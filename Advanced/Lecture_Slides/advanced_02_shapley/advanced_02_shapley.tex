...
\begin{frame}{Where this module is used in industry}
  ...
  \begin{tabular}{@{}p{2.6cm}p{5.2cm}p{4.6cm}@{}}
    ...
    Consumer credit & Adverse-action reasons for each declined loan
      application & Which ``principal reasons'' appear on the legally required notice \\
    Insurance & Justifying a boosted pricing model factor by factor &
      Whether a rating factor survives actuarial and regulatory review \\
    Healthcare & Which vitals and labs drove a patient's risk score &
      Whether the clinician trusts, overrides or escalates the alert \\
    Fraud \& AML & Explaining why a transaction was flagged &
      Investigator triage; evidence in the case file \\
    Tech / marketing & Debugging ranking and churn models: what drove a
      bad score & Fix the data pipeline vs.\ retrain the model \\
    Model risk / audit & Global driver analysis and drift monitoring &
      Model approval, re-validation, decommissioning \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{industry}[The common thread]
    In every row the model is a \emph{tree ensemble or neural net} that beats the
    interpretable alternative --- and a human downstream who must \emph{account} for a
    single prediction. Shapley values are the de-facto standard answer because they
    come with a guarantee: the attributions \textbf{sum exactly} to the prediction.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: explaining a credit decline}
  \footnotesize
  \begin{industry}[The setting: a boosted model decides --- the law wants reasons]
    A bank scores consumer-loan applications with a gradient-boosted model --- hundreds
    of trees over $\sim 40$ features. US regulation (ECOA / Regulation B) requires the
    bank to give each declined applicant the \textbf{principal reasons}; the EU AI Act
    classifies credit scoring as high-risk with transparency duties. ``The model said
    so'' is not a legal answer.
  \end{industry}
  \begin{readme}[How Shapley values enter the pipeline]
    For each declined applicant $x$ the bank computes attributions
    $\varphi_1,\dots,\varphi_p$ that split $f(x)-E[f(X)]$ across features. The most
    negative $\varphi_j$'s are mapped to standardised reason codes (``debt-to-income
    ratio too high''). The \emph{background} is chosen deliberately --- e.g.\ approved
    applicants --- so the reasons answer ``why you, compared to those who were
    approved''.
  \end{readme}
  \begin{takeaway}[Two honest caveats --- both taught in this module]
    The reasons describe the \emph{model}, not the applicant's causal reality --- they
    are not financial advice. And with correlated features (income, debt, limits) the
    split between related reasons is partly a modelling choice, so banks fix background,
    seed and model version in their model-risk documentation.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Cooperative games: players, coalitions, value}
  ...
  \end{readme}
  \begin{industry}[Cost allocation: the original use]
    Shapley's 1953 idea long predates machine learning: utilities split shared
    infrastructure costs, airports split runway costs across airline types, and
    firms allocate IT platform costs to business units --- all with the same formula
...
\begin{frame}{The coalition lattice (native diagram)}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,font=\small,
    ...
    \draw[-{Stealth},accent,thick] (B.north west) -- (AB.south east)
      node[midway,right=1pt,font=\scriptsize\color{accent}] {A joins: $+210$};
    \draw[-{Stealth},orange!70!black,thick] (E.west) to[bend left=18]
      node[midway,left=1pt,font=\scriptsize] {A joins: $+120$} (A.south);
  \end{tikzpicture}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{advanced\_02\_shapley\_lab.ipynb}]
    ...
    companion Jupyter notebook (in \texttt{Advanced/Lab\_Notebooks/}) and run it
    cell by cell:
    \begin{itemize}
...
\begin{frame}{The efficiency axiom, verified to machine precision}
  ...
  \begin{takeaway}
    ...
    (gradients, single-feature ablations) gives you for free.
  \end{takeaway}
  \begin{industry}[Model risk: the audit test]
    Validation teams re-run exactly this check on sampled instances: if reported
    attributions do not reproduce $f(x)-\varphi_0$ to numerical precision, the
    explanation pipeline (wrong background, stale model version) is flagged before
...
\begin{frame}{Monte-Carlo: sample arrival orders instead of enumerating}
  ...
  \begin{readme}
    \begin{itemize}
      ...
      \item Efficiency now holds only \emph{in expectation}; each run leaves a small
            random residual.
    \end{itemize}
  \end{readme}
\end{frame}
...
\begin{frame}{Convergence: sampled orders vs.\ the exact answer}
  \begin{center}
    \includegraphics[width=0.9\textwidth,height=0.56\textheight,keepaspectratio]{a2_convergence.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Scaling up in practice: KernelSHAP, TreeSHAP, \texttt{shap}}
  ...
  \begin{readme}[TreeSHAP: exactness for free on tree ensembles]
    ...
    noise. This is why boosted trees $+$ SHAP became the industry pairing.
  \end{readme}
  \begin{industry}[The production tool]
...
\begin{frame}{Local view: the waterfall of one prediction}
  \begin{center}
    \includegraphics[width=0.9\textwidth,height=0.56\textheight,keepaspectratio]{a2_local_waterfall.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Global view: mean $|\varphi_j|$ vs.\ permutation importance}
  \begin{center}
    \includegraphics[width=0.94\textwidth,height=0.52\textheight,keepaspectratio]{a2_global_importance.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Dependence view: how an attribution varies with its feature}
  \begin{center}
    \includegraphics[width=0.94\textwidth,height=0.52\textheight,keepaspectratio]{a2_dependence.png}
  \end{center}
  \begin{takeaway}
...
\begin{solutionbox}
  ...
  \begin{tabular}{@{}lrrrrrr@{}}
    ...
    \midrule
    $\varphi_j$ & $+0.287$ & $+0.177$ & $+0.112$ & $-0.037$ & $-0.100$ & $-0.125$ \\
    \bottomrule
  \end{tabular}
  \end{center}
...
\begin{frame}{Pitfall 1: correlated features put the model off-manifold}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.8\textwidth,height=0.42\textheight,keepaspectratio]{a2_offmanifold.png}
  \end{center}
  \vspace{-2pt}
  \begin{takeaway}
...
\begin{frame}{Pitfall 3: attributions move when the model is retrained}
  \begin{numexample}[Same data and accuracy --- different explanations]
    ...
    attribution follows.
  \end{numexample}
  \begin{industry}[Model risk \& regulation: pinning the explanation down]
    Under SR 11-7-style model governance and the EU AI Act, banks version
    \emph{everything the explanation depends on}: model artefact, background sample,
    seed, and \texttt{shap} version. A declined customer who reapplies must not
    receive contradictory ``principal reasons'' because a nightly retrain shuffled
    credit between two collinear features.
  \end{industry}
\end{frame}
