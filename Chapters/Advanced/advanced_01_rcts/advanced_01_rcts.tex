...
\begin{frame}{Where this module is used in industry}
  ...
  \end{tabular}
  \end{center}
  \begin{takeaway}[All three manufacture the counterfactual instead of assuming it]
    Ship it, approve it, keep funding it --- each is a \emph{causal} decision, and none of
    them can be read off a correlation. The counterfactual is created by
    \textbf{randomisation} rather than argued from a model, which is why the same design
    settles a drug approval and a checkout button. Tech industrialised what agriculture
    and medicine invented: Fisher's randomised plots became feature flags.
  \end{takeaway}
\end{frame}
