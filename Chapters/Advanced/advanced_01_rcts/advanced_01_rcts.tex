...
\begin{frame}{Where this module fits}
  ...
  \begin{tabular}{@{}p{3.6cm}p{8.6cm}@{}}
    ...
    Module A7 --- multiple testing & Peeking and metric panels as multiplicity problems \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{2pt}
  \begin{takeaway}[A new question --- not a better model]
    Chapters 2--8 estimated $\hat f(x)\approx E[Y\mid X=x]$ --- \textbf{correlational} by
  ...
  \end{takeaway}
  {\scriptsize One of twelve optional advanced modules; self-study, with a companion lab notebook.}
\end{frame}
...
\begin{frame}{The sign flips inside every stratum (Simpson's paradox)}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.80\textwidth,height=0.48\textheight,keepaspectratio]{cha1_simpson.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Confounding, drawn (native diagram)}
  ...
  \begin{tikzpicture}[font=\small,>=Stealth,
      ...
      var/.style={draw=black!50,rounded corners=2pt,minimum width=2.5cm,minimum height=0.7cm,align=center}]
    \node[var,fill=orange!14] (S) {Skill $X$};
    \node[var,fill=accent!16,below left=0.8cm and 1.7cm of S] (D) {Training $D$};
    \node[var,fill=green!14,below right=0.8cm and 1.7cm of S] (Y) {Wage $Y$};
  ...
  \begin{readme}[Reading the diagram]
    ...
    motivation, health or ability would confound invisibly. The same diagram explains ad
    attribution: ``customers who saw the ad bought $30\%$ more'' is the backdoor path,
    because the ad was targeted at likely buyers.
  \end{readme}
\end{frame}
...
\begin{frame}{SUTVA --- the assumption the whole notation rests on}
  ...
  \begin{readme}[Where it honestly fails]
    ...
    time slices) so that it holds. Ride-hailing platforms do exactly this: rider discounts
    are tested with \emph{switchback} designs, whole city--hour blocks treated or not,
    because rider-level randomisation leaks through the shared driver pool.
  \end{readme}
\end{frame}
...
\begin{frame}{Solution --- Exercise A1.1 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A1.1 (2/2)}
\begin{solutionbox}
  \smallskip
...
\begin{frame}{Solution --- Exercise A1.2 (1/2)}
\begin{solutionbox}
  \scriptsize
...
\begin{frame}{Solution --- Exercise A1.2 (2/2)}
\begin{solutionbox}
  \smallskip
...
\begin{frame}{Randomisation kills both bias terms --- in expectation}
  ...
  \begin{takeaway}
    ...
    --- bias is gone, variance remains. Hashing a user id into variant A or B \emph{is}
    that coin flip: every feature flag and holdout on a modern platform is this one
    identity, run $10^4$ times a year.
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: randomisation as a design
  principle is R.~A.\ Fisher's, developed in the 1920s for crop trials at
  Rothamsted --- famously illustrated by his ``lady tasting tea'' experiment.}
\end{frame}
...
\begin{frame}{Seeing it: the estimator's sampling distribution under both designs}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.80\textwidth,height=0.46\textheight,keepaspectratio]{cha1_sampling.png}
  \end{center}
  \begin{takeaway}
...
\begin{lstlisting}[basicstyle=\ttfamily\fontsize{6.5}{7.6}\selectfont]
import numpy as np
...
\begin{frame}{Should we adjust an RCT with regression? (bridge to Ch.\ 3)}
  ...
  \begin{mistake}
    ...
    steps --- module~A7 in new clothes. One pre-registered specification, reported
    whatever it says.
  \end{mistake}
\end{frame}
...
print(m0.params["D"], m0.bse["D"])                # tau-hat 4.122, SE 1.523
  ...
  \begin{readme}[Why \texttt{cov\_type="HC2"}]
    ...
    two-arm experiment, so it is the natural default for RCTs. The \texttt{print}
    reports the coefficient on $D$ and its SE: $4.122$ with SE $1.523$, within one
    SE of the $+5$ we built in.
  \end{readme}
\end{frame}
...
\begin{frame}{What adjustment buys: the numbers}
  ...
  \begin{takeaway}
    ...
    randomisation, that is expected, not alarming. Run at scale, this is CUPED: adjust for
    the \emph{same metric measured pre-experiment} and variance reductions around $50\%$
    are routine --- halving every experiment's duration.
  \end{takeaway}
\end{frame}
...
\begin{frame}[fragile]{Solution --- Exercise A1.4 (1/3)}
...
\vspace{1pt}
{\fontsize{7}{8.4}\selectfont
\textbf{How to read this:} \texttt{m0} regresses $y$ on $D$ alone, \texttt{m1} adds
the pre-treatment covariates; each \texttt{print} pairs $\hat\tau$ with its HC2 SE
--- $4.122/1.523$ against $3.732/1.315$, same estimand and a smaller SE.\par}
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Exercise A1.4 (2/3)}
...
\begin{frame}{Solution --- Exercise A1.4 (3/3)}
\begin{solutionbox}[Solution --- reading the output]
  \smallskip
...
\begin{frame}{Power and the minimum detectable effect}
  ...
  \begin{numexample}[The wage experiment --- planned properly]
    ...
    $n = 2 \times 7.85 \times 41.7^2 / 25 = \mathbf{1092}$ per arm. Conversely, at
...
\begin{frame}{The power curve: analytic formula vs simulation (bridge to Ch.\ 5)}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.72\textwidth,height=0.42\textheight,keepaspectratio]{cha1_power.png}
  \end{center}
  \begin{takeaway}
    ...
    Both agree: $80\%$ power needs $\mathbf{1092}$ per arm; at $500$ per arm you toss a
    slightly bent coin.
  \end{takeaway}
  \begin{mistake}
...
\begin{frame}{Design refinements: blocking and cluster randomisation}
  ...
  \begin{numexample}
    ...
    Geo experiments sit at the harsh end of the same formula --- a brand campaign can only
    randomise regions, and few large clusters make $\mathrm{DE}$ brutal, which is why geo
    tests run for months and user-level tests for weeks.
  \end{numexample}
\end{frame}
...
\begin{frame}{Solution --- Exercise A1.5 (1/2)}
\begin{solutionbox}
  \textbf{Part 1.} Variance term: $0.10 \times 0.90 + 0.12 \times 0.88 = 0.1956$.
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Exercise A1.5 (2/2)}
\begin{solutionbox}
  \[
...
\begin{frame}{Per-protocol analysis vs the compliers' effect (CACE)}
  ...
  \begin{block}{The instrumental-variables repair}
    ...
    \]
    (the Wald estimator; derivation in the appendix). \textbf{Worked example:}
...
\begin{frame}{What non-compliance costs, drawn}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.80\textwidth,height=0.45\textheight,keepaspectratio]{cha1_itt_dilution.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Balance checks: auditing the randomiser}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.68\textwidth,height=0.38\textheight,keepaspectratio]{cha1_balance.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Peeking: multiple testing in the time dimension (bridge to module A7)}
  \scriptsize
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      \includegraphics[width=\linewidth]{cha1_peeking.png}
    \end{column}
    \begin{column}{0.52\textwidth}
      \begin{takeaway}
        An A/A test (no true effect), checked after every $100$ users per arm and
        stopped at the first star: the realised false-positive rate is $4.6\%$ after
        one look but $\mathbf{14.5\%}$ by $5$ looks, $\mathbf{19.4\%}$ by $10$,
        $\mathbf{24.3\%}$ by $20$. Each look is another test on overlapping data ---
        correlated, so below the independent-tests bound $1-0.95^{20}=64\%$, yet five
        times nominal $\alpha$.
      \end{takeaway}
    \end{column}
  \end{columns}
...
\begin{frame}{The A/B playbook: this module, spoken in tech}
  ...
  \begin{tabular}{@{}p{4.1cm}p{7.9cm}@{}}
    ...
    Secondary metrics, segments & FDR control and ``exploratory'' labels --- module A7 \\
    \bottomrule
  \end{tabular}
  \end{center}
  The vocabulary changes, the design does not: the experiment config \emph{is} the
  pre-registration, guardrail metrics are the safety endpoints, and the ramp-up plan is
  the dose escalation. Every hard problem in this section --- interference, peeking,
  multiplicity --- is a live engineering concern, not a textbook footnote.
\end{frame}
...
\begin{frame}{Solution --- Exercise A1.6 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A1.6 (2/2)}
\begin{solutionbox}
  \smallskip
...
\begin{solutionbox}[Solution --- power arithmetic and run time]
  ...
  \[
    n = \frac{7.85 \times 0.0995}{0.005^2}
      = \frac{0.78108}{0.000025} \approx \mathbf{31\,243} \text{ per arm}.
  \]
  At $10\,000$ visitors per arm per week: $31\,243/10\,000 \approx 3.1$ weeks
...
\begin{solutionbox}[Solution --- peeking; units; metric discipline]
  ...
  label anything found only in segments as \emph{exploratory} --- module A7's rules,
  applied verbatim.
\end{solutionbox}
\begin{mistake}
...
\begin{frame}{Module A1 in one slide}
  ...
  \begin{itemize}
    ...
    \item Threats: attrition, non-compliance (ITT!), balance, peeking --- module~A7 in the
          time dimension.
  \end{itemize}
\end{frame}
...
\begin{frame}{Common pitfalls}
  \begin{alertblock}{Don't}
    \begin{enumerate}
      ...
      \item Test twenty segments and report the one that won (module A7!).
    \end{enumerate}
  \end{alertblock}
\end{frame}
...
\begin{frame}{Five things to remember}
  \begin{takeaway}[Module A1 in five lines]
    \begin{enumerate}
      \item \textbf{Causation is a property of the design, not the model.} No amount of
            flexibility fixes confounded data; randomised data answer a different question.
      \item \textbf{Naive comparison $=$ ATT $+$ selection bias.} The decomposition names
            exactly what randomisation deletes.
      \item \textbf{The analysis of a clean RCT is Chapter 0's $t$-test} --- difference in
            means, with regression adjustment buying precision, never truth.
      \item \textbf{Power is bought at design time:}
            $n = 2\sigma^2(z_{1-\alpha/2}+z_{1-\beta})^2/\delta^2$ --- it cannot be
            rescued in the analysis.
      \item \textbf{Analyse as randomised (ITT), and never peek} --- non-compliance and
            sequential looks are module A7's problem in a lab coat.
    \end{enumerate}
  \end{takeaway}
\end{frame}

\begin{frame}{What's next}
  The interventional question is answered; two extension modules continue from here:
  \begin{itemize}
    \item \textbf{Module A2 --- Explainable AI with Shapley Values}: a randomised
          experiment tells you \emph{that} the treatment works; A2 asks how a
          predictive model arrived at one prediction --- and why that is still
          not a cause.
    \item \textbf{Module A3 --- Conformal Prediction}: the RCT gave an honest
          \emph{average} effect; A3 gives honest uncertainty for \emph{individual}
          predictions, with the same finite-sample flavour as the randomisation test.
  \end{itemize}
\end{frame}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      ...
      \item \textbf{Fisher's randomisation test} --- exact $p$-values from re-randomising;
            a bridge to module~A7's permutation tests, beyond the module's core.
...
\begin{frame}{Fisher's randomisation test (bridge to module A7 resampling)}
  \footnotesize
  ...
  \begin{readme}
    \begin{itemize}
      \item Empirical $p$-value:
            ...
            re-randomisations --- the same construction as module~A7's permutation
