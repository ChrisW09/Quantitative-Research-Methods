...
\begin{frame}{Where this module fits}
  \footnotesize
  This is one of twelve \textbf{optional advanced modules} that sit outside the taught chapters.
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    Deep networks & Module A9 & the archetypal black box (same machinery) \\
  ...
  \begin{takeaway}[Today's question]
    Chapter 8 gave us a boosted model that predicts well; module A9 gave us networks
...
\begin{frame}{Solution --- Exercise A2.1 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A2.1 (2/2)}
\begin{solutionbox}
  \begin{enumerate}
...
\begin{frame}{Solution --- Exercise A2.2 (1/3)}
...
\begin{frame}{Solution --- Exercise A2.2 (2/3)}
...
\begin{frame}{Solution --- Exercise A2.2 (3/3)}
\begin{solutionbox}[Solution --- efficiency and the dummy check]
  \medskip
...
\begin{frame}{Solution --- Exercise A2.3 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A2.3 (2/2)}
\begin{solutionbox}
  \medskip
...
\begin{frame}{Solution --- Exercise A2.4 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A2.4 (2/2)}
\begin{solutionbox}
  \smallskip
...
\begin{frame}{Solution --- Extended Exercise A2.2 (1/4)}
...
\begin{frame}{Solution --- Extended Exercise A2.2 (2/4)}
\begin{solutionbox}[Solution --- parts 1--3: the architecture]
  \smallskip
...
\begin{frame}{Solution --- Extended Exercise A2.2 (3/4)}
...
\begin{frame}{Solution --- Extended Exercise A2.2 (4/4)}
\begin{solutionbox}[Solution --- parts 4--5: correlation and the customer's question]
  \smallskip
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{advanced\_02\_shapley\_lab.ipynb}]
    The code in this module is meant to be \textbf{demonstrated live}. Open the
...
\begin{frame}{Five things to remember}
  \begin{takeaway}[Module A2 in five lines]
    \begin{enumerate}
      \item \textbf{An attribution divides one prediction's departure from the baseline} ---
            efficiency guarantees the parts sum exactly: $\sum_j \varphi_j = f(x) - $ baseline.
      \item \textbf{The Shapley value is the only division satisfying the four axioms} ---
            the game theory is what licenses the word \emph{fair}.
      \item \textbf{Exact costs $2^p$; Monte-Carlo converges at $1/\sqrt{m}$} --- and
            TreeSHAP is exact \emph{and} fast for tree ensembles.
      \item \textbf{Correlated features put the value function off-manifold} --- marginal
            vs.\ conditional sampling is a modelling choice, and it changes the answer.
      \item \textbf{An explanation is not a cause, and it moves when the model is
            retrained} --- report attributions with the model version they explain.
    \end{enumerate}
  \end{takeaway}
\end{frame}
...
\begin{frame}{What's next}
  \begin{itemize}
    \item \textbf{Module A3 --- Conformal Prediction}: you can now say how a
          prediction was assembled; A3 says how far to trust it ---
          distribution-free intervals around the same boosted model.
    \item \textbf{Module A1 --- Randomised Controlled Trials}: Pitfall 2 said an
          explanation is not a cause. A1 is the design that \emph{earns} causal
          claims --- randomise, then compare.
  \end{itemize}
\end{frame}
...
\begin{frame}{Solution --- Exercise A2.6 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A2.6 (2/2)}
\begin{solutionbox}
  \smallskip
