...
\begin{frame}{Where conformal prediction is used in industry}
  ...
  \begin{tabular}{@{}p{2.6cm}p{5.2cm}p{4.6cm}@{}}
    ...
    \midrule
    Energy & Intervals around day-ahead load and wind forecasts, recalibrated on recent
      residuals & How many MW of reserve capacity to buy \\
    E-commerce, logistics & Delivery-time windows behind the checkout promise &
      Which arrival date is displayed and guaranteed \\
    Pharma R\&D & Conformal sets in virtual screening of compounds & Which molecules
      advance to the wet lab \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{takeaway}[All three are paid for the width of the interval rather than the point forecast]
    In every row the model's \emph{point} prediction was already available and already
    good; what was missing is a number that sizes the buffer, the promise or the
    shortlist. Conformal prediction supplies it \textbf{without trusting the model}: the
    coverage holds in finite samples for any $\hat f$, which is why a desk is willing to
    commit money to the width.
  \end{takeaway}
\end{frame}
