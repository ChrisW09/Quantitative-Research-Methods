...
\begin{frame}{Where this module fits}
  \footnotesize
  The twelve \textbf{advanced modules} are optional self-study decks that sit outside
  the taught chapters. This one turns any predictive model from the course into a model
...
\begin{frame}{Solution --- Exercise A3.1 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A3.1 (2/2)}
\begin{solutionbox}
  \begin{itemize}
...
\begin{frame}{The honest slide: what breaks exchangeability}
  ...
  \end{readme}
  \begin{takeaway}
    Realised coverage is therefore also a \emph{drift alarm}: sliding from 90\% to 80\%
    says the world has moved away from the calibration set, and it needs no labels
    beyond the ones you already collect.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Wage: the calibration quantile and the band it buys}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.92\textwidth,height=0.40\textheight,keepaspectratio]{a3_scores_band.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Solution --- Exercise A3.2 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A3.2 (2/2)}
\begin{solutionbox}
  \begin{itemize}
...
\begin{frame}{The guarantee is an average over splits --- see it hold}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.88\textwidth,height=0.40\textheight,keepaspectratio]{a3_coverage_hist.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{The honest slide: marginal is not conditional}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.74\textwidth,height=0.31\textheight,keepaspectratio]{a3_subgroup.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Solution --- Exercise A3.3 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A3.3 (2/2)}
\begin{solutionbox}
  \begin{itemize}
...
\begin{frame}{What the calibration set buys, drawn}
  \begin{center}
    \includegraphics[width=0.80\textwidth,height=0.54\textheight,keepaspectratio]{a3_staircase.png}
  \end{center}
  \begin{takeaway}
    Exercise A3.3 as a picture. Below $n=19$ the $95\%$ interval is
    $(-\infty,\infty)$; at exactly $n=19$ the quantile is the largest score;
    and the overshoot above $1-\alpha$ decays like $1/(n+1)$ --- the sawtooth
    is the ceiling function doing its work.
  \end{takeaway}
\end{frame}
...
\begin{frame}[fragile]{The engine: quantile regression with the pinball loss}
  ...
  \begin{readme}[Why it works --- in one sentence]
    ...
    Below, \texttt{alpha} \emph{is} $\tau$: the two fits trace the $5\%$ and
    $95\%$ edges of $Y \mid X$ --- the raw band that CQR then calibrates.
  \end{readme}
  \begin{lstlisting}
...
lo = GradientBoostingRegressor(loss="quantile", alpha=0.05,   # 5% curve
        n_estimators=200, max_depth=2, learning_rate=0.05)  # Ch. 8 knobs
hi = GradientBoostingRegressor(loss="quantile", alpha=0.95,   # 95% curve
        n_estimators=200, max_depth=2, learning_rate=0.05)  # same knobs
  \end{lstlisting}
\end{frame}
...
\begin{frame}{Wage: constant width vs adaptive width}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.94\textwidth,height=0.35\textheight,keepaspectratio]{a3_cqr.png}
  \end{center}
  \begin{takeaway}
...
from sklearn.ensemble import GradientBoostingRegressor   # the CQR engine

wage = pd.read_csv("Wage.csv")                       # lab: via load()
x, y = wage["age"].to_numpy(float), wage["wage"].to_numpy()  # x = age
rng = np.random.default_rng(2024)                    # the deck's split
i = rng.permutation(len(y)); tr, ca, te = i[:2000], i[2000:2500], i[2500:]
X = x.reshape(-1, 1)                                 # sklearn wants 2-D
...
k = int(np.ceil((len(ca) + 1) * 0.9))                # k = 451 of 500
qhat = np.sort(s)[k - 1]                             # 0.66: tiny correction
L, U = lo.predict(X[te]) - qhat, hi.predict(X[te]) + qhat  # pad both edges
print(round(qhat, 2), ((y[te] >= L) & (y[te] <= U)).mean())  # 0.66, cov 0.896
\end{lstlisting}
\end{solutionbox}
\end{frame}
...
% ============================================================
\section[Classification]{Conformal Classification}
% ============================================================
...
\begin{frame}{A 3-class simulation: sets earn their keep at the boundaries}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.92\textwidth,height=0.38\textheight,keepaspectratio]{a3_three_class.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{The price of certainty: coverage vs set size}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.7\textwidth,height=0.25\textheight,keepaspectratio]{a3_tradeoff.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Same data, two intervals: when the model is right, they agree}
  ...
  \begin{takeaway}
    ...
    requirement}. That makes the pair a cheap diagnostic: run both, and if they disagree
    materially, the \emph{model assumptions} are the prime suspect.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Heteroskedastic truth: where the 90\% quietly stops being 90\%}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.94\textwidth,height=0.42\textheight,keepaspectratio]{a3_hetero.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}[fragile]{Solution --- Extended Exercise A3.2 (1/3)}
\begin{solutionbox}[Solution --- code: the three intervals]
\begin{lstlisting}[basicstyle=\ttfamily\fontsize{7}{8.2}\selectfont]
import numpy as np, statsmodels.api as sm               # OLS and arrays
from sklearn.ensemble import GradientBoostingRegressor  # the flexible fit
rng = np.random.default_rng(2024)                      # the deck's seed
...
pred = ols.params[0] + ols.params[1] * x[te]           # test fitted values
...
\end{lstlisting}
{\scriptsize \textbf{Output:} \texttt{cov} returns [all, $x<2$, $x>8$] ---
marginal coverage first, then the two conditional ones.\par}
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise A3.2 (2/3)}
...
\begin{frame}{Solution --- Extended Exercise A3.2 (3/3)}
\begin{solutionbox}[Solution --- interpretation]
...
\begin{frame}{Pitfall 1: exchangeability violations in the wild}
  ...
  \begin{readme}[What to do instead]
    ...
    --- the guarantee you can check is the one you keep. A wind-power desk does exactly
    this, recalibrating on the trailing 30 days: a fixed calibration set from spring
    undercovers in autumn storms --- same model, dead guarantee.
  \end{readme}
\end{frame}
...
\begin{frame}{From ten lines to production}
  ...
  \begin{readme}[Practical sizing rules]
    ...
    \end{itemize}
  \end{readme}
  \begin{takeaway}[The production tool]
    Teams reach for \textbf{MAPIE} (\emph{Model Agnostic Prediction Interval
    Estimator}, scikit-learn compatible): split/cross conformal, CQR and classification
    sets behind one API, plus jackknife+ variants that recycle training folds. It is not
    installed here --- deliberately: everything in this module was ten lines from
    scratch, and those ten lines are the audit trail you will be asked for.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{advanced\_03\_conformal\_lab.ipynb}]
    ...
    \begin{itemize}
      ...
      \item \S5, \emph{OLS prediction interval vs conformal} --- the friendly
            homoskedastic case (the heteroskedastic stress test is worked as
            Extended Exercise A3.2).
    \end{itemize}
...
\begin{frame}[fragile]{Split conformal in ten lines}
  \begin{lstlisting}
import numpy as np, pandas as pd                      # arrays + CSV
from sklearn.linear_model import LinearRegression     # any model works
...
cov = (np.abs(y[te] - f.predict(P(x[te]))) <= qhat).mean()  # 4. coverage
print(k, qhat.round(2), cov)                         # 451 56.14 0.926
  \end{lstlisting}
  \vspace{-1mm}
  \begin{readme}
    \scriptsize
    \textbf{Key call:} \texttt{np.sort(s)[k-1]}, the $451$st smallest held-out
    residual: $\hat q = 56.14$, so $\hat f(x) \pm 56.14$ covers $92.6\%$.
  \end{readme}
\end{frame}
...
\begin{lstlisting}[basicstyle=\ttfamily\fontsize{6.5}{7.2}\selectfont,aboveskip=2pt,belowskip=0pt]
import numpy as np                                        # arrays only
from sklearn.linear_model import LogisticRegression       # Ch. 4 model
rng = np.random.default_rng(2024)                         # deck's seed
mu = np.array([[0, 0], [2.2, 0], [1.1, 1.9]])            # class centres
X = np.vstack([rng.normal(m, 1.0, (1000, 2)) for m in mu]) # 1000 each
y = np.repeat([0, 1, 2], 1000)                            # labels, in order
p = rng.permutation(3000); X, y = X[p], y[p]              # shuffle once
clf = LogisticRegression(max_iter=1000).fit(X[:1500], y[:1500])  # TRAIN
pc = clf.predict_proba(X[1500:2250])                      # calibration probs
s = 1 - pc[np.arange(750), y[1500:2250]]                  # score: 1 - p_true
k = int(np.ceil(751 * 0.9))                               # k = 676 of 750
qhat = np.sort(s)[k - 1]                                  # qhat = 0.7685
sets = clf.predict_proba(X[2250:]) >= 1 - qhat            # keep p >= 0.2315
cov = sets[np.arange(750), y[2250:]].mean()               # truth in the set?
print(qhat.round(4), cov, sets.sum(1).mean().round(3))    # 0.7685 0.9013 1.296
\end{lstlisting}
  {\scriptsize \textbf{Parts 3--4.} Coverage $0.9013$, average size $1.30$, singletons $71.3\%$.
  At $\alpha = 0.01$: $k = \lceil 751 \times 0.99 \rceil = 744$, $\hat q = 0.9638$,
  coverage $98.9\%$ --- and the average set grows to $2.17$ of 3 classes.\par}
\end{solutionbox}
\begin{mistake}
  \scriptsize
...
\begin{frame}{Five things to remember}
  \begin{takeaway}[Module A3 in five lines]
    \begin{enumerate}
      \item \textbf{One assumption: exchangeability.} No Gaussianity, no correct model ---
            but time order, distribution shift and feedback all break it.
      \item \textbf{Split conformal is rank arithmetic:} the
            $k = \lceil (n+1)(1-\alpha) \rceil$-th calibration score gives
            $\hat f(x) \pm \hat q$ with finite-sample coverage.
      \item \textbf{The guarantee is marginal} --- averaged over points and splits, not
            per subgroup and not conditional on $x$.
      \item \textbf{Honest width is not informative width} --- locally weighted scores and
            CQR make the interval track difficulty.
      \item \textbf{Classification gives prediction sets}, and the set size is the
            difficulty meter.
    \end{enumerate}
  \end{takeaway}
\end{frame}
...
\begin{frame}{What's next}
  \begin{itemize}
    \item \textbf{Module A1 --- Randomised Controlled Trials}: marginal coverage
          is a guarantee about \emph{predictions}, not interventions; A1 builds
          the design that answers ``what happens if \emph{we} act?''
          (read module A7 first --- A1 leans on it).
    \item \textbf{Module A2 --- Explainable AI with Shapley Values}: the interval
          says how much to trust a prediction; A2 says which features built it.
  \end{itemize}
\end{frame}
