...
\begin{frame}{Where this module is used in industry}
  ...
  \begin{tabular}{@{}p{2.6cm}p{5.2cm}p{4.6cm}@{}}
    ...
    \midrule
    Insurance & Claim \emph{frequency} per policy-year: Poisson/NB GLM with a
      $\log(\text{exposure})$ offset and a spline in driver age & The premium each tariff cell pays \\
    Energy & National load forecast as a GAM: smooth(temperature) $+$ smooth(hour) $+$
      calendar dummies & How much generation to schedule for tomorrow \\
    Retail & Daily demand counts per SKU, fitted negative-binomial because promotions
      overdisperse & The reorder quantity and safety stock per store \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{takeaway}[All three need a count model whose every term can be shown to a reviewer]
    Whenever the response is a \textbf{count} or a \textbf{rate}, this is the stack: a GLM
    for interpretability and valid uncertainty, smooth terms for the shapes nobody wants
    to guess, offsets for unequal exposure. A gradient-boosted model would forecast the
    counts too --- but no actuary signs off a tariff, and no system operator schedules a
    plant, on a curve they cannot see.
  \end{takeaway}
\end{frame}
