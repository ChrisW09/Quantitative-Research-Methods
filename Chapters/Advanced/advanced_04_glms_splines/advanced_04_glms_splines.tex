...
\begin{frame}{Two failures, one diagnosis}
  ...
  \begin{takeaway}[What we need instead]
    ...
    The cost of not having one is concrete: a fleet-rebalancing optimiser fed
    ``$-105$ rides expected'' will happily truck bikes \emph{away} from a station that
    will see riders at dawn.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Solution --- Exercise A4.1 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A4.1 (2/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A4.2 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A4.2 (2/2)}
\begin{solutionbox}
...
\begin{frame}{Logistic regression, re-derived in one line}
  ...
  \begin{takeaway}
    ...
    fitting --- all forced by ``Bernoulli $+$ canonical link''. Bank default models are
    exactly this GLM, and every $e^{\beta_j}$ carries a documented odds-ratio meaning ---
    a key reason GLMs survive in the deep-learning era.
  \end{takeaway}
\end{frame}
...
\begin{lstlisting}
import numpy as np, pandas as pd                     # arrays, data frames
import statsmodels.api as sm, statsmodels.formula.api as smf

bike = pd.read_csv('Bikeshare.csv', index_col=0)     # 8 645 hourly rows
bike['hr'] = bike['hr'].astype(int)                  # hour 0..23
form = 'bikers ~ C(hr) + temp + workingday + C(weathersit)'  # C(): dummies
pois = smf.glm(form, data=bike,                      # ML fit, IRLS inside
...
\end{lstlisting}
  \begin{labnote}
    \textbf{Reading the print-outs:} the deviance is this GLM's residual sum of
    squares; \texttt{exp} of a coefficient is a rate ratio. Notebook
    \texttt{advanced\_04\_glms\_splines\_lab.ipynb}, \S 3 \emph{Poisson
    regression on Bikeshare}: runs this fit, prints the coefficient table and
...
\begin{frame}{Reading the fitted coefficients as rate ratios}
  ...
  \begin{tabular}{@{}lrrr@{}}
    ...
    \midrule
    Intercept & $3.042$ & $0.008$ & $20.9$ bikers (baseline hour) \\
...
\begin{frame}{The rate ratios, drawn}
  \begin{center}
    \includegraphics[width=0.82\textwidth,height=0.54\textheight,keepaspectratio]{adv04_rate_ratios.png}
  \end{center}
  \begin{takeaway}
    The table on the previous slide as a forest plot, on the log scale where
    rate ratios are symmetric. Every interval is tight except
    \textbf{heavy rain} --- one observed hour, and the CI says so before you
...
\begin{frame}{Offsets: modelling rates when exposure varies}
  ...
  \begin{takeaway}
    ...
    every row is exactly one hour.) Claim counts per policy-year and disease counts per
    $100\,000$ population are \emph{always} fitted this way: without the offset, big
    portfolios and big cities dominate for size alone.
  \end{takeaway}
\end{frame}
...
\begin{labnote}
  ...
  \emph{Exercise A4.3 --- Rate ratios on Bikeshare [Python]}.
\end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise A4.3 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A4.3 (2/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A4.4 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A4.4 (2/2)}
\begin{solutionbox}
...
\begin{frame}{Seeing the violation: variance vs mean across cells}
  \begin{center}
    \includegraphics[width=0.88\textwidth,height=0.50\textheight,keepaspectratio]{adv04_meanvar.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Poisson vs quasi-Poisson vs negative binomial, side by side}
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    \midrule
    \texttt{temp} & $1.567$ & $0.005$ & $0.026$ & $1.893$ & $0.032$ \\
  ...
  \end{readme}
  \begin{takeaway}
    Treat $\hat\varphi$ as a diagnostic, not a nuisance: a jump from 3 to 30 after a
    promotion week says the \emph{mean} model is missing a driver. Inflating the standard
    errors then papers over a specification problem.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Solution --- Exercise A4.5 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A4.5 (2/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise A4.1 (1/3)}
...
\begin{frame}{Solution --- Extended Exercise A4.1 (2/3)}
...
\begin{frame}{Solution --- Extended Exercise A4.1 (3/3)}
\begin{solutionbox}[Solution --- re-model and decide]
...
\begin{frame}{The B-spline basis: local, overlapping bumps}
  \begin{center}
    \includegraphics[width=0.9\textwidth,height=0.45\textheight,keepaspectratio]{adv04_bspline_basis.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Effective degrees of freedom: how much did you really fit?}
  ...
  \begin{takeaway}
    ...
    smooth is worth $6.2$ parameters'' is a meaningful, comparable statement.
  \end{takeaway}
\end{frame}
...
\begin{frame}{One picture: \texttt{Wage} across three orders of magnitude of $\lambda$}
  \begin{center}
    \includegraphics[width=0.95\textwidth,height=0.50\textheight,keepaspectratio]{adv04_pspline_lambda.png}
  \end{center}
  \begin{takeaway}
    ...
    $\lambda = 261$, $\text{edf} = 6.2$ --- close to the df Chapter 7 found by CV for
    this very curve.
  \end{takeaway}
\end{frame}
...
\begin{exercise}
  ...
  \begin{enumerate}
    ...
    \item Your colleague reports ``$\lambda = 261$'' as the headline of their smooth.
          Why is ``$\text{edf} = 6.2$'' the better headline?
  \end{enumerate}
\end{exercise}
\end{frame}

\begin{frame}{Solution --- Exercise A4.6 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise A4.6 (2/2)}
\begin{solutionbox}
...
xs = bike[['hr', 'temp']].astype(float)            # the two smooth terms
bs = BSplines(xs, df=[12, 6], degree=[3, 3])       # rich bases for hr, temp
Xpar = ...   # intercept, workingday, 3 weather dummies (parametric part)

gam = GLMGam(bike.bikers, exog=Xpar, smoother=bs,  # alpha = one lambda each
             family=sm.families.Poisson(), alpha=[0.319, 0.00423])
res = gam.fit()                                    # penalized IRLS fit
# the two penalty weights above were chosen by the built-in criterion:
#   gam.select_penweight()[0]   ->  [0.319, 0.00423]
print(res.deviance)                                # 259 650 vs 269 006
\end{lstlisting}
  \begin{labnote}
    \textbf{How to read it:} \texttt{alpha} shrinks the over-rich 12 and 6 df
    bases to $\approx 11$ and $\approx 5$ edf. Notebook \S 7 \emph{A penalized-spline GAM for
    Bikeshare} builds \texttt{Xpar}, runs \texttt{select\_penweight}, and draws the
    partial-effect plots on the next slide with confidence bands.
  \end{labnote}
\end{frame}
...
\begin{frame}{The fitted smooths: hour of day and temperature}
  \begin{center}
    \includegraphics[width=0.92\textwidth,height=0.51\textheight,keepaspectratio]{adv04_gam_partial.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Interpreting the hour curve --- and one honest caveat}
  ...
  \end{mistake}
  \begin{takeaway}
    This readability is why system operators forecast national electricity load with
    exactly this structure: engineers must \emph{see and sign off} each curve, and the
    hot-weather bend caused by air conditioning is the standard argument for letting
    temperature be a smooth.
  \end{takeaway}
\end{frame}
...
\begin{labnote}[{{Run this live}}]
  The worked solution sits in the lab notebook under \emph{Lecture exercises ---
  worked Python solutions}: the cell headed
  \emph{Extended Exercise A4.2 --- Build and defend a count GAM [Python]}.
\end{labnote}
\end{frame}

\begin{frame}{Solution --- Extended Exercise A4.2 (1/3)}
...
\begin{frame}{Solution --- Extended Exercise A4.2 (2/3)}
...
\begin{frame}{Solution --- Extended Exercise A4.2 (3/3)}
\begin{solutionbox}[Solution --- coefficient shift and the shipping check]
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{advanced\_04\_glms\_splines\_lab.ipynb}]
    The code behind every number in this deck is meant to be \textbf{run live}.
    Open the companion Jupyter notebook (it sits beside this deck) and run it
    cell by cell:
    \begin{itemize}
      \item \S1, \emph{Bikeshare, and the linear model failing} --- negative predictions, fanning residuals;
      \item \S2, \emph{The exponential family in one numeric check};
      \item \S3, \emph{Poisson regression on Bikeshare} --- coefficients as rate ratios;
      \item \S4, \emph{Deviance, LRT and AIC} --- the three ways to compare fits;
      \item \S5, \emph{Overdispersion: quasi-Poisson and negative binomial};
      \item \S6, \emph{Penalized splines on Wage} --- the penalty path and edf;
      \item \S7, \emph{A penalized-spline GAM for Bikeshare}.
    \end{itemize}
...
\begin{frame}{Five things to remember}
  \begin{takeaway}[Module A4 in five lines]
    \begin{enumerate}
      \item \textbf{Match the family to the response} --- OLS on \texttt{Bikeshare} counts
            gave $10\%$ negative predictions; Poisson cannot.
      \item \textbf{A GLM is family $+$ linear predictor $+$ link;} $\exp(\hat\beta_j)$ is
            a rate ratio, and offsets handle exposure.
      \item \textbf{Deviance is the GLM's RSS} --- LRT and AIC do the comparing.
      \item \textbf{Check overdispersion before believing a Poisson SE} ---
            $\hat\varphi = 30.9$ here rescaled every SE by $\times 5.56$.
      \item \textbf{Penalized splines make flexibility one dial} --- $\lambda$, read off
            as edf, tuned by GCV/CV --- and the count GAM keeps every curve readable.
    \end{enumerate}
  \end{takeaway}
\end{frame}
...
\begin{frame}{What's next}
  \begin{itemize}
    \item \textbf{Module A6 --- Survival Analysis}: the next likelihood after
          counts --- censored times, the hazard, and the Cox model, fitted by the
          same maximum-likelihood machinery.
    \item \textbf{Module A3 --- Conformal Prediction}: the count GAM predicts;
          A3 wraps any such fit in intervals with guaranteed coverage.
  \end{itemize}
\end{frame}
...
\begin{frame}{Solution --- Drill Exercise A4.A (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Drill Exercise A4.A (2/2)}
\begin{solutionbox}
