...
\begin{frame}{Solution --- Exercise 9.1 (1/2)}
\begin{solutionbox}
  \scriptsize
...
\begin{frame}{Solution --- Exercise 9.1 (2/2)}
\begin{solutionbox}
  \scriptsize
...
\begin{frame}{Support vectors: the only points that matter}
  \begin{center}
    \includegraphics[width=0.38\textwidth,height=0.41\textheight,keepaspectratio]{ch09_maxmargin.png}
  \end{center}
  \begin{takeaway}
  ...
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: Vladimir Vapnik's idea from the
  1960s; kernels and soft margins made it the SVM in the 1990s.}
\end{frame}
...
\begin{frame}{Module A5 in one slide}
  \footnotesize
...
\begin{frame}{Eight things to remember}
  \footnotesize
  \begin{takeaway}[Module A5 in eight lines]
    \begin{enumerate}
  ...
  \begin{takeaway}[What's next]
    Module A9 (deep learning, ISLP Chapter 10) stops \emph{choosing} the enlarged
    features and \emph{learns} them instead: a neural network is a stack of adaptive
    basis functions with a linear rule on top.
  \end{takeaway}
\end{frame}
