...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.6cm}p{5.2cm}p{4.6cm}@{}}
    ...
    Subscription tech & Time to churn for customers who have \emph{not yet} churned &
      Which cohorts the retention budget is spent on \\
    Manufacturing & Time to failure of a component still in the field &
      The warranty term offered, and the maintenance interval \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{takeaway}[{{In all three, the informative subjects are the ones nothing has happened to yet}}]
    Drop them and every number moves the same way --- too pessimistic. A churn model that
    quietly discards still-active customers is fitted on the unhappy minority; a warranty
    analysis that ignores components still running will promise a shorter life than the
    product has; a trial analysed only on patients who have already progressed understates
    the drug. Censoring is not missing data to be cleaned away: it is the observation.
  \end{takeaway}
\end{frame}
...
\begin{frame}{What's next}
  ...
  \begin{readme}[Where this chapter connects]
    \begin{itemize}
      ...
      \item \textbf{Unsupervised learning (Chapter 12):} no response at all --- a sibling
