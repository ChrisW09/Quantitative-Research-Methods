...
\begin{frame}{Solution --- Exercise 11.1 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 11.1 (2/2)}
\begin{solutionbox}
  \medskip
...
\begin{frame}{The Kaplan--Meier (product-limit) estimator}
  ...
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: Kaplan and Meier submitted
  separately; the editor made them merge --- a 1958 citation classic.}
\end{frame}
...
\begin{frame}{Reading the curve without over-reading it}
  ...
  \begin{readme}[Three habits]
    \begin{itemize}
      ...
      \item Never extrapolate past the largest observed time ($82.56$ months). A curve
            that flattens at $0.45$ does not say that $45\%$ of subjects last forever ---
            it says follow-up ran out. Either extrapolate with a parametric tail and
            declare it, or report a restricted quantity such as $\mathrm{RMST}(\tau)$.
    \end{itemize}
  \end{readme}
\end{frame}
...
\begin{solutionbox}
  ...
  $\mathbf{8}$ and $\hat S(10)=3/6=0.500$ --- the curve is dragged down early and the
...
\begin{labnote}[Run this live]
  The worked solution sits in \texttt{advanced\_06\_survival\_lab.ipynb} under
...
km = KaplanMeierFitter().fit(BC['time'], BC['status'])   # censored rows lower r_k, not S
print('S(20) = %.4f' % km.survival_function_at_times(20).iloc[0])   # S(20) = 0.7132
...
for s in ['Female', 'Male']:                         # one curve per group
    m = BC['sex'] == s                               # boolean mask for this group
    k = KaplanMeierFitter().fit(BC.loc[m, 'time'], BC.loc[m, 'status'])  # separate KM fit
...
\begin{frame}{BrainCancer by sex: the real numbers}
  ...
  \begin{takeaway}
    ...
    medians is entirely consistent with chance at this sample size. Note what drives the
    precision: $V_1$ grows with the number of \emph{events}, not with the number of
    patients --- which is why trials are sized in events, and why recruiting more subjects
    into a low-event setting buys surprisingly little power.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Solution --- Exercise 11.4 (1/2)}
  \scriptsize
\begin{solutionbox}
  \tiny
...
\begin{frame}{Solution --- Exercise 11.4 (2/2)}
\begin{solutionbox}
  \medskip
...
\begin{frame}{The Cox proportional-hazards model}
  ...
  \begin{takeaway}
    ...
    since 1972.
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: David Cox (1972) --- the same Cox
  as in logistic regression (1958).}
\end{frame}
  ...
  \begin{readme}[Where the baseline went]
    \begin{itemize}
      ...
      \item \textbf{Buys:} no distribution for $T$; only the \emph{order} of the times
            matters --- so a prepayment model fitted on loan age transfers unchanged to a
            book measured in payment counts. \textbf{Costs:} no absolute risk from
            $\hat\beta$ alone.
    \end{itemize}
  \end{readme}
\end{frame}
...
\begin{frame}{Evaluating a survival model}
  ...
  \begin{readme}
    \begin{itemize}
      ...
      \item Those are the two questions any reviewer will ask, in order:
            \emph{discrimination} (does it rank?) and \emph{calibration} (are the
            probabilities right at the horizons the decision uses?). A model can pass the
  ...
  \begin{readme}[Chapter 6 transplanted]
    \begin{itemize}
      ...
      \item The resulting linear predictor $x^\top\hat\beta$ is the risk score behind
            commercial genomic assays: a handful of genes, one score, dichotomised at a
            threshold chosen on \emph{clinical} grounds and validated on an independent
            cohort. Sparsity is what makes the assay cheap enough to run.
    \end{itemize}
  \end{readme}
\end{frame}
...
\begin{frame}[fragile]{Solution --- Extended Exercise 11.2 (1/4)}
\begin{solutionbox}[Solution --- code]
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
import numpy as np, pandas as pd                      # arrays and data frames
from lifelines import CoxPHFitter                     # partial-likelihood fit
from lifelines.statistics import proportional_hazard_test   # Schoenfeld PH check
BC = load('BrainCancer', index_col=0).dropna()        # drops 1 NA row -> n=87
BC['diagnosis'] = pd.Categorical(BC['diagnosis'],     # Meningioma = reference
        categories=['Meningioma', 'HG glioma', 'LG glioma', 'Other'])  # HRs are vs. this
X = pd.get_dummies(BC, drop_first=True).astype(float) # drop the reference level
cph = CoxPHFitter().fit(X, duration_col='time', event_col='status')  # h_0 never estimated
print(cph.summary[['coef', 'exp(coef)', 'p']].round(4))  # log HR, HR, p-value
print('HR per 10 ki : %.3f' % np.exp(10 * cph.params_['ki']))   # 10 ki points: 0.577
print('C-index      : %.3f' % cph.concordance_index_)           # discrimination: 0.794
print(proportional_hazard_test(cph, X, time_transform='rank').summary['p'])  # PH p-values
...
\begin{frame}{Solution --- Extended Exercise 11.2 (2/4)}
\begin{solutionbox}[Solution --- output and reading]
  \begin{itemize}
...
\begin{frame}{Solution --- Extended Exercise 11.2 (3/4)}
\begin{solutionbox}[Solution --- output and reading]
  \begin{itemize}
...
\begin{frame}{Solution --- Extended Exercise 11.2 (4/4)}
\begin{solutionbox}[Solution --- output and reading]
  \begin{itemize}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{advanced\_06\_survival\_lab.ipynb}]
...
res = logrank_test(BC.loc[male, 'time'],   BC.loc[~male, 'time'],   # men vs women times
                   BC.loc[male, 'status'], BC.loc[~male, 'status']) # their event flags
print('chi2 = %.4f  p = %.4f' % (res.test_statistic, res.p_value))  # chi2 1.4405, p 0.23
  \end{lstlisting}
  \begin{takeaway}
...
BC['diagnosis'] = pd.Categorical(BC['diagnosis'],           # set the reference
        categories=['Meningioma', 'HG glioma', 'LG glioma', 'Other'])  # HRs vs. this
...
proportional_hazard_test(cph, X, time_transform='rank').print_summary()  # p per covariate
  \end{lstlisting}
  \begin{takeaway}
...
\begin{frame}{Module A6 in one slide}
...
\begin{frame}{Five things to remember}
  \begin{takeaway}[Module A6 in five lines]
    \begin{enumerate}
...
\begin{frame}{What's next}
  ...
  \begin{readme}[Where this chapter connects]
    \begin{itemize}
      ...
      \item \textbf{Unsupervised learning (module A8):} no response at all --- a sibling
            self-study module in \texttt{Chapters/Advanced/}, worth reading next.
      \item \textbf{Module A7:} multiple testing --- and a survival study with many
            subgroups is precisely a multiplicity problem.
    \end{itemize}
  \end{readme}
  \begin{takeaway}
    This module and the unsupervised-learning one are the two ``unusual response''
    topics: here the response is only partly observed, there it is absent. Both are
    handled by changing the likelihood, not the loss you happen to know.
  \end{takeaway}
\end{frame}
