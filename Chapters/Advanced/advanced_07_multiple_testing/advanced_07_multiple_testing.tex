...
\begin{frame}{Where this chapter is used in industry}
  ...
  \end{tabular}
  \end{center}
  \begin{takeaway}[{{The bar is set by the tests you ran, not the ones you report}}]
    In all three the number of hypotheses is fixed \emph{before} anyone looks at a
    $p$-value, and the threshold moves with it. Genomics has even frozen the arithmetic
    into a standing constant: the genome-wide threshold $5\times10^{-8}$ is Bonferroni at
    roughly a million effectively independent tests.
  \end{takeaway}
\end{frame}
