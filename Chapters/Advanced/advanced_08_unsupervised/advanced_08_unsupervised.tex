...
% Advanced module A8: Unsupervised Learning (was ISLP Ch. 12)
% ============================================================
...
\subtitle{Unsupervised Learning\\[2pt]{\small Advanced module A8 --- optional self-study}}
...
\begin{frame}{The advanced modules at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.72\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{A8} & \textbf{Unsupervised Learning}\,{\color{accent!75!black}--- PCA, $K$-means, hierarchical clustering} \\
  ...
  house style with a companion lab notebook.\par\medskip This module assumes Chapter 2 (what a test error buys you) and Chapter 5 (resampling and stability). It was ISLP Chapter 12 in earlier editions of this course.}
\end{frame}
...
\begin{frame}{So why do it at all? Two families of method}
  ...
  \begin{columns}[t]
    \begin{column}{0.49\textwidth}
      \begin{block}{Dimension reduction --- PCA}
        ...
        and as inputs to a later supervised model (principal-components regression).
      \end{block}
    \end{column}
    \begin{column}{0.49\textwidth}
  ...
  \end{columns}
  \begin{labnote}
    The companion notebook \texttt{advanced\_08\_unsupervised\_lab.ipynb} runs every method in this
...
\begin{frame}{The problem PCA solves}
  ...
  \end{numexample}
  \par{\tiny\itshape\color{gray}History: PCA is one of the oldest tools
  in this course --- Karl Pearson (1901), reinvented and named by Harold
  Hotelling (1933).}
\end{frame}
...
\begin{frame}{Seeing it in two dimensions}
  ...
  \begin{takeaway}
    ...
    only $0.45$. The grey stubs are the residuals PC1 \emph{discards} --- the shortest
    possible ones, which is the same thing as the widest possible spread.
  \end{takeaway}
\end{frame}
...
\begin{frame}{When to standardise, and when not to}
  ...
  \end{mistake}
  \begin{labnote}
    \S1 of the companion notebook runs both versions side by side and prints both PVE
...
\begin{labnote}[{{Run this live}}]
  The worked solution sits in \texttt{advanced\_08\_unsupervised\_lab.ipynb} under
...
from sklearn.decomposition import PCA                  # the PCA estimator
from sklearn.preprocessing import StandardScaler       # mean 0, sd 1 per column

USA = load('USArrests', index_col=0)                   # 50 states x 4 features
print(USA.var().round(1).to_dict())                    # Assault 6945, Murder 19

for tag, X in [('scaled',   StandardScaler().fit_transform(USA)),
               ('unscaled', (USA - USA.mean()).values)]:  # centred, NOT scaled
    p = PCA().fit(X)                                   # same estimator both ways
...
\begin{readme}
  One loop, one estimator, two preprocessings --- so any difference in the printout is
  the scaling and nothing else.
\end{readme}
\end{frame}
...
\begin{frame}{PCA when $p \gg n$: 1000 genes, 40 samples}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.94\textwidth,height=0.42\textheight,keepaspectratio]{ch12_gene_pca.png}
  \end{center}
  \begin{takeaway}
  ...
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Switch to \texttt{advanced\_08\_unsupervised\_lab.ipynb} \S4: $\mathrm{PVE}_1=0.127$, PC1 group means $\pm 11.75$.
  \end{labnote}
\end{frame}
...
\begin{frame}{Clustering: what we are asking for}
  ...
  \begin{takeaway}[{{Two designs, two commitments}}]
    ...
    And neither chooses the features or the distance for you: grouping insurance claims by
    cost alone puts a cheap fraud next to a cheap accident. Choosing what ``similar'' means
    \emph{is} the analysis; the algorithm is the easy part.
  \end{takeaway}
\end{frame}
...
\begin{frame}{The $K$-means objective}
  ...
  \begin{readme}
    \begin{itemize}
      \item $W(C_k)$ sums the squared Euclidean distances over all pairs inside
            cluster $k$ and divides by $|C_k|$ --- small means tight.
...
\begin{frame}{USArrests at $K=2$, $3$ and $4$}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.98\textwidth,height=0.38\textheight,keepaspectratio]{ch12_kmeans_K.png}
  \end{center}
  \begin{takeaway}
  ...
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Run the $K$-loop cell: $W = 200.0,\ 105.0,\ 79.9,\ 57.6$.
  \end{labnote}
  \par{\tiny\itshape\color{gray}History: the algorithm is Stuart Lloyd's
  (Bell Labs, 1957), unpublished until 1982; the name ``$K$-means'' is
  MacQueen's (1967).}
\end{frame}
...
\begin{frame}{Are the clusters interpretable? The $K=3$ profiles}
  ...
  \begin{readme}[{{The only honest check available on this slide}}]
    ...
    exactly what $K$-means would produce from a single unimodal cloud too. Note also that a
    table like this one, and not the vector of labels, is what anyone downstream actually
    uses: nothing can be written against a cluster number.
  \end{readme}
\end{frame}
...
\begin{frame}{Solution --- Exercise 12.4 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 12.4 (2/3)}
\begin{solutionbox}
  \[
    W=4\times 2^2=16.0 .
  \]
...
\begin{frame}{Solution --- Exercise 12.4 (3/3)}
  \footnotesize
\begin{solutionbox}
...
from sklearn.preprocessing import StandardScaler  # W depends on the scaling

Xs = StandardScaler().fit_transform(load('USArrests', index_col=0))

for K in (4, 8):                                 # rougher surface at K=8
    runs = [KMeans(n_clusters=K, n_init=1, random_state=s).fit(Xs)
            for s in range(20)]                  # 20 seeds, one start each
    ...
    print(f'K={K}: {len(np.unique(w.round(3)))} optima  min={w.min():.3f}  '
          ...
          f'n_init=50 -> {best:.3f}')            # the value 50 starts reach
    print('  best / worst sizes', sorted(np.bincount(runs[w.argmin()].labels_)),
          sorted(np.bincount(runs[w.argmax()].labels_)))   # a bad W is a bad split
\end{lstlisting}
\end{solutionbox}
\end{frame}
...
\begin{frame}{Cutting the tree chooses $K$ --- after you have seen it}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.98\textwidth,height=0.42\textheight,keepaspectratio]{ch12_dendro_cut.png}
  \end{center}
  \begin{takeaway}
    ...
    selection problem module~A7 formalises --- and here there is no $p$-value to correct.
  \end{takeaway}
  \begin{labnote}[{{Live demo --- \S3 of the notebook}}]
    Switch to \texttt{advanced\_08\_unsupervised\_lab.ipynb} \S3 and cut the tree: sizes $(31,19)$ and $(31,11,8)$.
  \end{labnote}
\end{frame}
...
\begin{frame}{Solution --- Exercise 12.5 (1/2)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 12.5 (2/2)}
\begin{solutionbox}
  \medskip
  \textbf{Part 2 --- cuts.}
  \[
...
\begin{frame}{How much does the linkage choice matter? Quantify it}
  ...
  \begin{takeaway}
    ...
    shape}: choose it before you see the answer, and say which you chose --- the distance,
    the linkage and the cut height, all three, so that a reader can reproduce the partition.
  \end{takeaway}
\end{frame}
...
\begin{frame}{The decisions you must make, and must report}
  ...
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    Switch to \texttt{advanced\_08\_unsupervised\_lab.ipynb} \S5: noise silhouette $0.323$, bootstrap ARI $0.68$.
  \end{labnote}
\end{frame}
...
\begin{frame}{The bridge to module A7 (multiple testing)}
  \begin{takeaway}[{{Turning structure into something testable}}]
    ...
    --- $1000$ two-sample $t$-tests, one per gene. Then you are squarely in module~A7:
...
\begin{solutionbox}[{{Solution --- code}}]
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
from sklearn.cluster import KMeans           # same tool on both data sets
from sklearn.metrics import silhouette_score # separation, per point

rng   = np.random.default_rng(2024)         # same seed as the slide figure
noise = rng.normal(size=(200, 2))           # ONE Gaussian: no clusters at all
mu    = np.array([[-2.5, 0.], [2.5, 0.], [0., 2.5]])   # 3 true centres
...
for name, X in [('noise', noise), ('real ', real)]:   # identical recipe
    W    = {K: km(X, K).inertia_ for K in range(1, 9)}   # W falls either way
    sil  = {K: silhouette_score(X, km(X, K).labels_) for K in range(2, 9)}
    peak = max(sil, key=sil.get)            # the K a scree/sil rule would pick
    print(f'{name} W(3)/W(1)={W[3]/W[1]:.3f} sil(3)={sil[3]:.3f} '
          f'peak K={peak} ({sil[peak]:.3f})')   # noise answers too
\end{lstlisting}
\end{solutionbox}
\end{frame}
...
\begin{frame}{Do the two methods find the same structure?}
  \begin{center}
    \includegraphics[width=0.42\textwidth,height=0.50\textheight,keepaspectratio]{ch12_method_agreement.png}
  \end{center}
  \begin{takeaway}
    $K$-means ($K=4$) against complete linkage (cut at 4) on scaled
    USArrests: most states land in matching pairs and the adjusted Rand
    index is $0.68$ --- the two algorithms broadly agree on the structure
    without agreeing on every state. Crosstabbing two clusterings is the
    same validation move the lab makes on NCI60.
  \end{takeaway}
\end{frame}


\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[{{Companion notebook: \texttt{advanced\_08\_unsupervised\_lab.ipynb}}}]
    ...
    \begin{itemize}
      ...
      \item \S2, \emph{K-means} --- $K=1,\dots,4$, cluster profiles in original units, and
            the local-optimum experiment;
      \item \S3, \emph{Hierarchical clustering} --- five linkages, cutting the tree, and
            how much the partition moves;
      \item \S4, \emph{High-dimensional data: gene expression} --- PCA and clustering on the
            bundled $1000\times40$ matrix (with the optional \texttt{NCI60} branch);
      \item \S5, \emph{Practical issues: outliers, and clusters in pure noise} --- plus the
...
USA = load('USArrests', index_col=0)                 # 50 states x 4 features
Xs  = StandardScaler().fit_transform(USA)            # standardise: mean 0, sd 1 per column
pca = PCA().fit(Xs)                                  # all min(n-1, p) directions

print(np.round(pca.explained_variance_ratio_, 4))    # [0.6201 0.2474 0.0891 0.0434]
print(np.round(np.cumsum(pca.explained_variance_ratio_), 4))   # 2 PCs = 86.8%
loadings = pd.DataFrame(pca.components_.T, index=USA.columns,
                        columns=[f'PC{i+1}' for i in range(4)])   # rows = variables
print(loadings.round(3))                             # signs may be flipped: fine
Z = pca.transform(Xs)                                # scores: states in PC space
  \end{lstlisting}
  \vspace{-10pt}\footnotesize
  \begin{readme}
    \texttt{components\_} holds the loadings, one \emph{row} per PC (hence \texttt{.T});
    \texttt{explained\_variance\_ratio\_} is the PVE. Without the scaler PC1 would be
    \texttt{Assault} alone at $96.6\%$.
  \end{readme}
\end{frame}
...
print(round(km.inertia_, 3))                          # W = 79.922, the objective
print(USA.assign(cluster=km.labels_)                  # profiles in RAW units
         .groupby('cluster').mean().round(1))         # cluster means per variable

w = [KMeans(n_clusters=4, n_init=1, random_state=s).fit(Xs).inertia_
     for s in range(20)]                              # twenty single-start runs
print(np.round(sorted(w), 3))                         # 57.554 best, 77.856 worst
print('distinct local optima:', len(set(np.round(w, 3))))  # 9 different answers
  \end{lstlisting}
  \footnotesize
  \begin{readme}[{{What \texttt{n\_init} is buying}}]
    Lloyd's algorithm only ever reaches a \emph{local} minimum of $W$, and which one is
    fixed by the random starting centroids. With \texttt{n\_init=1} the twenty seeds land
    on \textbf{nine} different values, $57.554$ up to $77.856$ --- $35\%$ apart on one data
    set. \texttt{n\_init=50} keeps the smallest $W$ of $50$ starts, and
    \texttt{random\_state} makes that choice reproducible; report both.
  \end{readme}
\end{frame}
...
from scipy.spatial.distance import squareform         # square D -> condensed

for method in ['complete', 'average', 'single', 'centroid']:
    Zl = linkage(Xs, method=method)                # n-1 merges: i, j, height, size
    lab = fcluster(Zl, 3, criterion='maxclust')    # slice the tree, 3 branches left
    print(f'{method:9s} sizes {np.bincount(lab)[1:]}  top fusion {Zl[-1,2]:.2f}')

Dcorr = 1 - np.corrcoef(Xs)                           # 1-r: shape, not level
...
dendrogram(Zl, labels=USA.index.tolist(), leaf_font_size=6) # HEIGHT is the message
  \end{lstlisting}
  \footnotesize
  \begin{readme}[{{What \texttt{linkage} returns, and what the cut does}}]
    \texttt{Zl} is the $(n-1)\times4$ \emph{linkage matrix}: one row per fusion --- the two
    things joined, the \textbf{height} at which they joined, the size of the new group. On
    raw data it assumes Euclidean distance, so any other dissimilarity must arrive
    condensed (\texttt{Zc}). \texttt{fcluster} re-fits nothing; it slices that same tree
    low enough to leave $3$ branches: $(31,11,8)$, $(30,19,1)$, $(48,1,1)$, $(30,19,1)$.
  \end{readme}
\end{frame}
...
\begin{frame}{Module A8 in one slide}
...
\begin{frame}{Five things to remember}
  \begin{takeaway}[{{Module A8 in five lines}}]
    \begin{enumerate}
...
\begin{frame}{What's next}
  \begin{itemize}
    \item \textbf{Module A7 --- Multiple Testing}: the gene example ended with
          $1000$ $t$-tests and $\sim 50$ false discoveries expected by chance ---
          A7 is the machinery (FWER, Holm, BH) that turns suggested structure
          into defensible claims.
    \item \textbf{Back to Chapter 6}: principal component regression is this
          module's PCA put to supervised work --- the scores as predictors.
  \end{itemize}
\end{frame}
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{The same tree, drawn two ways} --- a dendrogram has $2^{n-1}$ equally valid drawings; the rule is stated in the main deck, and the two-renderings figure lives here.
      ...
      \item \textbf{PCA with missing values} --- the matrix-completion algorithm, needed in
            practice and beyond what the main deck needs.
      ...
      \item \textbf{Extended Exercise 12.3} --- a full clustering workflow on the bundled
            gene-expression data. It needs $20$ minutes at the keyboard, so leave it until
            you have the notebook open.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{PCA with missing values: matrix completion}
  ...
  \begin{readme}
    ...
    their values. Under another name this is the classical recommender engine: a
    user-by-item rating matrix is mostly missing, and low-rank completion of it puts $M$
    latent factors in place of taste.
  \end{readme}
\end{frame}
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
import numpy as np
from scipy.cluster.hierarchy import linkage, fcluster   # build, then cut
from scipy.spatial.distance import squareform   # linkage wants D condensed
from sklearn.metrics import adjusted_rand_score # 1 = exact, 0 = chance
...
for nm, arg in [('eucl', X), ('1-r', squareform(1-np.corrcoef(X), checks=False))]:
    for m in ('complete', 'average', 'single'):   # 3 linkages x 2 distances
        lab = fcluster(linkage(arg, method=m), 2, criterion='maxclust')  # cut in 2
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
from scipy import stats                     # one t-test per gene
from sklearn.decomposition import PCA       # centred, not standardised

p = PCA().fit(X - X.mean(0)); Z = p.transform(X - X.mean(0))   # PCA on the samples
print('PVE1', round(p.explained_variance_ratio_[0], 3),   # 0.127: small, not useless
...
\vspace{-8pt}
\begin{readme}
  PCA runs on \emph{centred}, not standardised, data --- the genes already share one
  scale. \texttt{G} is genes\,$\times$\,samples, so \texttt{axis=1} tests each gene across
  the $40$ columns.
\end{readme}
\end{frame}
...
\begin{solutionbox}[{{Solution --- interpretation}}]
  \begin{itemize}
    ...
    \item \textbf{Part 4.} $159$ genes have $p<0.05$, but $\sim 50$ are expected under the
          ...
          the $100$ smallest $p$-values lie in genes $501$--$1000$ --- module~A7, and the
