...
\begin{frame}{The advanced modules at a glance}
  ...
  \end{center}
  \vspace{2mm}
  {\scriptsize Eight optional, self-study modules that sit outside the taught
...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.6cm}p{5.2cm}p{4.6cm}@{}}
    ...
    \midrule
    Fixed income & PCA of daily yield-curve moves into level, slope and curvature &
      How many futures to sell to bring the level exposure back inside the limit \\
    Process manufacturing & PCA of hundreds of correlated sensor channels into two or three
      monitored indices & Stop the line and inspect the batch, or let it run \\
    Genomics, pharma & Hierarchical clustering of tumour expression profiles into candidate
      subtypes & Which subtype hypothesis is written into the next trial protocol \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{takeaway}[{{What the three rows share: the check happens outside the data}}]
    None of the three can be validated by a held-out number, because there is no held-out
    number. Each is judged by what happens next --- the hedge holds through the next
    sell-off, the control chart catches a genuine excursion, the subtype replicates in the
    next cohort.
  \end{takeaway}
\end{frame}
...
\begin{frame}{USArrests at $K=2$, $3$ and $4$}
  \footnotesize
  \begin{center}
    \includegraphics[width=0.98\textwidth,height=0.42\textheight,keepaspectratio]{ch12_kmeans_K.png}
  \end{center}
  \begin{takeaway}
