...
\subtitle{Advanced Module A9: Deep Learning}
...
\begin{frame}{The advanced modules}
  \vspace{-1.5mm}
  \begin{center}
  \scriptsize
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.72\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{A9} & \textbf{Deep Learning}\,{\color{accent!75!black}--- MLPs, convolutions, training, regularisation} \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{2mm}
  ...
  house style with a companion lab notebook.\par\medskip This module assumes Chapter 3 (least squares and the linear predictor) and Chapter 5 (validation and overfitting). It was ISLP Chapter 10 and a taught session in earlier editions of this course.}
\end{frame}
...
\begin{frame}{Learning objectives}
  ...
  \begin{itemize}
    ...
    \item \textbf{Recognise} where convolutional networks belong ---
          the appendix develops them as optional self-study.
...
\begin{frame}{Roadmap of this chapter}
  \begin{takeaway}[A tour of modern deep learning]
    ...
    \end{enumerate}
    Convolutional networks for images sit in the \textbf{appendix}: the same
    ideas, developed as optional self-study rather than taught here.
  \end{takeaway}
\end{frame}
...
\begin{frame}{What is deep learning?}
  ...
  \begin{takeaway}
    ...
    consistently dominant on small to medium tabular data --- on the wide
    customer, policy and transaction tables of a bank or an insurer,
    gradient boosting (Chapter~8) usually matches or beats a neural network
    at a fraction of the cost, and is far easier to document for model
    validation. Deep learning earns its keep on the \emph{unstructured}
    assets those firms also hold: scanned documents, call recordings, images.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Single hidden layer}
  ...
  \begin{takeaway}
    ...
    combination. Logistic regression is the \emph{no hidden layer} case: a sigmoid
    applied straight to $\beta_0+\sum_j\beta_j X_j$.
  \end{takeaway}
\end{frame}

\begin{frame}{Single-layer network as a graph}
  \begin{center}
    \includegraphics[width=0.62\textwidth,height=0.46\textheight,keepaspectratio]{10_1.pdf}
  \end{center}
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Switch to \texttt{advanced\_09\_deep\_learning\_lab.ipynb} \S2 and run the model cell: the \texttt{MLP}
    class builds exactly this kind of graph in code ($3\to32\to32\to2$, ReLU between the
    layers), ending in two raw logits because \texttt{nn.CrossEntropyLoss} applies the
    softmax itself.
  \end{labnote}
...
\begin{frame}{Feed-forward network as a labelled graph}
  \begin{center}
  \begin{tikzpicture}[
      x=1.7cm,y=0.8cm,>={Stealth[length=2mm]},
      inp/.style={circle,draw=accent,fill=accent!12,minimum size=7mm,inner sep=0pt,font=\small},
      hid/.style={circle,draw=orange!70!black,fill=orange!15,minimum size=7mm,inner sep=0pt,font=\small},
      onode/.style={circle,draw=green!55!black,fill=green!15,minimum size=7mm,inner sep=0pt,font=\small},
    ...
    \node[font=\footnotesize\bfseries,text=accent] at (0,-1.9) {Input layer ($p=4$)};
    \node[font=\footnotesize\bfseries,text=orange!70!black] at (2,-1.9) {Hidden layer ($K=5$)};
    \node[font=\footnotesize\bfseries,text=green!45!black] at (4,-1.9) {Output};
  \end{tikzpicture}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Solution --- Exercise 10.1 (1/5)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 10.1 (2/5)}
\begin{solutionbox}
  \medskip
...
\end{solutionbox}
\begin{mistake}
  Forgetting the bias $w_{10} = -4$ and reporting $z_1 = 5$ --- always add the
  bias before applying $g$.
\end{mistake}
\end{frame}

\begin{frame}{Solution --- Exercise 10.1 (3/5)}
\begin{solutionbox}
  \textbf{Step 3 --- output.}
  \[
...
\begin{frame}{Solution --- Exercise 10.1 (4/5)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 10.1 (5/5)}
\begin{solutionbox}
  \medskip
...
\end{solutionbox}
\begin{mistake}
  Squashing the hidden units with the sigmoid too --- here $g$ is ReLU; only the
  output logit is passed through $\sigma$.
\end{mistake}
\end{frame}
...
\begin{frame}{Solution --- Exercise 10.2 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 10.2 (2/4)}
\begin{solutionbox}
  \medskip
...
\begin{frame}{Solution --- Exercise 10.2 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 10.2 (4/4)}
\begin{solutionbox}
  \medskip
...
\end{solutionbox}
\begin{mistake}
  Evaluating the derivative as $\sigma'(z) = z(1 - z)$ --- the formula plugs in
  the \emph{output} $\sigma(z)$, not the pre-activation $z$ itself.
\end{mistake}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 10.1 (1/6)}
\begin{solutionbox}[Solution --- Forward pass]
...
\begin{frame}{Solution --- Extended Exercise 10.1 (2/6)}
\begin{solutionbox}[Solution --- Forward pass]
  \medskip
...
\begin{frame}{Solution --- Extended Exercise 10.1 (3/6)}
\begin{solutionbox}[Solution --- Backpropagation: the chain rule]
...
\begin{frame}{Solution --- Extended Exercise 10.1 (4/6)}
\begin{solutionbox}[Solution --- Backpropagation: the chain rule]
  \[
...
\end{solutionbox}
\begin{mistake}
  Evaluating $\sigma'(s)$ by plugging $s=1$ into $u(1-u)$ --- the $u$ in
  $\sigma'(s)=u(1-u)$ is $u=\sigma(s)=0.731$.
\end{mistake}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 10.1 (5/6)}
\begin{solutionbox}[Solution --- Gradient step and interpretation]
...
\begin{frame}{Solution --- Extended Exercise 10.1 (6/6)}
\begin{solutionbox}[Solution --- Gradient step and interpretation]
...
\begin{frame}{Multilayer perceptron (MLP)}
  ...
  \end{block}
  \begin{readme}
    $h$ is the output activation: identity for regression, softmax for multiclass
    classification. A first layer can also \emph{embed} a high-cardinality identifier
    (SKU, store) as a short learned vector rather than a wide one-hot block.
  \end{readme}
\end{frame}

\begin{frame}{MLP as a graph: stacking hidden layers}
  \begin{center}
    \includegraphics[width=0.85\textwidth,height=0.48\textheight,keepaspectratio]{10_4.pdf}
  \end{center}
  \begin{takeaway}
  ...
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: the name honours Frank Rosenblatt's
  \emph{perceptron} (1958); backpropagation was popularised by Rumelhart,
...
\begin{labnote}[{{Live demo --- \S4 of the notebook}}]
  After the hand count, run \texttt{advanced\_09\_deep\_learning\_lab.ipynb} \S4: it prints
  $352+528+51=931$ and PyTorch's own parameter count confirms it in one line.
\end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 10.3 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 10.3 (2/3)}
\begin{solutionbox}
  \textbf{Step 3 --- total.}
  \[
...
\begin{frame}{Solution --- Exercise 10.3 (3/3)}
\begin{solutionbox}
  \medskip
...
\end{solutionbox}
\begin{mistake}
  Attaching parameters to the $10$ input units, or dropping the
  $+n_{\text{out}}$ bias term --- both corrupt the total.
\end{mistake}
\end{frame}
...
\begin{labnote}[{{Live demo --- \S3 of the notebook}}]
  Every iteration of the training loop in \texttt{advanced\_09\_deep\_learning\_lab.ipynb} \S3 is this
  update rule: run the cell and \texttt{opt.step()} applies it to all $1\,250$ weights
  at once (Adam-scaled rather than a fixed $\eta$), ending in test AUC $0.943$.
\end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 10.5 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 10.5 (2/4)}
\begin{solutionbox}
  \medskip
...
\begin{frame}{Solution --- Exercise 10.5 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 10.5 (4/4)}
\begin{solutionbox}
  \medskip
...
\end{solutionbox}
\begin{mistake}
  Concluding ``smaller is always safer, so smaller is better'' --- at
  $\eta = 0.01$ the ratio is $0.98$ and hundreds of steps are needed here.
\end{mistake}
\end{frame}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{advanced\_09\_deep\_learning\_lab.ipynb}]
    ...
    \begin{itemize}
      \item \S1, \emph{Data} --- loading \texttt{Default}, the train/test split and standardisation;
      \item \S2, \emph{PyTorch model} --- the \texttt{MLP} class, seeded with \texttt{torch.manual\_seed(1)} so the AUC below is reproducible;
      \item \S3, \emph{Training loop} --- mini-batches via \texttt{DataLoader}, then the test AUC;
      \item \S4, \emph{Counting the parameters} --- Exercise 10.3 re-done in code and confirmed by PyTorch; the convolution count (Exercise 10.4) follows in the same cell and is optional, appendix material;
      \item \S5, \emph{Does the network actually beat logistic regression?} --- the honest tabular comparison.
    \end{itemize}
    Requires \texttt{torch}; \S6, \emph{Exercises} ends the notebook with hands-on tasks.
  \end{labnote}
\end{frame}
...
\begin{labnote}[{{Run this live}}]
  The worked solution sits in \texttt{advanced\_09\_deep\_learning\_lab.ipynb} under
  \emph{Lecture exercises --- worked Python solutions}: the cell headed
  \emph{Exercise 10.6 --- Softmax and cross-entropy [Python]}.
\end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 10.6 (1/5)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 10.6 (2/5)}
\begin{solutionbox}
...
\begin{frame}[fragile]{Solution --- Exercise 10.6 (3/5)}
\begin{solutionbox}
\begin{lstlisting}
...
\end{lstlisting}
\end{solutionbox}
\begin{readme}{\scriptsize
  Line 3 holds \emph{logits} --- unbounded scores, not probabilities; line 4 turns them
  into a probability vector, line 8 keeps only \texttt{p[0]}. PyTorch fuses both into
  \texttt{nn.CrossEntropyLoss}.}
\end{readme}
\end{frame}

\begin{frame}{Solution --- Exercise 10.6 (4/5)}
...
\begin{frame}{Solution --- Exercise 10.6 (5/5)}
\begin{solutionbox}[Solution --- Interpretation]
  \medskip
...
\end{solutionbox}
\begin{mistake}
  Applying softmax yourself and then calling PyTorch's
...
\begin{labnote}[Run this live]
  The worked solution sits in \texttt{advanced\_09\_deep\_learning\_lab.ipynb} under
  \emph{Lecture exercises --- worked Python solutions}: the cell headed
  \emph{Extended Exercise 10.3 --- Overfitting and regularisation [Python]}.
\end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution --- Extended Exercise 10.3 (1/4)}
\begin{solutionbox}[Solution --- code: load / standardise / convert to tensors]
...
\begin{frame}[fragile]{Solution --- Extended Exercise 10.3 (2/4)}
\begin{solutionbox}[Solution --- code: train and record the loss after every epoch]
...
\begin{frame}{Solution --- Extended Exercise 10.3 (3/4)}
\begin{solutionbox}[Solution --- typical result and interpretation]
  \textbf{Typical values} (exact numbers vary with seed and version):
  \begin{center}\ttfamily\footnotesize
  unreg:\ train $\to$ 0.00\quad val-min $\approx$ 0.10 (epoch 16)\quad val-final $\approx$ 0.93\\[2pt]
  decay:\ train $\approx$ 0.19\quad val \emph{lower and flat} $\approx$ 0.18
  \end{center}
  \begin{itemize}
    \item \emph{Unregularised:} training loss dives toward $0$ (the net \textbf{memorises}
          ...
          epochs and then \textbf{climbs} almost tenfold --- textbook overfitting.
...
\begin{frame}{Solution --- Extended Exercise 10.3 (4/4)}
\begin{solutionbox}[Solution --- typical result and interpretation]
  \textit{Same recipe as the companion notebook} (\texttt{advanced\_09\_deep\_learning\_lab.ipynb});
  there the MLP adds dropout and mini-batches via \texttt{DataLoader}.

  \smallskip
  \textit{Common mistake:} judging the two nets by training loss --- the
  unregularised net ``wins'' at $0.00$ precisely because it memorises.
  Only the validation curve ranks them honestly.
\end{solutionbox}
\end{frame}
...
\begin{frame}{Module A9 in one slide}
  ...
  \begin{itemize}
    ...
    \item Convolutional networks for images --- in the \textbf{appendix},
          as optional self-study.
  \end{itemize}
\end{frame}
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{description}
    ...
    \item[Dense params] $\#\text{params}=n_{\text{in}}\,n_{\text{out}}+n_{\text{out}}$ \ (weight matrix $+$ one bias per unit).
    \item[Conv output (appendix)] size $=\big\lfloor (W-F+2P)/S\big\rfloor+1$ \ (input $W$, filter $F$, pad $P$, stride $S$).
    \item[Conv params (appendix)] $\#\text{params}=(F^2 d_{\text{in}}+1)\times(\text{\#filters})$; \ pooling has none.
  \end{description}
\end{frame}
...
\begin{frame}{Decision rules of thumb}
  ...
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    \S5 of \texttt{advanced\_09\_deep\_learning\_lab.ipynb} runs the first rule on \texttt{Default}:
    logistic regression AUC $0.946$ vs the MLP's $0.943$ --- the simpler model wins
    on tabular data.
  \end{labnote}
\end{frame}
...
\begin{frame}{Self-check questions}
  ...
  \begin{readme}[{{Quick self-assessment}}]
    \begin{enumerate}
      \item A dense layer maps $50$ inputs to $10$ units. How many parameters?
      \item Give the one-line description of training a neural network.
      \item The training loss explodes within a few epochs. What is the first
            thing to change?
      \item Early stopping watches one curve. Which one --- training or
            validation loss?
      \item Why is re-using the validation set over many tuning rounds
            dangerous?
      \item ReLU and sigmoid: give $g(-2)$ for each.
    \end{enumerate}
  \end{readme}
  \begin{takeaway}[{{Answers}}]
    (1) $50\times 10+10=510$. (2) Gradient descent on a loss:
    $\theta\leftarrow\theta-\eta\,\nabla_\theta L(\theta)$. (3) The learning
    rate $\eta$ --- too high, and the steps overshoot the minimum. (4) The
    validation loss; the training loss keeps falling while the model overfits.
    (5) Each round quietly runs a comparison and keeps the winner, so the
    validation score becomes optimistic --- module A7's problem in disguise.
    (6) $\max(0,-2)=0$ and $\sigma(-2)=1/(1+e^{2})\approx 0.12$.
  \end{takeaway}
\end{frame}

\begin{frame}{Five things to remember}
  \begin{takeaway}[Module A9 in five lines]
    \begin{enumerate}
      \item \textbf{A network is stacked adaptive basis functions} --- linear, activation,
            linear; drop the nonlinearity and it collapses to one linear model.
      \item \textbf{Training is gradient descent on a loss:}
            $\theta \leftarrow \theta - \eta\,\nabla_\theta L(\theta)$ --- and the learning
            rate is the hyperparameter that fails loudest.
      \item \textbf{Parameters count as $n_{\text{in}}\,n_{\text{out}} + n_{\text{out}}$
            per dense layer} --- width, not depth, drives model size.
      \item \textbf{Regularise by early stopping, dropout and weight decay} --- and watch
            the validation curve, never the training curve.
      \item \textbf{On tabular data a tuned boosted tree usually wins} --- reach for a
            network when the structure (images, text, audio) demands it.
    \end{enumerate}
  \end{takeaway}
\end{frame}

\begin{frame}{What's next}
  This module and \textbf{A7 --- Multiple Testing} (ISLP Chapter 13, also in
  \texttt{Chapters/Advanced/}) are the two exam-relevant self-study modules ---
  read A7 next if you have not already:
  \begin{itemize}
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{Gradient descent on a loss surface: the real path} --- the seeded 2-D descent figure; the learning-rate trade-off it illustrates is proved in Exercise 10.5.
      \item \textbf{Convolutional networks (CNNs)} --- convolution, pooling, a full architecture, and Exercise 10.4 on layer arithmetic. The ideas carry over from the MLP, so the main deck spends its time on training instead; read this block when you work with images.
      \item \textbf{CNN architecture arithmetic (Extended Exercise 10.2)} --- shapes and parameter counts through a whole network; Exercise 10.4 above does the same for a single layer.
...
\begin{frame}{Gradient descent on a loss surface: the real path}
  \begin{center}\includegraphics[width=0.88\textwidth,height=0.52\textheight,keepaspectratio]{ch10_x_gradient_descent.png}\end{center}
...
% ------------------------------------------------------------
% CONVOLUTIONAL NETWORKS --- moved out of the taught flow, optional self-study
...
\begin{frame}{The convolution operation}
  \begin{center}
  \begin{tikzpicture}[x=0.60cm,y=0.56cm,>={Stealth[length=2mm]},font=\small]
    ...
    \node[font=\scriptsize,text=gray] at (5.6,-6.4) {stride $=1$, valid $\Rightarrow 5-3+1=3$};
  \end{tikzpicture}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Max-pooling: downsampling by local maxima}
  ...
  \end{center}
  \begin{takeaway}
    Each outlined $2\times2$ window is replaced by its maximum, shrinking $H,W$ by the stride
...
\begin{frame}{Solution --- Exercise 10.4 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 10.4 (2/4)}
\begin{solutionbox}
  \medskip
...
\begin{frame}{Solution --- Exercise 10.4 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 10.4 (4/4)}
\begin{solutionbox}
  \medskip
...
\begin{frame}{Why convolution wins: the parameter count, drawn}
  \begin{center}
    \includegraphics[width=0.76\textwidth,height=0.54\textheight,keepaspectratio]{ch10_conv_vs_dense.png}
  \end{center}
  \begin{takeaway}
    Exercise 10.4 as a curve. The dense layer's cost grows with the
    \emph{fourth power} of the image side; the convolution stays at $1\,216$
    parameters at every size, because the same $5\times5$ filters slide
    everywhere. Weight sharing, not depth, is what makes images tractable.
  \end{takeaway}
\end{frame}
...
\begin{longexercise}
  ...
  \begin{enumerate}
    ...
    \item What input region does one unit of the final map, after Pool2, see
          (receptive field)?
  \end{enumerate}
\end{longexercise}
\end{frame}
...
\begin{solutionbox}[Solution --- spatial dimensions]
...
\end{solutionbox}
\begin{mistake}
  Flattening only $16\times16=256$ features --- the depth counts too:
  $16\times16\times64=16\,384$.
\end{mistake}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 10.2 (2/3)}
\begin{solutionbox}[Solution --- learnable parameters]
...
\begin{solutionbox}[Solution --- receptive field and interpretation]
