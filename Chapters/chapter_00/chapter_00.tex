...
\begin{frame}{The shape of a data set: $n$ observations, $p$ variables}
  ...
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    Run \texttt{chapter\_00\_lab.ipynb} \S1: \texttt{load('Wage')} is exactly this
...
\begin{frame}{Variable types decide everything that follows}
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    \bottomrule
  \end{tabular}
  \begin{industry}[Data engineering: ERP and CRM exports lie about types]
    Plant codes, cost centres and customer segments arrive as \emph{numbers}.
    Feed them in unchanged and the model reads ``plant 7 is seven times plant
    1''. Recode nominal fields as dummies at load time.
  \end{industry}
\end{frame}
...
\begin{frame}{Worked example: why a 95\% accurate test can mislead}
  ...
  \end{takeaway}
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Run \texttt{chapter\_00\_lab.ipynb} \S4: the \texttt{posterior} cell returns
    $0.154$ and prints the 1000-people tree; the next cell sweeps the prevalence.
  \end{labnote}
\end{frame}
...
\begin{frame}{The normal distribution}
  ...
  \begin{readme}[What is and is not assumed]
    ...
    work instead.
  \end{readme}
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    Run \texttt{chapter\_00\_lab.ipynb} \S5: the gallery cell draws the standard
    distributions, and the CLT cell shows means of skewed wages turning normal.
  \end{labnote}
\end{frame}
...
\begin{frame}{Worked example: a one-sample $t$-test}
  ...
  \begin{labnote}[{{Live demo --- \S7 of the notebook}}]
    ...
    \texttt{scipy} reproduces $t = 2.00$ and $p = 0.048$, and the next cell shades
    the $p$-value as an area under the null distribution.
  \end{labnote}
\end{frame}
...
\begin{frame}[fragile]{Exercise 0.7 --- Interrogating a regression table [Python]}
  \begin{exercise}
    \scriptsize
    A colleague regresses a city's monthly ice-cream sales (\euro 000s) on
    average temperature ($^\circ$C) and reports:
...
\end{lstlisting}
  ...
  \begin{labnote}[{{Run this live}}]
    Worked solution: \texttt{chapter\_00\_lab.ipynb}, \emph{Lecture exercises ---
    worked Python solutions}, cell \emph{Exercise 0.7 --- Interrogating a
    regression table [Python]}.
  \end{labnote}
\end{frame}
...
\end{lstlisting}
  ...
  \begin{labnote}[{{Run this live}}]
    Worked solution: \texttt{chapter\_00\_lab.ipynb}, \emph{Lecture exercises ---
    worked Python solutions}, cell \emph{Exercise 0.9 --- Reading Python
    output [Python]}.
  \end{labnote}
\end{frame}
...
\begin{frame}{Why this course speaks matrix}
  ...
  \end{alertblock}
  \begin{labnote}[{{Live demo --- \S\S9--10 of the notebook}}]
    Run \texttt{chapter\_00\_lab.ipynb} \S9: \texttt{np.linalg.solve} applies this
    formula to Exercise 0.8's data ($\hat\beta = (0.3333,\ 1.5)$); \S10 then
    minimises the same loss by gradient descent.
  \end{labnote}
\end{frame}
