...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \midrule
    \rowcolor{accent!18} \textbf{Ch.\ 0 + 0b} & \textbf{Statistics refresher and the toolkit}\,{\color{accent!75!black}--- one taught session} \\
    \rowcolor{black!3} {\color{accent}Ch.\ 1} & Introduction \\
     {\color{accent}Ch.\ 2} & Statistical learning\,{\color{black!45}--- accuracy, bias--variance} \\
    \rowcolor{black!3} {\color{accent}Ch.\ 3} & Linear regression\,{\color{black!45}--- estimation, inference, diagnostics} \\
     {\color{accent}Ch.\ 4} & Classification\,{\color{black!45}--- logistic regression, confusion matrix, ROC} \\
    \rowcolor{black!3} {\color{accent}Ch.\ 5} & Resampling\,{\color{black!45}--- validation, CV, bootstrap} \\
     {\color{accent}Ch.\ 6} & Model selection and regularisation\,{\color{black!45}--- ridge, lasso} \\
    \rowcolor{black!3} {\color{accent}Ch.\ 7} & Moving beyond linearity\,{\color{black!45}--- splines, GAMs} \\
     {\color{accent}Ch.\ 8} & Tree-based methods\,{\color{black!45}--- trees, forests, boosting} \\
    \bottomrule
  \end{tabular}
  \end{center}
\end{frame}
...
% ============================================================
\section[Data and notation]{Data, variables and notation}
% ============================================================
...
\begin{frame}{The shape of a data set: $n$ observations, $p$ variables}
  ...
  \end{takeaway}
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    \scriptsize Run \texttt{chapter\_00\_lab.ipynb} \S1: \texttt{load('Wage')} is exactly this
    picture ($n = 3000$, $p = 11$), and \texttt{.dtypes} separates the column types.
  \end{labnote}
\end{frame}
...
% ============================================================
\section[One variable]{Describing one variable}
% ============================================================
...
\begin{frame}{Measures of spread}\label{frm:spread}
  \footnotesize
...
\begin{frame}{Quantiles, the five-number summary, and the boxplot}
  ...
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Switch to \texttt{chapter\_00\_lab.ipynb} \S2 and run the quartile and boxplot
    cells: \texttt{numpy} interpolates $Q_3 = 7.5$ where the by-hand rule gives $8$,
    and \texttt{Wage["wage"].describe()} rebuilds this plot from real data.
  \end{labnote}
\end{frame}
...
\begin{frame}{Standardisation: the $z$-score}\label{frm:zscore}
  ...
  \begin{takeaway}[Why you will meet this constantly]
    ...
    \begin{itemize}
      ...
      \item \textbf{Module A9} (deep learning) --- neural networks train far better on
            standardised inputs.
    \end{itemize}
  \end{takeaway}
\end{frame}
...
\begin{frame}{Solution --- Exercise 0.2 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 0.2 (2/2)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    Dividing by $n$ instead of $n-1$ in $s^2$ ---
...
% ============================================================
\section[Two variables]{Describing two variables}
% ============================================================
...
\begin{frame}{Correlation: covariance made unit-free}
  ...
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: $r$ is Karl Pearson's (1896)
  formalisation of an idea of Francis Galton --- the same Galton who gave
  ``regression'' its name (Ch.~3).}
\end{frame}

\begin{frame}{What $r$ sees --- and what it misses}\label{frm:rsees}
  \begin{center}
  ...
  A correlation matrix over a dashboard of KPIs fails in both directions: a
  threshold or curved effect reads as $r \approx 0$, while a shared driver ---
  season, a price change, a campaign --- makes unrelated series move together.
\end{frame}

\begin{frame}{Anscombe's quartet: four data sets, one set of statistics}\label{frm:anscombe}
  \begin{center}
...
\begin{frame}{Solution --- Exercise 0.3 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 0.3 (2/2)}
  \begin{solutionbox}
...
% ============================================================
\section[Probability]{Probability essentials}
% ============================================================
...
\begin{frame}{Bayes' theorem: reversing the conditioning}
  ...
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: named for the Reverend Thomas Bayes,
  published posthumously in 1763; Laplace rediscovered and generalised it.}
\end{frame}

\begin{frame}{Worked example: why a 95\% accurate test can mislead}\label{frm:screening}
  \footnotesize
  ...
  \begin{block}{Rules you will use constantly}
    ...
    \[
      \mathrm{Var}(X + Y) = \mathrm{Var}(X) + \mathrm{Var}(Y)
        + 2\,\mathrm{Cov}(X, Y)
      \qquad (\text{the last term is } 0 \text{ iff } X \perp Y) .
    \]
  \end{block}
  \begin{takeaway}[Read the rules carefully]
    Expectation is linear \emph{without conditions}; variance is not ---
    variances add only when the covariances vanish, and a sum of $n$ terms
    carries one covariance per pair. \textbf{Positive correlation makes an
    average less precise than $n$ independent terms would}: the rule behind
    $\sigma/\sqrt n$, correlated CV folds (Ch.\ 5) and random-forest
    decorrelation (Ch.\ 8).
  \end{takeaway}
\end{frame}
...
\begin{frame}{Solution --- Exercise 0.4}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} answering (2) with ``$95\%$'' --- that is
    $P(\text{flag} \mid F)$, not $P(F \mid \text{flag})$. The tiny base
    rate drags the useful direction down to $19\%$.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Solution --- Exercise 0.5 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 0.5 (2/2)}
  \begin{solutionbox}
...
% ============================================================
\section[Distributions]{Distributions you will meet}
% ============================================================
...
\begin{frame}{The normal distribution}
  ...
  \begin{block}{$X \sim N(\mu, \sigma^2)$}
    ...
    \[
      ...
      \qquad
      \Phi(z) = \Pr(Z \le z) \ \text{for } Z = \frac{X-\mu}{\sigma} \sim N(0,1),
    \]
    with $\Phi$ the standard-normal CDF --- the ``area to the left'' that
    tables and \texttt{scipy.stats.norm.cdf} return. The exponent is a
    \emph{squared distance} from $\mu$, the reason this density recurs:
    maximising its likelihood minimises squared error (Ch.\ 0b appendix),
    and Chapters 2 and 4 build classifiers directly from it.
  \end{block}
  ...
  \begin{readme}[What is and is not assumed]
    Regression does \emph{not} require $X$ or $y$ to be normal --- only the
    \emph{errors}, and even then mainly for $t$- and $F$-tests in small
    samples; with large $n$ the CLT does the work instead.
  \end{readme}
\end{frame}
...
\begin{frame}{The 68--95--99.7 rule, drawn}
  ...
  It is the same arithmetic that puts process-control limits at $\pm 3\sigma$:
  an in-control process rarely trips the alarm, a real shift trips it fast.
  \par{\tiny\itshape\color{gray}History: the bell curve first appears in de
  Moivre (1733) as an approximation to the binomial; Gauss (1809) connected it
  to least squares --- hence ``Gaussian''.}
\end{frame}
...
% ============================================================
\section[Sampling and CIs]{Sampling, estimation and confidence intervals}
% ============================================================
...
\begin{frame}{The square-root law, drawn}\label{frm:sqrtlaw}
  \begin{center}
    \includegraphics[width=0.76\textwidth,height=0.54\textheight,keepaspectratio]{ch00_se_shrinks.png}
  \end{center}
  \begin{takeaway}
    $\mathrm{SE} = \sigma/\sqrt{n}$ is the most-used formula in this course,
    and its shape is the point: precision is bought at a \emph{square-root}
    rate, so quadrupling the data only halves the standard error. The dots
    are a seeded simulation --- 4\,000 sample means at each $n$.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Confidence intervals}
  ...
  \end{block}
  \begin{center}
    \includegraphics[width=0.66\textwidth,height=0.30\textheight,keepaspectratio]{ch00_ci_coverage.png}
  \end{center}
  \vspace{-2pt}
  \footnotesize
  ...
  \begin{labnote}[{{Live demo --- \S6 of the notebook}}]
    Run \texttt{chapter\_00\_lab.ipynb} \S6: the coverage cell redraws this picture
    from \texttt{Wage} samples, then repeats the procedure $5\,000$ times.
  \end{labnote}
\end{frame}

\begin{frame}{What a confidence interval does and does not say}\label{frm:cimeaning}
  \footnotesize
...
\begin{frame}{Two ways to be wrong}
  ...
  \begin{takeaway}[The same table under different names]
    ...
    negative); in module A7 (ISLP Ch.\ 13) the columns are relabelled to define the family-wise
    error rate and the false discovery rate. It is one idea wearing three
    costumes.
  \end{takeaway}
\end{frame}

\begin{frame}{What a $p$-value is not}\label{frm:pnot}
  \begin{alertblock}{Four standard misreadings}
    \begin{enumerate}
      ...
      \item \emph{``I tested 20 things and one was significant at 5\%.''}
            Expected by chance alone --- see module A7 (ISLP Ch.\ 13).
    \end{enumerate}
  \end{alertblock}
...
\begin{frame}{Power: the error nobody reports}
  ...
  \end{center}
  \footnotesize
  Moving the critical value to the right shrinks the Type~I error area
  (upper tail, $\alpha/2$)
  but grows the Type~II error area ($\beta$). Only \emph{more data} --- or a larger
...
\begin{frame}{Worked example: a one-sample $t$-test}
  ...
  \end{takeaway}
  \begin{labnote}[{{Live demo --- \S7 of the notebook}}]
    Switch to \texttt{chapter\_00\_lab.ipynb} \S7 and run the $t$-test cell:
    \texttt{ttest\_1samp} returns a \emph{pair} --- statistic first,
    $p$-value second: $(2.00,\ 0.048)$. The next cell shades that $p$ as an area.
  \end{labnote}
\end{frame}
...
\begin{frame}{Solution --- Exercise 0.6 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 0.6 (2/2)}
  \begin{solutionbox}
    ...
    unremarkable. Correcting for this is the entire content of ISLP Chapter 13
    --- self-study module A7 (Bonferroni, Holm, Benjamini--Hochberg).
  \end{solutionbox}
  \begin{mistake}
    Expecting double the data to halve the SE ---
...
\begin{frame}{Solution --- Extended Exercise 0.1 (1/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 0.1 (2/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 0.1 (3/3)}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} ``significant, so act''. The $p$-value compares
    the increase with \emph{zero}; the decision needed a comparison with
    \euro 5. Always test the threshold that actually matters.
  \end{solutionbox}
\end{frame}


% ============================================================
\section[Simple regression]{Simple linear regression: the bridge}
% ============================================================
...
\begin{frame}{Fitted values, residuals, and $R^2$}
  ...
  \end{center}
  \footnotesize
  Left: the fit on \texttt{Advertising}; the vertical segments are the residuals
  ...
  missing something. Both plots return in Chapter 3. A fit like this one starts
  most marketing conversations --- and is not causal: spend rises in strong
  seasons, so the slope quietly absorbs the season.
\end{frame}

\begin{frame}{How good is the fit? RSS, TSS and $R^2$}
  \footnotesize
  \begin{block}{Two sums of squares, and their ratio}
    \[
      \mathrm{RSS} = \sum_i (y_i - \hat y_i)^2,
      \qquad
      \mathrm{TSS} = \sum_i (y_i - \bar y)^2 = S_{yy},
      \qquad
      R^2 = 1 - \frac{\mathrm{RSS}}{\mathrm{TSS}} .
    \]
  \end{block}
  \begin{readme}[Reading it]
    TSS is the variation in $y$ \emph{before} the model (predict $\bar y$ for
    everyone); RSS is what is \emph{left over} after it. So $R^2$ is the
    fraction of the variation the model accounts for: $0 \le R^2 \le 1$, and
    $R^2 = 0.61$ reads ``$61\%$ of the variability in sales is accounted for
    by TV spend''. In \emph{simple} regression $R^2 = r^2$ exactly --- the
    same number you already met as the correlation, squared.
  \end{readme}
  \begin{takeaway}[What a high $R^2$ does not buy you]
    Not correctness, not causality, and not accurate predictions on new data.
    $R^2$ never falls when a predictor is added, so it cannot compare models
    of different sizes --- which is why Chapter 6 needs adjusted $R^2$, $C_p$,
    AIC and BIC.
  \end{takeaway}
\end{frame}
...
\begin{frame}[fragile]{Reading a regression output --- the table you will see all term}\label{frm:regtable}
  \scriptsize
\begin{lstlisting}
...
\begin{frame}{Solution --- Extended Exercise 0.2 (1/4)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 0.2 (2/4)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 0.2 (3/4)}
  \begin{solutionbox}
    \textbf{(4) Fit quality.}
...
\begin{frame}{Solution --- Extended Exercise 0.2 (4/4)}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} being impressed by $R^2 = 0.997$ when $n = 4$.
    A line has two parameters, so four points make a near-perfect fit cheap
    --- $R^2$ measures fit to \emph{these} points, not validity.
  \end{solutionbox}
\end{frame}
...
\begin{frame}[fragile]{Exercise 0.7 --- Interrogating a regression table [Python]}
  \begin{exercise}
    \scriptsize
    A colleague regresses monthly ice-cream sales (\euro 000s) on temperature ($^\circ$C):
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize,aboveskip=3pt,belowskip=1pt]
                coef   std err       t   P>|t|    [0.025   0.975]
...
\end{lstlisting}
    \begin{enumerate}
      ...
      \item Verify the $t$-value and check the CI against the
      ...
      \item Your colleague concludes: ``raising the temperature by
            $1^\circ$C would raise sales by \euro 900''. What is wrong,
            and what would make the claim defensible?
    \end{enumerate}
  \end{exercise}
  \vspace{-9pt}
  \begin{labnote}[{{Run this live}}]
    \scriptsize Worked solution: \texttt{chapter\_00\_lab.ipynb}, \emph{Lecture
    exercises --- worked Python solutions}, cell \emph{Exercise 0.7 ---
    Interrogating a regression table [Python]}.
  \end{labnote}
\end{frame}
...
\begin{frame}{Solution --- Exercise 0.7}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} the word ``effect''. With observational data a
    coefficient is a \emph{comparison} between months, not a lever you can
    pull --- write ``associated with'' and mean it.
  \end{solutionbox}
\end{frame}


% ============================================================
\section[Python]{The Python toolkit}
% ============================================================

\begin{frame}[fragile]{\texttt{numpy}: arrays and vectorised arithmetic}
\begin{lstlisting}
import numpy as np                     # arrays + fast vector maths

x = np.array([2, 4, 4, 5, 7, 8, 12])   # our n = 7 sample
x.mean(), np.median(x), x.std(ddof=1)  # 6.0, 5.0, 3.317

z = (x - x.mean()) / x.std(ddof=1)     # z-scores: SDs from the mean
A = np.array([[1, 1], [1, 2], [1, 3]]) # 3 cases, 2 columns
A.T @ A                                # the matrix behind every OLS fit
\end{lstlisting}
  \begin{readme}
    \footnotesize \texttt{.std(ddof=1)} divides by $n-1 = 6$ --- one degree of
    freedom is spent on $\bar x = 6$ --- and returns $s = 3.317$.
  \end{readme}
...
\begin{lstlisting}
import pandas as pd                    # tables with named columns

df = pd.read_csv("Wage.csv")           # each row is one person
df.shape          # (3000, 11) -- n = 3000 rows, p = 11 columns
df.head()         # eyeball the data before trusting any number
df.describe()     # count, mean, std, min, quartiles, max
df["wage"].median()                     # robust centre of pay
df["education"].value_counts()          # how many people per level
df.groupby("education")["wage"].mean()  # mean wage within each level
df[["age", "wage"]].corr()              # r for every pair of columns
df[df["wage"] > 200].shape[0]           # how many earn above 200
\end{lstlisting}
  \begin{takeaway}[The four verbs of the labs]
    \textbf{select} columns, \textbf{filter} rows, \textbf{group} and
    \textbf{summarise}. Almost every data-preparation step in the course's
    labs is a combination of these. None of them changes \texttt{df}.
  \end{takeaway}
\end{frame}
...
fig, ax = plt.subplots(figsize=(6, 4))     # one blank canvas
ax.hist(df["wage"], bins=40)               # shape: skew and gaps
ax.boxplot(df["wage"])                     # centre, spread, outliers
ax.scatter(df["age"], df["wage"], s=8)     # does y move with x?
ax.bar(counts.index, counts.values)        # counts per category
ax.set_xlabel("age"); ax.set_ylabel("wage")  # label both axes, always
...
\end{lstlisting}
  \begin{takeaway}[Plot first --- model second]
    ...
    No summary statistic replaces looking at the data. Each
    \texttt{ax.\dots()} call draws one more layer onto the same axes.
  \end{takeaway}
\end{frame}
...
import statsmodels.formula.api as smf  # formula interface, alias smf
fit = smf.ols("sales ~ TV", data=adv).fit()  # estimate the line y ~ x
print(fit.summary())        # slope 0.0475, R^2 = 0.612 on Advertising

# scikit-learn: prediction -- one API for every model
from sklearn.linear_model import LinearRegression  # estimator class
model = LinearRegression().fit(X_train, y_train)  # learn on train only
model.predict(X_test)       # same two verbs for every model
\end{lstlisting}
  \begin{takeaway}[Which to reach for]
    ...
    Chapters 5--10 on the second. \texttt{.fit()} hands back a
    \emph{result object}: \texttt{fit.params}, \texttt{fit.bse},
    \texttt{fit.rsquared}.
  \end{takeaway}
\end{frame}
...
\begin{lstlisting}
df.isna().sum()                    # count gaps before anything else
df["horsepower"].unique()[:5]      # '?' hides in Auto.csv as a string!
auto = pd.read_csv("Auto.csv", na_values="?").dropna()  # '?' -> NaN
df["education"] = df["education"].astype("category")   # a label
...
adv = pd.read_csv("Advertising.csv", index_col=0)   # 1. load, n = 200
adv.describe()                                      # 2. summarise
adv.plot.scatter(x="TV", y="sales")                 # 3. LOOK at it
fit = smf.ols("sales ~ TV", data=adv).fit()         # 4. fit the line
print(fit.params, fit.bse, fit.rsquared)            # 5. 0.0475, SE, 0.612
print(fit.conf_int())                               # 6. the CI for b
...
\end{lstlisting}
  ...
  \end{takeaway}
  \begin{labnote}[{{Live demo --- \S8 of the notebook}}]
    Run all of this in \texttt{chapter\_00\_lab.ipynb} \S8: the \texttt{statsmodels}
    cell prints the table above (slope $0.0475$, $R^2 = 0.612$).
  \end{labnote}
\end{frame}
...
\begin{frame}[fragile]{Exercise 0.9 --- Reading Python output [Python]}
  \begin{exercise}
    \scriptsize
    A student runs this on the \texttt{Wage} data:
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize,aboveskip=3pt,belowskip=1pt]
>>> df.shape                          # (3000, 11)
>>> df["wage"].describe()
# count 3000 | mean 111.70 | std 41.73 | 25% 85.38 | 50% 104.92 | 75% 128.68 | max 318.34
>>> df["age"].corr(df["wage"])        # 0.1956
\end{lstlisting}
    \begin{enumerate}
      \item What are $n$ and $p$?
      \item Compute the IQR. Would a wage of $220$ (thousand dollars) be
            flagged by the $1.5\times\mathrm{IQR}$ rule?
      \item Compare mean and median: what does that say about the shape,
  ...
  \begin{labnote}[{{Run this live}}]
    \scriptsize Worked solution: \texttt{chapter\_00\_lab.ipynb}, \emph{Lecture
    exercises --- worked Python solutions}, cell \emph{Exercise 0.9 --- Reading
...
\begin{frame}{Solution --- Exercise 0.9}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} deleting the $109$ flagged wages before
    modelling. In a right-skewed variable the fences mark the tail, not
    errors --- deleting them would bias every later model toward low earners.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Solution --- Exercise 0.10}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} defaulting to mean and SD. The first question
    is always the \emph{shape} --- skew, outliers, unequal groups --- and
    the summary follows from the shape, not the other way round.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 0.4 (2/2)}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} softening an unearned causal claim (``members
    tend to benefit\dots'') instead of dropping it. If only observational
    data exist, the honest product is an association \emph{plus its threats}.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{block}{Regression and optimisation}
    \[
      b_1 = \frac{S_{xy}}{S_{xx}} = r\,\frac{s_y}{s_x},
      \quad b_0 = \bar y - b_1\bar x,
      \quad
      R^2 = 1 - \frac{\mathrm{RSS}}{\mathrm{TSS}},
      \quad
      \hat{\boldsymbol\beta} = (\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{X}^\top\mathbf{y},
...
\begin{frame}{Where each topic reappears in the course}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Standardisation ($z$-scores) & Ch.\ 2 (KNN), Ch.\ 6 (ridge/lasso), module A9 \\
    ...
    Type I/II errors, $p$-values & Ch.\ 4 (confusion matrix), module A7 (FWER, FDR) \\
    Power, sample size & Ch.\ 5 (how much data is enough), module A7 \\
    ...
    Eigenvectors & The unsupervised-learning module (PCA) in \texttt{Advanced/} \\
    Derivatives, gradients, convexity & module A9 (backpropagation, training) \\
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    \item Why does the sample variance divide by $n-1$? (p.~\pageref{frm:spread})
    \item Give two variables with a strong relationship but $r \approx 0$. (p.~\pageref{frm:rsees})
    \item A test is 99\% accurate for a disease affecting 1 in 10{,}000.
          You test positive. Estimate $P(\text{ill}\mid+)$ --- roughly. (p.~\pageref{frm:screening})
    \item State precisely what ``95\% confident'' means. (p.~\pageref{frm:cimeaning})
    \item Why does quadrupling the sample size only halve the standard
          error? (p.~\pageref{frm:sqrtlaw})
    \item In $\hat y = 4 + 0.8x$ with $x$ in years and $y$ in \euro 000s,
          interpret both coefficients with units. (p.~\pageref{frm:regtable})
    \item If $\mathbf{X}$ is $200 \times 5$, what size is
          $(\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{X}^\top\mathbf{y}$?
          (appendix, p.~\pageref{frm:matrix})
    \item Why must predictors be standardised before ridge regression? (p.~\pageref{frm:zscore})
    \item Why does a large learning rate make gradient descent diverge?
          (appendix, p.~\pageref{frm:gddrawn})
    \item Two studies report the same estimate; one has twice the standard
          ...
          not the same as being more \emph{true}? (p.~\pageref{frm:pnot})
    \item Sketch Anscombe's fourth data set from memory and say why its
          correlation is untrustworthy. (p.~\pageref{frm:anscombe})
  \end{enumerate}
\end{frame}
...
\begin{frame}{Where to read more}
  ...
  \begin{labnote}[Practice beats reading]
    ...
    reproduce them in Python in \texttt{chapter\_00\_lab.ipynb}
    --- \S\S1--10 rebuild every figure of this deck from the course data, and
    \emph{Lecture exercises --- worked Python solutions} checks your answers. That single hour is the best
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{Four shapes you will actually meet} --- the distribution-shape gallery; the skew point is already made in the main deck, and the bimodal-means-two-populations idea lives on here.
      ...
      \item \textbf{Calculus and optimisation} --- derivatives, minimising a loss, gradient descent (with Extended Exercise 0.3). Software does this for you; come back here when the deep-learning module (A9) trains a network.
    \end{itemize}
  \end{block}
  \vspace{-1mm}
  Exercise numbers are unchanged: Exercise 0.8 and Extended Exercise 0.3 live
  here, which is why the main flow runs 0.7 $\to$ 0.9.
\end{frame}
...
\begin{frame}{Three distributions that come from the normal}
  \begin{center}
    \includegraphics[width=0.88\textwidth,height=0.52\textheight,keepaspectratio]{ch00_reference_dists.png}
  \end{center}
  \footnotesize
  \begin{tabular}{@{}lll@{}}
    \toprule
  ...
  $1.96$.
  \par{\tiny\itshape\color{gray}History: the $t$ is \emph{Student's}: W.~S.\
  Gosset (1908), a Guinness brewer made to publish under a pen name.}
\end{frame}
...
\begin{frame}{Decomposing the variation}
  ...
  \begin{block}{The three sums of squares}
    ...
    \]
  \end{block}
  \begin{readme}[Why the split is exact --- the cross term vanishes]
    Write $y_i - \bar y = (\hat y_i - \bar y) + (y_i - \hat y_i)$ and square.
    The cross term $2\sum_i (\hat y_i - \bar y)(y_i - \hat y_i)$ is
    \emph{zero}, because least squares makes the residuals sum to zero and
    makes them orthogonal to the fitted values --- the two normal equations
    again. So ``explained $+$ unexplained'' is an identity of the fitting
    method, not an approximation, and $R^2$ (main deck, ``How good is the
    fit?'') is just this identity divided through by TSS.
  \end{readme}
...
\begin{frame}{Why this course speaks matrix}\label{frm:matrix}
...
\begin{frame}{Norms, distances and eigenvectors --- where they show up}
  ...
  \begin{block}{Euclidean distance}
    ...
    \emph{Where:} KNN (Ch.\ 2, 4) and clustering (Ch.\ 12, the optional
    unsupervised-learning module) are built on it --- and it is why
    unstandardised variables wreck both.
  \end{block}
  \begin{block}{Eigenvectors and eigenvalues}
    $\mathbf{A}\mathbf{v} = \lambda\mathbf{v}$: directions the matrix only
    stretches. \emph{Where:} principal components (Ch.\ 12, the optional
    unsupervised-learning module) are the eigenvectors of the covariance
    matrix --- the directions of greatest variance in the data.
  \end{block}
\end{frame}
...
\begin{frame}{Solution --- Exercise 0.8 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 0.8 (2/2)}
  \begin{solutionbox}
    \textbf{(4)}
    \[
...
\begin{frame}{Gradient descent: when there is no closed form}
  ...
  \begin{readme}[The picture]
    ...
    it well is a recurring practical theme of the deep-learning module (A9).
  \end{readme}
  \begin{takeaway}[Convexity decides whether this is safe]
    ...
    schedules and early stopping matter so much in the deep-learning module (A9).
  \end{takeaway}
\end{frame}

\begin{frame}{Gradient descent, drawn}\label{frm:gddrawn}
  \begin{center}
...
\begin{frame}{Solution --- Extended Exercise 0.3 (1/4)}
  \begin{solutionbox}
    \scriptsize
    \textbf{(1) The loss.}
    \[
...
\begin{frame}{Solution --- Extended Exercise 0.3 (2/4)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 0.3 (3/4)}
  \begin{solutionbox}
    \scriptsize
    \textbf{(4) Did it help?}
    \[
...
\begin{frame}{Solution --- Extended Exercise 0.3 (4/4)}
  \begin{solutionbox}
    ...
    and tunes $\alpha$: the workflow of the deep-learning module (A9).

    \smallskip
    \textit{Common mistake:} blaming the model when descent diverges. The
    loss surface did not change --- the step size did. Divergence is a tuning
    failure: standardise the predictors and shrink $\alpha$.
  \end{solutionbox}
\end{frame}
