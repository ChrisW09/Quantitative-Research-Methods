...
\newtcolorbox{industry}[1][Where this shows up]{enhanced, fontupper=\footnotesize, colback=industryC!7, colframe=industryC, boxrule=0pt, leftrule=3pt, arc=2pt, fonttitle=\bfseries, title={In industry --- #1}, left=2mm, right=2mm, top=1mm, bottom=1mm}
...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{black!3} {\color{accent}Ch.\ 1} & Introduction \\
     {\color{accent}Ch.\ 2} & Statistical learning\,{\color{black!45}--- accuracy, bias--variance, Bayes classifier, KNN} \\
    ...
    \rowcolor{black!3} {\color{accent}Ch.\ 10} & Deep learning\,{\color{black!45}--- MLPs, CNNs, training} \\
     {\color{accent}Ch.\ 12} & Unsupervised learning\,{\color{black!45}--- PCA, clustering} \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{1mm}
  {\scriptsize The precourse is taught in one 180-minute session drawing on both decks, before Chapter 1; the ten chapter decks follow. It assumes nothing beyond an introductory undergraduate course. Today's session is highlighted.\par\medskip Support vector machines (Ch.\ 9), survival analysis (Ch.\ 11) and multiple testing (Ch.\ 13) are optional self-study modules in \texttt{Chapters/Advanced/}.}
\end{frame}
...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.3cm}p{4.7cm}p{5.4cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{Which decision it drives} \\
    \midrule
    Manufacturing & Statistical process control: a chart on a critical
      dimension, limits at $\pm 3\sigma$ &
      Stop the line, or let it keep running \\
    Insurance & Claim severity is strongly right-skewed, so the mean sits
      above the typical claim &
      Where reinsurance attaches, and the reserve booked \\
    E-commerce & Online A/B test of a checkout or ranking change &
      Ship the variant, or roll it back \\
    Market research & Brand and satisfaction trackers on a sample of a few
      thousand &
      Act on a movement in the tracker, or treat it as noise \\
    Supplier quality & Acceptance sampling of an incoming lot against an
      agreed plan &
      Accept the lot, or reject it on the sample \\
    Logistics, operations & Delivery-time distributions summarised by the
      median and the 95th percentile &
      The service-level promise quoted to customers \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{industry}[The common thread]
    Every row is the same two-step move: \emph{describe} what the data show,
    then quantify \emph{how much of it could be noise}.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: an online A/B test}
  \footnotesize
  A retailer wants to know whether a redesigned checkout page raises the
  purchase rate. This is a hypothesis test, run as an experiment.
  \begin{industry}[The set-up]
    \textbf{Response} $y_i$: did session $i$ end in a purchase (0/1)?
    \textbf{Unit of randomisation}: the visitor, not the session --- otherwise
    the same person sees both designs. \textbf{Fixed before launch}: the
    primary metric, the segments to be reported (device, new vs.\ returning,
    market), the run length, and the smallest effect worth acting on.
  \end{industry}
  \begin{numexample}[Illustrative sizing]
    The rule of thumb for $80\%$ power at $\alpha = 0.05$ is
    $n \approx 16\,\pi(1-\pi)/\delta^2$ per arm. At a baseline of $\pi=2.5\%$,
    detecting a relative lift of $10\%$ ($\delta = 0.0025$) needs tens of
    thousands of visitors per arm --- which is why small sites cannot test
    small effects.
  \end{numexample}
  \begin{takeaway}[Who signs off --- and the honest caveat]
    The product owner, plus a central experimentation platform that enforces
    the pre-registration. Its consumer is the launch reviewer who ships it.
    Caveat: stopping the test the first time it looks
    significant (``peeking''), or reporting whichever of twenty metrics moved,
    breaks the $5\%$ guarantee --- the problem of Chapter~13.
  \end{takeaway}
\end{frame}
...
\begin{frame}{What $r$ sees --- and what it misses}
  ...
  \begin{industry}[KPI dashboards: the correlation-matrix trap]\scriptsize
    A threshold or curved effect reads as $r \approx 0$, and a shared driver ---
    season, a price change, a campaign --- makes unrelated KPIs move together.
  \end{industry}
\end{frame}
...
\begin{frame}{The 68--95--99.7 rule, drawn}
  ...
  \begin{industry}[Manufacturing: the arithmetic behind Six Sigma]\scriptsize
    Process-control charts set limits at $\pm 3\sigma$: an in-control process
    rarely trips the alarm, while a real shift in the process trips it fast.
  \end{industry}
\end{frame}
...
\begin{frame}{Fitted values, residuals, and $R^2$}
  ...
  \begin{industry}[Marketing: the first-pass driver analysis]\scriptsize
    Sales on advertising spend starts most marketing-mix conversations --- and
    is not causal: spend rises in strong seasons, so the slope absorbs the season.
  \end{industry}
\end{frame}
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    \item Why does the sample variance divide by $n-1$? (p.~22)
    \item Give two variables with a strong relationship but $r \approx 0$. (p.~35)
    \item A test is 99\% accurate for a disease affecting 1 in 10{,}000.
          You test positive. Estimate $P(\text{ill}\mid+)$ --- roughly. (p.~45)
    \item State precisely what ``95\% confident'' means. (p.~60)
    \item Why does quadrupling the sample size only halve the standard
          error? (p.~58)
    \item In $\hat y = 4 + 0.8x$ with $x$ in years and $y$ in \euro 000s,
          interpret both coefficients with units. (p.~77)
    \item If $\mathbf{X}$ is $200 \times 5$, what size is
          $(\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{X}^\top\mathbf{y}$?
          (appendix, p.~111)
    \item Why must predictors be standardised before ridge regression? (p.~27)
    ...
    \item Two studies report the same estimate; one has twice the standard
          ...
          not the same as being more \emph{true}? (p.~64)
    \item Sketch Anscombe's fourth data set from memory and say why its
          correlation is untrustworthy. (p.~36)
  \end{enumerate}
\end{frame}
