...
\begin{frame}{A coin, worked end to end}
  ...
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Switch to \texttt{chapter\_00b\_lab.ipynb} \S4 and run the coin and
    \texttt{Default} cells: the grid maximiser lands at $0.699 \approx 7/10$,
    and \texttt{scipy} recovers the same $\hat\pi = 0.0333$ as the closed form.
  \end{labnote}
\end{frame}
...
\end{lstlisting}
  ...
  \begin{labnote}[{{Live demo --- \S6 of the notebook}}]
    Switch to \texttt{chapter\_00b\_lab.ipynb} \S6 and run the \texttt{rmse}
    and split/fit/score cells: the same loop on \texttt{Auto} prints test
    RMSEs $4.955$ down to $4.442$ and crowns degree 5.
  \end{labnote}
\end{frame}
...
\end{lstlisting}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_00b\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Exercise 0b.6 --- Reading and fixing code [Python]}.
  \end{labnote}
\end{frame}
...
\begin{frame}{Extended Exercise 0b.2 --- A complete small experiment [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_00b\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Extended Exercise 0b.2 --- A complete small experiment [Python]}.
  \end{labnote}
\end{frame}
...
\begin{frame}{How many models are there?}
  ...
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    Switch to \texttt{chapter\_00b\_lab.ipynb} \S5 and run the counting cells:
    at $p = 30$, best subset needs $2^{30}$ fits ($\approx 124$ days at
    $0.01$\,s each) while stepwise needs $466$.
  \end{labnote}
\end{frame}
