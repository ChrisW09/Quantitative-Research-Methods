...
\begin{frame}{Why this second session exists}
  \footnotesize
  These five topics were counted in the nine chapter decks. Each is used
...
\begin{frame}{Where this chapter is used in industry}
  ...
  \end{tabular}
  \end{center}
  \begin{takeaway}[All three are linear on a scale nobody decides on]
    In each row the model is linear on a \emph{transformed} scale --- log,
    log-odds --- while the decision is taken on the original one. Someone must
    transform back before the number is used, and that step is where the
    mistakes happen.
  \end{takeaway}
\end{frame}
