  ...
  \begin{columns}[t]
    \begin{column}{0.5\textwidth}
      \tableofcontents[currentsection,sectionstyle=show/shaded,
                       subsectionstyle=show/show/hide,sections={1-4}]
    \end{column}
    \begin{column}{0.5\textwidth}
      \tableofcontents[currentsection,sectionstyle=show/shaded,
                       subsectionstyle=show/show/hide,sections={5-7}]
    \end{column}
  \end{columns}
\end{frame}}
...
\begin{frame}{Why this second session exists}
  \footnotesize
  These five topics were counted across the eight chapter decks and the deep-learning
  module (A9, ISLP Ch.\ 10), which was a taught chapter when the tally was made. Each is used
...
\begin{frame}{Learning objectives for this session}
  ...
  \begin{readme}[{{How to use this deck --- self-study and revision}}]
    ...
    \texttt{chapter\_00b\_lab.ipynb} alongside the lab section. Revise from the
...
% ============================================================
\section[Mathematical notation]{Reading mathematical notation}
% ============================================================
...
\begin{frame}{The anatomy of a formula}
  \begin{center}
    \begin{tikzpicture}[every node/.style={font=\footnotesize}]
      ...
      \draw[->,accent,thick] (-3.0,1.3) -- (-1.95,0.55);
      \node[accent,align=center,anchor=south] at (-3.3,1.35) {\dots and stop at $i = n$};
      \draw[->,orange!70!black,thick] (1.1,1.3) -- (0.0,0.5);
      \node[orange!70!black,align=center,anchor=south] at (1.4,1.35) {do this to every observation};
...
\begin{frame}{Sums and products}\label{frm:sums}
  \footnotesize
...
\begin{frame}{$\arg\min$, $\arg\max$ and the indicator}\label{frm:argmin}
  \footnotesize
...
\begin{frame}{Exercise 0b.1 --- Translating notation [Math]}
  ...
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    After solving by hand, run \texttt{chapter\_00b\_lab.ipynb} \S1: its
    notation cells compute this exact data --- RSS $2.0$, error rate $0.667$,
    $|\mathcal{S}| = 2$ with mean $3.5$ --- each in one line of numpy.
  \end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 0b.1 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 0b.1 (2/2)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    Writing $\min$ when $\arg\min$ is meant.
    Every estimator ahead is an $\arg\min$: $\hat\beta$ is the
    \emph{coefficient value} that minimises RSS, never the RSS value itself.
  \end{mistake}
\end{frame}


% ============================================================
\section[Logs and exponentials]{Logs, exponentials and growth}
% ============================================================

\begin{frame}{The two functions Chapter 4 is written in}\label{frm:logexp}
  \begin{center}
  ...
  \begin{takeaway}[Three jobs a log does]
    \begin{enumerate}
      ...
      \item \textbf{Tames skew and turns effects into percentages.} A right tail
            ...
            \emph{relative} change --- in a $\log$--$\log$ demand model the
            slope \emph{is} the price elasticity.
    \end{enumerate}
  \end{takeaway}
...
\begin{frame}{Exercise 0b.2 --- Logs in practice [Math]}
  ...
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Run \texttt{chapter\_00b\_lab.ipynb} \S2 alongside: it checks part (2)
    numerically ($t = \log 5/0.4 = 4.024$) and shows logging \texttt{Boston}'s
    \texttt{medv} move $r$ from $-0.74$ to $-0.81$.
  \end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 0b.2 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 0b.2 (2/2)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    Inventing a rule for sums. $\log(a+b)$
    simplifies to nothing --- the log rules turn \emph{products} into sums;
...
% ============================================================
\section[Odds and the logit]{Probability, odds and the logit}
% ============================================================

\begin{frame}{Three ways to say the same thing}\label{frm:threeways}
  \footnotesize
...
\begin{frame}{The odds ratio --- and why a coefficient \emph{is} one}\label{frm:oddsratio}
  \footnotesize
  \begin{block}{Comparing two groups}
    \[
      \mathrm{OR} = \frac{\mathrm{odds}_1}{\mathrm{odds}_0}
                  = \frac{p_1/(1-p_1)}{p_0/(1-p_0)},
      \qquad
      \log \mathrm{OR} = \mathrm{logit}(p_1) - \mathrm{logit}(p_0) .
    \]
  \end{block}
  \begin{readme}[Differences on the log-odds scale are ratios on the odds scale]
    Put a $0/1$ dummy $x$ into $\log\frac{p(x)}{1-p(x)} = \beta_0 + \beta_1 x$:
    group $0$ has log-odds $\beta_0$, group $1$ has $\beta_0 + \beta_1$. Their
    \emph{difference} is $\beta_1$, so
    \[
      \beta_1 = \log \mathrm{OR}
      \qquad\text{and}\qquad
      e^{\beta_1} = \mathrm{OR} .
    \]
    That is how every logistic coefficient in Chapter 4 is read: exponentiate
    it and report ``the odds multiply by $e^{\beta}$''. For a \emph{continuous}
    predictor the same holds per one-unit increase.
  \end{readme}
  \begin{alertblock}{An odds ratio is not a risk ratio}
    $p$ from $0.25$ to $0.45$ is $1.8\times$ the \emph{probability} but
    $2.45\times$ the \emph{odds}. The two agree only for rare events; quoting
    an OR as ``times as likely'' overstates the effect.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Exercise 0b.3 --- Odds and odds ratios [Math]}
  ...
  \begin{labnote}[{{Live demo --- \S3 of the notebook}}]
    Run \texttt{chapter\_00b\_lab.ipynb} \S3: \texttt{odds()}, \texttt{logit()}
    and \texttt{sigmoid()} rebuild the conversion table, and the worked cell
    under \emph{Lecture exercises} verifies every part of this exercise.
  \end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 0b.3 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 0b.3 (2/2)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    Reading odds as probabilities. Odds of $0.818$
    do not mean ``$82\%$'' --- convert back with
    $p = \mathrm{odds}/(1+\mathrm{odds}) = 0.45$.
  \end{mistake}
\end{frame}


% ============================================================
\section[Maximum likelihood]{Likelihood and maximum likelihood}
% ============================================================

\begin{frame}{The idea in one sentence}\label{frm:mle}
...
\begin{frame}{Maximum likelihood outside the classroom}
  \footnotesize
  Least squares is the special case, not the rule. Claim frequency and severity
  models, probability-of-default models and demand models are nearly all fitted
  by maximum likelihood instead --- because the response is a count, a $0/1$
  event, or a skewed positive amount, and none of those has normal errors.
  Durations behave the same way: a retention curve falls away roughly
  exponentially, so the log of the share still active is close to linear in
  time, and fitting on that scale returns the expected customer lifetime.
  \begin{takeaway}[What one principle buys]
    The same estimation idea covers continuous responses, classes, counts and
    durations --- which is why Chapters~4, 6 and~10 all report a
    log-likelihood instead of an $R^2$.
  \end{takeaway}
\end{frame}


\begin{frame}{The price of a probability: $-\log \hat p$}
  \begin{center}
    \includegraphics[width=0.64\textwidth,height=0.48\textheight,keepaspectratio]{ch00b_log_loss.png}
  \end{center}
  \begin{takeaway}
    Maximising the log-likelihood means paying $-\log \hat p$ for the
    probability you gave what actually happened: hedging at $0.5$ costs
    $0.69$, confident-and-wrong costs without bound. This curve is the loss
    of logistic regression (Ch.~4) and of cross-entropy (module A9).
  \end{takeaway}
\end{frame}
...
\begin{frame}{Exercise 0b.4 --- What a likelihood is and is not [Concept]}
  ...
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Run \texttt{chapter\_00b\_lab.ipynb} \S4 alongside: the coin cells show $L$
    and $\ell$ peaking at the same $\hat\pi = 0.699$ (statement 4 settled by
    machine), and \texttt{scipy} recovers $\hat\pi = 0.0333$ on \texttt{Default}.
  \end{labnote}
\end{frame}
...
\begin{frame}{Solution --- Exercise 0b.4}
  \begin{solutionbox}
    ...
    \medskip
    \textit{Common mistake:} the root of (1), (2) and (4) is one flipped
    conditioning: $L$ is the probability of \emph{this data} given $\pi$,
    never of $\pi$ given the data. Keep that direction straight and the
    statements sort themselves.
  \end{solutionbox}
\end{frame}


% ============================================================
\section[Python in practice]{The Python you will actually write}
% ============================================================

\begin{frame}[fragile]{Functions and loops --- used in every lab}
\begin{lstlisting}
def rmse(y, y_hat):                     # define once, reuse everywhere
    """Root mean squared error."""      # what help(rmse) shows
    return np.sqrt(np.mean((y - y_hat) ** 2))   # the value handed back

scores = []                             # accumulate results in a list
for degree in range(1, 6):              # range(1, 6) -> 1, 2, 3, 4, 5
    fit = np.polyfit(x_train, y_train, degree)  # fit on train data only
    scores.append(rmse(y_test, np.polyval(fit, x_test)))  # test error

best = int(np.argmin(scores)) + 1       # argmin: position 0 -> degree 1
print(f"best degree: {best}  (RMSE {min(scores):.3f})")   # {} filled in
\end{lstlisting}
...
\begin{lstlisting}
squares = [d ** 2 for d in range(5)]          # a loop that fills a list
mask = df["balance"] > 1000                   # True/False for each row
df.loc[mask, "income"].mean()                 # mean income where True

for name, group in df.groupby("education"):   # one loop pass per level
    print(name, group["wage"].median())       # median wage per group

for ax, col in zip(axes.ravel(), columns):    # pairs panel with column
    ax.hist(df[col])                          # one histogram per panel
\end{lstlisting}
  \begin{alertblock}{Two things that will bite you}
    \begin{enumerate}
      ...
      \item \textbf{Chained assignment}: \texttt{df[df.x > 1]["y"] = 0} silently
            does nothing. Use \mbox{\texttt{df.loc[df.x > 1, "y"] = 0}} ---
            the \texttt{.loc} above: rows and column in one bracket.
    \end{enumerate}
  \end{alertblock}
\end{frame}
...
sample = rng.choice(data, size=100)      # same 100 draws on every run

from sklearn.model_selection import train_test_split   # the split helper
...
\end{lstlisting}
  ...
  \texttt{test\_size=0.3} sets aside $30\%$ of the rows as the test set.
\end{frame}

\begin{frame}[fragile]{The scikit-learn contract: fit, predict, score}\label{frm:contract}
\begin{lstlisting}
from sklearn.linear_model import LinearRegression     # the model class
from sklearn.metrics import mean_squared_error        # a scoring rule

model = LinearRegression()               # 1. choose a model (empty)
...
model.coef_, model.intercept_            # what fit() stored; note the _
\end{lstlisting}
...
\begin{frame}[fragile]{Exercise 0b.6 --- Reading and fixing code [Python]}
  \begin{exercise}[{{Exercise --- a student writes this to compare polynomial degrees}}]
    \scriptsize
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize,aboveskip=2pt,belowskip=1pt]
scores = []
...
\end{lstlisting}
    \begin{enumerate}\setlength{\itemsep}{0pt}
      ...
      \item Line (c) prints \texttt{3}. Why is that answer wrong, and what is
            the correct degree?
      \item Lines (a) and (b) use the same data. How will \texttt{scores}
            behave as the degree rises, and why is the comparison meaningless?
  ...
  \begin{labnote}[{{Run this live}}]
    Worked solution: \texttt{chapter\_00b\_lab.ipynb}, \emph{Lecture exercises
    --- worked Python solutions}, cell \emph{Exercise 0b.6 --- Reading and fixing code [Python]}.
  \end{labnote}
\end{frame}
...
for degree in range(1, 6):                  # now all five degrees
    fit = np.polyfit(X_tr, y_tr, degree)    # learn on the training half
    scores.append(rmse(y_te, np.polyval(fit, X_te)))   # score on test
\end{lstlisting}
    ...
    \smallskip
    \textit{Common mistake:} the thread through all five bugs --- trusting a
    printed number without asking what was computed. Off-by-one and leakage
    errors run \emph{silently}; print intermediate objects and check them.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Extended Exercise 0b.2 --- A complete small experiment [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    Worked solution: \texttt{chapter\_00b\_lab.ipynb}, \emph{Lecture exercises
    --- worked Python solutions}, cell \emph{Extended Exercise 0b.2 --- A complete small experiment [Python]}.
  \end{labnote}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 0b.2}
  \begin{solutionbox}
    \scriptsize
    \textbf{(1) The six steps.} Load; \emph{look} (scatter, and check for
    missing values --- \texttt{Auto} stores them as \texttt{'?'}); split with
    a seed; fit each candidate on train; score each on test; report the
    chosen model \emph{with} its test error.

    \textbf{(2) Looking, and splitting.} Plot before modelling --- the
    curvature is obvious and says a polynomial is worth trying. The split must
    precede every fitting decision, so nothing learned from the test set can
    influence the model.

    \textbf{(3) Only the first step is real.} The drop from $4.96$ to $4.52$
    is large and reflects genuine curvature: a line is the wrong shape. Beyond
    that the numbers differ by less than $0.12$ --- ordinary split-to-split
    noise. Degree $5$ is nominally lowest, but nothing meaningful separates it
    from degree $2$ --- so choose \textbf{degree 2}, the simplest model that
    is not beaten.

    \textbf{(4) The winner is unstable, so do not trust it.} A single split
    is a high-variance estimate, and the seed sweep proves it: three different
    ``best'' degrees appear. The fix is $k$-fold cross-validation (Chapter 5):
    here ($5$-fold, repeated $10\times$) it gives RMSEs of
    ...
    \textbf{(5) Leakage.} The scaler learned its mean and SD from data that
    includes the test set, so the test error is no longer an honest estimate
    of performance on new data. Fit the scaler on the training set alone.

    \textit{Common mistake:} rerunning with new seeds until the ranking
    looks right --- the sweep is a diagnostic, not a menu; a fixed seed buys
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{block}{Logs, odds and the logistic function}
    \[
      \log(ab) = \log a + \log b, \quad \log(a^k) = k \log a, \quad
      \log\frac{p(x)}{1-p(x)} = \beta_0 + \beta_1 x
      \;\Longrightarrow\;
      \text{odds ratio} = e^{\beta_1} .
    \]
    \[
      \mathrm{odds} = \frac{p}{1-p}, \quad
      p = \frac{\mathrm{odds}}{1+\mathrm{odds}}, \quad
      \mathrm{logit}(p) = \log\frac{p}{1-p}, \quad
...
\begin{frame}{Vocabulary check}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Cross-entropy & $-\ell$ for a classifier; the loss of module A9 \\
...
\begin{frame}{Where each topic reappears in the course}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Sets and $|\mathcal{S}|$ & Ch.\ 8 (tree regions), Ch.\ 12 clusters (Advanced) \\
    Logs and exponentials & Ch.\ 3 (logged responses), Ch.\ 4, module A9 \\
    Odds, logits, the sigmoid & Ch.\ 4 (logistic regression), module A9 (activations) \\
    Likelihood and log-likelihood & Ch.\ 4 (fitting), Ch.\ 6 (AIC, BIC) \\
    Maximum likelihood & Ch.\ 4, Ch.\ 6, module A9 (cross-entropy) \\
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    \item Read $\frac{1}{n}\sum_{i=1}^{n} I(y_i \ne \hat y_i)$ aloud, in words.
          (pp.~\pageref{frm:sums} and \pageref{frm:argmin})
    \item What is the difference between $\min_\theta f$ and
          $\arg\min_\theta f$? (p.~\pageref{frm:argmin})
    \item Simplify $\log(a^2 b) - 2\log a$. (p.~\pageref{frm:logexp})
    \item A logistic coefficient is $-0.7$. What happens to the odds? (p.~\pageref{frm:oddsratio})
    \item Convert log-odds $+1.6$ into a probability. (p.~\pageref{frm:threeways})
    \item Why is a log-likelihood a sum when a likelihood is a product? (p.~\pageref{frm:mle})
    \item Under what assumption is least squares equivalent to maximum
          likelihood? (appendix, p.~\pageref{frm:lsml})
    \item How many models does best-subset selection fit with $p = 20$?
          (appendix, p.~\pageref{frm:counting})
    \item What does \texttt{np.argmin} return --- and what does it not
          return? (p.~\pageref{frm:argmin})
    \item Name two things that must never be fitted on the test set. (p.~\pageref{frm:contract})
  \end{enumerate}
\end{frame}
...
\begin{frame}{What's next}
  ...
  \end{itemize}
  \begin{labnote}[{{Companion notebook: \texttt{chapter\_00b\_lab.ipynb}}}]
    The notebook (next to this deck) runs everything here, live:
    \S1 Reading mathematical notation; \S2 Logs and exponentials;
    \S3 Probability, odds and the logit; \S4 Likelihood and maximum likelihood;
    \S5 Counting: why best-subset selection dies; \S6 The Python patterns
    every lab uses --- plus worked solutions to the [Python] exercises.
  \end{labnote}
\end{frame}
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{The three scales, drawn} --- probability, odds and log-odds side by side; the conversion table in the main deck carries the same content numerically.
...
\begin{frame}{The three scales, drawn}
  ...
  probability. Read the picture right to left and you have the standing warning
  about odds: they are not probabilities, and converting a full set back gives
  numbers that sum to more than one.
\end{frame}

\begin{frame}{Least squares is maximum likelihood in disguise}\label{frm:lsml}
  \begin{center}
  ...
  \begin{takeaway}[One principle under two names]
    Maximising the likelihood \emph{is} minimising squared error whenever
    the errors are normal --- and with $\mu_i = \beta_0+\beta_1 x_i$ the
    same algebra makes Chapter 3's least squares maximum likelihood too.
    Chapter 4 keeps the principle and drops the normal assumption.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 0b.1 (1/5)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 0b.1 (2/5)}
  \begin{solutionbox}
    \textbf{(3) Maximise.}
    \[
...
\begin{frame}{Solution --- Extended Exercise 0b.1 (3/5)}
  \begin{solutionbox}
    \scriptsize
...
\begin{frame}{Solution --- Extended Exercise 0b.1 (4/5)}
  \begin{solutionbox}
    ...
    \]
    \emph{Read it against part (2):} the same Bernoulli log-likelihood,
    except each customer now has their \emph{own} $\pi_i$, tied to $x_i$
    through the sigmoid. Each $y_i=1$ contributes $\log\pi_i$, each
    $y_i=0$ contributes $\log(1-\pi_i)$ --- the sum rewards coefficients
    that assign high probability to what actually happened, case by case.
    This is exactly what Chapter 4's \texttt{statsmodels} call maximises.
  \end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 0b.1 (5/5)}
  \begin{solutionbox}
    ...
    module A9 (deep learning): the same quantity under a different name.
  \end{solutionbox}
  \begin{mistake}
    Treating $\hat\pi = k/n$ as exact. With $k=0$
    the MLE announces ``defaults are impossible'' --- an estimate inherits the
    sample's noise and needs a standard error, not blind faith.
  \end{mistake}
\end{frame}

\begin{frame}{How many models are there?}\label{frm:counting}
  \footnotesize
  ...
  \begin{exercise}
    ...
    \begin{enumerate}
      ...
      \item Testing every model against the data and keeping the best
            ...
            is it? (One sentence; module A7 gives it a name.)
    \end{enumerate}
  \end{exercise}
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    Run \texttt{chapter\_00b\_lab.ipynb} \S5 alongside: the counting cells print
    $\binom{6}{2} = 15$, $2^6 = 64$ and $22$ stepwise fits --- parts (1)--(3) ---
    plus the table behind part (4)'s $124$ days at $p = 30$.
  \end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 0b.5 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 0b.5 (2/2)}
  \begin{solutionbox}
    ...
    That is \textbf{multiple testing} (module A7, ISLP Ch.\ 13), and it is also why model
    choice must be judged on held-out data (Chapter 5).
  \end{solutionbox}
  \begin{mistake}
    Assuming the $22$ stepwise fits search the same
    space as the $64$. Stepwise follows one \emph{path} through the subsets,
    so it can miss the best model --- the speed is bought with a restricted
    search.
  \end{mistake}
\end{frame}
