...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.3cm}p{4.8cm}p{5.3cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{Which decision it drives} \\
    \midrule
    Banking & Credit scorecards and probability-of-default models, built on
      the log-odds scale &
      Approve, refer or decline --- and at what price \\
    Retail, airlines & Log--log demand models, whose coefficient \emph{is}
      the price elasticity &
      How far a price is allowed to move \\
    Insurance & Frequency and severity models fitted by maximum likelihood &
      The technical premium quoted for a policy \\
    Finance & Continuously compounded log returns; CAGR &
      What gets reported as performance and volatility \\
    Telco, SaaS & Retention curves that decay roughly exponentially &
      How much may be spent to acquire a customer \\
    Operations, call centres & Arrival counts and service times fitted by
      Poisson and exponential likelihoods &
      Staffing levels set for each interval \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{industry}[The common thread]
    In each row the model is linear on a \emph{transformed} scale --- log,
    log-odds --- and someone must transform back before the number is used.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: price elasticity of demand}
  \footnotesize
  A retailer or an airline needs one number: how much volume a price rise costs.
  \vspace{-6pt}
  \begin{industry}[The model]
    \textbf{Response}: $\log(\text{units sold})$, per product and week.
    \textbf{Predictors}: $\log(\text{price})$ plus controls --- a promotion
    flag, seasonality, a competitor price index, the channel.
    \textbf{Why both sides are logged}: the slope of a $\log$--$\log$
    regression \emph{is} the elasticity --- read straight off the output,
    nothing to transform back.
  \end{industry}
  \vspace{-6pt}
  \begin{numexample}[Illustrative]
    An estimated $\beta_1 \approx -1.8$ says a $1\%$ price rise costs about
    $1.8\%$ of volume. For a bigger move the multiplicative form is the honest
    one: a $10\%$ price cut lifts volume by $0.9^{-1.8}-1 \approx 21\%$ (not
    $18\%$), so revenue \emph{rises}. Comparing $|\beta_1|$ with $1$ is the
    whole calculation: above $1$ demand is elastic and a price rise loses
    revenue; below $1$ it gains.
  \end{numexample}
  \vspace{-6pt}
  \begin{takeaway}[Who signs off --- and the honest caveat]
    Pricing or category management own the list price, promotion depth and
    fare-class prices; finance checks the margin. The number itself lands with
    the category manager setting next season's price ladder. Caveat: past prices were cut
    \emph{because} demand was weak, so the estimate is confounded unless it
    comes from experiments or genuinely exogenous price moves --- and never
    extrapolate to a price never tried.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Why logs are everywhere in statistics}
  ...
  \end{numexample}
  \begin{industry}[Finance and pricing: why logs are everywhere]\scriptsize
    Log returns add up over time; and in a $\log$--$\log$ demand model the slope
    \emph{is} the price elasticity.
  \end{industry}
\end{frame}
...
\begin{frame}{Maximum likelihood outside the classroom}
  \footnotesize
  \begin{industry}[Insurance and banking: pricing models]
    Claim frequency and severity models, probability-of-default models and
    demand models are nearly all fitted by maximum likelihood rather than least
    squares --- because the response is a count, a $0/1$ event, or a skewed
    positive amount, and none of those has normal errors.
  \end{industry}
  \begin{industry}[Telco and software: exponential decay]
    Retention curves fall away roughly exponentially, so the log of the share
    still active is close to linear in time. Fitting on that scale gives the
    expected customer lifetime that acquisition budgets and lifetime-value
    figures rest on.
  \end{industry}
...
\begin{frame}{The price of a probability: $-\log \hat p$}
  ...
  \begin{takeaway}
    ...
    of logistic regression (Ch.~4) and of cross-entropy (Ch.~10).
  \end{takeaway}
\end{frame}
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    ...
    \item What is the difference between $\min_\theta f$ and
          $\arg\min_\theta f$? (p.~12)
    \item Simplify $\log(a^2 b) - 2\log a$. (p.~17)
    \item A logistic coefficient is $-0.7$. What happens to the odds? (p.~23)
    \item Convert log-odds $+1.6$ into a probability. (p.~23)
    \item Why is a log-likelihood a sum when a likelihood is a product? (p.~30)
    \item Under what assumption is least squares equivalent to maximum
          likelihood? (appendix, p.~53)
    \item How many models does best-subset selection fit with $p = 20$?
          (appendix, p.~57)
    \item What does \texttt{np.argmin} return --- and what does it not
          return? (p.~12)
    \item Name two things that must never be fitted on the test set. (p.~38)
  \end{enumerate}
\end{frame}
...
\begin{frame}{The three scales, drawn}
  ...
  probability.
  \begin{industry}[Betting and insurance: odds are not probabilities]\scriptsize
    A bookmaker's price is odds; converting back gives implied probabilities
    that sum to more than one --- the margin. Read odds as probabilities and
    you misprice the risk.
  \end{industry}
\end{frame}
