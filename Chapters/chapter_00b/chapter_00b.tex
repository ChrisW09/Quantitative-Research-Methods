...
\begin{frame}{The odds ratio --- and why a coefficient \emph{is} one}
  \footnotesize
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    ...
    \item What is the difference between $\min_\theta f$ and
          $\arg\min_\theta f$? (p.~11)
    \item Simplify $\log(a^2 b) - 2\log a$. (p.~16)
    \item A logistic coefficient is $-0.7$. What happens to the odds? (p.~22)
    \item Convert log-odds $+1.6$ into a probability. (p.~22)
    \item Why is a log-likelihood a sum when a likelihood is a product? (p.~29)
    \item Under what assumption is least squares equivalent to maximum
          likelihood? (appendix, p.~52)
    \item How many models does best-subset selection fit with $p = 20$?
          (appendix, p.~56)
    \item What does \texttt{np.argmin} return --- and what does it not
          return? (p.~11)
    \item Name two things that must never be fitted on the test set. (p.~37)
  \end{enumerate}
\end{frame}
