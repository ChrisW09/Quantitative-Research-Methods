...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \midrule
    \rowcolor{accent!18} \textbf{Ch.\ 1} & \textbf{Introduction} \\
...
\begin{frame}{Notation in this chapter}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    $\mathbf{y}$ & $n$-vector stacking the responses $y_1,\dots,y_n$ \\
...
\begin{frame}{Learning objectives for this chapter}
  ...
  \begin{readme}[{{How to use this deck --- self-study and revision}}]
    ...
    \texttt{chapter\_01\_lab.ipynb} alongside the lab section. Revise from the
...
\begin{frame}{Two business problems we follow all semester}
  \scriptsize
  ...
  \begin{column}{0.48\textwidth}
    \textbf{A retail bank's credit scorecard}\\[0.4ex]
    \begin{tabular}{@{}p{1.0cm}p{5.9cm}@{}}
      \toprule
      ...
      A7      & fairness testing across many subgroups \\
      \bottomrule
    \end{tabular}
  \end{column}
  \begin{column}{0.48\textwidth}
    \textbf{A retailer's pricing and demand question}\\[0.4ex]
    \begin{tabular}{@{}p{1.0cm}p{5.9cm}@{}}
      \toprule
      ...
      A9      & one network across thousands of series \\
      \bottomrule
    \end{tabular}
  \end{column}
  \end{columns}
  ...
  \begin{takeaway}[One decision --- many chapters]
    Neither list is a tour of techniques. The same decision --- accept this applicant, set this
    price --- keeps coming back, and each chapter adds the piece the previous tool could not
    supply: a validated card, a shorter list of attributes, a curve that saturates. Methods are
    chosen by what the decision needs, never because they are new.
  \end{takeaway}
\end{frame}
...
\begin{frame}{What is statistical learning?}
  \begin{block}{In one sentence}
    Statistical learning is the use of \textbf{past data} to build a \textbf{rule} that turns
    what we \emph{can} measure into what we \emph{want to know} --- together with an honest
    account of how far that rule can be trusted.
  \end{block}
  \begin{readme}[The same sentence with a data set attached]
    \emph{Past data}: $3\,000$ recorded salaries, each with the age, education and year that
    came with it. \emph{The rule}: what to expect a 45-year-old graduate to earn.
    \emph{How far to trust it}: two people with identical records still earn different
    salaries, and a useful model says how wide that spread is.
  \end{readme}
  \begin{takeaway}The second half is what makes it \emph{statistical}. Any program can print a number; a statistical model also says how much of that number is noise.\end{takeaway}
\end{frame}

\begin{frame}{The one equation the whole course rests on}
  \begin{block}{The statistical-learning model}
    \setlength{\abovedisplayskip}{3pt}\setlength{\belowdisplayskip}{1pt}
    With a response $Y$ and predictors $X=(X_1,\dots,X_p)$, assume a systematic relationship:
    \[ \boxed{\;Y=f(X)+\varepsilon\;} \]
  \end{block}
  \begin{readme}
    $f$ is the \emph{systematic} part --- what $X$ genuinely says about $Y$: fixed, real, never
    observed. $\varepsilon$ is everything else (variables nobody recorded, plain chance), with
    mean zero. \emph{Learning} means estimating $f$ by an $\hat f$ built from $n$ past cases.
  \end{readme}
  \begin{takeaway}Every method in this course is another way of estimating the same $f$; they differ only in what they assume about its shape --- and none can beat $\varepsilon$.\end{takeaway}
\end{frame}

\begin{frame}{Which box is my problem in? Two questions decide (native diagram)}
  Do not start from the method. Start from two questions about the data you already have.
  \vspace{-1mm}
  \begin{center}
  \begin{tikzpicture}[
    q/.style={rounded corners=3pt,draw=accent,fill=accent,text=white,font=\small\bfseries,text width=2.8cm,align=center,inner sep=2.5pt,minimum height=0.95cm},
    cat/.style={rounded corners=3pt,draw=accent,fill=accent!15,font=\small\bfseries,text width=2.4cm,align=center,inner sep=2.5pt,minimum height=0.9cm},
    ex/.style={rounded corners=2pt,draw=green!55!black,fill=green!8,font=\scriptsize,text width=2.4cm,align=center,inner sep=2.5pt,minimum height=0.8cm},
    ed/.style={-{Latex[length=2mm]},thick,draw=accent},
    lab/.style={font=\scriptsize\itshape,inner sep=1.5pt,fill=white}]
   \node[q]   (q1)  at (-5.4, 0.00) {Q1. Is a response $Y$ recorded for the training cases?};
   \node[q]   (q2)  at (-1.7,-1.35) {Q2. Is that $Y$ a number or a class?};
   \node[cat] (uns) at ( 2.05, 1.35) {Unsupervised\\{\scriptsize no $Y$ at all}};
   \node[cat] (reg) at ( 2.05,-0.35) {Regression\\{\scriptsize $Y$ is a number}};
   \node[cat] (cla) at ( 2.05,-2.05) {Classification\\{\scriptsize $Y$ is a class}};
   \node[ex]  (e3)  at ( 5.30, 1.35) {\textbf{NCI60}\\gene expression, no label at all};
   \node[ex]  (e1)  at ( 5.30,-0.35) {\textbf{Wage}\\a salary in dollars};
   \node[ex]  (e2)  at ( 5.30,-2.05) {\textbf{Smarket}\\market up or down};
   \draw[ed] (q1.east) -- ++(0.45,0) |- (uns.west);
   \draw[ed] (q1.east) -- ++(0.45,0) |- (q2.west);
   \draw[ed] (q2.east) -- ++(0.35,0) |- (reg.west);
   \draw[ed] (q2.east) -- ++(0.35,0) |- (cla.west);
   \node[lab] at (-3.55, 0.85) {no};
   \node[lab] at (-3.55,-0.85) {yes};
   \draw[ed] (uns.east)--(e3.west);
   \draw[ed] (reg.east)--(e1.west);
   \draw[ed] (cla.east)--(e2.west);
  \end{tikzpicture}
  \end{center}
  \vspace{-3.5mm}
  \begin{takeaway}Two answers fix the branch. Which algorithm you then use --- and how flexible it is --- is a choice \emph{inside} the box, and that is what the rest of the course is about.\end{takeaway}
\end{frame}

\begin{frame}{Answering the two questions --- and the usual traps}
  \vspace{-1mm}
  \begin{readme}[Q1 asks what your rows contain --- not what you wish they contained]
    A firm that wants to score churn but never recorded who churned has no supervised problem
    --- it has a labelling project. Outcome written down: credit decisions, weekly demand,
    churn. Nothing written down: segmentation, anomaly detection.
  \end{readme}
  \begin{readme}[Q2 asks about the response only --- never the predictors]
    A postcode stored as an integer is a class; a five-star rating stored as text is a number.
    The test: what would being \emph{wrong} mean? Off by how much $\Rightarrow$ regression; in
    the wrong bucket $\Rightarrow$ classification.
  \end{readme}
  \begin{takeaway}[Why the branch matters at all]
    It fixes how you keep score: squared error, or misclassification rate --- and for
    unsupervised problems, no ``accuracy'' at all: nothing to be right about.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Two distinct goals: prediction vs.\ inference}\label{frm:predinf}
...
\begin{frame}{Solution --- Exercise 1.1 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 1.1 (2/3)}
\begin{solutionbox}
  \begin{enumerate}
...
\begin{frame}{Solution --- Exercise 1.1 (3/3)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
Classifying by the \emph{tool} instead of the
...
\begin{frame}{Three motivating examples set the tone}
  ...
  \begin{takeaway}
    \begin{itemize}
      \item \textbf{Wage}: regression, mostly inference --- the same
            shape as a pay-equity review, where the coefficient
            \emph{is} the deliverable.
      \item \textbf{Smarket}: classification, weak signal --- the same
            shape as a trading or forecasting signal with a very low
            signal-to-noise ratio.
      \item \textbf{NCI60}: unsupervised, structure discovery --- the
            same shape as segmenting customers or products where no
            label exists.
    \end{itemize}
  \end{takeaway}
\end{frame}
...
\begin{frame}{Example 1: Wage --- a regression problem}
  ...
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    Switch to \texttt{chapter\_01\_lab.ipynb} \S1 and run the loading and plotting cells: \texttt{Wage.shape} prints \texttt{(3000, 11)}, and the three-panel reproduction of Fig.~1.1 shows the wage--age curvature live.
  \end{labnote}
\end{frame}
...
\begin{frame}{Example 2: Smarket --- a classification problem}
  ...
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Switch to \texttt{chapter\_01\_lab.ipynb} \S2 and run the boxplot cell: the \texttt{Lag1}--\texttt{Lag3} boxes for \texttt{Up} vs.\ \texttt{Down} days overlap almost completely --- the weak signal the next slides discuss, drawn from the raw data.
  \end{labnote}
\end{frame}
...
\begin{frame}{Reading the boxplots}
  Each panel compares returns from one, two, and three days ago
  for days where the market went \texttt{Up} vs.\
  \texttt{Down}.
...
\begin{frame}{Example 3: NCI60 --- an unsupervised problem}\label{frm:nci60}
  \begin{block}{The data}
  ...
  \begin{labnote}[{{Live demo --- \S3 of the notebook}}]
    Switch to \texttt{chapter\_01\_lab.ipynb} \S3 and run the PCA cell (needs the \texttt{ISLP} package; Colab installs it automatically): $6\,830$ genes collapse to two components, and cell lines of the same cancer type land next to each other.
  \end{labnote}
\end{frame}
...
\begin{frame}{The three problems on one map}\label{frm:map}
  \begin{center}
    \includegraphics[width=0.74\textwidth,height=0.50\textheight,keepaspectratio]{ch01_n_vs_p.png}
  \end{center}
  \begin{takeaway}
    Where a data set sits decides which chapters apply. Below the diagonal
    ($n \gg p$) least squares behaves; NCI60 sits at $p/n \approx 107$,
    where no unique least-squares fit exists --- hence Chapter~6, and the
    unsupervised Advanced module (Ch.~12).
  \end{takeaway}
\end{frame}
...
\begin{frame}{What the three examples illustrate}
  ...
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Run the summary-table cell in \texttt{chapter\_01\_lab.ipynb} \S4: it prints $n$, $p$ and $p/n$ for the three data sets side by side --- $p/n \approx 0.003$ for Wage against $107$ for NCI60.
  \end{labnote}
\end{frame}
...
\begin{frame}{Notation: $n$, $p$, and the design matrix}\label{frm:notation}
...
\begin{frame}{Solution --- Exercise 1.2 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 1.2 (2/3)}
\begin{solutionbox}
\begin{enumerate}\setcounter{enumi}{3}
  ...
  \item \textbf{Unsupervised} (dimension reduction); no response $Y$.
        ...
        answers --- PCA / factor-analysis territory (Ch.~12, an
        optional Advanced module).
  \end{enumerate}
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Exercise 1.2 (3/3)}
\begin{solutionbox}
  \begin{enumerate}
...
\end{solutionbox}
\begin{mistake}
Letting the predictors vote --- e.g.\ calling
...
\begin{frame}{Why Python?}
  ...
  \begin{readme}
    ...
    throughout.
  \end{readme}
  \begin{takeaway}[{{Two libraries, two jobs}}]
    Reach for \texttt{statsmodels} where a coefficient has to be defended, and for
    \texttt{scikit-learn} where a score has to be produced every night.
  \end{takeaway}
\end{frame}
...
\begin{frame}[fragile]{Standard imports}
  \begin{lstlisting}
import numpy as np                 # arrays/linear algebra: maths layer
import pandas as pd                # labelled tables: the data layer
import matplotlib.pyplot as plt    # plots: look before you model

# scikit-learn, in the order you use it: model, split, score
from sklearn.linear_model    import LinearRegression, LogisticRegression
from sklearn.model_selection import train_test_split, cross_val_score
from sklearn.metrics         import mean_squared_error, accuracy_score

from ISLP import load_data         # book datasets, loaded by name
  \end{lstlisting}
  \begin{readme}
    Adopt these as the default first cell of every notebook. Nothing is
    computed here --- the cell only makes tools available, so it prints
    nothing. The \texttt{sklearn} lines supply, in the order you use them, the
    \emph{model}, the \emph{split} that keeps a score honest, and the
    \emph{metric}.
  \end{readme}
\end{frame}
...
A @ A         # @ is the matrix product, not elementwise
np.linalg.inv(A)                # inverse -- appears in least squares
np.mean(x), np.std(x)           # summaries: centre and spread
  \end{lstlisting}
  \begin{readme}
    Shapes are what you check first: \texttt{(4,)} is a vector of four numbers,
    \texttt{(2, 2)} is two rows by two columns. Every design matrix in this
    course is $n \times p$ (rows = observations, columns = predictors), so an
    unexpected shape means a transposed matrix, not a broken formula.
  \end{readme}
\end{frame}
...
df = pd.read_csv("Wage.csv")    # CSV on disk -> a table in memory

df.shape           # (3000, 11) -- n = 3000 rows, 11 columns
df.head()          # eyeball the first 5 rows before anything
df["wage"].mean()  # a column behaves like a vector
df.groupby("education")["wage"].mean()  # does wage differ by group?
df.describe()      # count/mean/std/quartiles, all columns
  \end{lstlisting}
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    Read \texttt{.shape} first: those two numbers are the $n$ and (almost) the $p$ of the problem. These calls run for real in \texttt{chapter\_01\_lab.ipynb} \S1: \texttt{Wage.describe(include='all').T} shows which columns are numeric and which categorical --- e.g.\ mean age $42.4$, top education level \emph{2.\ HS Grad} (971 of $3\,000$).
  \end{labnote}
\end{frame}
...
X = df[["age", "year"]].values  # design matrix X: n rows, p = 2
y = df["wage"].values           # response y: one number per row
model = LinearRegression()      # pick the model class (not fitted yet)
model.fit(X, y)                 # estimate f-hat from the data

print(model.coef_, model.intercept_)  # the fitted f-hat, in numbers
yhat = model.predict(X)         # apply f-hat: y-hat for every row
  \end{lstlisting}
  Every scikit-learn model follows the same \texttt{fit / predict}
  API.
  \begin{readme}
    \texttt{fit} is the estimation step, and it prints nothing: it
    \emph{changes the model object}, storing the estimates in \texttt{coef\_}
    and \texttt{intercept\_} --- the underscore marks `learned from data'.
    \texttt{predict} reuses them on any \texttt{X} with the same $p$ columns.
  \end{readme}
\end{frame}
...
\begin{frame}{Datasets used throughout the book}
  ...
  \begin{takeaway}
    ...
    laptop in seconds. Outside a textbook no such tidy table exists: it
    has to be joined together \emph{as of the decision date}, and a
    field recorded only \emph{after} the decision is the classic
    leakage bug.
  \end{takeaway}
  \begin{labnote}[{{Run this live}}]
    Exercise 1.3 (next slides) is worked in Python in \texttt{chapter\_01\_lab.ipynb}, under \emph{Lecture exercises --- worked Python solutions}: the cell recomputes every number ($n=5$, $p=3$, $x_{2,3}=82$).
  \end{labnote}
\end{frame}
...
\end{solutionbox}
\begin{mistake}
Reading $x_{2,3}$ column-first as ``column 2,
...
\begin{frame}{The book in one slide}
  \begin{tabular}{rl}
    ...
    Ch.\,9  & Support Vector Machines\,{\color{black!45}--- Advanced module} \\
    Ch.\,10 & Deep Learning \\
    Ch.\,11 & Survival Analysis and Censored Data\,{\color{black!45}--- Advanced module} \\
    Ch.\,12 & Unsupervised Learning (PCA, clustering)\,{\color{black!45}--- Advanced module} \\
    Ch.\,13 & Multiple Testing\,{\color{black!45}--- Advanced module} \\
    \bottomrule
  \end{tabular}
  \vspace{1mm}

  {\scriptsize The four modules marked \emph{Advanced} are not taught in the nine sessions ---
  they sit in \texttt{Chapters/Advanced/} as optional self-study decks.}
\end{frame}
...
\begin{frame}{The narrative arc}
  \begin{itemize}
    ...
    \item Chapter 12 returns to unsupervised problems --- an optional
          Advanced module, and the natural sequel to this chapter's
          NCI60 example.
...
\begin{frame}{Solution --- Extended Exercise 1.1 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 1.1 (2/4)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
Counting the response \texttt{defaulted} among the predictors (giving $p=8$) --- the response is never a column of $\mathbf X$.
\end{mistake}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 1.1 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 1.1 (4/4)}
\begin{solutionbox}
  ...
  \begin{alertblock}{The classic mistakes}
    \begin{enumerate}
      ...
      \item Forgetting to standardise features for distance-based
            methods.
      \item Treating $p$-values as truth instead of evidence (module A7).
      \item Re-using the test set during model selection.
    \end{enumerate}
  \end{alertblock}
  \begin{takeaway}[The two that kill whole projects]
    Training on a population different from the one you will score, and shipping a score nobody
    can explain to the person who has to act on it.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Self-check questions}
  ...
  \begin{enumerate}
    ...
    \item For the Boston housing data ($n = 506$, $p = 12$), what
          ...
          Explain. (pp.~\pageref{frm:nci60} and \pageref{frm:notation})
    \item Write the design matrix $\mathbf{X}$ for $n = 4$
          observations of $p = 2$ predictors plus an intercept. (appendix, pp.~\pageref{frm:design}--\pageref{frm:anatomy})
  \end{enumerate}
\end{frame}
...
\begin{frame}{Get hands-on: the companion lab notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_01\_lab.ipynb}]
    ...
    \begin{itemize}
      \item \S1 \emph{The Wage data} --- reproducing Fig.~1.1;
      \item \S2 \emph{The Stock Market data} --- \emph{Smarket} (Fig.~1.2);
      \item \S3 \emph{The NCI60 cancer cell lines};
      \item \S4 \emph{The three problem types, side by side} --- $n$, $p$ and $p/n$ in one table.
    \end{itemize}
...
\begin{frame}{Key formulas and notation at a glance}
  \footnotesize
  \begin{description}
    \item[Data] $n$ observations, $p$ predictors; design matrix $\mathbf{X}\in\mathbb{R}^{n\times p}$, response $\mathbf{y}\in\mathbb{R}^n$.
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{The two recreated overview figures} --- Wage close up and the Smarket lag boxplots, computed from the raw data. The main deck teaches from the ISLP originals; these show the same pictures can be rebuilt in a dozen lines.
      \item \textbf{The design matrix in full} --- the anatomy of $\mathbf{X}$ and $\mathbf{y}$ drawn out entry by entry, and the notation for estimates. The one-slide version in the main deck (``Notation: $n$, $p$, and the design matrix'') is all the chapter needs.
      \item \textbf{The dataset catalogue} --- the two reference tables of every ISLP data set. Look them up when a lab asks for a data set you have not met.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Recreating Figure 1.2 from the raw Smarket data}
  \begin{center}\includegraphics[width=0.88\textwidth,height=0.5\textheight,keepaspectratio]{ch01_x_smarket_lag_boxplots.png}\end{center}
...
\begin{frame}{The design matrix}\label{frm:design}
  ...
  \begin{readme}
    \begin{itemize}
      ...
      \item $x_{ij}$: value of the $j$-th predictor for the $i$-th
            observation.
      \item The book's convention (ISLP \S1, notation): vectors are
            \emph{columns} by default, so by rows
            $\mathbf{X} = (x_1\ \cdots\ x_n)^\top$ (transposes needed), by
            columns simply $(\mathbf{x}_1\ \cdots\ \mathbf{x}_p)$ --- the
            same matrix, sliced two ways.
    \end{itemize}
  \end{readme}
\end{frame}

\begin{frame}{Anatomy of the data: the matrix $\mathbf{X}$ and the vector $\mathbf{y}$}\label{frm:anatomy}
  \begin{center}
  \begin{tikzpicture}[
...
\begin{frame}{Datasets at a glance (1/2)}
  ...
  \begin{tabular}{lrrll}
    ...
    \midrule
    Advertising  &  200 &   3 & sales (num.)       & Ch.~2, 3 --- the running example \\
    Wage         & 3000 &  10 & wage (num.)        & Ch.~1, 7 \\
    Smarket      & 1250 &   8 & Direction (binary) & Ch.~1, 4 \\
    NCI60        &   64 & 6830 & --- (unsup.)       & Ch.~1, 12$^{\dagger}$ \\
    Income1 / 2  & 30 / 30 & 1 / 2 & Income (num., simulated) & Ch.~2 \\
  ...
  {\scriptsize $\dagger$ optional Advanced self-study module.}
\end{frame}
...
\begin{frame}{Datasets at a glance (2/2)}
  ...
  \begin{tabular}{lrrll}
    ...
    \textbf{Name} & $n$ & $p$ & \textbf{Response} & \textbf{Used in} \\
    \midrule
    Weekly       & 1089 &   8 & Direction (binary) & Ch.~4 exercises \\
    Caravan      & 5822 &  85 & Purchase (binary)  & Ch.~4 \\
    Portfolio    &  100 &   2 & --- (bootstrap)    & Ch.~5 \\
    OJ           & 1070 &  17 & Purchase (binary)  & Ch.~8, 9$^{\dagger}$ \\
    Hitters      &  322 &  19 & Salary (num.)      & Ch.~6, 8 \\
    College      &  777 &  17 & Apps (num.)        & Ch.~6 \\
    BrainCancer  &   88 &   6 & survival (time, status) & Ch.~11$^{\dagger}$ \\
    Publication  &  244 &   7 & survival (time, status) & Ch.~11$^{\dagger}$ \\
    Fund         &   50 & 2000 & --- (testing)      & Ch.~13$^{\dagger}$ \\
    USArrests    &   50 &   4 & --- (unsup.)       & Ch.~12$^{\dagger}$ \\
    Bikeshare    & 8645 &  14 & bikers (num.)      & Ch.~4 \\
    \bottomrule
  \end{tabular}
  \vspace{1mm}

  {\scriptsize $\dagger$ optional Advanced self-study module. \quad
  (\texttt{Ch12Ex13.csv} is an exercise file for module A8, not a named data set.)}
\end{frame}
