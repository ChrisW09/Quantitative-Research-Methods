...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.2cm}p{6.0cm}p{5.0cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{Which decision it drives} \\
    \midrule
    Banking & Credit scoring at application: probability of default from bureau and application data & Accept or decline this applicant; where to set the cut-off and the credit limit \\
    Retail & Weekly demand forecast per article and store & How many units to order and where to set the reorder point \\
    Telco, SaaS & Churn score per subscriber for the coming quarter & Which subscribers receive a retention offer, and which are left alone \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{takeaway}[A number is only worth modelling if it moves a lever]
    All three rows need the same three ingredients: a response somebody actually recorded,
    predictors that are \emph{available before} the decision is taken, and a lever that moves
    when the number moves. Without the third, the model is a report, not a tool.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    ...
    \item For the Boston housing data ($n = 506$, $p = 12$), what
          ...
          Explain. (pp.~36 and 40)
    \item Write the design matrix $\mathbf{X}$ for $n = 4$
          observations of $p = 2$ predictors plus an intercept. (pp.~54--55)
  \end{enumerate}
\end{frame}
...
\begin{frame}{Datasets at a glance (2/2)}
  ...
  \begin{tabular}{lrrll}
    ...
    BrainCancer  &   88 &   8 & survival           & Ch.~11$^{\dagger}$ \\
