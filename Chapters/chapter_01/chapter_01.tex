...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{industry}[The common thread]
    Every row needs the same three things: a response worth predicting, predictors that are
    \emph{available before} the decision, and a decision that changes when the number changes.
    Without the third, the model is a report, not a tool.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: predictive maintenance at a manufacturer}
  \footnotesize
  \begin{industry}[The learning problem]
    \textbf{Response} $Y$: will this asset need an intervention within the next $N$ operating
    days or cycles? --- binary, supervised and heavily imbalanced. \textbf{Predictors}:
    telemetry aggregates (vibration, temperature, pressure), operating hours and load,
    maintenance and fault history, asset age, ambient conditions.
  \end{industry}
  \begin{readme}[The fitted output --- and who signs it off]
    A \emph{risk score per asset per period}, not a failure date. The decision: pull the asset
    into the next planned maintenance window, or let it run. Maintenance planning and
    reliability engineering own the score; operations sign off, because an intervention costs
    production time. The score is read by the production planner on the affected line, who
    either releases the asset into the window or pushes back because the order book cannot
    absorb the downtime.
  \end{readme}
  \begin{takeaway}[One honest caveat]
    Failures are rare, so labels are scarce --- and every failure the model prevents is a
    \emph{censored} observation nobody can confirm: its own success degrades its future
    training data. The value lies in the decision it changes, not in the score.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Two business problems we follow all semester}
  ...
  \begin{industry}[Banking and retail: one problem --- many chapters]
...
\begin{frame}{What is statistical learning?}
  ...
  \begin{readme}
    ...
    \end{itemize}
  \end{readme}
  \begin{industry}[Analytics teams: which half a firm's questions fall into]
    Supervised, because the outcome was recorded: churn scores, credit decisions, demand
    forecasts. Unsupervised: segmentation, anomaly detection --- no label, so no ``accuracy''.
  \end{industry}
\end{frame}
...
\begin{frame}{Three motivating examples set the tone}
  ...
  \begin{takeaway}
    ...
    \end{itemize}
  \end{takeaway}
  \begin{industry}[From HR to trading: the same three flavours]
    A \emph{Wage}-type question is a pay-equity or compensation review --- the coefficient is
    the deliverable. A \emph{Smarket}-type question is a trading or forecasting signal with a
    very low signal-to-noise ratio. An \emph{NCI60}-type question is segmenting customers or
    products where no label exists.
  \end{industry}
\end{frame}
...
\begin{frame}{The three problems on one map}
  ...
  \begin{takeaway}
    ...
    where no unique least-squares fit exists --- hence Chapters~6 and~12.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Why Python?}
  ...
  \begin{readme}
    ...
    throughout.
  \end{readme}
  \begin{industry}[Banking and retail: the production stack]
    pandas, scikit-learn and statsmodels are the default analytics stack in banks, insurers
    and retailers: \texttt{statsmodels} where a coefficient has to be defended,
...
\begin{frame}{Datasets used throughout the book}
  ...
  \begin{takeaway}
    ...
    laptop in seconds.
  \end{takeaway}
  \begin{industry}[Data engineering: why company data never arrives tidy]
    The equivalent table does not exist in a firm: it has to be built by joining warehouse
    tables \emph{as of the decision date}, and most of a project's time goes into that join.
    Pulling in a field that was only recorded \emph{after} the decision is the classic
    leakage bug --- the model looks excellent in testing and fails in production.
  \end{industry}
...
\begin{frame}{Pitfalls to remember from day one}
  \begin{alertblock}{The classic mistakes}
    ...
    \end{enumerate}
  \end{alertblock}
  \begin{industry}[Project delivery: what kills a model project]
    Target leakage from fields written after the decision; training on a population different
    from the one being scored; nobody owning the model once it is live; and a score nobody can
    explain to a regulator or to a sales director.
  \end{industry}
\end{frame}
