...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{Ch.\ 2} & \textbf{Statistical learning}\,{\color{accent!75!black}--- accuracy, bias--variance, Bayes classifier, KNN} \\
...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.2cm}p{6.0cm}p{5.0cm}@{}}
    ...
    \midrule
    Telco, SaaS & Churn score per subscriber & A \emph{ranking}: who gets a retention offer \\
  ...
  \begin{industry}[The common thread]
    Before picking a method, each row answers two questions from this chapter: do we need a
    \emph{rank} or a \emph{coefficient}, and how much of the variation is irreducible?
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: an item-to-item recommender}
  \footnotesize
  \begin{industry}[The learning problem: this chapter's own method]
    There is no coefficient to estimate: the ``model'' is a \emph{nearest-neighbour lookup} in
    a similarity space built from co-purchase and co-view behaviour, or from learned item
    embeddings. $K$ is simply how many neighbours are retrieved for the item on the page.
  \end{industry}
  \begin{readme}[The fitted output --- and who signs it off]
    A \emph{ranked slate} of items, which fills the recommendation module on the product page
    or the home screen. Only the ordering matters --- nothing in this model is interpretable,
    and nobody asks it to be. Merchandising and personalisation own the module, and the
    slate is judged by the category merchandiser who carries its revenue target --- whose
    challenge is almost always ``why is my range never shown?''.
  \end{readme}
  \begin{takeaway}[Honest caveats]
    Wide sparse behaviour vectors put the curse of dimensionality centre stage --- hence
    embeddings and approximate nearest-neighbour search in production. Add popularity bias,
    cold start for new items, and a feedback loop: showing an item creates the very co-view
    data the next model trains on.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Worked example: prediction goal}
  \begin{numexample}[Direct-mail marketing]
    ...
    \end{itemize}
  \end{numexample}
  \begin{industry}[Telco and SaaS: churn scoring]
    A churn model only has to \emph{rank} subscribers well: the retention team calls the top
    few per cent, and nobody asks why a customer is on the list --- so a black box is
    acceptable. Rank badly and the discount budget goes to customers who would have stayed
    anyway.
  \end{industry}
\end{frame}
...
\begin{frame}{Worked example: inference goal}
  \begin{numexample}[Wage data]
    ...
    \end{itemize}
  \end{numexample}
  \begin{industry}[Pricing and pay equity: the coefficient \emph{is} the deliverable]
    A price-elasticity model must hand over a signed, quantified effect --- ``a $1\,\%$ price
    rise costs us this much volume'' --- because a person sets the price from it. A pay-equity
    review is the same: only a model whose structure can be reviewed answers it.
  \end{industry}
\end{frame}
...
\begin{frame}{Training MSE vs.\ test MSE}
  ...
  \begin{alertblock}{The cardinal sin}
    ...
    itself.
  \end{alertblock}
  \begin{industry}[Procurement: buying a model from a vendor]
    An accuracy figure quoted on the data the model was fitted to is worth nothing. Ask for
    performance on a \emph{later} period than the one used for fitting --- out-of-time
    validation is exactly what a model risk function will demand before the model goes live.
  \end{industry}
\end{frame}
...
\begin{frame}{Bias and variance trade off}
  ...
  \end{itemize}
  \begin{industry}[Asset management: why parsimony wins]
    In settings where the signal is tiny relative to the noise --- asset returns are the
    textbook case --- flexible models mostly fit noise, so the variance term dominates and
    small, heavily constrained models routinely beat them out of sample. The same logic
    explains why a few well-chosen predictors often outperform a large automated feature set.
  \end{industry}
\end{frame}
...
\begin{frame}{Choosing $K$ in practice}
  \begin{takeaway}[Practical advice]
    ...
    \end{itemize}
  \end{takeaway}
  \begin{industry}[E-commerce and marketing: where nearest neighbours run]
    Item--item ``customers who bought this'' recommenders, lookalike audience targeting,
    duplicate and entity resolution, and $k$-NN over embeddings for vector search and
    retrieval. The curse of dimensionality is the practical catch: run naively over a wide
    customer table, the ``nearest'' neighbours are not near, so firms reduce dimension or
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_02\_lab.ipynb}]
    ...
    \begin{itemize}
      ...
      \item \S5 \emph{KNN classification};
      \item \S6 \emph{KNN by hand, then by scikit-learn}.
    \end{itemize}
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    \item Sketch the bias-variance decomposition curves. Where is the
          optimal flexibility? (p.~58)
    \item Why is the irreducible error not a function of $\hat f$? (p.~14)
    \item Construct an example where a more flexible method is
          preferable to a linear model. (pp.~23 and 98)
    \item For KNN, what happens to bias and variance as $K$ grows? (p.~78)
