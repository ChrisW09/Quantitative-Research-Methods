...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{Ch.\ 2} & \textbf{Statistical learning}\,{\color{accent!75!black}--- accuracy, bias--variance} \\
...
\begin{frame}{Notation in this chapter}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    $\Phi(z)$ & standard-normal CDF, $\Pr(Z\le z)$ --- appendix exercises \\
    \bottomrule
  \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}{Learning objectives for this chapter}
  ...
  \begin{itemize}
    ...
    \item \textbf{Derive} and \textbf{interpret} the bias-variance
          decomposition.
    \item \textbf{Define} the Bayes classifier and the Bayes error rate,
          and \textbf{implement} the $K$-nearest-neighbours classifier ---
          both developed in the \textbf{appendix} as optional self-study.
  \end{itemize}
  \begin{readme}[{{How to use this deck --- self-study and revision}}]
    \footnotesize Attempt each exercise \emph{before} its solution; run
    \texttt{chapter\_02\_lab.ipynb} alongside the lab section. Revise from the
...
\begin{frame}{Roadmap of this chapter}
  \begin{takeaway}[The conceptual journey]
    \begin{enumerate}
      ...
      \item Classification --- the Bayes classifier and KNN --- is carried in
            the \textbf{appendix}; Chapter 4 teaches it properly.
    \end{enumerate}
  \end{takeaway}
\end{frame}
...
\begin{frame}{A simulated example: Income data}
  ...
  \end{center}
  Income vs.\ years of education. The smooth curve added on the right is the
  \emph{true} $f$ --- in real data we never see it.
...
\begin{frame}{Why irreducible error exists}\label{frm:irred}
...
\begin{frame}{Worked example: prediction goal}
  \begin{numexample}[Direct-mail marketing]
    ...
    \end{itemize}
  \end{numexample}
  A churn score has exactly this shape: the retention team calls the top few per cent, so
  only the \emph{ranking} matters and a black box is acceptable. Rank badly and the discount
  budget goes to customers who would have stayed anyway.
\end{frame}
...
\begin{frame}{Worked example: inference goal}
  \begin{numexample}[Wage data]
    ...
    \end{itemize}
  \end{numexample}
  A price-elasticity model is the same shape of question: it must hand over a signed,
  quantified effect --- ``a $1\,\%$ price rise costs us this much volume'' --- because a
  person sets the price from it. Here the coefficient \emph{is} the deliverable.
\end{frame}
...
\begin{frame}{Parametric vs.\ nonparametric: a one-predictor illustration}
  \begin{center}
    \includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{ch02_param_vs_nonparam.png}
  \end{center}
...
\begin{frame}{The parametric approach: two-step recipe}
  ...
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Switch to \texttt{chapter\_02\_lab.ipynb} \S4 and run the fits cell: on the same
    simulated data a straight line (two parameters) sits beside two splines --- the
    line misses the curvature exactly as the flat plane misses it in Figure 2.4.
  \end{labnote}
\end{frame}
...
\begin{frame}{Linear fit on Income}
  ...
  \end{center}
  The flat plane is the linear regression estimate. It captures the
...
\begin{frame}{Nonparametric Income fit (smooth thin-plate spline)}
  ...
  \end{center}
  The fitted surface is now curved, matching the true $f$ much more
  closely. No misspecification bias --- this is what you want when you
  do not know the functional form.
  \islpsource{Figure 2.5}
\end{frame}

\begin{frame}{Nonparametric Income fit (rough thin-plate spline)}\label{frm:roughfit}
  \begin{center}
...
\begin{frame}{Solution --- Exercise 2.1 (1/3)}
\begin{solutionbox}
\begin{enumerate}
...
\begin{frame}{Solution --- Exercise 2.1 (2/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 2.1 (3/3)}
\begin{solutionbox}
  \begin{enumerate}
...
\begin{frame}{The trade-off as a map: where each method sits}\label{frm:tradeoffmap}
  \begin{center}
  \begin{tikzpicture}[xscale=1.0,yscale=0.68,
    ...
    \draw[dashed,gray,-{Stealth[length=2.5mm]},thick]
          (0.9,4.70) -- (10.35,0.12)
          node[pos=0.44,above=4mm,sloped,font=\scriptsize\itshape,
...
\begin{frame}{Why ever choose a less-flexible method?}\label{frm:lessflex}
  ...
  \begin{takeaway}
    ...
    \end{itemize}
  \end{takeaway}
  \begin{labnote}[{{Live demo --- \S\S2--3 of the notebook}}]
    Switch to \texttt{chapter\_02\_lab.ipynb} \S\S2--3 and run the \texttt{Auto} cells:
    \texttt{groupby('origin')} splits mean \texttt{mpg} ($20.0$/$27.6$/$30.5$), and the pairplot shows the curvature a good model must bend to.
  \end{labnote}
\end{frame}
...
\end{solutionbox}
\begin{mistake}
Reading the flexibility ranking in part 1 as
...
\begin{frame}{Supervised vs.\ unsupervised learning}
  ...
  \end{columns}
  \begin{takeaway}
    This course is supervised learning throughout. The unsupervised case
    --- principal components and clustering --- is the optional module A8
    (ISLP Ch.\ 12) in \texttt{Chapters/Advanced/}.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Training MSE vs.\ test MSE}
  ...
  \begin{alertblock}{The cardinal sin}
    ...
    itself. An accuracy figure quoted on the data the model was fitted
    to is worth nothing: ask instead for performance on a \emph{later}
    period than the one used for fitting.
  \end{alertblock}
\end{frame}

\begin{frame}{Simulated data: candidate fits and their train/test MSE}
  \begin{center}
    \includegraphics[width=0.95\textwidth,height=0.52\textheight,keepaspectratio]{2_9.pdf}
  \end{center}
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Switch to \texttt{chapter\_02\_lab.ipynb} \S4 and run the MSE-curve cell: training MSE
    only falls, test MSE turns up past moderate flexibility, and neither drops below the
    grey dashed $\mathrm{Var}(\varepsilon)=0.16$ floor.
  \end{labnote}
...
\begin{frame}{Reading the simulation}
  \begin{readme}
    Left: simulated data (open circles), the true $f$ (the smooth black
    curve), and three fits: a straight line (orange), a moderate spline that
    almost coincides with the truth (blue), and a very flexible spline that
    wiggles through the noise (green).\\
    Right: the training MSE is the curve that only falls; the test MSE is the
    U-shaped one.
  \end{readme}
...
\end{solutionbox}
\begin{mistake}
Treating zero training error as proof that
...
\begin{frame}{Bias and variance trade off}
  ...
  \end{itemize}
  Where the U has its minimum depends on the signal-to-noise ratio. When the signal is tiny
  --- asset returns are the textbook case --- a flexible method mostly fits noise, the
  variance term dominates, and small, heavily constrained models routinely win out of
  sample. It is the same reason a few well-chosen predictors often beat a large automated
  feature set.
\end{frame}

\begin{frame}{The bias-variance trade-off, schematically}
  \begin{center}
  \begin{tikzpicture}[xscale=0.94,yscale=0.50]
   ...
   \begin{scope}[shift={(2.3,7.9)}]
     \draw[draw=accent,line width=1.3pt] (0,0)--(0.55,0); \node[right,font=\scriptsize] at (0.55,0){Bias$^2$};
     \draw[draw=orange!85!black,line width=1.3pt] (2.9,0)--(3.45,0); \node[right,font=\scriptsize] at (3.45,0){Variance};
     \draw[draw=green!55!black,line width=1.2pt,dashed] (0,-1.0)--(0.55,-1.0); \node[right,font=\scriptsize] at (0.55,-1.0){Irreducible $\sigma^2$};
     \draw[draw=red!75!black,line width=1.7pt] (2.9,-1.0)--(3.45,-1.0); \node[right,font=\scriptsize] at (3.45,-1.0){Test MSE (sum)};
   \end{scope}
  \end{tikzpicture}
  \end{center}
...
\begin{frame}{Bias, variance, and test MSE across flexibility}\label{frm:bvcurves}
  \begin{center}
    \includegraphics[width=0.95\textwidth,height=0.66\textheight,keepaspectratio]{2_12.pdf}
  \end{center}
  \footnotesize
  The curve that falls is the squared bias (\emph{Bias} in the legend); the
  one that rises is the variance (\emph{Var}); their sum plus $\sigma^2$ is
  the U-shaped test MSE (\emph{MSE}). Horizontal dashed line: the irreducible
  $\sigma^2$. Vertical dotted line: the flexibility that minimises test MSE.
...
\begin{frame}{Solution --- Exercise 2.4 (1/3)}
\begin{solutionbox}
\begin{enumerate}
...
\begin{frame}{Solution --- Exercise 2.4 (2/3)}
\begin{solutionbox}
  \begin{enumerate}
...
\begin{frame}{Solution --- Exercise 2.4 (3/3)}
\begin{solutionbox}
\begin{enumerate}\setcounter{enumi}{2}
...
\end{solutionbox}
\begin{mistake}
Forgetting to square the bias (adding $-1.0$
...
\end{solutionbox}
\begin{mistake}
Picking the flexibility where the
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_02\_lab.ipynb}]
    ...
    \begin{itemize}
      \item \S\S1--3 \emph{NumPy refresher}, \emph{pandas} and \emph{Plots};
      \item \S4 \emph{Reproducing Figure 2.9 --- bias--variance};
      \item \S\S5--6 \emph{KNN classification} and \emph{KNN by hand} --- these
            go with the appendix block, not with the taught session.
    \end{itemize}
...
import pandas as pd                # tables: one row per observation
import matplotlib.pyplot as plt    # every figure in this chapter
from ISLP import load_data         # fetches a book data set by name

rng = np.random.default_rng(2024)  # seed 2024: same draws every run
plt.rcParams['figure.dpi'] = 110   # sharper inline figures
  \end{lstlisting}
  \begin{readme}
    These six lines open every code slide that follows. \texttt{np}
    supplies arrays and the random-number generator, \texttt{pd}
    supplies data frames, \texttt{plt} draws. \texttt{load\_data}
    pulls a book data set by name (\texttt{"Auto"}, \texttt{"Default"}).
    The \texttt{rng} object is the \emph{only} source of randomness:
    seeding it is what makes your numbers match the slide's.
  \end{readme}
\end{frame}

\begin{frame}[fragile]{NumPy basics}
  \begin{lstlisting}
x = np.array([3, 4, 5])           # 1-D array: three measurements
y = np.array([4, 9, 7])           # a second variable, same length

x.sum(), x.mean(), x.std(ddof=1)  # basic stats (sample sd)
A = np.array([[1, 2], [3, 4]])    # nested list -> 2 x 2 matrix
A.shape, A.T, np.linalg.inv(A), A @ A  # shape, transpose, inverse, product
  \end{lstlisting}
  \begin{readme}
    \footnotesize An \emph{array} is a block of numbers you compute
    with all at once; a nested list becomes a matrix and
    \texttt{.shape} gives (rows, columns). \texttt{@} multiplies
    matrices, \texttt{*} works element by element. Every data set
    ahead is such a matrix: one row per observation, one column per
    predictor.
  \end{readme}
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    Switch to \texttt{chapter\_02\_lab.ipynb} \S1 and run these cells live: note
    \texttt{std(ddof=1)} returns the \emph{sample} sd ($1.0$ here) --- NumPy's default
    \texttt{ddof=0} divides by $n$, while pandas defaults to \texttt{ddof=1}.
  \end{labnote}
\end{frame}
...
\begin{labnote}[{{Run this live}}]
  The worked solution sits in \texttt{chapter\_02\_lab.ipynb} under
  \emph{Lecture exercises --- worked Python solutions}: the cell headed
  \emph{Exercise 2.8 --- Scatter and Flexibility on Auto [Python]}.
\end{labnote}
\end{frame}
...
\begin{solutionbox}
\begin{lstlisting}
import pandas as pd                 # read and clean the table
import matplotlib.pyplot as plt     # draw the scatter plot

Auto = pd.read_csv("../ALL CSV FILES - 2nd Edition/Auto.csv")  # raw file
Auto = Auto[Auto["horsepower"] != "?"].copy()   # drop the 5 "?" rows
Auto["horsepower"] = Auto["horsepower"].astype(float)  # text -> number

plt.scatter(Auto["horsepower"], Auto["mpg"], s=10)  # one dot per car
plt.xlabel("horsepower"); plt.ylabel("mpg")         # always label axes
plt.show()                                          # render the figure

print(Auto.shape[0])                              # 392 rows survive
...
\end{solutionbox}
\begin{mistake}
Skipping the conversion (the correlation
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{description}
    ...
    \item[MSE] $\dfrac1n\sum_{i=1}^n\big(y_i-\hat f(x_i)\big)^2$ --- average squared error on \emph{whichever} sample it is computed on.
    \item[Training MSE] that average over the $n$ points used to fit $\hat f$; always over-optimistic.
    \item[Test MSE] that average over \emph{unseen} observations; at a new point, $E\big[(y_0-\hat f(x_0))^2\big]$ --- the honest one.
    \item[Bias] $\operatorname{Bias}(\hat f(x_0))=E[\hat f(x_0)]-f(x_0)$, expectation over training sets; enters \emph{squared} below.
...
\begin{frame}{Self-check questions}
  ...
  \begin{enumerate}
    \item Sketch the bias-variance decomposition curves. Where is the
          optimal flexibility? (p.~\pageref{frm:bvcurves})
    \item Why is the irreducible error not a function of $\hat f$? (p.~\pageref{frm:irred})
    \item Construct an example where a more flexible method is
          preferable to a linear model. (pp.~\pageref{frm:roughfit} and \pageref{frm:lessflex})
    \item For KNN, what happens to bias and variance as $K$ grows? (appendix, p.~\pageref{frm:chooseK})
    \item State the Bayes classifier in words; why is it the gold
          standard? (appendix, p.~\pageref{frm:bayes})
    \item Suppose you have $n = 50$ observations of $p = 1$ predictor
          and want to estimate $E[Y \mid X]$. Pick a method and
          justify. (pp.~\pageref{frm:tradeoffmap} and \pageref{frm:lessflex})
  \end{enumerate}
\end{frame}
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{Simulating the U-shape} --- a third rendering of the training-vs-test U-curve, kept after ISLP Fig 2.9 and the annotated version said the same thing.
      \item \textbf{Bias--variance from first principles} --- Extended Exercise 2.1 derives the decomposition instead of quoting it. The main deck states the result and Exercise 2.4 uses it with numbers --- that is what the exam asks for.
      \item \textbf{The Bayes boundary for two Gaussians} --- Extended Exercise 2.3 computes the optimal boundary analytically. It is the algebra behind LDA, so it fits better \emph{after} Chapter 4.
      \item \textbf{Classification: the Bayes classifier and KNN} --- conditional class probabilities, the Bayes rule and its error rate, KNN with the $K=1$/$K=100$ boundaries, and Exercises 2.6, 2.7, E2.2 and E2.4. Chapter 4 teaches classification properly; this block keeps the thread available, and the mock exams still draw on it.
    \end{itemize}
  \end{block}
  \vspace{-1mm}
  Exercise numbers are unchanged: 2.6, 2.7, E2.2 and E2.4 live here, which is
  why the main flow runs 2.5 $\to$ 2.8.
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 2.1 (1/5)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 2.1 (2/5)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 2.1 (3/5)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 2.1 (4/5)}
\begin{solutionbox}
\textbf{4. Apply.} Test MSE $=\mathrm{Bias}^2+\mathrm{Var}+\sigma^2$.
The table gives the \emph{bias}, so square it first --- for model A,
\[
  \underbrace{2.0^2}_{\mathrm{Bias}^2 = 4.00}
  + \underbrace{0.5}_{\mathrm{Var}}
  + \underbrace{1.0}_{\sigma^2}
  = 5.50 ,
\]
and likewise for B and C (next slide). Note the three ingredients are on
different scales: bias in $y$-units, variance and $\sigma^2$ in squared
units --- only after squaring the bias do they add.
\end{solutionbox}
\begin{mistake}
Adding the raw bias ($2.0$) instead of its square ($4.00$) --- model A
would wrongly score $3.5$ and \emph{win}. The decomposition holds for
$\mathrm{Bias}^2$; the table's column is deliberately labelled Bias.
\end{mistake}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 2.1 (5/5)}
\begin{solutionbox}
\vspace{-1mm}
\begin{center}\footnotesize
\begin{tabular}{lcccc}
...
\end{solutionbox}
\begin{mistake}
Judging by a single column --- C has the lowest bias and A the lowest variance, yet B wins, because only the sum $\mathrm{Bias}^2+\mathrm{Var}+\sigma^2$ is the test MSE.
\end{mistake}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 2.3 (1/4)}
\begin{solutionbox}
\textbf{1. Bayes rule.} By Bayes' theorem the posterior is
\[
  \Pr(Y{=}k\mid x)
  = \frac{\pi_k f_k(x)}{\pi_0 f_0(x) + \pi_1 f_1(x)},
  \qquad
  f_k(x)=\frac{1}{\sqrt{2\pi}\,\sigma}
         \exp\!\Bigl(-\frac{(x-\mu_k)^2}{2\sigma^2}\Bigr).
\]
The denominator is the \emph{same} for both classes --- it only renormalises
--- so comparing posteriors is comparing numerators:
$\Pr(Y{=}k\mid x)\propto\pi_k f_k(x)$, and the Bayes classifier predicts
class~1 iff $\pi_1 f_1(x)>\pi_0 f_0(x)$. This ``compare the numerators''
step is the engine of every generative classifier in Chapter 4's appendix
(LDA, QDA, naive Bayes).
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 2.3 (2/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 2.3 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 2.3 (4/4)}
\begin{solutionbox}
...
% ------------------------------------------------------------
% Classification, the Bayes classifier and KNN --- moved out of the main flow.
% The taught session stops at the bias--variance trade-off; this block carries
% the classification thread in full, and Extended Exercise 2.3 above is its
...
\begin{frame}{The Bayes classifier}\label{frm:bayes}
  \begin{block}{Definition}
  ...
  \begin{takeaway}
    ...
    not an algorithm.
  \end{takeaway}
  \islpsource{Section 2.2.3, equation (2.10)}
\end{frame}
...
\begin{frame}{The Bayes error rate}
  ...
  \end{readme}
  \islpsource{Section 2.2.3, equation (2.11)}
\end{frame}
...
\begin{frame}{$K$-nearest neighbours: definition}
  ...
  \begin{labnote}[{{Live demo --- \S6 of the notebook}}]
    Switch to \texttt{chapter\_02\_lab.ipynb} \S6 and run the voting cell: this algorithm coded
    in a few lines and checked against \texttt{KNeighborsClassifier} --- watch the prediction
    flip when $K$ widens from $1$ to $3$.
  \end{labnote}
  \par{\tiny\itshape\color{gray}History: a 1951 US Air Force report by Fix \&
  Hodges; Cover \& Hart (1967) proved its error is at most twice the Bayes
  error.}
  \islpsource{Section 2.2.3, equation (2.12)}
\end{frame}
...
\begin{frame}{KNN with $K = 1$ (left) and $K = 100$ (right)}
  \begin{center}
    \includegraphics[width=0.85\textwidth,height=0.66\textheight,keepaspectratio]{2_16.pdf}
  \end{center}
  \footnotesize
  \textbf{$K = 1$:} each training point gets its own little island, so the
  boundary is extremely jagged --- \emph{low bias, very high variance};
  training error is zero but test error is poor. \textbf{$K = 100$:} an almost
  straight boundary that smooths the real structure away ---
  \emph{low variance, high bias}.
...
\begin{frame}{Choosing $K$ in practice}\label{frm:chooseK}
  \begin{takeaway}[Practical advice]
    ...
    \end{itemize}
  \end{takeaway}
  \begin{mistake}[Trusting ``nearest'' in a wide table]
    Run KNN naively over a customer table with hundreds of columns and the nearest
    neighbours are not near at all --- the curse of dimensionality again. Reduce the
    dimension, or learn an embedding, before the distance means anything.
  \end{mistake}
\end{frame}
...
\begin{frame}{Solution --- Exercise 2.6 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 2.6 (2/3)}
\begin{solutionbox}
\begin{enumerate}\setcounter{enumi}{2}
...
\begin{frame}{Solution --- Exercise 2.6 (3/3)}
\begin{solutionbox}
  \begin{enumerate}
...
\end{solutionbox}
\begin{mistake}
Averaging the three local errors without the
...
\begin{frame}{Solution --- Exercise 2.7 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 2.7 (2/3)}
\begin{solutionbox}
\begin{center}\footnotesize
\begin{tabular}{ccc}
\toprule
...
\begin{frame}{Solution --- Exercise 2.7 (3/3)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
Mixing scales when ranking --- comparing one
...
\begin{frame}{Exercise 2.7, drawn}
  \begin{center}
    \includegraphics[width=0.44\textwidth,height=0.52\textheight,keepaspectratio]{ch02_knn_by_hand.png}
  \end{center}
  \begin{takeaway}
    The two circles are the story: $K=1$ reaches only $P_1$ (Blue, $d=1$),
    prediction Blue; widening to $K=3$ pulls in $P_2, P_3$ (both Red,
    $d=1.414$) and the prediction \emph{flips} to Red at $\hat P = 2/3$.
    That volatility is the variance in the $K$ trade-off.
  \end{takeaway}
\end{frame}
...
\begin{labnote}[{{Run this live}}]
  The worked solution sits in \texttt{chapter\_02\_lab.ipynb} under
  \emph{Lecture exercises --- worked Python solutions}: the cell headed
  \emph{Extended Exercise 2.2 --- KNN simulation study [Python]}.
\end{labnote}
\end{frame}
...
import numpy as np                                   # arrays + the RNG
from sklearn.neighbors import KNeighborsClassifier   # the vote-based model
rng = np.random.default_rng(1)                       # seeded RNG (reproducible)
def sample(n):                                       # draw n points, half per class
    n0 = n // 2                                      # class 0 gets half of n
...
tr, te = [], []                                      # one error rate per K
for K in range(1, 51):                               # fit KNN for K = 1..50
    m = KNeighborsClassifier(K).fit(Xtr, ytr)        # .fit() just stores rows
...
\end{lstlisting}
\smallskip
{\scriptsize KNN never really ``fits'': each prediction re-scans the stored rows, keeps the $K$ nearest, and votes.}
\end{solutionbox}
\end{frame}
...
inv = [1/K for K in range(1, 51)]   # flexibility axis: K=1 lands far right
# plotted this way the U-shape opens up: rigid left, overfit right
...
\smallskip
\textit{Common mistake:} over-trusting the exact winner $K=14$. The basin is flat --- any $K$ in $10$--$20$ is statistically equivalent, and the training curve ($0$ at $K=1$ by construction) is no evidence at all.
\end{solutionbox}
\end{frame}
...
\begin{frame}[fragile]{KNN classification}
  \begin{lstlisting}
from sklearn.neighbors import KNeighborsClassifier  # majority vote of K
from sklearn.model_selection import train_test_split  # holds rows back
from sklearn.preprocessing import StandardScaler  # puts X on one scale
...
Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=0.3,
                                       random_state=0)  # 70/30, fixed
scaler = StandardScaler().fit(Xtr)         # means/sds from TRAIN only
# 10-NN: fitting only memorises the scaled training rows
knn = KNeighborsClassifier(n_neighbors=10).fit(scaler.transform(Xtr), ytr)
print(knn.score(scaler.transform(Xte), yte))  # accuracy on unseen rows
  \end{lstlisting}
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    Run \S5 of \texttt{chapter\_02\_lab.ipynb}: the same code, looped over $K$ from $1$ to $200$,
    turns this single fit into the U-shaped test-error curve against $1/K$.
  \end{labnote}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 2.4 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 2.4 (2/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 2.4 (3/4)}
\begin{solutionbox}
  \begin{itemize}
...
\begin{frame}{Solution --- Extended Exercise 2.4 (4/4)}
\begin{solutionbox}
\textbf{The three cases at a glance.}
\begin{center}\footnotesize
\begin{tabular}{llll}
\toprule
Case & Winner & Deciding force & Chapter that revisits it \\
\midrule
(a) linear truth & linear & zero bias at lower variance & Ch.\ 3 (Fig.\ 3.18) \\
(b) curved truth, big $n$ & KNN & misspecification bias & Ch.\ 3, 7 \\
(c) $p$ large, $n$ modest & linear & curse of dimensionality & Ch.\ 3 (Fig.\ 3.20), 6 \\
\bottomrule
\end{tabular}
\end{center}
The same trade-off is re-run with real estimates in Chapter 3's appendix
(linear vs.\ KNN regression, ISLP \S3.5) --- same conclusions, drawn from
simulations instead of reasoning.

\smallskip
\textit{Common mistake:} judging KNN by its training error --- with $K=1$ that is exactly zero on \emph{every} data set. Only the test error can compare these models.
\end{solutionbox}
\end{frame}
