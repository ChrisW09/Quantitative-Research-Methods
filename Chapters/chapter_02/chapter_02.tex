...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.2cm}p{6.0cm}p{5.0cm}@{}}
    ...
    \midrule
    Telco, SaaS & Churn score per subscriber: a pure prediction problem, and nobody asks the model why & A \emph{ranking}: who gets a retention offer \\
    Pricing & Price elasticity estimated for a coefficient, not a ranking & The list price a manager sets \\
    E-commerce & Item--item nearest neighbours (``customers who bought this'') --- this chapter's own method, run at scale & The products shown next to the basket \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{takeaway}[All three answer ``rank or coefficient?'' before choosing a method]
    Row one needs only an ordering, row two needs a number a person can act on, and row
    three needs neighbours in a sensible distance. The method follows from that answer ---
    together with how much of the variation is irreducible whatever we fit.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    ...
    \item Construct an example where a more flexible method is
          preferable to a linear model. (pp.~23 and 95--97)
