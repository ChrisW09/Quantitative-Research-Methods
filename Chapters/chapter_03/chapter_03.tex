% ============================================================
% Beamer — ISLP Lecture Series (English) — IMPROVED EDITION
...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{Ch.\ 3} & \textbf{Linear regression}\,{\color{accent!75!black}--- estimation, inference, diagnostics} \\
  ...
  \begin{takeaway}[The path through linear regression]
    \begin{enumerate}
      ...
      \item Diagnostics: nonlinearity, heteroscedasticity, leverage,
            collinearity.
      \item Comparison with KNN regression --- in the \textbf{appendix}.
    \end{enumerate}
  \end{takeaway}
\end{frame}
...
\begin{frame}{Why is linear regression still in every textbook?}
  Linear regression dates back to the early 19th century --- the method
  of least squares, published by Legendre (1805) and Gauss (1809). Why
  are we still teaching it two centuries later?
...
\begin{frame}{The simple linear regression model}
  ...
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: the odd name is Francis
  Galton's (1886). Studying heights, he found that children of tall parents
  tend back towards the average --- they ``regress'' towards the mean ---
  and the label stuck to the line-fitting method itself.}
\end{frame}
...
\begin{frame}{Least-squares geometry: minimise the squared residuals}
  \begin{center}
    \begin{tikzpicture}[scale=0.92,>=Stealth]
      ...
      \node[accent,right,font=\small] at (5.25,3.9) {$(x_i,y_i)$};
...
\begin{frame}{Deriving the slope: step 1 of 2}\label{frm:deriveslope}
...
\begin{frame}{Deriving the intercept: step 2 of 2}\label{frm:deriveint}
...
\begin{frame}{The same bowl, seen from above: RSS contours on real data}
  \begin{center}
    \includegraphics[width=0.85\textwidth,height=0.66\textheight,keepaspectratio]{3_2a.pdf}
  \end{center}
  Each blue curve joins the $(\beta_0,\beta_1)$ pairs with the same RSS on the
  Advertising data. The red dot at the centre is the least-squares solution
  $(\hat\beta_0,\hat\beta_1)=(7.03,\,0.0475)$ --- the bottom of the bowl.
...
\begin{frame}{Solution --- Exercise 3.1 (1/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 3.1 (2/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 3.1 (3/3)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    treating $\hat y(6)=6.1$ as just as
...
\begin{frame}{Population vs.\ sample lines (illustration)}
  ...
  \end{center}
  \begin{takeaway}
    The flatter heavy line is the \emph{population} line: fixed, and never
    observed. Every other line is least squares on one sample; they wobble
    around it, and a standard error measures that wobble from a
    \emph{single} sample.
  \end{takeaway}
...
\begin{frame}{Confidence interval: construction}
  ...
  \begin{numexample}[Advertising example]
    ...
    confidence.
  \end{numexample}
  The interval, not the point estimate, is what a controller asks for before
  signing off a budget shift: it shows whether the channel's payback is
  comfortably positive or could plausibly be zero.
\end{frame}
...
\begin{frame}{A picture of the 95\% confidence interval}
  ...
  \begin{tikzpicture}[scale=0.92,>=Stealth]
    ...
    \node[above,green!45!black,font=\small] at (\ctr,{\amp+0.10})
...
\begin{frame}{Solution --- Exercise 3.2 (1/2)}
  \begin{solutionbox}
    \scriptsize
...
\begin{frame}{Solution --- Exercise 3.2 (2/2)}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} dividing the RSE by $\sqrt n$. The slope's
    SE divides by $\sqrt{\sum(x_i-\bar x)^2}$ --- more spread-out
    predictor values give a more precise slope, regardless of $n$.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Reading the regression output}
  ...
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    Switch to \texttt{chapter\_03\_lab.ipynb} \S1 and run the first cell: the
    same table for Boston's \texttt{medv}\,$\sim$\,\texttt{lstat} --- slope
    $-0.95$, $t=-24.5$, $R^2=0.544$.
  \end{labnote}
\end{frame}
...
\begin{frame}{Common misinterpretations of $R^2$}\label{frm:r2mis}
  ...
  \begin{alertblock}{Low $R^2$ does \emph{not} prove}
    ...
    \end{itemize}
  \end{alertblock}
  A factor model makes the second point concrete: regress a fund's excess return
  on market, size and value factors and $R^2$ is the share of its movement
  explained by known factors --- the rest is what the manager charges for.
\end{frame}
...
\begin{frame}{Solution --- Exercise 3.3 (1/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 3.3 (2/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 3.3 (3/3)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    reading $R^2=0.49$ as ``49\% of points
...
\begin{frame}{Solution --- Extended Exercise 3.L1 (1/4)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 3.L1 (2/4)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 3.L1 (3/4)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 3.L1 (4/4)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    dropping the ``$1+$'' under the root. That
    gives the \emph{confidence} interval for the average 7-year-old car ---
    $8 \pm 2.58$ --- far too narrow for one specific car.
  \end{mistake}
\end{frame}
...
\begin{frame}[fragile]{Exercise 3.4 --- Multiple OLS on Advertising [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_03\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Exercise 3.4 --- Multiple OLS on Advertising [Python]}.
  \end{labnote}
\end{frame}
...
import statsmodels.api as sm         # OLS reporting SEs and p-values

ad = pd.read_csv("../ALL CSV FILES - 2nd Edition/Advertising.csv",
                 index_col=0)        # col 0 = market ID, not an input
X = sm.add_constant(ad[["TV", "radio", "newspaper"]])  # 3 media + b0
res = sm.OLS(ad["sales"], X).fit()   # .fit() = solve for the 4 betas
print(res.summary())                 # SE, t, p, 95% CI per coef; R2, F
# TV ~ 0.0458, radio ~ 0.189, newspaper ~ -0.001 (p ~ 0.86)
# R-squared ~ 0.897
\end{lstlisting}
  \end{solutionbox}
  \begin{readme}\footnotesize
    \texttt{add\_constant} makes $\beta_0$ estimable; \texttt{.fit()}
    returns what \texttt{.summary()} prints.
  \end{readme}
\end{frame}
...
\begin{frame}{Solution --- Exercise 3.4 (2/2)}
  ...
  \end{solutionbox}
  \begin{mistake}
    reading the near-zero newspaper slope as
...
\begin{frame}{A cautionary tale: the Advertising slopes}\label{frm:confound}
...
\begin{frame}{Solution --- Exercise 3.5 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 3.5 (2/2)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    concluding ``newspaper advertising cannot
    work''. The model says newspaper adds nothing \emph{given TV and
    radio, in these data} --- an observational statement, not a causal
    verdict.
  \end{mistake}
\end{frame}
...
\begin{frame}{Four questions to ask of every multiple regression}
  ...
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Switch to \texttt{chapter\_03\_lab.ipynb} \S2 and run the five-predictor
    cell: one summary table answers Q1--Q3 at once --- $F=214.5$, a $t$-test
    per slope (\texttt{age}: $p=0.054$), $R^2=0.682$.
  \end{labnote}
\end{frame}
...
\begin{frame}{Why not just $p$ separate $t$-tests?}
  ...
  \begin{takeaway}
    ...
    corrections in module A7 (ISLP Ch.\ 13).
  \end{takeaway}
\end{frame}
...
\begin{frame}{Q4.\ How precise is a prediction?}\label{frm:predint}
...
\begin{frame}{Confidence band vs.\ prediction band, on Auto}\label{frm:bands}
  \begin{center}
    \includegraphics[width=0.74\textwidth,height=0.54\textheight,keepaspectratio]{ch03_ci_vs_pi.png}
  \end{center}
  \begin{takeaway}
    Both bands are $95\%$; mid-range the confidence band is $\sim 1.3$\,mpg
    wide, the prediction band $\sim 19$\,mpg. One answers ``where is the
    \emph{average} mpg at this horsepower''; the other ``where will
    \emph{this car} land'' --- and no sample size shrinks the second below
...
\begin{frame}{Solution --- Exercise 3.6 (1/2)}
  \begin{solutionbox}
    \textbf{(1) $F$-statistic.}
    \[
...
\begin{frame}{Solution --- Exercise 3.6 (2/2)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    reading the significant $F$ as ``all three
    predictors matter''. It says only that \emph{at least one} does; the
    individual $t$-tests (and Chapter~6) decide which.
  \end{mistake}
\end{frame}
...
\begin{frame}[fragile]{Extended Exercise 3.L3 --- Multiple regression on Advertising [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_03\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Extended Exercise 3.L3 --- Multiple regression on Advertising [Python]}.
  \end{labnote}
\end{frame}
...
ad = pd.read_csv("../ALL CSV FILES - 2nd Edition/Advertising.csv",
                 index_col=0)          # col 0 = market ID, not an input
X   = sm.add_constant(ad[["TV", "radio", "newspaper"]])  # 3 media + b0
res = sm.OLS(ad["sales"], X).fit()     # solve for the 4 coefficients
print(res.summary())                   # the whole inference table
...
\end{lstlisting}
  \end{solutionbox}
  \begin{readme}\footnotesize
    The coefficient block holds one row per column of \texttt{X}:
    \texttt{coef} is $\hat\beta_j$, \texttt{std err} its standard error,
    \texttt{t} their ratio, \texttt{P>|t|} the two-sided $p$-value, the
    last pair the $95\%$ CI. $R^2$, adjusted $R^2$ and $F$ sit above it.
  \end{readme}
\end{frame}
...
\begin{lstlisting}
X2   = sm.add_constant(ad[["TV", "radio"]])  # same X, no newspaper
res2 = sm.OLS(ad["sales"], X2).fit()         # refit; did slopes move?
...
\end{lstlisting}
  ...
  \end{solutionbox}
  \begin{mistake}
    citing the unchanged $R^2$ as the evidence.
    Plain $R^2$ can only fall when a predictor is dropped --- adjusted
    $R^2$ and RSE, which penalise dead weight, are the fair judges.
  \end{mistake}
\end{frame}
...
\begin{frame}{Two levels: a single 0/1 dummy}
  ...
  \end{readme}
  The adjusted pay gap is exactly this coefficient: regress $\log$(salary) on
  job grade, tenure, performance and location \emph{plus} a protected-group
  dummy, and the dummy's coefficient is the gap that survives once the
  legitimate factors are held fixed.
\end{frame}
...
\begin{frame}{Solution --- Exercise 3.7}
  \begin{solutionbox}
    ...
    \medskip
    \textit{Common mistake:} comparing South and West directly by their
    coefficients as if each were a mean --- both are contrasts against
    \emph{East}. South vs.\ West is $-12.5-(-18.7)=+6.2$: South's average
    balance is \$6.20 \emph{above} West's.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{The interaction model}\label{frm:interact}
  ...
  \begin{readme}
    ...
    wanted to model.
  \end{readme}
  Price elasticity is the standard example: it is not one number, because
  shoppers react far more strongly to a cut that is also displayed. Hence
  price $\times$ promotion in pricing models, TV $\times$ digital in media ones.
\end{frame}
...
\begin{frame}{The hierarchy principle}
  ...
  \begin{labnote}[{{Live demo --- \S3 of the notebook}}]
    Switch to \texttt{chapter\_03\_lab.ipynb} \S3 and run the
    \texttt{ModelSpec} cell: it fits \texttt{poly(lstat,\,2)} plus an
    \texttt{lstat:age} interaction with both main effects kept in --- the
    interaction is significant ($t=-4.4$).
  \end{labnote}
\end{frame}

\begin{frame}{Why Advertising needs an interaction}\label{frm:interadv}
  \begin{center}
    \includegraphics[width=0.72\textwidth,height=0.46\textheight,keepaspectratio]{3_5.pdf}
  \end{center}
  \begin{takeaway}[What the figure shows]
    The additive model can only fit a \emph{flat plane}: it under-predicts
    when the budget is split between TV and radio and over-predicts when it is
    concentrated in one medium. A \texttt{TV}$\times$\texttt{radio} term lets
    the surface \emph{bend}, lifting $R^2$ from about $0.90$ to $0.97$.
  \end{takeaway}
...
\begin{frame}{Solution --- Exercise 3.8 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 3.8 (2/2)}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} quoting $0.019$ as ``the'' TV effect. With
    an interaction present, that is the slope only at $\texttt{radio}=0$;
    the TV effect must be stated at a chosen radio level.
  \end{solutionbox}
\end{frame}

\begin{frame}{Seeing the interaction: the TV slope depends on radio}
  \begin{center}\includegraphics[width=0.88\textwidth,height=0.5\textheight,keepaspectratio]{ch03_x_tv_radio_interaction.png}\end{center}
...
\begin{frame}[fragile]{Exercise 3.9 --- Polynomial fit on Auto [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_03\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Exercise 3.9 --- Polynomial fit on Auto [Python]}.
  \end{labnote}
\end{frame}
...
\begin{frame}[fragile]{Solution --- Exercise 3.9 (1/2)}
  \begin{solutionbox}
    ...
    \vspace{-2mm}
\begin{lstlisting}
import pandas as pd, numpy as np            # tables and arrays
import statsmodels.api as sm                # OLS with inference

auto = pd.read_csv("../ALL CSV FILES - 2nd Edition/Auto.csv",
                   na_values="?").dropna()  # "?" marks missing hp
hp = auto["horsepower"].astype(float)       # it was read in as text

# Linear model: a single straight line
X1 = sm.add_constant(hp)                    # design: 1s and hp
m1 = sm.OLS(auto["mpg"], X1).fit()          # the straight-line fit

# Quadratic model: hp^2 is just one more column
X2 = sm.add_constant(np.column_stack([hp, hp**2]))  # now 1s, hp, hp^2
m2 = sm.OLS(auto["mpg"], X2).fit()          # same OLS, one more x

print(m1.rsquared, m2.rsquared)   # 0.606 -> 0.688: curved
\end{lstlisting}
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Solution --- Exercise 3.9 (2/2)}
  ...
  \end{solutionbox}
  \begin{mistake}
    justifying the quadratic by the $R^2$
...
\begin{frame}{Solution --- Extended Exercise 3.L4 (1/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 3.L4 (2/3)}
  \begin{solutionbox}
    \textbf{(3) Predictions.}
    \[
...
\begin{frame}{Solution --- Extended Exercise 3.L4 (3/3)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    reading $\hat\beta_4=2$ as the South's
    slope. It is the \emph{difference} from East: the South's own slope
    is $6+2=8$. Dummy interactions, like dummies, are contrasts against
    the baseline.
  \end{mistake}
\end{frame}
...
\begin{frame}{Diagnostic 1: nonlinearity}
  ...
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Switch to \texttt{chapter\_03\_lab.ipynb} \S4 and run the diagnostics
    cell: residuals vs fitted plus a Q--Q plot for the full Boston fit ---
    a curved residual band and the capped $\texttt{medv}=50$ streak.
  \end{labnote}
\end{frame}

\begin{frame}{Residuals vs.\ fitted on the Auto data}
  \begin{center}
    \includegraphics[width=0.9\textwidth,height=0.56\textheight,keepaspectratio]{ch03_residual_diag.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Solution --- Exercise 3.10 (1/2)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 3.10 (2/2)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    judging these assumptions from the raw
    $y$-vs-$x$ scatter or from $R^2$. A high $R^2$ can coexist with
    panels (1), (2) and (4) --- only the residual plot shows them cleanly.
  \end{mistake}
\end{frame}
...
\begin{frame}{Outliers and leverage: visual examples}
  ...
  \end{center}
  Left: point 41 has an extreme $x$ and drags the fit (solid line) away from
  the fit without it (dashed); point 20 is unusual in $y$ only. Middle: 41 is
  unremarkable in $X_1$ or $X_2$ alone, yet still high-leverage. Right:
  studentised residual against leverage --- 41 stands out on both axes.
...
\begin{frame}[fragile]{Exercise 3.11 --- Leverage and outliers [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_03\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Exercise 3.11 --- Leverage and outliers [Python]}.
  \end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution --- Exercise 3.11 (1/3)}
  \begin{solutionbox}
    \textbf{(1)--(2) Code.}
    \vspace{-2mm}
\begin{lstlisting}
import pandas as pd, numpy as np        # tables and arrays
import statsmodels.api as sm            # OLS + diagnostics

boston = pd.read_csv("../ALL CSV FILES - 2nd Edition/Boston.csv")
X = sm.add_constant(boston["lstat"])    # 1s column + lstat
res = sm.OLS(boston["medv"], X).fit()   # fit medv ~ lstat

infl = res.get_influence()              # all per-point diagnostics
lev  = infl.hat_matrix_diag             # h_ii: unusual x, not y
stud = infl.resid_studentized_external  # unusual y, on a t scale
...
\begin{frame}{Solution --- Exercise 3.11 (2/3)}
  \begin{solutionbox}
    \textbf{(3) Meaning.}
    \begin{itemize}
...
\begin{frame}{Solution --- Exercise 3.11 (3/3)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    treating the thresholds as deletion rules.
...
\begin{frame}{Diagnostic 4: collinearity}\label{frm:collin}
  ...
  interpretation of individual coefficients is corrupted. Marketing mix models
  are the classic casualty: campaign weeks raise TV, digital and in-store spend
  at once, so channel coefficients are barely identified --- one turns negative,
  and the next refresh flips it back. Check the VIFs before anyone reallocates
  a budget on them.
\end{frame}
...
\begin{frame}{Variance inflation factor (VIF)}\label{frm:vif}
...
\begin{frame}[fragile]{Exercise 3.12 --- Collinearity and VIF [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_03\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Exercise 3.12 --- Collinearity and VIF [Python]}.
  \end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution --- Exercise 3.12 (1/3)}
  \begin{solutionbox}
    \textbf{(1) Code.}
\begin{lstlisting}
import pandas as pd
...
X = sm.add_constant(credit[cols])   # the const row's VIF is ignored
# vif_(X, i) regresses column i of X on all the *other* columns
# and returns 1/(1 - R2_i); loop over i to get one VIF per predictor
vifs = {col: vif_(X.values, i) for i, col in enumerate(X.columns)}
print(vifs)   # Limit 160.59, Rating 160.67, Age 1.01
\end{lstlisting}
  \end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Exercise 3.12 (2/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 3.12 (3/3)}
  \begin{solutionbox}
    \textbf{(3) Fixes.} Drop one of the redundant predictors (keep
    \texttt{limit} \emph{or} \texttt{rating}); or combine them into a single
    index (an average or a principal component); or use a penalised method
    such as ridge regression, which tolerates collinearity.

    \medskip
    \textit{Common mistake:} concluding the model \emph{predicts} poorly.
    Collinearity corrupts the attribution to \emph{individual} coefficients;
    the joint fit and its predictions are typically fine.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 3.L5 (1/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 3.L5 (2/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 3.L5 (3/3)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    conflating the diagnoses --- an outlier is
    unusual in $y$, a leverage point in $x$. The dangerous case is both
    at once, and only Cook's distance measures that combination.
  \end{mistake}
\end{frame}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_03\_lab.ipynb}]
    ...
    \begin{itemize}
      \item \S1 \emph{Simple linear regression} and \S2 \emph{Multiple linear regression}, with \texttt{statsmodels};
      \item \S3 \emph{Polynomials and interactions via the ISLP} \texttt{ModelSpec};
      \item \S4 \emph{Diagnostics}; \S5 \emph{Comparison with KNN} goes with the appendix block.
    \end{itemize}
...
\begin{frame}[fragile]{Simple regression with statsmodels}
  \begin{lstlisting}
import pandas as pd, numpy as np       # tables and array maths
import statsmodels.api as sm           # OLS with full inference
from ISLP import load_data             # the textbook's data sets

Boston = load_data("Boston")           # 506 census tracts
X = sm.add_constant(Boston["lstat"])   # lstat + a column of 1s (b0)
y = Boston["medv"]                     # response: median home value
res = sm.OLS(y, X).fit()               # .fit() runs least squares
print(res.summary())                   # coef | std err | t | P>|t| | CI
  \end{lstlisting}
  \begin{readme}\footnotesize
    \textbf{In:} the response \texttt{y} and a design matrix \texttt{X}
    whose columns are the predictors plus a column of ones ---
    \texttt{add\_constant} is what makes the intercept $\beta_0$ estimable.
    \textbf{Key call:} \texttt{.fit()} solves the least-squares problem and
    hands back a \emph{results} object; nothing prints until you ask.
    \textbf{Out:} \texttt{.summary()} gives one row per column of
    \texttt{X} --- \texttt{coef} $=\hat\beta_j$, \texttt{std err}
    $=\mathrm{SE}(\hat\beta_j)$, \texttt{t} $=$ their ratio,
    \texttt{P>|t|} the two-sided $p$-value, then the $95\%$ CI --- with
    $R^2$ and the $F$-statistic in the header block above.
  \end{readme}
\end{frame}

\begin{frame}[fragile]{Confidence and prediction intervals}
  \begin{lstlisting}
xnew = pd.DataFrame({"const": 1.0, "lstat": [5, 10, 15]})  # 3 new x
pred = res.get_prediction(xnew)   # plug them into the fitted line
pred.summary_frame(alpha=0.05)    # alpha=.05 -> 95% intervals
# columns: mean (the fitted y), mean_se,
#   mean_ci_lower / upper,        <- CI: the average y at this x
#   obs_ci_lower  / upper         <- PI: one new tract at this x
  \end{lstlisting}
  \begin{readme}\footnotesize
    Same fit, two questions. \texttt{mean\_ci} brackets the \emph{average}
    response at that $x$ and shrinks as $n$ grows; \texttt{obs\_ci} brackets
    \emph{one new observation}, so it also carries the irreducible noise
    $\sigma$ and stays wide however large $n$ is. Quote the wider one to a
    client asking about a single case.
  \end{readme}
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    Switch to \texttt{chapter\_03\_lab.ipynb} \S1 and run the
    \emph{Confidence and prediction intervals} cell: at $\texttt{lstat}=5$
    the CI is $[29.0,\,30.6]$ but the prediction interval is
    $[17.6,\,42.0]$ --- an order of magnitude wider.
  \end{labnote}
\end{frame}
...
X = sm.add_constant(Boston[predictors])  # 5 columns + a column of 1s
res = sm.OLS(Boston["medv"], X).fit()    # one intercept + five slopes
print(res.summary())                     # each t: this x, others fixed
  \end{lstlisting}
  Adding more predictors always increases $R^2$; the effect on the
  per-coefficient standard errors is ambiguous --- they fall only if the new
  variables cut $\hat\sigma$ by more than collinearity inflates them.
  \begin{readme}\footnotesize
    Nothing about the call changes --- only \texttt{X} gets wider, so the
    coefficient block now has six rows. Each \texttt{t} and
    \texttt{P>|t|} tests $H_0:\beta_j=0$ \emph{with the other four
    predictors held fixed}, which is why a predictor can be significant on
    its own and insignificant here. The header's $F$-statistic tests all
    five slopes at once.
  \end{readme}
\end{frame}
...
terms = [poly("lstat", degree=2), "age",   # lstat, lstat^2 and age
         ("lstat", "age")]                 # tuple = the product term
design = MS(terms).fit_transform(Boston)   # builds the X columns
res2 = sm.OLS(Boston["medv"], design).fit()  # then fit as usual
print(res2.summary())                      # intercept + 4 term rows
  \end{lstlisting}
  \begin{readme}[Reading a model specification]\footnotesize
    \texttt{ModelSpec} is ISLP's model \emph{formula}: it turns a list of
    terms into the numeric design matrix \texttt{statsmodels} expects.
    A bare column name is a main effect; \texttt{poly} of degree $2$
    expands to \texttt{lstat} \emph{and} \texttt{lstat}$^2$; a
    \emph{tuple} of two names is their product --- what an R-style formula
    writes as \texttt{lstat:age}. Listing both names \emph{and} the tuple
    gives main effects plus interaction (the formula shorthand
    \texttt{lstat*age}), exactly what the hierarchy principle demands; a
    categorical column is expanded into $K-1$ dummies for you.
  \end{readme}
\end{frame}
...
fig, axes = plt.subplots(1, 2, figsize=(10, 4))    # side-by-side panels
axes[0].scatter(res.fittedvalues, res.resid, s=8)  # e_i against yhat_i
axes[0].axhline(0, color="grey", linewidth=0.8)    # e = 0: no pattern
axes[0].set_xlabel("Fitted"); axes[0].set_ylabel("Residual")
sm.qqplot(res.resid, line="45", ax=axes[1])        # are errors normal?
plt.tight_layout(); plt.show()                     # draw both panels
  \end{lstlisting}
  Two diagnostic plots that should be made for every regression: the
  residual-vs.-fitted plot and the Q-Q plot.
  \begin{readme}\footnotesize
    \texttt{res.fittedvalues} are the $\hat y_i$ and \texttt{res.resid}
    the $e_i=y_i-\hat y_i$. \textbf{Left:} a shapeless band around $0$ is
    what we want; a U-shape says the mean is mis-specified (add a
    nonlinear term), a widening funnel says the variance is not constant.
    \textbf{Right:} points on the $45^\circ$ line mean the errors are
    close to normal, so the $t$- and $F$-based $p$-values can be trusted.
  \end{readme}
\end{frame}
...
infl = res.get_influence()   # per-observation diagnostics
lev = infl.hat_matrix_diag   # h_ii: how unusual x_i is
high_lev = np.where(lev > 2*X.shape[1]/X.shape[0])[0]  # h > 2(p+1)/n

vifs = [vif_(X.values, j) for j in range(X.shape[1])]  # j on the others
print(dict(zip(X.columns, vifs)))  # VIF near 1 fine; > 5 problematic
  \end{lstlisting}
  Always inspect leverage and VIF before declaring an analysis
  complete. \texttt{vif\_(X, j)} regresses column $j$ of \texttt{X} on
  all the \emph{other} columns and returns $1/(1-R_j^2)$: the factor by
  which collinearity inflates $\mathrm{Var}(\hat\beta_j)$.
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Switch to \texttt{chapter\_03\_lab.ipynb} \S4 and run the VIF cell: on
    the full Boston fit only \texttt{rad} ($7.5$) and \texttt{tax} ($9.0$)
...
\begin{frame}{Chapter 3 in one slide}
  ...
  \begin{itemize}
    ...
    \item Compared linear regression with KNN regression (appendix).
  \end{itemize}
\end{frame}
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{block}{Tests}
    ...
    \]
  \end{block}
  \begin{block}{Fit and intervals \normalfont\scriptsize
      (simple regression; $S_{xx}=\sum(x_i-\bar x)^2$,
       $\mathrm{TSS}=\sum(y_i-\bar y)^2$, $t=t_{0.025,\,n-2}$)}
    \vspace{-2.6ex}
    \begin{gather*}
      \mathrm{SE}(\hat\beta_1)=\frac{\mathrm{RSE}}{\sqrt{S_{xx}}},
      \qquad
      \mathrm{SE}(\hat\beta_0)=\mathrm{RSE}\sqrt{\tfrac1n+\tfrac{\bar x^2}{S_{xx}}},
      \qquad
      \mathrm{RSE}=\sqrt{\tfrac{\mathrm{RSS}}{n-2}},
      \qquad
      R^2 = 1 - \frac{\mathrm{RSS}}{\mathrm{TSS}} \\[-2pt]
      \underbrace{\hat y_0\pm t\,\mathrm{RSE}
        \sqrt{\tfrac1n+\tfrac{(x_0-\bar x)^2}{S_{xx}}}}
        _{\text{CI for the mean response } E[Y\mid X=x_0]}
      \qquad
      \underbrace{\hat y_0\pm t\,\mathrm{RSE}
        \sqrt{1+\tfrac1n+\tfrac{(x_0-\bar x)^2}{S_{xx}}}}
        _{\text{PI for a new observation } Y_0 \text{ --- note the } ``1+{}"}
    \end{gather*}
  \end{block}
\end{frame}
...
\begin{frame}{Self-check questions}
  ...
  \begin{enumerate}
    \item Derive the OLS slope $\hat\beta_1$ from the normal equations.
          (pp.~\pageref{frm:deriveslope}--\pageref{frm:deriveint}; in full in the appendix, p.~\pageref{frm:ee3l2})
    \item Why does adding any predictor never decrease $R^2$? What is
          the fix? (p.~\pageref{frm:r2mis})
    \item Suppose two predictors are perfectly correlated. What goes
          wrong with OLS? With ridge? (pp.~\pageref{frm:collin}--\pageref{frm:vif})
    \item Interpret an interaction coefficient
          $\beta_3\,X_1 X_2 = 0.4$ when $X_1$ = TV spend in \$1k and
          $X_2$ = radio spend in \$1k. (pp.~\pageref{frm:interact} and \pageref{frm:interadv})
    \item In the Boston data, the simple regression of \texttt{medv}
          on \texttt{age} is significantly negative, but in the
          multiple regression it is not. Explain. (p.~\pageref{frm:confound})
    \item State the prediction interval for a new observation at $x_0$
          and contrast with the confidence interval. (pp.~\pageref{frm:predint}--\pageref{frm:bands})
  \end{enumerate}
\end{frame}
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{The RSS surface and ISLP Figure 3.1 recreated} --- two more renderings of the least-squares geometry; the contour picture in the main deck carries the teaching.
      \item \textbf{Squared versus absolute loss} --- the trade-offs of the squared criterion and the robust alternatives. Interesting, but this course always uses squared loss.
      \item \textbf{Deriving least squares (Extended Exercise 3.L2)} --- the full calculus derivation of $\hat\beta_0$ and $\hat\beta_1$. The main deck derives the two normal equations already; this is the same argument done slowly, as homework.
      \item \textbf{Matrix form of multiple regression} --- $\hat{\boldsymbol\beta}=(\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{X}^\top\mathbf{y}$. You never compute it by hand; it reappears in Chapter 6 for ridge.
      \item \textbf{Linear vs.\ polynomial vs.\ KNN (Extended Exercise 3.L6)} --- a full simulation study in Python. Excellent practice, and a natural bridge to Chapter 5 --- but it needs a machine, not a lecture slot.
      \item \textbf{Linear regression vs.\ KNN regression} --- the parametric/nonparametric comparison and the curse of dimensionality. It needs KNN, which Chapter 2 also carries in its appendix, so the two blocks are self-study together.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Recreating Figure 3.1 on the Advertising data}
  \begin{center}
    \includegraphics[width=0.9\textwidth,height=0.56\textheight,keepaspectratio]{ch03_ols_residuals.png}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Extended Exercise 3.L2 --- Deriving least squares [Math]}\label{frm:ee3l2}
  \begin{longexercise}
...
\begin{frame}{Solution --- Extended Exercise 3.L2 (1/5)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 3.L2 (2/5)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 3.L2 (3/5)}
  \begin{solutionbox}
    \[
...
\begin{frame}{Solution --- Extended Exercise 3.L2 (4/5)}
  \begin{solutionbox}
    ...
    \medskip
    \emph{A sketch of where unbiasedness comes from} (used in (4)):
    since $\sum_i(x_i-\bar x)=0$, the slope can be written as
    \[
      \hat\beta_1=\frac{\sum_i (x_i-\bar x)\,y_i}{S_{xx}}
      =\beta_1+\frac{\sum_i (x_i-\bar x)\,\varepsilon_i}{S_{xx}} ,
    \]
    substituting $y_i=\beta_0+\beta_1x_i+\varepsilon_i$. The estimate is
    the true slope \emph{plus a noise-weighted average} --- and that noise
    term has expectation zero.
  \end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 3.L2 (5/5)}
  \begin{solutionbox}
  ...
  \begin{readme}
    \begin{itemize}
      ...
      \item Diagonal entries $h_{ii}$ are leverages --- we will use
            them in diagnostics.
      \item $\operatorname{Var}(\hat{\boldsymbol\beta})
            = \sigma^2(\mathbf{X}^\top\mathbf{X})^{-1}$: every standard
            error in the summary table is the square root of one diagonal
            entry. Near-collinear predictors make
            $\mathbf{X}^\top\mathbf{X}$ nearly singular, its inverse huge
            --- that is VIF, in matrix form.
    \end{itemize}
  \end{readme}
\end{frame}
...
\begin{frame}[fragile]{Extended Exercise 3.L6 --- Linear vs polynomial vs KNN [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_03\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Extended Exercise 3.L6 --- Linear vs polynomial vs KNN [Python]}.
  \end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution --- Extended Exercise 3.L6 (1/4)}
  \begin{solutionbox}
    \textbf{Load, clean and split.}
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
import pandas as pd
from sklearn.model_selection import train_test_split   # hold-out split
from sklearn.linear_model import LinearRegression      # the OLS baseline
from sklearn.preprocessing import PolynomialFeatures, StandardScaler
from sklearn.pipeline import make_pipeline             # chain the steps
from sklearn.neighbors import KNeighborsRegressor      # flexible rival
...
\begin{frame}[fragile]{Solution --- Extended Exercise 3.L6 (2/4)}
  \begin{solutionbox}
...
for name, m in [("linear", lin), ("poly-2", pol), ("KNN10", knn)]:
    print(name, round(mse(yte, m.predict(Xte)), 1))  # MSE on unseen cars
# linear 26.2 | poly-2 19.8 | KNN(K=10) 21.1
\end{lstlisting}
  \end{solutionbox}
  \begin{readme}\footnotesize
    \texttt{make\_pipeline} glues the preprocessing to the model, so the
    squaring and the standardising are learned on the \emph{training} rows
    only and then replayed on \texttt{Xte} --- no leakage. KNN needs the
    scaler because it measures distances; OLS does not. All three are then
    scored on the same held-out $30\%$, which is what makes $26.2$, $19.8$
    and $21.1$ comparable.
  \end{readme}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 3.L6 (3/4)}
  \begin{solutionbox}
    \textbf{Test MSE.} Linear $\approx 26.2$, degree-2 polynomial
    $\approx 19.8$, KNN $(K{=}10)\approx 21.1$: both flexible models
    ...
    polynomial captures the curvature with one extra parameter; a
    moderate-$K$ KNN is an equally strong non-parametric competitor.
  \end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 3.L6 (4/4)}
  \begin{solutionbox}
    ...
    $\approx 38$ at $K{=}1$); the best $K$ balances the two.
    \emph{Takeaway:} this one split cannot rank the two flexible fits ---
    $19.8$ against $19.3$ at $K{=}25$ is noise, and the ranking flips with
    the seed. Scored by 10-fold CV on all $392$ cars (see the lab notebook),
    KNN at $K\approx25$--$50$ is marginally \emph{ahead} ($18.2$--$18.6$ vs
    $19.2$) --- but inside one standard error. Prefer the degree-2 fit for
    \emph{interpretability}, not for accuracy.
  \end{solutionbox}
  \begin{mistake}
    choosing $K$ by training MSE --- it falls
    monotonically to zero at $K=1$. Only test or CV error reveals the
    U-shape that picks the right $K$.
  \end{mistake}
\end{frame}


% ------------------------------------------------------------
% Linear regression vs KNN regression --- moved out of the main flow.
% The comparison needs KNN, which Chapter 2 now also carries in its appendix,
% so the two blocks are self-study together.
% ------------------------------------------------------------
...
\begin{frame}{Linear vs.\ KNN: simulation}
  ...
  \end{center}
  The truth here \emph{is} linear (left), so least squares (blue dashed) nails
  it. Right: KNN's test MSE (green) against $1/K$ stays above the
  least-squares MSE (dashed).
