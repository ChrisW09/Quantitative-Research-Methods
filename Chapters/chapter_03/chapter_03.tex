...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.85cm}p{5.15cm}p{4.6cm}@{}}
    \toprule
    ...
    HR, legal & Pay-equity audit: log salary on grade, tenure, location \emph{plus} a group indicator & Which pay adjustments the board signs off \\
  ...
  \begin{industry}[The common thread]
    In all six, the \emph{coefficient} is the deliverable --- not the
    prediction. Someone has to defend that number in a meeting, which is
    why standard errors, intervals and diagnostics matter as much as the fit.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: marketing mix modelling (MMM)}
  \footnotesize
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{industry}[The model an agency actually fits]
        \begin{itemize}\setlength{\itemsep}{0pt}
          \item Response: weekly $\log$(sales) per brand and region;
                $n$ is a few hundred brand--weeks.
          \item Predictors: spend per channel (TV, digital, print,
                in-store), price index, promotion depth, distribution
                coverage, month dummies.
        \end{itemize}
      \end{industry}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{readme}[What it is used for --- and who signs off]
        Each channel coefficient is read as a contribution and an
        elasticity, then aggregated into a decomposition of the year's
        sales --- the starting point for next year's budget split,
        signed off by the CMO with finance and re-fitted quarterly.
        Its reader is the brand manager whose channel budget it moves.
      \end{readme}
    \end{column}
  \end{columns}
  \vspace{-1mm}
  \begin{industry}[Every diagnostic in this chapter shows up here]
    Channels are flighted together, so spend variables are collinear (VIF);
    spend has diminishing returns, so log terms are needed; promotional weeks
    are high-leverage points; error spread grows with the sales level.
  \end{industry}
  \vspace{-2mm}
  \begin{takeaway}[The honest caveat]
    MMM estimates \emph{association}, so turning a coefficient
    into a budget decision is a judgement --- good teams cross-check it
    against geo experiments or holdout regions.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Confidence interval: construction}
  ...
  \begin{numexample}[Advertising example]
    ...
    confidence.
  \end{numexample}
  \begin{industry}[Marketing: what the controller actually asks for]
    Before signing off a budget shift, finance asks for the \emph{interval},
    not the point estimate: it shows whether the channel's payback is
...
\begin{frame}{Common misinterpretations of $R^2$}
  ...
  \begin{alertblock}{Low $R^2$ does \emph{not} prove}
    ...
    \end{itemize}
  \end{alertblock}
  \begin{industry}[Asset management: factor models]
    Regress a fund's excess return on market, size and value factors: $R^2$
    is the share of its movement explained by known factors --- the rest is
    what the manager charges for.
  \end{industry}
\end{frame}
...
\begin{frame}{Two levels: a single 0/1 dummy}
  ...
  \end{readme}
  \begin{industry}[HR and legal: the adjusted pay gap]
    Pay-equity audits regress $\log$(salary) on job grade, tenure,
    performance and location \emph{plus} a protected-group dummy. That single
    coefficient --- the gap remaining after the legitimate factors are held
    fixed --- is the number HR and legal report to the board.
  \end{industry}
\end{frame}
...
\begin{frame}{The interaction model}
  ...
  \begin{readme}
    ...
    wanted to model.
  \end{readme}
  \begin{industry}[Retail: price $\times$ promotion]
    Price elasticity is not one number --- shoppers react far more strongly
    to a cut that is also displayed. Pricing teams model
    price $\times$ promotion, media teams TV $\times$ digital.
  \end{industry}
\end{frame}
...
\begin{frame}{Diagnostic 4: collinearity}
  ...
  \begin{industry}[Marketing mix modelling: channels move together]
    Campaign weeks raise TV, digital and in-store spend at once, so channel
    coefficients are barely identified: one turns negative, the next refresh
    flips it back. Check VIFs before anyone reallocates budget on them.
  \end{industry}
\end{frame}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_03\_lab.ipynb}]
    ...
    \begin{itemize}
      ...
      \item \S4 \emph{Diagnostics}, then \S5 \emph{Comparison with KNN}.
    \end{itemize}
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    \item Derive the OLS slope $\hat\beta_1$ from the normal equations.
          (pp.~25--26; in full in the appendix, pp.~148--150)
