...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}r l >{\raggedright\arraybackslash}p{0.60\textwidth}@{\hspace{2pt}}}
    ...
    \midrule
     1 & {\color{accent}Ch.\ 1 + 2} & Introduction\,{\color{black!45}--- what is statistical learning} \\
    \rowcolor{black!3} 2 & {\color{accent}Ch.\ 2} & Assessing model accuracy\,{\color{black!45}--- bias--variance, Bayes classifier, KNN} \\
     3--4 & {\color{accent}Ch.\ 3} & Linear regression\,{\color{black!45}--- estimation, inference, diagnostics} \\
    \rowcolor{accent!18} \textbf{5--6} & \textbf{Ch.\ 4} & \textbf{Classification}\,{\color{accent!75!black}--- logistic regression, confusion matrix, ROC} \\
     7 & {\color{accent}Ch.\ 5} & Resampling\,{\color{black!45}--- validation, CV, bootstrap} \\
    \rowcolor{black!3} 8 & {\color{accent}Ch.\ 6} & Model selection and regularisation\,{\color{black!45}--- ridge, lasso} \\
     9 & {\color{accent}Ch.\ 7} & Moving beyond linearity\,{\color{black!45}--- splines, GAMs} \\
    \rowcolor{black!3} 10 & {\color{accent}Ch.\ 8} & Tree-based methods\,{\color{black!45}--- trees, forests, boosting} \\
     11 & {\color{accent}Ch.\ 9} & Support vector machines \\
    \rowcolor{black!3} 12 & {\color{accent}Ch.\ 10} & Deep learning\,{\color{black!45}--- MLPs, CNNs, training} \\
     13 & {\color{accent}Ch.\ 11} & Survival analysis\,{\color{black!45}--- censored data} \\
    \rowcolor{black!3} 14 & {\color{accent}Ch.\ 12} & Unsupervised learning\,{\color{black!45}--- PCA, clustering} \\
     15 & {\color{accent}Ch.\ 13} & Multiple testing\,{\color{black!45}--- FWER, FDR} \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{1mm}
  {\scriptsize Fifteen lectures of 180 minutes, after a taught precourse session. Today's chapter is highlighted. Short exercises every $\sim$20 min; extended exercises every $\sim$45 min; each chapter has a companion lab notebook.}
\end{frame}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_04\_lab.ipynb}]
    ...
    \begin{itemize}
      ...
      \item the \emph{ROC curves} subsection of \S2;
      \item \emph{optional:} the rest of \S2, \emph{LDA, QDA, naive Bayes, KNN --- head-to-head}, and \S3 \emph{Poisson regression on Bikeshare} --- both appendix material.
    \end{itemize}
...
for name, mdl in [("LDA", LDA()),
  ...
  comparable. Add each model to the ROC axes with one more
  \texttt{RocCurveDisplay.from\_estimator(...)} call.
\end{frame}
