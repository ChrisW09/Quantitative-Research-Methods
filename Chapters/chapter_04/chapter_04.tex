...
% The title always prints the words "In industry", so the box does not rely on its
% tint alone; every call passes an explicit title, so the default is never used.
...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{black!3} {\color{accent}Ch.\ 12} & Unsupervised learning\,{\color{black!45}--- PCA, clustering} \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{1mm}
...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.5cm}p{5.0cm}p{5.4cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{Which decision it drives} \\
    \midrule
    Retail banking &
      Probability of default (PD) on card, loan and mortgage applications &
      Accept/decline cut-off, risk-based pricing, IFRS 9 loss provisions \\
    Payments &
      Card-fraud and anti-money-laundering transaction screening &
      Block at checkout, or send to a human review queue \\
    Telco, SaaS, banking &
      Monthly churn (attrition) propensity scoring &
      Who receives a retention offer this month \\
    Insurance &
      Underwriting referral; claims-fraud and claims-severity triage &
      Which applications and claims a human examines \\
    Healthcare, pharma &
      Diagnostic and screening tests; biomarker and spectral panels (classic LDA territory: appendix) &
      Who is referred for a confirmatory test \\
    Tech, operations &
      Spam, content and support-ticket classification (multinomial) &
      Auto-route to a queue, or escalate to an agent \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{2pt}
  \begin{industry}[The common thread]
    All six deliver a \emph{probability}, not a verdict. The verdict comes from a
    \textbf{threshold chosen on business costs} --- and where the decision has to be
    explained to a customer or a regulator, the explainable model usually wins.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: credit scoring (probability of default)}
  \footnotesize
  \begin{industry}[The model behind an accept/decline decision]
    \textbf{Response:} 90+ days past due within 12 months of scoring.\quad
    \textbf{Predictors:} utilisation, months since last delinquency, recent
    bureau enquiries, income and tenure, loan-to-value.
  \end{industry}
  \vspace{-3mm}
  \begin{numexample}[The fitted output (illustrative)]
    Utilisation gets a positive coefficient, a delinquency-free year a negative
    one; scorecards rescale the log-odds into points calibrated so that a fixed
    number of points \emph{doubles the odds}.
  \end{numexample}
  \vspace{-3mm}
  \begin{readme}[Which decision --- and who signs off]
    The PD drives the cut-off, the rate offered, IFRS 9 expected credit loss
    ($\text{PD}\times\text{LGD}\times\text{EAD}$) and Basel capital --- and an independent
    validation function, not the analyst, signs it off (SR 11-7, ECB TRIM).
    Day to day it is the credit-committee member and the branch lead who live
    with that cut-off.
  \end{readme}
  \vspace{-3mm}
  \begin{takeaway}[Why logistic regression --- and one caveat]
    A declined applicant must be told \emph{why}: adverse-action reason codes
    under US ECOA/Reg B come straight from the coefficients. Caveat: the data
    contain only \emph{accepted} applicants, so the sample is selected.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Worked example: Default vs.\ balance}
  ...
  \end{numexample}
  \begin{industry}[Credit: coefficients become reason codes]
    A declined applicant must be told \emph{which} factors counted against them;
    the signed coefficients supply exactly that, which is why scorecards stay
    logistic. Quote an odds ratio as if it were a change in probability and the
    notice is simply wrong.
  \end{industry}
\end{frame}
...
\begin{frame}{The student paradox in the Default data}
  ...
  \end{takeaway}
  \begin{industry}[Portfolio analytics: mix effects]
    The same trap sits in every management report: a segment or a channel looks
    riskier (or more churn-prone) only because it carries larger balances,
    younger accounts or cheaper tariffs. Repricing a segment on the raw
    cross-tabulation prices the mix, not the risk.
  \end{industry}
\end{frame}
...
\begin{frame}{Why we need more than ``accuracy''}
  ...
  \end{takeaway}
  \begin{industry}[Payments: fraud and AML screening]
    This is not a toy example --- it is how card fraud and money-laundering alerts
    actually look. A missed fraud costs the chargeback; a false positive blocks a
    good customer at checkout. Fraud teams therefore price each cell of the
    matrix, and set the threshold to the number of alerts their reviewers can
    actually work through in a day.
  \end{industry}
\end{frame}
...
\begin{frame}{Self-check questions}
  ...
  \begin{enumerate}
    ...
    \item Write the logistic model and its log-odds form. What exactly does
          $e^{\beta_j}=1.15$ tell a client? (pp.~19--20 and~27)
    \item In the Default data, why does \texttt{student} flip sign
          once \texttt{balance} is included? (pp.~33--34)
    ...
    \item The threshold drops from $0.5$ to $0.2$. Which way do
          ...
          what decides whether that is an improvement? (pp.~50 and~54--56)
  \end{enumerate}
  \vspace{1mm}
  {\footnotesize \textbf{Appendix questions (optional).}
  Derive the LDA discriminant from Bayes' rule assuming
  $X\mid Y=k\sim\mathcal{N}(\mu_k,\Sigma)$ (pp.~90--91; step-by-step pp.~108--111).
  When does naive Bayes outperform logistic regression even though its
  independence assumption is wrong? (pp.~104 and~116)}
\end{frame}
...
\begin{frame}{Two philosophies of classification}
  ...
  \end{takeaway}
  \begin{industry}[Screening: the prior is the prevalence]
    Generative models make the base rate explicit: in a diagnostic or fraud
    screen, $\pi_k$ \emph{is} the prevalence. When the condition is rare, even a
    sharp test flags mostly false positives --- which is why a screening result is
    confirmed, not acted on.
  \end{industry}
\end{frame}
...
\begin{frame}{Method comparison: when each wins}
  ...
  \end{takeaway}
  \begin{industry}[Risk and campaign teams: how it is actually reported]
    Risk teams quote AUC as the Gini coefficient ($2\,\text{AUC}-1$) and the KS
    statistic; campaign teams compare models at the \emph{top decile}, because
    that is where the budget goes. Boosted trees (Chapter~8) often win outright ---
    which is why regulated decisions keep the model they can explain.
  \end{industry}
\end{frame}
