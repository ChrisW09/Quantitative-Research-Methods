...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{black!3} {\color{accent}Ch.\ 2} & Statistical learning\,{\color{black!45}--- accuracy, bias--variance} \\
     {\color{accent}Ch.\ 3} & Linear regression\,{\color{black!45}--- estimation, inference, diagnostics} \\
    \rowcolor{accent!18} \textbf{Ch.\ 4} & \textbf{Classification}\,{\color{accent!75!black}--- logistic regression, confusion matrix, ROC} \\
...
\begin{frame}{Contents}\small
  ...
  \vfill
  {\tiny Optional and advanced material is collected in the \textbf{appendix} at the end of the
  deck: the \textbf{generative models} (Bayes refresher, LDA, QDA, naive Bayes, Exercises
  4.5--4.7), the classifier comparison (Extended Exercise 4.4), the derivations
  (Extended Exercises 4.2--4.3), and GLMs / Poisson regression.}
\end{frame}
...
\begin{frame}{Notation in this chapter}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    $\pi_k$ & prior probability of class $k$ \quad\emph{(appendix)} \\
    $f_k(x)$ & class-conditional density of $X$ within class $k$ \quad\emph{(appendix)} \\
    $\mu_k$, $\Sigma$ & class-$k$ mean vector; (shared) covariance matrix \quad\emph{(appendix)} \\
    $\delta_k(x)$ & discriminant score of class $k$; predict the largest \quad\emph{(appendix)} \\
  ...
  \end{center}
  \vspace{2pt}
  {\scriptsize Symbols marked \emph{(appendix)} belong to the generative
  models (LDA, QDA, naive Bayes), which are optional material at the end of
  the deck.}
\end{frame}
...
\begin{frame}{Learning objectives for this chapter}
  ...
  \begin{itemize}
    ...
    \item \textbf{Extend} the model to several predictors and
          \textbf{recognise} confounding in the fitted coefficients.
    \item \textbf{Read} confusion matrices, sensitivity, specificity,
          precision, ROC (receiver operating characteristic) curves, and AUC;
          \textbf{tune} the threshold to your loss function.
    \item \textbf{Run} the whole workflow in Python
          (\texttt{statsmodels} and \texttt{scikit-learn}).
  \end{itemize}
  \vspace{1mm}
  {\footnotesize \textbf{Optional (appendix).} The \emph{generative} classifiers ---
  the Bayes refresher, LDA, QDA and naive Bayes, their comparison with logistic
  regression and KNN, and the GLM / Poisson extension --- are covered in the
  appendix and are not part of the taught thread.}
  \begin{readme}[{{How to use this deck --- self-study and revision}}]
    \footnotesize Attempt each exercise \emph{before} its solution; run
    \texttt{chapter\_04\_lab.ipynb} alongside the lab section. Revise from the
    back --- key formulas, vocabulary, self-check (each question names its
    page) --- then redo the exercises; the appendix is fully solved.
  \end{readme}
\end{frame}


\begin{frame}{Roadmap of this chapter}
  \begin{takeaway}[The taught thread --- one model done properly]
    \begin{enumerate}
      \item \textbf{Why} linear regression fails on a categorical response.
      \item \textbf{Logistic regression} --- discriminative, linear in the
            log-odds: the model, maximum likelihood, coefficients on the odds
            scale, several predictors and confounding.
      \item \textbf{Evaluation} --- confusion matrix, sensitivity /
            specificity / precision, ROC and AUC, choosing the threshold.
      \item \textbf{Python lab} --- the same workflow end-to-end.
    \end{enumerate}
  \end{takeaway}
  \begin{block}{In the appendix (optional)}
    \footnotesize
    The \emph{generative} family --- LDA, QDA and naive Bayes, built from
    Bayes' theorem --- together with the head-to-head comparison against
    logistic regression and KNN, and the GLM framework that ties linear,
    logistic and Poisson regression together.
  \end{block}
\end{frame}
...
\begin{frame}{Why ordinary linear regression fails on 0/1 data}\label{frm:whynotols}
...
\begin{frame}{Solution \textemdash{} Exercise 4.1 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.1 (2/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.1 (3/4)}
\begin{solutionbox}
...
same classification as linear discriminant analysis --- see the appendix
--- but its fitted values are still not valid probabilities.)

\vspace{1mm}
\end{solutionbox}
\end{frame}

\begin{frame}{Solution \textemdash{} Exercise 4.1 (4/4)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
hoping a larger sample fixes the problem. No
...
\begin{frame}{Why we need a logistic curve}
  ...
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: the curve is Verhulst's (1838), a
  model of population growth; logistic regression is largely David Cox's
  (1958).}
\end{frame}

\begin{frame}{The logistic regression model}\label{frm:logmodel}
...
\begin{frame}{The logistic curve in pictures}
  \begin{center}
    \includegraphics[width=0.78\textwidth,height=0.38\textheight,keepaspectratio]{4_2.pdf}
  \end{center}
  ...
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    Switch to \texttt{chapter\_04\_lab.ipynb} \S1 (\emph{Predicted probabilities}) and run the
    plot cell: the fitted $\Pr(\text{default})$ traces exactly this S-shape over
    \texttt{balance} on the real Default data --- one curve per student status.
  \end{labnote}
...
\begin{frame}{Solution \textemdash{} Exercise 4.2 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.2 (2/3)}
\begin{solutionbox}
\textbf{(3) Odds.}
\[
...
\begin{frame}{Solution \textemdash{} Exercise 4.2 (3/3)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
reading $\eta=-2.40$ itself as a probability.
...
\begin{frame}{Interpreting the coefficients}\label{frm:coefint}
...
\begin{frame}{Worked example: Default vs.\ balance}
  ...
  \end{numexample}
  \begin{mistake}
    Quoting the odds ratio as if it were a change in probability. ``The odds
    multiply by $245$'' does \emph{not} mean the probability rises by a factor
    of $245$ --- probabilities are capped at one.
  \end{mistake}
\end{frame}
...
\begin{frame}{Solution \textemdash{} Exercise 4.3 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.3 (2/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.3 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.3 (4/4)}
\begin{solutionbox}
\vspace{1mm}
...
\end{solutionbox}
\begin{mistake}
reporting part (1) as ``students are $47.6\%$
...
\begin{frame}{The student paradox in the Default data}\label{frm:student}
  ...
  \begin{takeaway}
    ...
    regression when several predictors might be related. The same trap sits in
    every segment-level report: a group looks riskier only because it carries
    larger balances, so acting on the raw cross-tabulation acts on the
    \emph{mix}, not on the risk.
  \end{takeaway}
\end{frame}

\begin{frame}{The student paradox visualised}\label{frm:studentviz}
  \begin{center}
    \includegraphics[width=0.85\textwidth,height=0.38\textheight,keepaspectratio]{4_3.pdf}
  \end{center}
  ...
  story.
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    Switch to \texttt{chapter\_04\_lab.ipynb} \S1 and run the GLM cell: in the
    \texttt{statsmodels} summary, \texttt{student\_d} is \emph{negative}
    ($\hat\beta\approx-0.65$) once \texttt{balance} is held fixed --- the sign flip
    behind this figure, live.
  \end{labnote}
...
\begin{frame}{Solution \textemdash{} Exercise 4.4 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.4 (2/3)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.4 (3/3)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.1 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.1 (2/4)}
\begin{solutionbox}
\textbf{(2) Odds for A.}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.1 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.1 (4/4)}
\begin{solutionbox}
\textbf{(6) Estimation.}
...
\end{solutionbox}
\begin{mistake}
plugging income in dollars. $\beta_2=0.003$ is
...
\begin{frame}{Why we need more than ``accuracy''}\label{frm:accuracy}
  \vspace{-2mm}{\hfill\scriptsize\itshape\color{gray}$\blacktriangleright$~session 2 of this chapter starts here\;}\par
  ...
  \begin{takeaway}
    ...
    decompose it into the four cells of a \emph{confusion matrix} --- the two
    errors cost different amounts, and only the decomposition shows them.
  \end{takeaway}
\end{frame}

\begin{frame}{The confusion matrix}\label{frm:confmat}
...
\begin{frame}{The confusion matrix as a picture}
  ...
  \end{center}
  \begin{takeaway}
    The diagonal cells --- TP and TN --- are the correct decisions; FP and
    FN are the two error types. Sensitivity reads down the true-positive
    column; specificity down the true-negative column; precision across the
...
\begin{exercise}
A classifier fitted to the Default data ($n=10{,}000$), scored at
threshold $0.5$, gives:
\begin{center}\small
\begin{tabular}{l|cc|c}
\toprule
...
\end{exercise}
{\scriptsize Nothing here depends on \emph{which} classifier produced the
table --- these are the ISLP Table~4.4 counts (LDA; appendix).}
\end{frame}
...
rate of $333/10000=3.33\%$ --- barely worse than the model's $2.75\%$. So
almost all of the headline accuracy comes from the $96.7\%$ base rate
of non-defaulters, not from skill. The
high accuracy hides a poor sensitivity: the model \emph{misses
...
\end{solutionbox}
\begin{mistake}
mixing up sensitivity and precision. Both have
...
\begin{frame}{Why a single threshold is not enough}\label{frm:threshold}
...
\begin{frame}{The trade-off on the real data}
  \begin{center}
    \includegraphics[width=0.72\textwidth,height=0.27\textheight,keepaspectratio]{ch04_threshold_tradeoff.png}
  \end{center}
  \begin{takeaway}
    The lab's logistic regression, swept over every threshold.
    \texttt{predict()}'s silent $t=0.5$ catches $39$ of $113$ defaulters;
    $t=0.10$ catches $91$, but two thirds of the flagged never default.
    No statistic picks the point --- the cost of a missed default does.
  \end{takeaway}
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Switch to \texttt{chapter\_04\_lab.ipynb} \S4 and run the threshold cell to reproduce
    both operating points live; the printed AUC stays $0.946$ at either threshold.
  \end{labnote}
\end{frame}

\begin{frame}{The ROC curve}\label{frm:roc}
  \begin{center}
...
\begin{frame}{ROC and AUC on the Default data}
  \begin{center}
    \includegraphics[width=0.88\textwidth,height=0.33\textheight,keepaspectratio]{ch04_roc.png}
  \end{center}
  \begin{takeaway}
  ...
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Switch to \texttt{chapter\_04\_lab.ipynb} \S2 (\emph{ROC curves}) and run the plot cell:
    all five classifiers on one axes --- logistic regression, LDA and QDA tie at
    AUC $0.946$; KNN trails at $0.811$.
  \end{labnote}
\end{frame}

\begin{frame}{AUC: area under the ROC curve}\label{frm:auc}
  \begin{block}{Probabilistic interpretation}
    AUC $= \Pr\bigl(\hat p(\text{positive}) > \hat p(\text{negative})\bigr)$
...
\begin{exercise}
Lowering the threshold of the same classifier to $0.2$ on the same
Default data gives:
\begin{center}\small
\begin{tabular}{l|cc|c}
\toprule
...
\begin{frame}{Solution \textemdash{} Exercise 4.9 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.9 (2/3)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.9 (3/3)}
\begin{solutionbox}
\textbf{(4) AUC.}
...
\end{solutionbox}
\begin{mistake}
expecting the AUC to improve after lowering the
...
\begin{labnote}[{{Run this live}}]
  The worked solution sits in \texttt{chapter\_04\_lab.ipynb} under
  \emph{Lecture exercises --- worked Python solutions}: the cell headed
  \emph{Extended Exercise 4.5 --- Default: logistic, confusion, ROC/AUC [Python]}.
\end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution \textemdash{} Extended Exercise 4.5 (1/4)}
\begin{solutionbox}
\begin{lstlisting}
...
clf   = LogisticRegression(max_iter=10000).fit(X, y)  # fit by max. lik.
proba = clf.predict_proba(X)[:, 1]  # col 1 = P(Yes), not a 0/1 label
\end{lstlisting}
\end{solutionbox}
\end{frame}

\begin{frame}[fragile]{Solution \textemdash{} Extended Exercise 4.5 (2/4)}
\begin{solutionbox}
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize\linespread{0.9}\selectfont]
for t in (0.5, 0.2):          # same fitted model, two decision cut-offs
    tn, fp, fn, tp = confusion_matrix(y, proba > t).ravel()  # order TN,FP,FN,TP
    sens, spec = tp/(tp+fn), tn/(tn+fp)    # caught defaulters / caught payers
    prec, acc  = tp/(tp+fp), (tp+tn)/len(y)  # alarms that were real; overall
    print(t, tn, fp, fn, tp,               # one row of the table below
          round(acc,3), round(sens,3), round(spec,3), round(prec,3))

print("AUC =", round(roc_auc_score(y, proba), 3))  # no cut-off involved
RocCurveDisplay.from_predictions(y, proba); plt.show()  # every cut-off at once
\end{lstlisting}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.5 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.5 (4/4)}
\begin{solutionbox}
  \begin{itemize}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_04\_lab.ipynb}]
    ...
    \begin{itemize}
      \item \S1 \emph{Logistic regression}, including \emph{Predicted probabilities};
      \item \S2 \emph{LDA, QDA, naive Bayes, KNN --- head-to-head} --- run its \emph{ROC curves} subsection now; the rest is appendix material;
      \item \S3 \emph{Poisson regression on Bikeshare} --- optional, appendix material;
      \item \S4 \emph{The threshold is a business decision, not a statistical one} --- the sweep behind the trade-off slide.
    \end{itemize}
...
y = (Default["default"] == "Yes").astype(int)        # 1 = defaulted
X = sm.add_constant(Default[["balance", "income"]])  # beta_0 column
...
print(res.summary())                  # coefficients, SEs, z, p-values
  ...
  \begin{readme}
    \texttt{family=Binomial()} \emph{is} logistic regression: that family's
    default link is the logit, so the fit solves
    $\log[p/(1-p)]=\beta_0+\beta_1\,$balance$\,+\beta_2\,$income by
    maximum likelihood.
  \end{readme}
\end{frame}
...
print(classification_report(yte, proba > 0.5))  # same cut as predict()
print("AUC:", roc_auc_score(yte, proba))        # needs no cut-off at all
  \end{lstlisting}
  \vspace{-1mm}
  \begin{readme}
    Column~1 of \texttt{predict\_proba} is $P(\text{default}=\text{Yes})$.
    Comparing it to $0.5$ is exactly what \texttt{predict} does silently
    --- writing the cut-off out is what lets you move it.
  \end{readme}
\end{frame}
...
RocCurveDisplay.from_estimator(lr, Xte, yte, ax=ax)      # all cut-offs
ax.plot([0,1], [0,1], "k--", linewidth=0.8)   # chance line (AUC = 0.5)
ax.set_title("Default: ROC on the held-out 30%"); plt.show()
  \end{lstlisting}
  Every extra classifier is one more
  \texttt{RocCurveDisplay.from\_estimator(...)} call on the same
  \texttt{ax} --- the appendix does exactly that for LDA and QDA.
  \begin{readme}
    Internally \texttt{roc\_curve} slides the cut-off from~1 down to~0 and
    records the pair (false-positive rate, true-positive rate) at each one;
    the curve simply joins those points. So a point on the curve is a
    \emph{threshold}, and moving along it re-tunes the decision, never the
    fitted model --- which is why the AUC of a curve does not depend on any
    cut-off.
  \end{readme}
\end{frame}
...
\begin{labnote}[{{Run this live}}]
  The worked solution sits in \texttt{chapter\_04\_lab.ipynb} under
  \emph{Lecture exercises --- worked Python solutions}: the cell headed
  \emph{Exercise 4.10 --- Logistic regression + confusion matrix in Python [Python]}.
\end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution \textemdash{} Exercise 4.10 (1/3)}
\begin{solutionbox}
\begin{lstlisting}
import pandas as pd                                  # data handling
from sklearn.linear_model import LogisticRegression  # classifier
...
\end{lstlisting}
\smallskip
Keeping \texttt{proba} is the whole point: \texttt{clf.predict(X)} would
already have collapsed it at the $0.5$ cut-off.
\end{solutionbox}
\end{frame}

\begin{frame}[fragile]{Solution \textemdash{} Exercise 4.10 (2/3)}
\begin{solutionbox}
\begin{lstlisting}
for t in (0.5, 0.2):        # predict() would nail t at 0.5 for us
    tn, fp, fn, tp = confusion_matrix(y, proba > t).ravel()  # this order
    sens = tp / (tp + fn)               # share of defaulters we catch
...
\begin{frame}[fragile]{Solution \textemdash{} Exercise 4.10 (3/3)}
\begin{solutionbox}
...
misses most true defaulters, exactly as in Exercise 4.8. Dropping the threshold
...
\begin{enumerate}
  ...
  \item Do an \emph{honest} out-of-sample test: train on
        ...
        logistic regression --- and, as a one-line bonus, for LDA
        (appendix material; here it is a single \texttt{scikit-learn} import).
...
\end{longexercise}
\begin{labnote}[{{Run this live}}]
  The worked solution sits in \texttt{chapter\_04\_lab.ipynb} under \emph{Lecture
  exercises --- worked Python solutions}: the cell headed \emph{Extended Exercise
  4.6 --- Predicting market direction out-of-sample [Python]}.
\end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution \textemdash{} Extended Exercise 4.6 (1/4)}
\begin{solutionbox}
\begin{lstlisting}
import pandas as pd                        # data handling
from sklearn.linear_model import LogisticRegression  # logistic model
from sklearn.discriminant_analysis import LinearDiscriminantAnalysis as LDA
from sklearn.metrics import accuracy_score, confusion_matrix  # scoring
...
print("acc:", round(full.score(wk[lags], y), 3))  # in-sample: optimistic
...
\begin{frame}[fragile]{Solution \textemdash{} Extended Exercise 4.6 (2/4)}
\begin{solutionbox}
\begin{lstlisting}
...
for nm, mdl in [("logit", LogisticRegression(max_iter=10000)),
    ...
    pred = mdl.predict(wk.loc[te, ["Lag2"]])       # labels: cut at 0.5
...
\smallskip
\textbf{Reading the code.} \texttt{predict} silently applies the $0.5$
cut-off; \texttt{predict\_proba(...)[:,1]} would return the probability
and leave the choice to you. \texttt{ravel()} lists the cells as
TN, FP, FN, TP.
\end{solutionbox}
\end{frame}

\begin{frame}{Solution \textemdash{} Extended Exercise 4.6 (3/4)}
\begin{solutionbox}
\textbf{Reading it.}
\begin{itemize}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.6 (4/4)}
\begin{solutionbox}
  \begin{itemize}
...
\begin{frame}{Chapter 4 in one slide}
  \begin{block}{What we accomplished}
    One classifier done properly --- \textbf{logistic regression} --- plus the
    tools to judge \emph{any} classifier: the confusion matrix, ROC and AUC.
  \end{block}
  \begin{itemize}
    \item Saw why linear regression cannot model a categorical response.
    \item Modelled $\Pr(Y=1\mid X)$ as a logistic curve, linear in the
          log-odds; fitted it by maximum likelihood.
    \item Read coefficients on the odds scale, and watched a slope flip
          sign under confounding (the student paradox).
    \item Evaluated by confusion matrix, sensitivity / specificity /
          precision, ROC and AUC --- and chose the threshold by cost.
    \item Ran the whole workflow in \texttt{statsmodels} and
          \texttt{scikit-learn}.
  \end{itemize}
  \vspace{1mm}
  {\footnotesize \textbf{Appendix (optional):} the generative family --- Bayes
  refresher, LDA, QDA, naive Bayes --- their comparison with logistic
  regression and KNN, and the GLM / Poisson extension.}
\end{frame}
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{block}{Logistic regression}
    $p(X) = e^{\beta_0 + \beta^\top X}/(1 + e^{\beta_0 + \beta^\top X})$,
    \qquad
    $\log\frac{p}{1-p} = \beta_0 + \beta^\top X = \eta$.
  \end{block}
  \vspace{-2.6mm}
  \begin{block}{Evaluation: confusion-matrix rates, ROC, AUC}
    Sensitivity (recall, TPR) $=\mathrm{TP}/(\mathrm{TP+FN})$;\ \
    specificity (TNR) $=\mathrm{TN}/(\mathrm{TN+FP})$;\ \
    precision $=\mathrm{TP}/(\mathrm{TP+FP})$;\ \
    FPR $=1-$spec.\ $=\mathrm{FP}/(\mathrm{FP+TN})$.
    ROC: TPR vs.\ FPR across thresholds;
    AUC $=\Pr\bigl(\hat p(\text{pos})>\hat p(\text{neg})\bigr)$, $0.5$ chance, $1$ perfect.
  \end{block}
  \vspace{-2.6mm}
  \textbf{Threshold $\to$ boundary:}
  $p>t \iff \eta > \log\frac{t}{1-t}$ \ ($t=0.5\iff\eta>0$).
  \vspace{-0.5mm}
  \begin{block}{Generative models --- appendix, kept here for reference}
    Bayes posterior:
    $\Pr(Y = k \mid X) = \pi_k f_k(X) / \sum_\ell \pi_\ell f_\ell(X)$.
    \\[1pt]
    LDA: $\delta_k(x) = x^\top \Sigma^{-1}\mu_k - \tfrac12 \mu_k^\top \Sigma^{-1}\mu_k + \log\pi_k$;
    QDA gives each class its own $\Sigma_k$, so the quadratic term survives:
    $\delta_k(x) = -\tfrac12 (x-\mu_k)^\top \Sigma_k^{-1}(x-\mu_k)
      - \tfrac12\log|\Sigma_k| + \log\pi_k$.
    \\[1pt]
    1-D LDA, common $\sigma^2$:
    $x^\star = \frac{\mu_0+\mu_1}{2} - \frac{\sigma^2}{\mu_1-\mu_0}\log\frac{\pi_1}{\pi_0}$.
  \end{block}
\end{frame}

\begin{frame}{Decision rules of thumb}
  \begin{itemize}
    \item Binary or categorical response $\Rightarrow$ never ordinary least
          squares; start with logistic regression.
    \item Large $n$, modest $p$, no idea about the distribution of $X$
          $\Rightarrow$ logistic regression is the safe default.
    \item Report a \emph{probability}, then set the threshold; never let
          $0.5$ be an unexamined default.
    \item On imbalanced data, judge by sensitivity / specificity / AUC,
          never by accuracy alone.
    \item Interpret coefficients on the odds scale, and only ever
          \emph{holding the other predictors fixed}.
    \item Highly nonlinear boundary, plenty of data $\Rightarrow$ trees and
          ensembles (Ch.~8), KNN or kernel SVM (module A5).
  \end{itemize}
  \vspace{1mm}
  {\footnotesize \textbf{Appendix rules of thumb.} Small $n$ with roughly
  Gaussian, equally spread classes $\Rightarrow$ LDA; visibly unequal class
  covariances with enough data $\Rightarrow$ QDA; many binary / categorical
  features with $p\gg n$ $\Rightarrow$ naive Bayes. Standardise features
  before any distance-based model.}
\end{frame}
  ...
  \begin{alertblock}{Don't}
    \begin{enumerate}
      ...
      \item Treat logistic-regression probabilities as calibrated
            without checking.
      \item Read $e^{\beta_j}$ as a change in \emph{probability} --- it is a
            multiplier on the \emph{odds}, and the probability effect
            depends on where you sit on the curve.
      \item Interpret a univariate slope as a causal effect: adding
            \texttt{balance} flips the sign of \texttt{student}.
      \item Report a confusion matrix without saying which threshold
            produced it --- change the threshold, change every cell.
      \item Ignore the base rate --- in rare-event detection a
            ``perfect-looking'' classifier can be useless.
    \end{enumerate}
  \end{alertblock}
  {\footnotesize \emph{Appendix pitfalls:} LDA on visibly unequal class
  covariances; forgetting that QDA needs $\sim K p^2/2$ parameters.}
\end{frame}

\begin{frame}{Connections to the rest of the course}
  \begin{itemize}
    \item Ch.~2: the Bayes classifier and KNN --- the benchmark today's
          logistic regression is measured against.
    \item Ch.~3: linear regression is logistic regression's continuous
          twin (both are GLMs; see the appendix).
    \item Ch.~5: cross-validation for choosing thresholds and tuning.
    \item Ch.~6: ridge / lasso regularisation extends to logistic
          regression.
    \item Ch.~8: tree ensembles often beat logistic regression on
          tabular data --- but are harder to explain to a regulator.
    \item Module A5 (ISLP Ch.~9): SVMs as another linear / kernelised classifier.
    \item Module A9 (ISLP Ch.~10): deep learning is logistic regression with
          representation learning baked in (the softmax output layer is
          the multinomial logit of the appendix).
  \end{itemize}
\end{frame}

\begin{frame}{Self-check questions}
  Each question names the slide that answers it --- a wrong answer tells you
  where to read.
  \small
  \begin{enumerate}
    \item Why does ordinary linear regression fail for binary $Y$?
          (p.~\pageref{frm:whynotols})
    \item Write the logistic model and its log-odds form. What exactly does
          $e^{\beta_j}=1.15$ tell a client? (pp.~\pageref{frm:logmodel} and~\pageref{frm:coefint})
    \item In the Default data, why does \texttt{student} flip sign
          once \texttt{balance} is included? (pp.~\pageref{frm:student}--\pageref{frm:studentviz})
    \item Sketch a confusion matrix for a fraud detector with $99\,\%$
          accuracy on data with $1\,\%$ fraud. Is it useful?
          (pp.~\pageref{frm:accuracy}--\pageref{frm:confmat})
    \item Explain ROC and AUC. What does a $45^\circ$ ROC mean?
          (pp.~\pageref{frm:roc}--\pageref{frm:auc})
    \item The threshold drops from $0.5$ to $0.2$. Which way do
          sensitivity, specificity and the ROC operating point move, and
          what decides whether that is an improvement? (pp.~\pageref{frm:confmat} and~\pageref{frm:threshold})
  \end{enumerate}
  \vspace{1mm}
  {\footnotesize \textbf{Appendix questions (optional).}
  Derive the LDA discriminant from Bayes' rule assuming
  $X\mid Y=k\sim\mathcal{N}(\mu_k,\Sigma)$ (pp.~\pageref{frm:ldaassume}--\pageref{frm:ldadisc}; step-by-step p.~\pageref{frm:ee42}).
  When does naive Bayes outperform logistic regression even though its
  independence assumption is wrong? (pp.~\pageref{frm:nb} and~\pageref{frm:compare})}
\end{frame}

\begin{frame}{Ten things to remember}
  \begin{enumerate}
    \item Linear regression is wrong for a categorical $Y$.
    \item Logistic regression: linear in the log-odds, $S$-shaped in $p$.
    \item $e^{\beta_j}$ multiplies the \emph{odds}, not the probability.
    \item Estimation is by maximum likelihood; no closed form.
    \item Univariate slopes can flip sign --- watch for confounding.
    \item A model outputs a probability; a \emph{threshold} turns it into
          a decision.
    \item Accuracy alone is meaningless on imbalanced data.
    \item Confusion matrix $\Rightarrow$ sensitivity, specificity,
          precision --- always at a stated threshold.
    \item ROC sweeps every threshold; AUC $=\Pr(\hat p_{\text{pos}}>\hat p_{\text{neg}})$
          is threshold-free.
    \item Threshold choice is a business decision, driven by the relative
          cost of a false negative and a false positive.
  \end{enumerate}
  \vspace{1mm}
  {\footnotesize \emph{Eleventh, if you read the appendix:} LDA, QDA and naive
  Bayes are all one idea --- model $\pi_k$ and $f_k(x)$, then apply Bayes'
  rule; they differ only in how they model $f_k$.}
\end{frame}
...
\begin{frame}{Appendix --- what is in here, and why}
  \scriptsize
  Nothing here is needed to follow the chapter. The taught thread is
  \textbf{logistic regression and classifier evaluation}; everything below is a
  side topic, a formal derivation or a longer worked exercise.
  \begin{block}{Contents of the appendix}
    \begin{itemize}\itemsep2pt
      \item \textbf{Generative models --- Bayes refresher, LDA, QDA, naive
            Bayes} (Exercises 4.5--4.7): the whole family, from the
            prior~$\times$~likelihood picture to the two discriminants and the
            naive independence assumption. Read it as one block.
      \item \textbf{Deriving LDA from Bayes' theorem} (Extended Exercise 4.2):
            where that discriminant comes from, line by line.
      \item \textbf{Naive Bayes by hand} (Extended Exercise 4.3): a longer
            second pass over Exercise 4.7, and how naive Bayes relates to LDA.
      \item \textbf{Comparing the classifiers} (Extended Exercise 4.4):
            logistic regression vs.\ LDA / QDA / naive Bayes / KNN --- when each
            wins, the Bayes optimum, and the head-to-head lab cell.
      \item \textbf{Behind logistic regression:} log-likelihood, deviance and
            IRLS as three views of one optimisation; the softmax for $K>2$.
      \item \textbf{GLMs and Poisson regression:} the umbrella over linear,
            logistic and Poisson regression, with Bikeshare. Not examinable.
      \item \textbf{ISLP Figure 4.2 recreated on Default:} the straight line
            vs.\ the logistic curve, side by side. The main deck makes the
            point once; the book's two-panel version is kept here.
    \end{itemize}
  \end{block}
  \vspace{-1mm}
...
\begin{frame}{Three equivalent ways to think about the optimisation}
  ...
  \end{itemize}
  \begin{readme}[The number the software prints: deviance]
    \texttt{statsmodels} reports the fit as the \textbf{deviance}
    $=-2\log L(\hat\beta)$ (up to a constant): smaller is better. It is
    the GLM generalisation of RSS --- for normal errors it \emph{is} the
    RSS --- and the drop from the null deviance (intercept only) to the
    residual deviance plays $R^2$'s role from Chapter 3.
  \end{readme}
  \begin{takeaway}
    All four are one object: maximise $\log L$, minimise cross-entropy,
    minimise deviance, iterate IRLS --- the same $\hat\beta$ every way.
  \end{takeaway}
\end{frame}
...
\begin{frame}{From two classes to $K$: the softmax}
  ...
  \begin{takeaway}
    ...
    numerator.
  \end{takeaway}
  \islpsource{Section 4.3.5, equations (4.10)--(4.13)}
\end{frame}

% ---------------- Generative models (moved to appendix) ----------------
...
\begin{frame}{Two philosophies of classification}
  ...
  \begin{takeaway}[Which to prefer?]
    ...
    situations where logistic regression can be unstable. They also make the
    base rate explicit: $\pi_k$ \emph{is} the prevalence, so when a class is
    rare even a sharp rule flags mostly false positives.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Bayes' theorem in the classification setting}
  ...
  \islpsource{Section 4.4, equation (4.15)}
\end{frame}
...
\begin{frame}{Bayes' theorem as a flow: prior $\times$ likelihood $\to$ posterior}
  ...
  \begin{tikzpicture}[font=\footnotesize,>=Latex,
    ...
    \node[box,text width=4.0cm,fill=green!6,draw=green!50!black] (post) at (10.3,0)
      {\textbf{Posterior}\\[2pt]
       $\displaystyle\Pr(Y=k\mid x)=\frac{\pi_k f_k(x)}{\sum_\ell \pi_\ell f_\ell(x)}$};
    \draw[->] (prior.east) -- (times);
    \draw[->] (lik.east)   -- (times);
    \draw[->] (times) -- (joint);
    \draw[->] (joint) -- node[above=1pt,font=\scriptsize,align=center]
                          {divide by\\evidence} (post);
  \end{tikzpicture}}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Linear Discriminant Analysis: the assumption}\label{frm:ldaassume}
  ...
  \begin{takeaway}[Why this assumption is interesting]
    ...
    design.
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: LDA is R.~A.\ Fisher's (1936).
  The paper that introduced it also introduced the iris flower data set ---
  still the classic demo data set of machine learning today.}
\end{frame}

\begin{frame}{The LDA discriminant}\label{frm:ldadisc}
  ...
  ``linear'' in LDA.
  \islpsource{Section 4.4.2, equation (4.24); one predictor: (4.18)}
\end{frame}
...
\begin{frame}{LDA decision boundary on simulated data}
  ...
  \end{center}
  Three-class LDA on simulated Gaussians: dashed = Bayes optimum,
...
\begin{frame}{LDA with one predictor: two Gaussians and a cutoff}
  \begin{center}\includegraphics[width=0.88\textwidth,height=0.5\textheight,keepaspectratio]{ch04_x_lda_1d_densities.png}\end{center}
  \begin{takeaway}LDA compares the prior-weighted densities $\pi_k f_k(x)$: equal priors put the cutoff at the midpoint $0$; $\pi_2 = 0.8$ slides it by $\log(0.8/0.2)\,\sigma^2/(\mu_2{-}\mu_1) = 0.46$ toward the rarer class. The shaded overlap is error even Bayes cannot remove.\end{takeaway}
\end{frame}
...
\begin{frame}{Solution \textemdash{} Exercise 4.5 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.5 (2/3)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.5 (3/3)}
\begin{solutionbox}
...
\begin{frame}{Quadratic Discriminant Analysis: relaxing the assumption}
  ...
  not lines. {\tiny\itshape\color{gray}(ISLP \S4.4.3, eq.\ (4.28).)}
\end{frame}
...
\begin{frame}{Solution \textemdash{} Exercise 4.6 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.6 (2/4)}
\begin{solutionbox}
\textbf{(2) For $p=5$, $K=3$.}
...
\begin{frame}{Solution \textemdash{} Exercise 4.6 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.6 (4/4)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
counting $p^2$ entries per covariance matrix ---
...
\begin{frame}{Naive Bayes: extreme simplification}\label{frm:nb}
  \footnotesize
  \begin{block}{The independence assumption (ISLP eq.\ (4.29))}
    Within each class, the features are independent:
    $f_k(x) = \prod_{j=1}^p f_{kj}(x_j)$.
  \end{block}
  \begin{readme}[How each 1-D marginal $f_{kj}$ is actually estimated]
    \begin{itemize}\itemsep1pt
      \item \textbf{Quantitative $X_j$, parametric:} fit a univariate
            normal, $X_j \mid Y{=}k \sim N(\mu_{jk},\sigma_{jk}^2)$ ---
            this is \texttt{GaussianNB}, and equals QDA with all
            covariance matrices forced \emph{diagonal}.
      \item \textbf{Quantitative $X_j$, nonparametric:} a histogram or
            kernel density estimate of the class-$k$ values.
      \item \textbf{Qualitative $X_j$:} simply the class-$k$ category
            proportions --- the spam-filter case of Exercise 4.7.
    \end{itemize}
  \end{readme}
  \begin{takeaway}
    The assumption is almost always wrong --- features are usually
    correlated. But it reduces a $p$-dimensional density problem to $p$
    one-dimensional ones, slashing variance; and even a miscalibrated
    posterior often \emph{ranks} cases well enough to classify, which is
    why naive Bayes stays competitive when $n$ is small and $p$ large.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Solution \textemdash{} Exercise 4.7 (1/3)}
\begin{solutionbox}
...
\medskip
\textbf{Set up the ingredients before any arithmetic.} Each class needs
its prior and its two feature likelihoods, read straight off the table:
\begin{center}\small
\begin{tabular}{lccc}
\toprule
 & prior $\pi_k$ & $\Pr(F_1{=}1\mid k)$ & $\Pr(F_2{=}1\mid k)$ \\
\midrule
Spam & 0.4 & 0.6 & 0.7 \\
Ham  & 0.6 & 0.1 & 0.2 \\
\bottomrule
\end{tabular}
\end{center}
Because \emph{both} features are present, each class's score is the
product of its whole row --- three numbers, nothing more. That is the
entire parameter budget the naive assumption leaves.
\end{solutionbox}
\end{frame}

\begin{frame}{Solution \textemdash{} Exercise 4.7 (2/3)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Exercise 4.7 (3/3)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
dropping the prior and normalising the
...
\begin{frame}{Extended Exercise 4.2 \textemdash{} LDA from Bayes' theorem [Math]}\label{frm:ee42}
\begin{longexercise}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.2 (1/5)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.2 (2/5)}
\begin{solutionbox}
\scriptsize
\textbf{(2) Boundary.} Set $\delta_0(x)=\delta_1(x)$ and solve for $x$:
\[
  x\frac{\mu_0}{\sigma^2}-\frac{\mu_0^2}{2\sigma^2}+\log\pi_0
  = x\frac{\mu_1}{\sigma^2}-\frac{\mu_1^2}{2\sigma^2}+\log\pi_1
  \;\Longrightarrow\;
  x\,\frac{\mu_1-\mu_0}{\sigma^2}
  = \frac{\mu_1^2-\mu_0^2}{2\sigma^2}+\log\frac{\pi_0}{\pi_1}.
\]
Dividing by $(\mu_1-\mu_0)/\sigma^2$ and using
$\mu_1^2-\mu_0^2=(\mu_1-\mu_0)(\mu_1+\mu_0)$,
\[
  x^\star=\frac{\mu_0+\mu_1}{2}
          -\frac{\sigma^2}{\mu_1-\mu_0}\log\frac{\pi_1}{\pi_0}
        ...
        =3+0.847=3.85 ,
\]
with $\log\frac{\pi_1}{\pi_0}=\log\frac{0.3}{0.7}=-0.847$. Assign class
$1$ if $x>3.85$. The prior tilt moves the boundary $0.85$ units from the
midpoint $3$ \emph{toward} the rarer class, which needs stronger
evidence.
\end{solutionbox}
\end{frame}

\begin{frame}{Solution \textemdash{} Extended Exercise 4.2 (3/5)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.2 (4/5)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.2 (5/5)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.3 (1/4)}
\begin{solutionbox}
\textbf{Method.} Bayes' rule scores each class by its numerator
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.3 (2/4)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
normalising the likelihoods alone and dropping the
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.3 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.3 (4/4)}
\begin{solutionbox}
  \begin{itemize}
...
\begin{frame}{Method comparison: when each wins}\label{frm:compare}
  \footnotesize
  \begin{tabular}{lll}
    \toprule
  ...
  \end{tabular}
  \vspace{0.3em}
  \begin{readme}[Logistic regression vs.\ LDA: closer than they look]
    Both boundaries are linear in $x$ --- the same form on the log-odds
    scale. They differ only in \emph{estimation}: logistic maximises the
    likelihood of $\Pr(Y\mid X)$ alone; LDA plugs Gaussian estimates of
    $\Pr(X\mid Y)$ into Bayes' rule --- assumptions that buy efficiency
    when true, cost robustness when false (ISLP \S4.5.1).
  \end{readme}
  \begin{takeaway}[In practice]
    No method dominates universally: try all five, pick by held-out AUC
    --- one scikit-learn line each. Boosted trees (Ch.~8) often beat them
    all on raw AUC, so the choice is rarely accuracy alone.
  \end{takeaway}
\end{frame}

\begin{frame}{Bayes optimum and our methods}
  \begin{center}
    \includegraphics[width=0.80\textwidth,height=0.46\textheight,keepaspectratio]{4_12.pdf}
  \end{center}
  \begin{takeaway}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.4 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.4 (2/3)}
\begin{solutionbox}
\vspace{1mm}
...
\begin{frame}{Solution \textemdash{} Extended Exercise 4.4 (3/3)}
\begin{solutionbox}
\scriptsize
...
for name, mdl in [("LDA", LDA()),
    ...
    mdl.fit(Xtr, ytr)                        # class means + covariances
    p = mdl.predict_proba(Xte)[:, 1]         # Bayes posterior P(class 1)
    print(name, "AUC:", roc_auc_score(yte, p))  # rank quality, no cut-off
  \end{lstlisting}
  Continues the Python lab: \texttt{Xtr, Xte, ytr, yte} are the same
  train/test split used for logistic regression, so the AUCs are directly
  comparable.
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Switch to \texttt{chapter\_04\_lab.ipynb} \S2 and run the head-to-head cell: on one
    split, logit, LDA and QDA all reach AUC $0.946$, naive Bayes $0.935$, KNN $0.811$ ---
    the linear boundary is simply adequate here.
  \end{labnote}
\end{frame}
...
\begin{frame}{Bikeshare: Poisson captures the commute rhythm}
  \begin{center}
    \includegraphics[width=0.80\textwidth,height=0.47\textheight,keepaspectratio]{4_15.pdf}
  \end{center}
  \begin{takeaway}
    Fitted Poisson coefficients by month and hour: ridership rises through the
    warmer months and peaks at the two commutes. On the \emph{log}-rate scale
    these act \emph{multiplicatively} on the expected count, so predicted
    ridership can never go negative.
  \end{takeaway}
  \islpsource{Figure 4.15}
\end{frame}
...
res = sm.GLM(Bike["bikers"], X,          # response is a count, not 0/1
             family=sm.families.Poisson()).fit()  # log link: rate > 0
print(res.summary())                     # coefficients on log-rate scale
  \end{lstlisting}
  Coefficients are on the log-rate scale; \texttt{exp(coef)} gives
  the multiplicative effect on hourly ridership. It is the same
  \texttt{sm.GLM} call as the logistic fit --- only the \emph{family}
  changes: \texttt{Binomial()} with a logit link for $0/1$ labels,
  \texttt{Poisson()} with a log link for counts.
  \begin{labnote}[{{Live demo --- \S3 of the notebook}}]
    Switch to \texttt{chapter\_04\_lab.ipynb} \S3 and run the two Bikeshare cells: the
    Poisson GLM summary on \texttt{temp}, \texttt{hum} and \texttt{windspeed} --- read
    each \texttt{exp(coef)} as a multiplicative effect on the hourly rental rate.
  \end{labnote}
\end{frame}
