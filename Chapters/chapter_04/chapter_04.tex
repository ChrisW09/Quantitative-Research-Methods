...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.5cm}p{5.0cm}p{5.4cm}@{}}
    ...
    Retail banking &
      ...
      Where the accept/decline cut-off sits, and the rate a borderline
      applicant is offered \\
    Payments &
      Card-fraud and anti-money-laundering transaction screening &
      Block the transaction at checkout, or queue it for a human reviewer \\
    Healthcare, pharma &
      Diagnostic and screening tests; biomarker panels (the classic LDA
      setting: appendix) &
      Who is called back for a confirmatory test \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{2pt}
  \begin{takeaway}[The common thread: the model gives a probability and the threshold gives the verdict]
    None of the three acts on the fitted probability directly. Each picks a
    \textbf{threshold from the cost of the two errors} --- a missed default, a
    blocked good customer, a missed diagnosis --- so moving the cut-off, not
    refitting the model, is the lever that changes the decision.
  \end{takeaway}
\end{frame}
