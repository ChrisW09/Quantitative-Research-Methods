...
\begin{frame}{Learning objectives for this chapter}
  ...
  \begin{readme}[{{How to use this deck --- self-study and revision}}]
    ...
    \texttt{chapter\_05\_lab.ipynb} alongside the lab section. Revise from the
...
\begin{frame}{Validation set: pros and cons}\label{frm:valprocon}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Pros}
      \begin{itemize}
...
\begin{frame}{Validation across splits: how erratic is it?}\label{frm:valerratic}
  \begin{center}
...
\begin{frame}{Solution --- Exercise 5.1 (1/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 5.1 (2/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 5.1 (3/3)}
  \begin{solutionbox}
...
\begin{frame}{LOOCV's hidden cost}
  ...
  \begin{takeaway}[The OLS shortcut]
    ...
    is a one-fit shortcut using the leverage statistic. LOOCV earns the
    cost when observations are expensive --- a few dozen production
    batches, a handful of clinical sites --- where surrendering $20\%$ of
    forty samples to a hold-out is not an option.
  \end{takeaway}
\end{frame}
...
\begin{frame}{The OLS LOOCV shortcut}\label{frm:shortcut}
...
\begin{frame}{$k$-fold cross-validation, step by step (native diagram)}
  ...
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: formalised in the 1970s by Mervyn
  Stone and Seymour Geisser --- decades before computing power made it
  routine.}
\end{frame}

\begin{frame}{LOOCV vs.\ $k$-fold: the bias-variance trade-off}\label{frm:loocvkfold}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{LOOCV}
...
\begin{frame}{Why $k \approx 10$ wins: visualising the trade-off}\label{frm:k10}
  \begin{center}
...
\begin{frame}{Three pitfalls that quietly invalidate CV}\label{frm:cvpitfalls}
  ...
  \end{alertblock}
  \begin{takeaway}
    All three failures share a signature: accuracy that came from leakage
    looks excellent in validation and collapses after deployment --- never
    before.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Solution --- Exercise 5.4 (1/6)}
  \begin{solutionbox}
    \textbf{(1) CV estimate.}
    \[
...
\begin{frame}{Solution --- Exercise 5.4 (2/6)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 5.4 (3/6)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 5.4 (4/6)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 5.4 (5/6)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 5.4 (6/6)}
  \begin{solutionbox}
...
\begin{frame}{Why we need the bootstrap}
  ...
  \par{\tiny\itshape\color{gray}History: invented by Bradley Efron
  (1979). The name is the point: getting something from seemingly nothing,
...
\begin{frame}{When the bootstrap works --- and when it fails}\label{frm:bootfails}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
\begin{frame}[fragile]{Solution --- Exercise 5.6 (1/3)}
...
\begin{frame}[fragile]{Solution --- Exercise 5.6 (2/3)}
  \begin{lstlisting}
rng = np.random.default_rng(0)               # reproducible RNG
n, B = len(X), 1000                          # B bootstrap replications
...
\begin{frame}[fragile]{Solution --- Exercise 5.6 (3/3)}
  \begin{solutionbox}[Solution --- Expected result]
...
\begin{frame}{Extended Exercise 5.3 --- Leak-Free CV for Tuning and Evaluation [Integrative]}\label{frm:ee53}
  \begin{longexercise}
...
\begin{frame}{Solution --- Extended Exercise 5.3 (1/4)}
...
\begin{frame}[fragile]{Solution --- Extended Exercise 5.3 (2/4)}
...
\begin{frame}[fragile]{Solution --- Extended Exercise 5.3 (3/4)}
  \begin{lstlisting}
...
\begin{frame}{Solution --- Extended Exercise 5.3 (4/4)}
...
\begin{frame}{Decision rules of thumb}
  ...
  \end{itemize}
  \begin{takeaway}[What you report is never the tuned CV score]
    Searching hyperparameters and reporting on the same folds is
    optimistic by construction. Quote the nested-CV figure, or a hold-out
    the search never touched, and give its spread alongside it.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Connections to the rest of the course}
  \begin{itemize}
    ...
    \item Module A9: tune learning rate, dropout via CV.
    \item Module A7: resampling-based $p$-values and FDR-control
          procedures.
  \end{itemize}
\end{frame}
...
\begin{frame}{Exercise 5.2 --- LOOCV Fits and the Leverage Shortcut [Math]}\label{frm:ex52}
  \begin{exercise}
...
\begin{frame}{Solution --- Exercise 5.2 (1/3)}
  \begin{solutionbox}
    ...
    \textbf{(2) The OLS shortcut} (ISLP equation (5.2)).
...
\begin{frame}{Solution --- Exercise 5.2 (2/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 5.2 (3/3)}
  \begin{solutionbox}
...
\begin{frame}{Extended Exercise 5.1 --- LOOCV Shortcut vs.\ 5-Fold CV [Math]}\label{frm:ee51}
  \begin{longexercise}
...
\begin{frame}{Solution --- Extended Exercise 5.1 (1/4)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 5.1 (2/4)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 5.1 (3/4)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 5.1 (4/4)}
  \begin{solutionbox}
