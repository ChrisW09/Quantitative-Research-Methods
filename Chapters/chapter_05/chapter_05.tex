...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{Ch.\ 5} & \textbf{Resampling}\,{\color{accent!75!black}--- validation, CV, bootstrap} \\
...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.5cm}p{5.1cm}p{4.6cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{Which decision it drives} \\
    \midrule
    Banking &
      Independent validation of a credit-risk (PD) model &
      Whether the model may go live \\
    Retail, energy &
      Walk-forward backtest of a demand or load forecast &
      Replenishment and hedging settings \\
    Insurance &
      Bootstrap of a loss-reserve or capital figure &
      The reserve that gets booked \\
    Pharma, medtech &
      Patient-grouped CV of a diagnostic or triage model &
      Whether the model goes into a submission \\
    Tech, platforms &
      Bootstrap interval for revenue per user in an A/B test &
      Ship or do not ship \\
    Manufacturing &
      $k$-fold CV to tune a quality- or yield-prediction model &
      Which model complexity goes to production \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{industry}[The common thread]
    In every row the number that leaves the room is an \emph{out-of-sample}
    number. Resampling is what turns a model someone built into a model
    someone is willing to sign.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: validating a credit-risk model}
  \footnotesize
  \begin{industry}[The setting]
    A bank's retail portfolio needs a probability-of-default (PD) model
    --- the Chapter~4 scorecard, now facing independent review.
    \textbf{Response:} default within the next 12 months ($0/1$).
    \textbf{Predictors:} months since worst delinquency, revolving-limit
    utilisation, recent credit enquiries, months on file,
    instalment-to-income ratio. \textbf{Output:} a PD per account, banded
    into rating grades, with a Gini / AUC reported on \emph{two} hold-outs
    --- one random, one \emph{out-of-time} from a later origination cohort.
  \end{industry}
  \begin{readme}[Which decision it feeds and who signs it off]
    The PD drives approve/decline cut-offs, risk-based pricing and the
    IFRS~9 expected-credit-loss provision. The modelling team does not
    sign it off: an \emph{independent} validation unit re-runs the tests
    for a model-risk committee (US: Federal Reserve SR~11-7 guidance;
    euro area: ECB supervisory model reviews). The person who has to vote
    on the out-of-time Gini is a committee member from finance or the
    business, not a modeller.
  \end{readme}
  \begin{takeaway}[The honest caveat]
    Random $k$-fold folds across origination dates let the future inform
    the past: the Gini looks excellent and then evaporates on the next
    cohort.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Validation set: pros and cons}
  ...
  \end{takeaway}
  \begin{industry}[Banking: a single split is only the floor]
    A single hold-out is a fine first sanity check, but an independent
    validation unit will ask for an \emph{out-of-time} hold-out as well:
    score a later cohort, not just other rows from the same one.
  \end{industry}
\end{frame}
...
\begin{frame}{Exercise 5.1 --- Drawbacks of the Validation Set [Concept]}
  ...
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Switch to \texttt{chapter\_05\_lab.ipynb} \S2 and run the validation-split
    cell: with \texttt{random\_state=1} the MSE falls from $24.8$ (degree 1) to
    $18.8$ (degree 2); rerun with seeds 2 and 3 and watch every number move.
  \end{labnote}
\end{frame}
...
\begin{frame}{Solution --- Exercise 5.1}
  \begin{solutionbox}
    ...
    \medskip
    \textit{Common mistake:} averaging many repeated 50/50 splits instead.
    That tames the variance but not the bias --- every fit still uses half
    the data. Only larger training folds (CV) fix both at once.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{LOOCV's hidden cost}
  ...
  \end{takeaway}
  \begin{industry}[Chemometrics and pilot studies: where $n$ is small]
    LOOCV earns its keep when observations are expensive: a chemometric
    calibration built on a few dozen production batches, a pilot study, a
    handful of clinical sites. Wasting $20\%$ of forty samples on a
    hold-out is not an option --- so hold out one at a time.
  \end{industry}
\end{frame}
...
\begin{frame}{The OLS LOOCV shortcut}
  ...
  \begin{readme}
    ...
    fits.
  \end{readme}
  \begin{labnote}[Run this live: \S4 \emph{LOOCV (leave-one-out)}]
    The notebook scores degrees $1$--$5$ with \texttt{LeaveOneOut} --- all
    $n = 392$ fits --- which is the cost this shortcut removes.
  \end{labnote}
\end{frame}
...
\begin{frame}{LOOCV vs.\ $k$-fold: the bias-variance trade-off}
  ...
  \end{takeaway}
  \begin{labnote}[{{Live demo --- \S3 and \S4 of the notebook}}]
    Run the $k$-fold cell in \S3 and the LOOCV cell in \S4: both drop from MSE
    $\approx 24.2$ at degree 1 to $\approx 19.2$ at degree 2 --- but LOOCV needs
    $392$ fits per degree against ten.
  \end{labnote}
\end{frame}
...
\begin{frame}{Three pitfalls that quietly invalidate CV}
  \begin{alertblock}{The classic CV mistakes}
    ...
    \end{enumerate}
  \end{alertblock}
  \begin{industry}[Forecasting and credit: expensive --- not textbook --- mistakes]
    Forecasts and trading strategies are backtested \emph{walk-forward}
    (train to $t$, score $t+1$) for reason~2; churn, credit and healthcare
    models split by \emph{customer} or \emph{patient} for reason~3.
    Accuracy that came from leakage fails after deployment, not before.
  \end{industry}
\end{frame}
...
\begin{frame}{Solution --- Exercise 5.3}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} reading ``bias'' here as model bias. It is
    the bias of the \emph{test-error estimate}: training on fewer rows
    makes every model look worse than it will be when fit to all $n$.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Solution --- Exercise 5.4 (2/2)}
  ...
  \end{solutionbox}
  \begin{mistake}
    averaging fold MSEs unweighted when fold
...
\begin{frame}{Why we need the bootstrap}
  ...
  \begin{industry}[Insurance and platforms: where the formula is missing]
    Actuaries quote loss-reserve \emph{ranges}; risk teams put error bars
    on value-at-risk and stress figures; platform teams need a confidence
    interval for revenue per user, which is heavy-tailed enough that the
    textbook $t$-interval is hard to defend. None of these statistics has
    a usable closed-form standard error --- so all of them are
    bootstrapped.
  \end{industry}
\end{frame}
...
\begin{frame}{The bootstrap, schematically (native diagram)}
  ...
  \end{center}
  \begin{takeaway}
    Resample the rows \emph{with replacement} (the repeated observation in each resample is set in bold), recompute $\hat\alpha$ on each of the $B$ samples, and read the standard error off the spread of the replicates.
  \end{takeaway}
\end{frame}
...
\begin{frame}{The bootstrap sampling distribution, visualised}
  \begin{center}
    \includegraphics[width=0.95\textwidth,height=0.55\textheight,keepaspectratio]{5_10.pdf}
  \end{center}
  Left: $\hat\alpha$ across $1{,}000$ \emph{simulated} data sets --- the true
  sampling distribution. Centre: $\hat\alpha$ across $1{,}000$ \emph{bootstrap}
  samples drawn from one data set. Right: the same two distributions as
  boxplots --- the bootstrap reproduces the spread, which is why its SD serves
  as the SE.
  \islpsource{Figure 5.10}
\end{frame}
...
\begin{frame}{Exercise 5.5 --- Bootstrap Inclusion Probability [Math]}
  ...
  \begin{labnote}[{{Live demo --- \S6 of the notebook}}]
    After solving, switch to \texttt{chapter\_05\_lab.ipynb} \S6 and run the
    simulation cell: $20\,000$ resamples per $n$ confirm your part-(2) answers
    to Monte-Carlo accuracy and show the limit is reached by $n \approx 100$.
  \end{labnote}
\end{frame}
...
\begin{frame}{Solution --- Exercise 5.5 (2/2)}
  ...
  \end{solutionbox}
  \begin{mistake}
    concluding that a bootstrap sample is
...
\begin{frame}{What a bootstrap resample actually contains}
  \begin{center}
    \includegraphics[width=0.74\textwidth,height=0.52\textheight,keepaspectratio]{ch05_inclusion_probability.png}
  \end{center}
  \begin{takeaway}
    The Exercise 5.5 result, drawn --- convergence is essentially complete by
    $n = 100$. Every resample holds about $63.2\%$ of the distinct rows; the
    missing $36.8\%$ return as Chapter~8's \emph{out-of-bag} validation set.
    The dots are the lab's seeded simulation.
  \end{takeaway}
\end{frame}
...
\begin{frame}[fragile]{Extended Exercise 5.2 --- Bootstrap SE: Statistic and Coefficient [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_05\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Extended Exercise 5.2 --- Bootstrap SE: Statistic and Coefficient [Python]}.
  \end{labnote}
\end{frame}
...
Port = pd.read_csv(D+"Portfolio.csv")    # paired X,Y returns
...
def alpha(x, y):                             # min-variance allocation
    ...
    return (sy - sxy) / (sx + sy - 2*sxy)    # the statistic alpha

rng = np.random.default_rng(0)               # reproducible RNG
n, B = len(X), 1000                          # B bootstrap replications
reps = np.empty(B)                           # one alpha per replicate
for b in range(B):                           # one pass = one replicate
...
print(round(ols.params[1], 4), round(ols.bse[1], 5))  # slope, its SE

m = len(hp); slopes = np.empty(1000)           # B = 1000 replications
for b in range(1000):                          # resample, refit, store
    idx = rng.integers(0, m, m)                # rows with replacement
...
\begin{frame}{Solution --- Extended Exercise 5.2 (3/3)}
  ...
  \end{solutionbox}
  \begin{mistake}
    resampling \texttt{X} and \texttt{Y} as two
...
\begin{frame}{The bootstrap in action: the sampling distribution of $\hat\alpha$}
  \begin{center}\includegraphics[width=0.88\textwidth,height=0.52\textheight,keepaspectratio]{ch05_x_bootstrap_alpha_hist.png}\end{center}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_05\_lab.ipynb}]
    ...
    \begin{itemize}
      \item \S1 \emph{The Auto data set}, then \S2 \emph{Validation set approach};
      \item \S3 \emph{$k$-fold cross-validation} and \S4 \emph{LOOCV (leave-one-out)};
      \item \S5 \emph{The bootstrap} and \S6 \emph{What is actually in a bootstrap sample?}
    \end{itemize}
...
Auto = load_data("Auto")             # mpg and horsepower, n = 392
X = Auto[["horsepower"]].values; y = Auto["mpg"].values  # 392 rows
Xtr,Xte,ytr,yte = train_test_split(X, y, test_size=0.5,  # 50% held out
                                   random_state=1)   # reproducible
mdl = LinearRegression().fit(Xtr, ytr)       # fit on training half only
mean_squared_error(yte, mdl.predict(Xte))    # MSE on the unseen half
  \end{lstlisting}
  \begin{readme}
    Four arrays out: one half to fit on, one held out and scored. Lab MSE:
    $24.8$ at degree~1, $18.8$ at degree~2; reseed and both numbers move.
  \end{readme}
\end{frame}
...
cv = KFold(n_splits=10, shuffle=True, random_state=0)  # 10 splits
mse = -cross_val_score(LinearRegression(), X, y,  # refits once per fold
                       cv=cv, scoring="neg_mean_squared_error")
print(mse.mean(), mse.std())        # CV_(10) = mean of 10 fold MSEs
  \end{lstlisting}
  \begin{readme}
    \texttt{KFold} does not split the data --- it yields ten
    \emph{index pairs}. \texttt{cross\_val\_score} walks them and
    \emph{refits} the model on each nine-fold training part before scoring
    the held-out tenth, so no fold ever scores a model that saw it. The ten
    scores are then averaged. On Auto: $\approx 24.2$ at degree~1,
    $\approx 19.2$ at degree~2.
  \end{readme}
  Always shuffle (unless time-ordered). The \texttt{neg\_} sign is a
  scikit-learn quirk: scores are ``larger is better'', so MSE is negated.
\end{frame}
...
def boot_one(rng):                     # one bootstrap replicate:
    idx = rng.integers(0, len(X), size=len(X))  # n rows, repeats OK
    return LinearRegression().fit(X[idx], y[idx]).coef_[0]  # refit slope

B = 1000                                     # bootstrap replications
boots = np.array([boot_one(rng) for _ in range(B)])  # 1000 refit slopes
print("SE:",  boots.std(ddof=1))             # SD of replicates = SE
print("95%:", np.quantile(boots, [0.025, 0.975]))  # percentile CI
  \end{lstlisting}
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    One call to \texttt{boot\_one} is one replicate: draw $n$ row indices with
    replacement, \emph{refit} the regression on those rows, keep the slope.
    Run \S5: the $1000$ slopes give $\widehat{\mathrm{SE}} = 0.0075$ against
    the OLS formula's $0.0064$, with $95\%$ interval $[-0.173,\,-0.144]$.
  \end{labnote}
\end{frame}
...
\begin{frame}[fragile]{Exercise 5.6 --- Bootstrap SE of the Portfolio $\alpha$ [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_05\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Exercise 5.6 --- Bootstrap SE of the Portfolio $\alpha$ [Python]}.
  \end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution --- Exercise 5.6 (1/2)}
  \begin{solutionbox}[Solution --- Method: why the bootstrap is the right tool]
...
a_hat = alpha(X, Y)              # point estimate on the full sample
  \end{lstlisting}
\end{frame}
...
rng = np.random.default_rng(0)               # reproducible RNG
n, B = len(X), 1000                          # B bootstrap replications
reps = np.empty(B)                           # holds the B replicates
for b in range(B):                           # one pass = one replicate
    idx = rng.integers(0, n, size=n)   # one shared idx: keeps pairs
    reps[b] = alpha(X[idx], Y[idx])    # recompute alpha on resample

print("alpha_hat:", round(a_hat, 4))                # 0.5758
print("bootstrap SE:", round(reps.std(ddof=1), 4))  # 0.0912
  \end{lstlisting}
  \begin{solutionbox}[Solution --- Expected result]
...
\begin{frame}{Solution --- Extended Exercise 5.3 (1/3)}
  \begin{solutionbox}[Solution --- Why full-data preprocessing leaks]
...
\begin{frame}[fragile]{Solution --- Extended Exercise 5.3 (2/3)}
  \begin{solutionbox}[Solution --- Correct procedure: everything inside the split]
    ...
    on each fold's training part, so the scaler's mean and SD never see
...
pipe = Pipeline([("scale", StandardScaler()),  # scaling INSIDE each fold
                 ...
                 ("lm",    LinearRegression())])  # fitted last
...
test_err = -cross_val_score(search, X, y, cv=outer,  # refits the search
                            scoring="neg_mean_squared_error")
print("chosen degree:", search.fit(X, y).best_params_)  # tuned d
print("unbiased test MSE:", test_err.mean(), "+/-", test_err.std())
  \end{lstlisting}
  \begin{solutionbox}[Solution --- Nested CV: two loops and two jobs]
  ...
  \end{solutionbox}
  \begin{mistake}
    reporting \texttt{search.best\_score\_} as
    the test error. That score \emph{chose} the winner, so it is
    optimistically biased --- only the outer loop's score is honest.
  \end{mistake}
\end{frame}
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{block}{$k$-fold and LOOCV estimates}
    ...
    \]
  \end{block}
  \begin{block}{Why LOOCV has high variance (mean of $n$ errors, each with
    variance $\sigma^2$, average pairwise correlation $\rho$)}
    \[
      \mathrm{Var}(\bar e) = \tfrac{1}{n}\sigma^2 + \tfrac{n-1}{n}\rho\,\sigma^2
      \;\longrightarrow\; \rho\,\sigma^2 \quad (n \to \infty).
    \]
  \end{block}
  \begin{block}{Bootstrap SE and inclusion probability ($\approx 37\%$ left out)}
    \[
      \widehat{\mathrm{SE}}(\hat\alpha) =
        \sqrt{\tfrac{1}{B-1}\sum_{b=1}^{B}(\hat\alpha^{*b} - \bar\alpha^*)^2},
      \qquad
      1 - \bigl(1 - \tfrac1n\bigr)^{n} \to 1 - e^{-1} \approx 0.632 .
    \]
  \end{block}
\end{frame}
...
\begin{frame}{Decision rules of thumb}
  ...
  \end{itemize}
  \begin{industry}[Governance: what you report to a steering committee]
    Never the tuned CV score. Searching hyperparameters and reporting on
    the same folds is optimistic by construction; quote the nested-CV
    figure or a hold-out the search never touched, with its spread.
  \end{industry}
\end{frame}
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    ...
    \item How would you cross-validate a model on a time series?
          (p.~35, and the rules of thumb on p.~74)
    ...
    \item Outline a nested-CV procedure for unbiased evaluation while
          tuning a hyperparameter. (pp.~66--68)
    \item For OLS, derive the LOOCV shortcut formula using leverage.
          (p.~25; reasoning and practice in the appendix, pp.~82--83)
  \end{enumerate}
\end{frame}
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{Drilling the leverage shortcut (Exercise 5.2 and Extended Exercise 5.1)} --- both work the identity that gives LOOCV for a linear model from a \emph{single} least-squares fit. The identity itself stays in the main deck (``The OLS LOOCV shortcut''); what moves here is the arithmetic practice, which is heavy going in a lecture and ideal as homework. Remember that the shortcut is special: it holds for least squares (and ridge) only.
      \item \textbf{ISLP Figure 5.5 (5-fold CV as sliding folds)} --- the book's rendering of the fold layout. The native step-by-step grid in the main deck says the same thing, so this copy is kept for reference against the text.
    \end{itemize}
  \end{block}
\end{frame}


\begin{frame}{ISLP's view of 5-fold CV}
  \begin{center}
    \includegraphics[width=0.95\textwidth,height=0.45\textheight,keepaspectratio]{5_5.pdf}
  \end{center}
  Each row is one of the five fits. The single highlighted block is the
  validation fold: it is the leftmost fifth of the data in row~1 and slides
  one fifth to the right in each row below, so every observation is held out
  exactly once. The rest of each row is the training data.
...
\begin{frame}{Solution --- Exercise 5.2 (2/2)}
  ...
  \end{solutionbox}
  \begin{mistake}
    forgetting that $y_i - \hat y_i$ and $h_i$
...
\begin{frame}{Solution --- Extended Exercise 5.1 (2/2)}
  ...
  \end{solutionbox}
  \begin{mistake}
    reading $3.88 < 4.20$ as ``LOOCV is more
