...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.5cm}p{5.1cm}p{4.6cm}@{}}
    ...
    \midrule
    Retail, energy &
      Walk-forward backtest of a demand or load forecast (rolling-origin
      CV, never random folds) &
      The reorder point and the size of the hedge booked for next week \\
    Insurance &
      Bootstrap of a loss-reserve or capital figure, where no closed-form
      standard error exists &
      The reserve that actually gets booked \\
    Pharma, medtech &
      Patient-grouped CV of a diagnostic or triage model &
      Whether the model goes into a regulatory submission at all \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{takeaway}[The common thread: the resampling scheme has to match how the model will be used]
    Each row resamples \emph{differently} --- by time, by resampling the
    sample itself, by patient --- because each faces a different leak. Get
    the scheme wrong and the number is optimistic in exactly the way that
    matters, which is why the resampling design, not the score, is what an
    independent reviewer checks first.
  \end{takeaway}
\end{frame}
