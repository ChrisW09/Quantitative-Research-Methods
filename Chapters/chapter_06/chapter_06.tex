...
\subtitle{Chapter 6: Linear Model Selection and Regularisation}
...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{Ch.\ 6} & \textbf{Model selection and regularisation}\,{\color{accent!75!black}--- ridge, lasso} \\
...
\begin{frame}{Learning objectives for this chapter}
  ...
  \begin{itemize}
    ...
    \item \textbf{Tune} $\lambda$ by cross-validation and \textbf{avoid}
          the leakage that comes from standardising outside the CV loop.
  \end{itemize}
  \begin{readme}[{{How to use this deck --- self-study and revision}}]
    \footnotesize Attempt each exercise \emph{before} its solution; run
    \texttt{chapter\_06\_lab.ipynb} alongside the lab section. Revise from the
...
\begin{frame}{Roadmap of this chapter}
  \begin{takeaway}[{{Select, then shrink, then select \emph{by} shrinking}}]
    \begin{enumerate}
      \item Why plain OLS needs help: variance, interpretability,
            collinearity.
      \item \textbf{Subset selection}: best subset, forward and backward
            stepwise --- and how to choose the size with $C_p$, AIC, BIC
            or adjusted $R^2$.
      \item \textbf{Ridge} ($\ell_2$): shrink every coefficient, keep
            every predictor.
      \item \textbf{Lasso} ($\ell_1$): shrink \emph{and} select, so some
            coefficients are exactly zero.
      \item Which to reach for when, and how to tune $\lambda$ honestly.
    \end{enumerate}
  \end{takeaway}
...
\begin{frame}{Three pressures that push us beyond OLS}
  ...
  \begin{takeaway}[The three motivations]
    \begin{enumerate}
      ...
      \item \textbf{Collinearity.} When predictors move together,
            $\mathbf{X}^\top\mathbf{X}$ is near-singular and the OLS
            coefficients swing wildly with every data refresh.
    \end{enumerate}
  \end{takeaway}
\end{frame}

\begin{frame}{Two families of solutions}
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      \begin{block}{Subset selection}
        Pick a subset of predictors, then fit OLS on it. Discrete and
        combinatorial.
      \end{block}
    ...
    \begin{column}{0.47\textwidth}
      \begin{block}{Shrinkage}
        Keep all predictors but penalise the size of their
        coefficients. Continuous, tuned by one $\lambda$.
      \end{block}
      Ridge, lasso, elastic net.
    \end{column}
  \end{columns}
  \begin{takeaway}
    Both answer \emph{which predictors should carry weight?} --- one by
    searching, the other by taxing.
  \end{takeaway}
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    Switch to \texttt{chapter\_06\_lab.ipynb} \S1 and run the loading cell: the
    Hitters data ($263$ players, $19$ predictors after dummy coding, heavily
    collinear career totals) is the testbed for both families today.
  \end{labnote}
\end{frame}
...
\begin{frame}{Best subset selection: idea}
  ...
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Switch to \texttt{chapter\_06\_lab.ipynb} \S2: best subset capped at $k \le 5$ still fits $16\,663$ models.
  \end{labnote}
\end{frame}
...
\begin{frame}{Best subset on the Credit data}
  ...
  \end{center}
  RSS and $R^2$ as $k$ varies: both improve monotonically with more
  predictors, so they cannot by themselves pick $k$ --- that is what $C_p$,
  BIC and adjusted $R^2$ (next) are for.
...
\begin{frame}{Best subset vs.\ forward stepwise: two search strategies}
  ...
  \begin{takeaway}
    ...
    keeps only the winner at each level (the bold subsets on the arrowed
    path) and never even fits the dashed subset. At $p = 20$:
    $2^{20}\approx 10^6$ vs.\ $211$ models.
  \end{takeaway}
\end{frame}

\begin{frame}{How fast the search explodes}
  \begin{center}
    \includegraphics[width=0.74\textwidth,height=0.52\textheight,keepaspectratio]{ch06_search_space.png}
  \end{center}
  \begin{takeaway}
    The lattice, counted. On Hitters ($p=19$) best subset means $524\,288$
    fits against forward stepwise's $191$ --- and the log scale is doing the
    arguing: by $p=40$ exhaustive search needs a \emph{trillion} fits.
    This is why the lab caps best subset at $k \le 5$.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Solution --- Exercise 6.2}
  \begin{solutionbox}
    ...
    \smallskip
    % Kept as italic text: lifting this into a mistake box overflows the
    % slide by ~23pt (the solutionbox is already full).
    \textit{Common mistake:} comparing models of different sizes by
    training RSS or $R^2$ --- the biggest model always wins by
    construction; that is exactly the problem these criteria solve.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Solution --- Exercise 6.3 (1/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 6.3 (2/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 6.3 (3/3)}
  \begin{solutionbox}
  ...
  \end{solutionbox}
  \begin{mistake}
    Using $\log_{10}(100) = 2$ instead of the natural log
    $\ln(100) \approx 4.605$ --- with base-$10$ logs the BIC penalty would
    collapse to $C_p$'s $2d\hat\sigma^2$ here.
  \end{mistake}
\end{frame}
...
\begin{frame}{Why ridge: numerical stability via a penalty}
  ...
  \begin{readme}
    ...
    predictors are collinear --- where plain OLS returns huge,
    sign-flipping coefficients that move with every data refresh --- and
    (ii) \emph{lowers variance} by refusing to chase noise with large
    coefficients.
  \end{readme}
\end{frame}
...
\begin{frame}{The ridge regression objective}
  ...
  \begin{takeaway}
    ...
    zero.
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: proposed by Hoerl \& Kennard
  (1970) to stabilise regressions with correlated predictors in industrial
  chemistry; the same idea appears in maths as Tikhonov regularisation.}
\end{frame}

\begin{frame}{Reading the ridge objective}\label{frm:ridgeobj}
  \begin{readme}
    \begin{tabular}{@{}cl@{}}
      \toprule
  ...
  \begin{labnote}[{{Live demo --- \S3 of the notebook}}]
    Switch to \texttt{chapter\_06\_lab.ipynb} \S3 and run the ridge cell: CV picks
    $\alpha \approx 2.8$ and all $19$ coefficients stay nonzero --- shrunk, never zeroed.
  \end{labnote}
\end{frame}
...
\begin{frame}{Solution --- Exercise 6.4 (1/2)}
  \begin{solutionbox}
    ...
    A predictor measured in metres has a coefficient $1000\times$
    larger than the same predictor in millimetres, so it would be penalised
...
\begin{frame}{Solution --- Exercise 6.4 (2/2)}
  \begin{solutionbox}
    ...
    origin). So ridge keeps all $p$ predictors; it improves prediction and
    stability but does \emph{not} select variables --- for that, use the lasso.

    \medskip
    % Kept as italic text: lifting this into a mistake box overflows the
    % slide by ~18pt (the solutionbox is already full).
    \textit{Common mistake:} treating standardisation as cosmetic. For
    plain OLS it is (predictions unchanged); with a penalty it \emph{changes
    the model} --- ridge on raw and on standardised predictors gives
...
\begin{frame}{Choosing $\lambda$ by cross-validation on Hitters}
  \begin{center}\includegraphics[width=0.88\textwidth,height=0.52\textheight,keepaspectratio]{ch06_x_ridge_cv.png}\end{center}
...
\begin{frame}{Lasso: coefficient paths}
  ...
  \end{center}
  \par{\tiny\itshape\color{gray}History: the lasso is Robert Tibshirani's
  (1996) --- one of the authors of our textbook. The name is an acronym:
...
\begin{frame}{Why does lasso produce zeros? --- the geometry}\label{frm:lassogeom}
  \begin{columns}[T]
...
\begin{frame}{Choose $\lambda$ by cross-validation}\label{frm:cvlambda}
  ...
  \end{readme}
  \begin{takeaway}
    It is $\lambda$ --- not the coefficient list --- that you defend.
  \end{takeaway}
\end{frame}

\begin{frame}{Ridge vs.\ lasso vs.\ elastic net: when each wins}\label{frm:elastic}
  \begin{columns}[T]
  ...
  \begin{takeaway}[Default workflow]
    ...
    estimated effects, fall back to elastic net or ridge. And remember
    that the lasso's chosen \emph{set} is itself unstable under
    correlation --- re-run it over bootstrap samples and believe only the
    variables that survive most of them.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Mini-check: ridge vs.\ lasso}
  ...
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Switch to \texttt{chapter\_06\_lab.ipynb} \S4 and run the lasso cell: CV picks
    $\alpha \approx 2.3$ and zeroes $5$ of the $19$ predictors; the paths cell then
    shows them hitting zero one by one.
  \end{labnote}
\end{frame}
...
\begin{frame}{Solution --- Exercise 6.5 (2/2)}
  ...
  \end{solutionbox}
  \begin{mistake}
    Thresholding at $\lambda$ instead of $\lambda/2$. Here both give zero,
    but at $\lambda = 2$ the wrong threshold yields
    $1.5 - 2 < 0 \Rightarrow 0$, killing a coefficient that actually
    survives as $1.5 - 1.0 = 0.5$.
  \end{mistake}
\end{frame}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_06\_lab.ipynb}]
    ...
    \begin{itemize}
      \item \S2, \emph{Best-subset and forward stepwise selection} --- on the Hitters data loaded in \S1;
      \item \S3, \emph{Ridge regression}, and \S4, \emph{Lasso} --- each with its coefficient-path subsection;
      \item \S5, \emph{How many models did we skip?} --- the search space, counted.
    \end{itemize}
    Data loads via the \texttt{ISLP} package or the bundled CSV files; \S6, \emph{Exercises}, closes the notebook.
  \end{labnote}
\end{frame}
...
\begin{frame}[fragile]{Best subset and stepwise (manual)}
  \begin{lstlisting}
import statsmodels.api as sm, itertools  # OLS fits + subset lists
from ISLP import load_data
Hitters = load_data("Hitters").dropna()      # NA salaries out: n=263
y = Hitters["Salary"]                         # response we predict
preds = [c for c in Hitters.columns if c not in ("Salary",)] # 19 preds

best = {}
for k in range(1, 6):                         # model sizes k = 1..5
    best_rss = None
    for combo in itertools.combinations(preds, k):  # all C(p,k) subsets
        X = sm.add_constant(Hitters[list(combo)])   # subset + intercept
        rss = sm.OLS(y, X).fit().ssr          # training RSS of subset
        if best_rss is None or rss < best_rss:      # lower RSS wins
            best_rss, best_combo = rss, combo  # keep best model per size
    best[k] = (best_combo, best_rss)
# RSS only ranks equal-size models; Cp/BIC/CV then pick k. k<=5 with
# p=19 is already 16,663 OLS fits -- and 2^19 is the full search.
  \end{lstlisting}
\end{frame}

\begin{frame}[fragile]{Ridge regression}\label{frm:labridge}
  \begin{lstlisting}
from sklearn.linear_model import Ridge      # L2 penalty: sum beta_j^2
from sklearn.model_selection import GridSearchCV  # sweeps the grid
from sklearn.pipeline import make_pipeline  # scaler + model as one unit
from sklearn.preprocessing import StandardScaler  # z-score each column
import numpy as np

X, y = Hitters[preds], Hitters["Salary"].values   # n=263, p=19
# Scale INSIDE the search: re-fit per fold, else CV flatters itself.
ridge = GridSearchCV(make_pipeline(StandardScaler(), Ridge()),  # 50 alphas
                     {"ridge__alpha": np.logspace(-2, 4, 50)}, cv=10,
                     scoring="neg_mean_squared_error").fit(X, y)  # not R^2
print("best alpha:", ridge.best_params_["ridge__alpha"])  # 2.81 here
print(dict(zip(preds, ridge.best_estimator_[-1].coef_)))  # all stay nonzero
  \end{lstlisting}
  \begin{readme}
    The penalty is not scale-invariant, so standardising is mandatory.
  \end{readme}
\end{frame}

\begin{frame}[fragile]{The lasso}
  \begin{lstlisting}
from sklearn.linear_model import Lasso   # L1 penalty: sum |beta_j|

# same discipline; 50 alphas, 1e-3 to 10: near-OLS -> heavily sparse
lasso = GridSearchCV(make_pipeline(StandardScaler(), Lasso(max_iter=10000)),
                     {"lasso__alpha": np.logspace(-3, 1, 50)}, cv=10,
                     scoring="neg_mean_squared_error").fit(X, y)
print("best alpha:", lasso.best_params_["lasso__alpha"])  # CV winner
coefs = lasso.best_estimator_[-1].coef_   # [-1] = the fitted Lasso step
print([(p, c) for p, c in zip(preds, coefs) if c != 0])  # only the survivors
  \end{lstlisting}
  \begin{readme}
    Only the penalty changed: $\sum|\beta_j|$ replaces $\sum\beta_j^2$.
    The $\ell_1$ ball has \emph{corners} on the axes, so the CV-chosen
    $\alpha$ sets weak coefficients to \emph{exactly} zero instead of
    merely small, so the printed list is shorter than \texttt{preds};
    ridge on the same columns leaves all $19$ nonzero. Standardise for
    the same reason --- the penalty compares raw coefficients.
  \end{readme}
\end{frame}

\begin{frame}[fragile]{Exercise 6.6 --- CV Ridge and Lasso on Hitters [Python]}
  \begin{exercise}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_06\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Exercise 6.6 --- CV Ridge and Lasso on Hitters [Python]}.
  \end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution --- Exercise 6.6 (1/3)}
  \begin{lstlisting}
import numpy as np, pandas as pd
from sklearn.linear_model    import Lasso, Ridge  # L1 and L2 penalty
from sklearn.model_selection import GridSearchCV
from sklearn.pipeline        import make_pipeline
from sklearn.preprocessing   import StandardScaler  # z-score columns
# path as seen from Chapters/, where you run the notebook
df = pd.read_csv("../ALL CSV FILES - 2nd Edition/Hitters.csv").dropna()
y = df["Salary"].values                      # response, n=263 rows
X = pd.get_dummies(df.drop(columns=["Salary"]), drop_first=True)  # p=19
preds = X.columns                            # scale inside the CV, not here
lasso = GridSearchCV(make_pipeline(StandardScaler(), Lasso(max_iter=10000)),
                     {"lasso__alpha": np.logspace(-3, 2, 50)}, cv=10,
                     scoring="neg_mean_squared_error").fit(X, y)
print("lasso alpha:", round(lasso.best_params_["lasso__alpha"], 3))
coefs   = lasso.best_estimator_[-1].coef_  # [-1] = the fitted Lasso
kept    = [p for p, c in zip(preds, coefs) if abs(c) > 1e-8]  # 14 of 19
dropped = [p for p, c in zip(preds, coefs) if abs(c) <= 1e-8]  # 5 zeros
print("kept   :", kept, "\ndropped:", dropped)  # retained vs zeroed exactly
  \end{lstlisting}
\end{frame}

\begin{frame}[fragile]{Solution --- Exercise 6.6 (2/3)}
  \begin{lstlisting}
ridge = GridSearchCV(make_pipeline(StandardScaler(), Ridge()),   # scale in CV
                     {"ridge__alpha": np.logspace(-2, 4, 50)}, cv=10,
                     scoring="neg_mean_squared_error").fit(X, y)
rc = ridge.best_estimator_[-1].coef_       # [-1] = the fitted Ridge
print("ridge alpha:", round(ridge.best_params_["ridge__alpha"], 3),
      "-- nonzero coefs:", int(np.sum(np.abs(rc) > 1e-8)))  # all 19
  \end{lstlisting}
\end{frame}

\begin{frame}{Solution --- Exercise 6.6 (3/3)}
  \begin{solutionbox}[Solution --- expected result and interpretation]
    The lasso selects a moderate $\lambda$ and \emph{drives several
    coefficients to exactly zero} --- typically keeping strong career and
    recent-performance predictors (\texttt{CRBI}, \texttt{Hits},
    \texttt{Walks}, \texttt{PutOuts}, \texttt{DivisionW}) and dropping the
    weak, redundant ones. Ridge keeps \emph{all} predictors nonzero, only
    shrinking them: lasso buys a sparse, interpretable model by doing
    variable selection; ridge stabilises the fit and keeps every predictor.
  \end{solutionbox}
  \begin{mistake}
    Comparing the printed \texttt{alpha} with a textbook $\lambda$ ---
    \texttt{scikit-learn} scales the lasso penalty by $1/(2n)$, so the
    numbers differ across formulations even when the fitted models agree.
  \end{mistake}
\end{frame}
...
\begin{frame}[fragile]{Extended Exercise 6.3 --- CV Ridge and Lasso Test MSE on Hitters [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    The worked solution sits in \texttt{chapter\_06\_lab.ipynb} under
    \emph{Lecture exercises --- worked Python solutions}: the cell headed
    \emph{Extended Exercise 6.3 --- CV Ridge and Lasso Test MSE on Hitters [Python]}.
  \end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution --- Extended Exercise 6.3 (1/4)}
  \begin{lstlisting}
import numpy as np, pandas as pd
from sklearn.model_selection import train_test_split
from sklearn.preprocessing  import StandardScaler
from sklearn.linear_model   import RidgeCV, LassoCV  # built-in CV
from sklearn.metrics        import mean_squared_error  # test score

D  = "../ALL CSV FILES - 2nd Edition/"   # path from Chapters/
df = pd.read_csv(D+"Hitters.csv").dropna(subset=["Salary"])  # n=263
y  = df["Salary"].values                 # response: the 263 salaries
X  = pd.get_dummies(df.drop(columns=["Salary"]),
                    drop_first=True).astype(float)  # -> 19 predictors
Xtr, Xte, ytr, yte = train_test_split(X.values, y,
                        test_size=0.33, random_state=1)  # 2/3-1/3 split
# The penalty acts on raw coefficients, so units decide who gets shrunk:
# standardise, but learn the means and sds from training rows only.
sc = StandardScaler().fit(Xtr)      # fit on TRAIN only: no leakage
Xtr, Xte = sc.transform(Xtr), sc.transform(Xte)  # apply to both sets
  \end{lstlisting}
\end{frame}

\begin{frame}[fragile]{Solution --- Extended Exercise 6.3 (2/4)}
  \begin{lstlisting}
ridge = RidgeCV(alphas=np.logspace(-2, 4, 50)).fit(Xtr, ytr)  # CV on Xtr
lasso = LassoCV(cv=10, max_iter=100000,
                random_state=0).fit(Xtr, ytr)   # 10-fold CV for lambda

for name, m in [("ridge", ridge), ("lasso", lasso)]:  # same report
    mse = mean_squared_error(yte, m.predict(Xte))  # MSE on held-out
    nz  = int(np.sum(np.abs(m.coef_) > 1e-8))   # 19 ridge vs 16 lasso
    print(name, "lambda=", round(m.alpha_, 3),
          "testMSE=", round(mse, 0), "nonzero=", nz)

dropped = [c for c, w in zip(X.columns, lasso.coef_)
           if abs(w) <= 1e-8]            # Years, CRBI, Errors here
print("lasso drops:", dropped)
  \end{lstlisting}
  \begin{readme}
    $\lambda$ comes from CV on the training rows; the MSE is on the test third.
  \end{readme}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 6.3 (3/4)}
  \begin{solutionbox}[Solution --- expected result and interpretation]
...
\begin{frame}{Solution --- Extended Exercise 6.3 (4/4)}
  \begin{solutionbox}[Solution --- expected result and interpretation]
  \begin{itemize}
  ...
  \end{solutionbox}
  \begin{mistake}
    Reading the dropped variables as ``irrelevant to salary''.
    \texttt{Years} and \texttt{CRBI} are highly correlated with the kept
    career totals --- the lasso drops \emph{redundant} predictors, and a
    different split can swap which of a correlated group survives.
  \end{mistake}
\end{frame}
...
\begin{frame}{Chapter 6 in one slide}
  \begin{block}{What we accomplished}
    Two families of methods that turn the linear model into a workhorse
    for wide, collinear and sparse problems.
  \end{block}
  \begin{itemize}
    \item Subset selection (best, forward, backward), scored by $C_p$,
          AIC, BIC or adjusted $R^2$.
    \item Shrinkage (ridge, lasso, elastic net).
    \item The geometry that makes the lasso --- and only the lasso ---
          set coefficients to exactly zero.
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{description}
    ...
    \item[\quad closed form] $\hat\beta^{\,\mathrm R} = (\mathbf{X}^\top \mathbf{X} + \lambda I)^{-1}\mathbf{X}^\top \mathbf{y}$ \quad ($\beta_0$ unpenalised).
    \item[Lasso ($\ell_1$)] $\min_\beta\ \mathrm{RSS}+\lambda\sum_{j=1}^p|\beta_j|$ \quad (shrinks and selects: some $\hat\beta_j=0$).
    \item[\quad soft-threshold] $\hat\beta_j^{\,\mathrm L} = \mathrm{sign}(\hat\beta_j^{\,\mathrm{OLS}})\bigl(|\hat\beta_j^{\,\mathrm{OLS}}| - \lambda/2\bigr)_{+}$ \quad (orthonormal $\mathbf{X}^\top\mathbf{X}=I$ only; no general closed form).
...
\begin{frame}{Vocabulary checklist}
  ...
  \begin{tabular}{ll}
    ...
    One-standard-error rule & Largest $\lambda$ within one SE of the CV minimum \\
...
\begin{frame}{Decision rules of thumb}
  \begin{itemize}
    \item Always standardise predictors before ridge or lasso.
    ...
    \item Use BIC for parsimonious model selection; AIC / $C_p$ for
          predictive accuracy.
    \item With strongly collinear predictors prefer ridge (or elastic
          net): the lasso's chosen \emph{set} is the unstable part.
...
\begin{frame}{Common pitfalls}
  \begin{alertblock}{Don't}
    \begin{enumerate}
      ...
      \item Treat post-selection $p$-values as valid (module A7).
      \item Use AIC / BIC once $p$ approaches $n$ --- they assume
            $n \gg p$.
      \item Tune $\lambda$ on the same data you evaluate on.
      \item Standardise on the full data before cross-validating ---
            the scaler must be re-fit inside every fold.
      \item Read the lasso's dropped variables as ``irrelevant'' when
            they are merely redundant with a kept one.
    \end{enumerate}
  \end{alertblock}
\end{frame}
...
\begin{frame}{Connections to the rest of the course}
  \begin{itemize}
    ...
    \item Ch.~8: trees and boosting tame many correlated predictors in a
          quite different way.
    \item Module A9: deep nets use weight decay (ridge in disguise).
    \item Module A7: post-selection inference and the lasso.
  \end{itemize}
\end{frame}
...
\begin{frame}{Self-check questions}
  ...
  \begin{enumerate}
    \item Why does ridge regression always have a unique solution?
          (p.~\pageref{frm:ridgeobj})
    \item Why can lasso set coefficients to exactly zero, but ridge
          cannot? Sketch the unit balls. (p.~\pageref{frm:lassogeom})
    \item For two perfectly correlated predictors, what does ridge do?
          What does lasso do? What does elastic net do? (p.~\pageref{frm:elastic})
    \item Compute the ridge estimate when $\mathbf{X}^\top\mathbf{X} = I$
          --- a clean special case. (appendix, p.~\pageref{frm:ee62})
    \item Why must the scaler be re-fit inside every cross-validation
          fold rather than once on the full data? (p.~\pageref{frm:labridge})
    \item Outline the one-standard-error rule. (p.~\pageref{frm:cvlambda})
  \end{enumerate}
\end{frame}
...
\begin{frame}{Seven things to remember}
  \begin{enumerate}
    ...
    \item $C_p$, AIC, BIC and adjusted $R^2$ all read ``training fit
          $+$ complexity penalty''; CV needs no such assumption.
    \item Ridge: $\ell_2$ penalty $\to$ smooth shrinkage, no zeros.
    \item Lasso: $\ell_1$ penalty $\to$ sparse solutions.
    \item Always standardise before shrinkage --- inside the CV loop.
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}\itemsep2pt
      \item \textbf{Extended Exercise 6.1 --- all three criteria across model sizes} --- the full table, three solution slides. Exercise 6.3 in the main deck is the exam-shaped short version.
      \item \textbf{The constraint geometry, drawn a second time} --- a duplicate of the diamond-vs-circle picture that is already in the main deck.
      \item \textbf{Counting the models (Exercise 6.1)} --- how many subsets best subset and forward stepwise actually fit, $2^p$ against $1+p(p+1)/2$. The lattice picture in the main deck makes the same point in one slide.
      \item \textbf{Ridge and lasso under an orthonormal design (Extended Exercise 6.2)} --- the closed forms that explain \emph{why} the lasso thresholds coefficients to zero and ridge only shrinks them. The cleanest proof in the chapter --- and the hardest algebra.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 6.1 (2/3)}
  ...
  \end{solutionbox}
  \begin{mistake}
    Dividing by $n - d$ instead of $n - d - 1$ in the numerator --- the
    extra $-1$ is the intercept. At $d = 4$ the correct divisor is $95$,
    not $96$: $314/95 = 3.305$.
  \end{mistake}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 6.1 (3/3)}
  \begin{solutionbox}
    ...
    $\mathrm{BIC}\le C_p\le\mathrm{adj}\,R^2$ --- exactly the pattern of
    ISLP Figure~6.2 on the Credit data.
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.5\textwidth}\centering
      ...
      \begin{tikzpicture}[scale=0.8]
        ...
        \node[font=\scriptsize,align=center] at (1.95,-0.6) {both\\$\ne 0$};
      \end{tikzpicture}
    \end{column}
  \end{columns}
  \begin{takeaway}RSS contours first touch the $\ell_1$ diamond at a \emph{corner} ($\hat\beta_j=0$); the smooth $\ell_2$ circle almost never does. \hfill{\tiny\itshape\color{gray}ISLP Figure 6.7, redrawn}\end{takeaway}
\end{frame}
...
\begin{frame}{Exercise 6.1 --- Counting Models in Subset Selection [Math]}
  ...
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    Switch to \texttt{chapter\_06\_lab.ipynb} \S5 and run the counting cell: it prints
    $2^{19} = 524\,288$ best-subset fits against $191$ for forward stepwise on Hitters,
    plus this slide's $p=10$ case ($1\,024$ vs.\ $56$).
  \end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 6.1 (1/3)}
  \begin{solutionbox}
    \textbf{(1) Best subset.}
...
\begin{frame}{Solution --- Exercise 6.1 (2/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 6.1 (3/3)}
  \begin{solutionbox}
    \textbf{(3) The gap.}
  ...
  \end{solutionbox}
  \begin{mistake}
    Dropping the null model from either count --- both include it
    ($2^{10} = 1024$ subsets \emph{including} $\varnothing$; $1 + 55 = 56$
    stepwise fits, the leading $1$ being $M_0$).
  \end{mistake}
\end{frame}

\begin{frame}{Extended Exercise 6.2 --- Orthonormal Design: Ridge vs.\ Lasso [Math]}\label{frm:ee62}
  \begin{longexercise}
...
\begin{frame}{Solution --- Extended Exercise 6.2 (1/3)}
  \begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 6.2 (2/3)}
  \begin{solutionbox}
    \textbf{Why the clamp at zero?} The penalty $\lambda|\beta_j|$ has a
    \emph{kink} at $\beta_j=0$. Away from zero calculus applies and gives
    $\beta_j=\hat\beta_j^{\mathrm{OLS}}\mp\lambda/2$; but when
    $|\hat\beta_j^{\mathrm{OLS}}|\le\lambda/2$ each branch overshoots past
    zero and contradicts its own sign assumption, so the minimum sits
    \emph{at the kink}: $\hat\beta_j^{\mathrm L}=0$ exactly. A whole
    \emph{interval} of OLS values, $[-\lambda/2,\lambda/2]$, maps to zero
    --- that is why lasso solutions are sparse, while ridge's smooth
    penalty maps only the single point $0$ to $0$.
    (These are ISLP equations (6.14)--(6.15).)

    \smallskip
    \textbf{(3) Apply with $\lambda=1$} (ridge factor
    $1/(1+\lambda)=0.5$, lasso threshold $\lambda/2=0.5$) --- next slide.
  \end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 6.2 (3/3)}
  \begin{solutionbox}
    \[
      \begin{array}{c|cccc}
  ...
  \end{solutionbox}
  \begin{mistake}
    Thinking the lasso shrinks proportionally. It subtracts the same
    \emph{absolute} amount from every coefficient; ridge is the
    proportional one --- compare the $3.0$ and $0.4$ rows.
  \end{mistake}
\end{frame}
