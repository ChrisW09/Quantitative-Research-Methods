...
\begin{frame}{Three families of solutions}
  ...
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    ...
    collinear career totals) is the testbed for all three families today.
  \end{labnote}
\end{frame}
...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.9cm}p{4.9cm}p{4.4cm}@{}}
    ...
    \midrule
    Banking &
      Attribute shortlisting for a credit \mbox{scorecard} &
      Which attributes ship on the scorecard \\
    Pharma, food, chemicals &
      PLS on near-infrared spectra (hundreds of wavelengths) &
      Release the batch, or send it to the lab \\
    Manufacturing, energy &
      Ridge on collinear sensor and telemetry channels &
      Which process settings to trust \\
    Marketing &
      Mix model where channel spends move together &
      A defensible budget reallocation \\
    Health, biotech &
      Lasso screening of gene-expression features &
      Which features get a follow-up study \\
    Quantitative finance &
      Sparse selection among candidate return predictors &
      Which signals get capital \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{industry}[The common thread]
    Wide, correlated data plus a business need for a model small enough to
    explain --- that is what regularisation delivers.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: near-infrared spectroscopy and PLS}
  \footnotesize
  \begin{industry}[The setting]
    A plant needs the active-ingredient concentration of a blend, and the
    reference method is a slow destructive lab assay.
    \textbf{Response:} the reference value (\% w/w).
    \textbf{Predictors:} absorbance at several hundred NIR wavelengths,
    adjacent ones nearly identical, on calibration sets of only tens of
    samples ($p \gg n$). \textbf{Output:} a PLS fit on a few components
    ($M$ by CV), scored by root mean squared error of prediction.
  \end{industry}
  \begin{readme}[Which decision it feeds and who signs it off]
    The calibration replaces routine lab testing: it feeds in-process
    control and real-time release of a batch. QA and regulatory affairs
    own the sign-off, because it forms part of a \emph{validated}
    analytical method. The predicted concentration itself reaches a QA
    release manager or production supervisor, who must release the batch
    --- or hold it for the reference assay --- on the strength of it.
  \end{readme}
  \begin{takeaway}[The honest caveat]
    Calibrations drift --- a new supplier or instrument moves the spectra,
    so the model must be revalidated. And PLS mixes \emph{all}
    wavelengths: it predicts without pointing at a cause.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Forward stepwise selection}
  ...
  \begin{readme}
    ...
    subset.
  \end{readme}
  \begin{industry}[Banking: from a thousand candidate attributes to twelve]
    Bureau files offer hundreds of candidate attributes; the scorecard that
    ships has perhaps 10--15. Parsimony here is a monitoring and governance
    requirement, not merely statistical taste.
  \end{industry}
\end{frame}
...
\begin{frame}{Why ridge: numerical stability via a penalty}
  ...
  \end{readme}
  \begin{industry}[Manufacturing and energy: collinear sensors]
    Process telemetry is full of channels that move together. Plain OLS
    then returns huge, sign-flipping coefficients that move with every
    data refresh; ridge returns a fit an engineer can act on.
  \end{industry}
\end{frame}
...
\begin{frame}{Choose $\lambda$ by cross-validation}
  ...
  \end{readme}
  \begin{industry}[Retail analytics: wide SKU and customer tables]
    With hundreds of engineered columns, $\lambda$ --- not the
    coefficient list --- is what you defend.
  \end{industry}
\end{frame}
...
\begin{frame}{Ridge vs.\ lasso vs.\ elastic net: when each wins}
  ...
  \end{takeaway}
  \begin{industry}[Quantitative finance: is the selected set stable?]
    With many candidate predictors and a tiny signal-to-noise ratio,
    shrinkage beats unconstrained least squares. But the lasso's chosen
    \emph{set} is unstable under correlation --- so practitioners re-run
    it over bootstrap samples and only believe a variable that survives
    most of them.
  \end{industry}
\end{frame}
...
\begin{frame}{Principal components regression (PCR)}
  ...
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    Switch to \texttt{chapter\_06\_lab.ipynb} \S5 and run the PCR/PLS cell: both CV
    curves bottom out near an MSE of $1.15\times10^5$, but PLS is close to its best
    by $M \approx 8$ components while PCR needs $M \gtrsim 16$.
  \end{labnote}
\end{frame}
...
\begin{frame}{The high-dimensional regime}
  ...
  \end{takeaway}
  \begin{industry}[Biotech: biomarker screening as triage]
    Biomarker studies have thousands of features on a few hundred
    subjects; the lasso's job there is \emph{triage}.
  \end{industry}
\end{frame}
...
\begin{frame}{Lasso in high dimensions}
  ...
  \end{center}
  Lasso keeps working when $p \gg n$ if the truth is sparse \emph{and} the
  sample can see it: with $n = 100$ and $20$ true predictors, by $p = 2{,}000$
  (right) no amount of shrinkage recovers the fit.
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_06\_lab.ipynb}]
    ...
    \begin{itemize}
      ...
      \item \S5, \emph{PCR and PLS};
      \item \S6, \emph{How many models did we skip?} --- the search space, counted.
    \end{itemize}
...
# 50 log-spaced alphas, 1e-3 to 10: from near-OLS to the all-zero model
...
\begin{frame}[fragile]{Principal components regression}
  \begin{lstlisting}
from sklearn.decomposition import PCA  # max-variance directions of X
from sklearn.linear_model  import LinearRegression  # unpenalised fit
from sklearn.pipeline      import Pipeline  # PCA and OLS as one model
...
# M, not lambda, is the knob: M=19 reproduces OLS, small M shrinks hard.
  \end{lstlisting}
  \begin{readme}
    PCA never sees \texttt{y}: nothing makes the top components predict it.
  \end{readme}
\end{frame}
...
\begin{frame}[fragile]{Exercise 6.7 --- CV Ridge and Lasso on Hitters [Python]}
  ...
  \begin{labnote}[{{Run this live}}]
    ...
    \emph{Exercise 6.7 --- CV Ridge and Lasso on Hitters [Python]}.
  \end{labnote}
\end{frame}
...
\begin{frame}{Self-check questions}
  ...
  \begin{enumerate}
    ...
    \item Why can lasso set coefficients to exactly zero, but ridge
          cannot? Sketch the unit balls. (p.~45)
    \item For two perfectly correlated predictors, what does ridge do?
          What does lasso do? What does elastic net do? (p.~47)
    \item Compute the ridge estimate when $\mathbf{X}^\top\mathbf{X} = I$
          --- a clean special case. (appendix, p.~88)
    \item PCR ignores $Y$ when forming components. When can this hurt
          and when does it help? (p.~55)
    \item Outline the one-standard-error rule. (p.~46)
  \end{enumerate}
\end{frame}
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}\itemsep2pt
      ...
      \item \textbf{Partial least squares} --- PLS as the supervised cousin of PCR, with Exercise 6.6. PCR is the version that appears in the lab and the exam.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Exercise 6.1 --- Counting Models in Subset Selection [Math]}
  ...
  \begin{labnote}[{{Live demo --- \S6 of the notebook}}]
    Switch to \texttt{chapter\_06\_lab.ipynb} \S6 and run the counting cell: it prints
...
\begin{frame}{Solution --- Exercise 6.6}
  ...
  \end{solutionbox}
  \begin{mistake}
    Concluding supervised must be better --- using $Y$ also raises
    variance, so PCR often matches PLS. And neither selects variables:
    every component $Z_m$ mixes \emph{all} $p$ predictors.
  \end{mistake}
\end{frame}
