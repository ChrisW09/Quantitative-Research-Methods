...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.9cm}p{4.9cm}p{4.4cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{Which decision it drives} \\
    \midrule
    Banking &
      Stepwise shortlisting of bureau attributes for a credit
      \mbox{scorecard} &
      Which ten to fifteen attributes ship on the scorecard \\
    Marketing &
      Ridge on a mix model whose channel spends move almost in lockstep &
      How much budget moves from one channel to another next quarter \\
    Quantitative finance &
      Lasso over a long list of candidate return predictors &
      Which signals get capital, and which are switched off \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{takeaway}[All three ship a shortlist that someone has to defend]
    The deliverable is never the fit --- it is a small set of predictors a
    committee will challenge. Subset selection reaches it by searching,
    ridge and lasso by taxing; in each case the penalty (or the criterion)
    is the part you must justify.
  \end{takeaway}
\end{frame}
...
\begin{frame}[fragile]{Solution --- Exercise 6.6 (1/2)}
  \begin{lstlisting}
...
\begin{frame}[fragile]{Solution --- Exercise 6.6 (2/2)}
  \begin{lstlisting}
...
\begin{frame}{Self-check questions}
  ...
  \begin{enumerate}
    ...
    \item Why can lasso set coefficients to exactly zero, but ridge
          cannot? Sketch the unit balls. (p.~41)
    \item For two perfectly correlated predictors, what does ridge do?
          What does lasso do? What does elastic net do? (p.~43)
    \item Compute the ridge estimate when $\mathbf{X}^\top\mathbf{X} = I$
          --- a clean special case. (appendix, p.~81)
    \item Why must the scaler be re-fit inside every cross-validation
          fold rather than once on the full data? (p.~52)
    \item Outline the one-standard-error rule. (p.~42)
  \end{enumerate}
\end{frame}
