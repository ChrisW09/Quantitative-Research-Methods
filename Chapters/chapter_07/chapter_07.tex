...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{Ch.\ 7} & \textbf{Moving beyond linearity}\,{\color{accent!75!black}--- splines, GAMs} \\
...
\begin{frame}{Learning objectives for this chapter}
  ...
  \begin{readme}[{{How to use this deck --- self-study and revision}}]
    ...
    \texttt{chapter\_07\_lab.ipynb} alongside the lab section. Revise from the
...
\begin{frame}{Three reasons nonlinearity matters}
  \begin{block}{Concrete examples}
    \begin{itemize}
      \item Wage data peaks in middle age and then drops --- not linear.
  ...
  \begin{labnote}[{{Live demo --- \S1 of the notebook}}]
    Switch to \texttt{chapter\_07\_lab.ipynb} \S1 and run the two polynomial cells: a raw-power degree-4 OLS on Wage ($R^2\approx0.086$) whose grid plot shows exactly that middle-age wage hump.
  \end{labnote}
\end{frame}
...
\begin{frame}{Step functions: piecewise constant}
  ...
  \begin{readme}[Pros and cons]
    ...
    exist (age groups, salary tiers), or when banding is chosen
    deliberately for auditability --- at the price of a visible jump for
    two cases either side of a cut-off.
  \end{readme}
\end{frame}
...
\begin{frame}{Solution --- Exercise 7.1 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 7.1 (2/4)}
\begin{solutionbox}
\medskip
...
\begin{frame}{Solution --- Exercise 7.1 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 7.1 (4/4)}
\begin{solutionbox}
\medskip
...
\begin{labnote}[{{Live demo --- \S2 of the notebook}}]
  Switch to \texttt{chapter\_07\_lab.ipynb} \S2 and run the binning cell: four age bins give mean wages $94.2 \to 118.2 \to 117.8 \to 101.8$ --- the hump no straight line can produce, at the price of a jump at age $33.5$.
\end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 7.2 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 7.2 (2/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 7.2 (3/3)}
\begin{solutionbox}
\medskip
...
\end{solutionbox}
\begin{mistake}
Reading $\beta_2=25$ as the jump from bin $2$ to bin $3$. Both coefficients compare to the \emph{reference} bin; the bin-$2$-to-bin-$3$ step is $\beta_2-\beta_1=10$.
\end{mistake}
\end{frame}
...
\begin{frame}{Regression splines: idea}
  ...
  \begin{labnote}[{{Live demo --- \S3 of the notebook}}]
    Switch to \texttt{chapter\_07\_lab.ipynb} \S3 and run the \texttt{SplineTransformer} cell: a cubic B-spline basis with $6$ knots, fitted by plain least squares --- the curve bends like the polynomial but stays smooth with no jumps.
  \end{labnote}
  \par{\tiny\itshape\color{gray}History: a ``spline'' was originally a
  thin flexible strip draughtsmen bent around pins to draw smooth ship hulls;
  Isaac Schoenberg (1946) developed the mathematical version.}
\end{frame}
...
\begin{frame}{Why the continuity constraints? Broken vs.\ smooth joins}
  ...
  \begin{tikzpicture}[scale=0.62,>=Stealth,font=\small]
    ...
    % ---------- LEFT: no constraints ----------
    \begin{scope}
      \node[font=\bfseries] at (3,4.9) {No constraints};
      \draw[->,thick] (-0.2,0)--(6.6,0) node[right]{$x$};
      \draw[->,thick] (0,-0.2)--(0,4.3);
      \draw[gray,dashed] (3,-0.1)--(3,3.85) node[above,black,font=\footnotesize]{$\xi$};
    ...
    \begin{scope}[xshift=8.4cm]
      \node[font=\bfseries] at (3,4.9) {Cubic-spline constraints};
      \draw[->,thick] (-0.2,0)--(6.6,0) node[right]{$x$};
      \draw[->,thick] (0,-0.2)--(0,4.3);
      \draw[gray,dashed] (3,-0.1)--(3,3.85) node[above,black,font=\footnotesize]{$\xi$};
      \draw[cL,very thick,domain=0.2:3,smooth,samples=50] plot(\x,{0.8+0.7*\x-0.06*\x*\x});
...
\begin{frame}{How a cubic spline is built: continuity at the knots}
  ...
  \begin{tikzpicture}[scale=0.55,>=Stealth]
    ...
    \node[font=\footnotesize] at (5,-1.45) {three knots = piece boundaries};
...
\begin{frame}{Choosing knots and degrees of freedom}\label{frm:knots}
  ...
  \end{readme}
  \begin{takeaway}
    The knot count is the real tuning parameter: more knots track every
    wiggle in the data, fewer give a smooth curve that misses some of it.
    Choose it by cross-validation, not by eye.
  \end{takeaway}
\end{frame}
...
\begin{frame}{How do we actually fit one? The truncated-power basis}
  \footnotesize
  A cubic spline is just OLS on the right columns. Start from a cubic, then
  add \emph{one} extra term per knot:
  \begin{block}{The basis, with $K$ knots $\xi_1 < \cdots < \xi_K$}
    \[
      1,\; x,\; x^2,\; x^3,\;
      (x-\xi_1)^3_+,\; \ldots,\; (x-\xi_K)^3_+,
      \qquad
      (x-\xi)^3_+ =
      \begin{cases}
        (x-\xi)^3 & x > \xi,\\
    ...
    \]
  \end{block}
  \begin{takeaway}[{{Why this works, and what it costs}}]
    Each truncated term is \emph{zero left of its knot}, so it switches on
    exactly where its piece begins --- the locality a global polynomial can
    never have. It changes only the \emph{third} derivative at $\xi$, so
    value, slope and curvature stay continuous automatically. Count the
    columns: $4 + K$ coefficients, i.e.\ $K+4$ degrees of freedom, matching
    the count you are about to derive.
  \end{takeaway}
  \islpsource{Section 7.4.3, equations (7.9)--(7.10)}
\end{frame}

\begin{frame}{The truncated-power basis, drawn}
  \begin{center}
    \includegraphics[width=0.94\textwidth,height=0.52\textheight,keepaspectratio]{ch07_spline_basis.png}
  \end{center}
  \begin{takeaway}
    The same basis, plotted: four global polynomial terms plus one truncated
    cubic per knot ($K=3$ knots at the Wage age quartiles) $=$ $K+4=7$ basis
    functions. Note where each orange curve leaves the axis --- that is its
    knot.
  \end{takeaway}
\end{frame}
...
\begin{labnote}[{{Live demo --- \S6 of the notebook}}]
  Switch to \texttt{chapter\_07\_lab.ipynb} \S6 and run the basis-building cell: it constructs the truncated-power basis on Wage ($K=3$ knots at the age quartiles) and confirms the design matrix has rank $7=K+4$ --- the count you are about to derive.
\end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 7.3 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 7.3 (2/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 7.3 (3/3)}
\begin{solutionbox}
\medskip
...
% by ~24pt (the solutionbox is already full).
...
\begin{frame}{Solution --- Exercise 7.4 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 7.4 (2/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 7.4 (3/3)}
\begin{solutionbox}
\medskip
...
\end{solutionbox}
\begin{mistake}
Thinking the natural spline is linear \emph{between} the boundary knots too --- inside the boundary knots it is every bit as flexible a cubic spline; only the two tails are straightened.
\end{mistake}
\end{frame}
...
\begin{frame}{Smoothing splines: let the data choose}\label{frm:smoothobj}
  ...
  \end{block}
  \begin{takeaway}
    Two terms pull opposite ways: the RSS wants a curve through every
    point, the penalty wants a straight line. $\lambda$ sets the exchange
    rate --- and over-smoothing flattens exactly the peaks that matter most.
  \end{takeaway}
\end{frame}

\begin{frame}{Reading the smoothing-spline objective}\label{frm:smoothread}
  \begin{readme}
    \begin{tabular}{@{}cl@{}}
      \toprule
  ...
  \begin{takeaway}[Three observations]
    \begin{itemize}
      ...
      \item The fit is linear in $y$ --- $\hat g = S_\lambda y$ --- so
            smoothness is reported as the \textbf{effective degrees of
            freedom} $\mathrm{df}_\lambda = \operatorname{trace}(S_\lambda)$,
            running from $n$ (interpolation) down to $2$ (a straight line).
            Quote $\mathrm{df}_\lambda$, not $\lambda$: it is comparable
            across data sets.
    \end{itemize}
  \end{takeaway}
\end{frame}
...
\begin{frame}{Smoothing spline flexibility: $\lambda$ is the dial}
  \begin{center}\includegraphics[width=0.88\textwidth,height=0.50\textheight,keepaspectratio]{ch07_x_smooth_spline.png}\end{center}
...
\begin{frame}{Local regression (LOESS): a different philosophy}\label{frm:loess}
...
\begin{frame}{Solution --- Exercise 7.5 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 7.5 (2/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 7.5 (3/3)}
\begin{solutionbox}
\medskip
...
\end{solutionbox}
\begin{mistake}
Picking $\lambda$ by minimising \emph{training} RSS --- that always selects $\lambda=0$ (interpolation). Only out-of-sample criteria can trade fit against roughness.
\end{mistake}
\end{frame}
...
\begin{frame}{From one predictor to many: GAMs}\label{frm:gam}
  ...
  \end{readme}
  \par{\tiny\itshape\color{gray}History: GAMs were introduced by Trevor
  Hastie and Robert Tibshirani (1986) --- two of the authors of our
  textbook.}
\end{frame}

\begin{frame}{Why GAMs are so useful}\label{frm:gamwhy}
...
\begin{frame}{GAM component functions fitted on Wage}\label{frm:gamwage}
  \begin{center}
    \includegraphics[width=0.9\textwidth,height=0.53\textheight,keepaspectratio]{ch07_gam_components.png}
  \end{center}
...
\begin{frame}[fragile]{Solution --- Extended Exercise 7.2 (1/4)}
\begin{solutionbox}
...
W['edu'] = W['education'].astype('category').cat.codes  # levels -> 0..4
X = W[['age', 'year', 'edu']].values  # cols 0, 1, 2 in that order
y = W['wage'].values                  # response
gam = LinearGAM(s(0) + l(1) + f(2)).fit(X, y)  # smooth, linear, factor
gam.summary()   # EDF per term: age ~15, year ~1, edu 4
\end{lstlisting}
...
\begin{frame}{Solution --- Extended Exercise 7.2 (2/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 7.2 (3/4)}
\begin{solutionbox}
\textbf{(c) Non-linearity \& EDF.} Only \texttt{age} is genuinely non-linear: its smooth spends many effective degrees of freedom ($\approx15$ at pyGAM's default penalty, dropping to $\approx10$ once $\lambda$ is tuned by \texttt{gam.gridsearch}). \texttt{year} enters with EDF $\approx1$ (essentially a straight line); \texttt{education} as a factor contributes df $=$ (levels $-1$) $=4$. The EDF is how a GAM quantifies ``how wiggly'' each term is.
\end{solutionbox}
\begin{mistake}
Reading a partial-effect curve as an absolute wage level. Each panel is centred around zero; only its \emph{shape} and within-panel differences are meaningful, with all other predictors held fixed.
\end{mistake}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 7.2 (4/4)}
\begin{solutionbox}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_07\_lab.ipynb}]
    ...
    \begin{itemize}
      \item \S1, \emph{Polynomial regression}, and \S2, \emph{Step functions};
      \item \S3, \emph{Regression splines}, and \S4, \emph{LOESS};
      \item \S5, \emph{GAM}, and \S6, \emph{Where the degrees of freedom come from}.
    \end{itemize}
    Data loads via the \texttt{ISLP} package or the bundled CSV files; \S7, \emph{Exercises}, closes the notebook.
  \end{labnote}
\end{frame}
...
Wage = load_data("Wage")  # 3000 workers: wage, age, year
design = MS([poly("age", degree=4)]).fit_transform(Wage)  # 4 age cols
res = sm.OLS(Wage["wage"], design).fit()  # plain OLS on that design
print(res.summary())  # one coefficient per basis column
  \end{lstlisting}
  \begin{readme}[What the design matrix actually holds]
    \texttt{fit\_transform} is what builds the columns OLS sees: an
    intercept plus \emph{four} columns for \texttt{age}. \texttt{poly}
    returns \emph{orthogonal} polynomials --- rotated combinations of
    $\texttt{age},\texttt{age}^2,\texttt{age}^3,\texttt{age}^4$ that span
    the same space, so the fitted curve and $R^2\approx0.086$ match raw
    powers exactly, but the columns are uncorrelated. Judge the term by
    the fitted curve, never by one column's $t$-statistic.
  \end{readme}
\end{frame}
...
design  = MS([bs("age", df=6)]).fit_transform(Wage)  # 6 cubic cols
design2 = MS([ns("age", df=4)]).fit_transform(Wage)  # 4 cols, linear tails
res = sm.OLS(Wage["wage"], design2).fit()  # still ordinary least squares
print(res.summary())  # one weight per basis function
  \end{lstlisting}
  \begin{readme}[\texttt{df} and knots are two ways to say the same thing]
    Each call returns a matrix whose columns are the basis functions
    $b_j(\texttt{age})$ of the slide before. \texttt{df} says \emph{how
    many columns}; the knots are then placed for you at quantiles of
    \texttt{age}. Naming the knots instead gives the identical fit,
    because cubic-spline df $=K+4$ and natural-spline df $=K$ --- which
    is why \texttt{ns} at the same df, spending nothing on the tails,
    can afford more knots where the data actually are.
  \end{readme}
\end{frame}
...
l = nps.lowess(endog=Wage["wage"], exog=Wage["age"],  # y first, then x
               frac=0.2, return_sorted=True)  # span: 20% of the points
import matplotlib.pyplot as plt
plt.scatter(Wage["age"], Wage["wage"], s=4, alpha=0.3)  # raw data
plt.plot(l[:, 0], l[:, 1], color="orange")  # col 0 = age, col 1 = fit
plt.show()
  \end{lstlisting}
  \begin{readme}[No coefficients come back --- just the curve]
    One $(x,\hat y)$ row per point, sorted by $x$ so it can be plotted.
  \end{readme}
  \begin{labnote}[{{Live demo --- \S4 of the notebook}}]
    Switch to \texttt{chapter\_07\_lab.ipynb} \S4 and run the \texttt{lowess} cell: with span $0.2$ each local fit uses $20\%$ of the $3\,000$ workers, and the curve traces the same rise--plateau--fall as the splines.
  \end{labnote}
\end{frame}
...
X = Wage[["year", "age"]].values  # col 0 = year, col 1 = age
y = Wage["wage"].values           # response
gam = LinearGAM(s(0) + s(1)).fit(X, y)  # s(j): smooth of column j
gam.summary()  # EDF per term: year ~7, age ~14
  \end{lstlisting}
  \begin{readme}[Reading the EDF column]
    \texttt{s(0)} and \texttt{s(1)} index \emph{columns} of \texttt{X}.
    A term's effective df says how much flexibility the penalty let it
    use: $1$ is a straight line, year~$\approx7$ a gentle drift,
    age~$\approx14$ the pronounced hump.
  \end{readme}
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    Switch to \texttt{chapter\_07\_lab.ipynb} \S5 and run the two GAM cells: the summary reports $\approx21.2$ total effective df (age's smooth alone spends $\approx14$), and the partial-dependence panels show the age hump next to the gentle year drift.
  \end{labnote}
\end{frame}
...
\begin{labnote}[{{Run this live}}]
  The worked solution sits in \texttt{chapter\_07\_lab.ipynb} under \emph{Lecture exercises --- worked Python solutions}: the cell headed \emph{Exercise 7.6 --- Natural spline / GAM on Wage [Python]}.
\end{labnote}
\end{frame}
...
fit = smf.ols('wage ~ cr(age, df=5)', data=Wage).fit()  # 5 ns columns
print(fit.summary())  # read the joint F, not single t's
\end{lstlisting}
\emph{What the output shows:} $6$ coefficients (intercept $+$ $5$ basis columns), $R^2\approx0.087$, and an overall $F$-test with $p\approx6\times10^{-58}$ --- the spline terms are jointly highly significant, even though \texttt{age} alone explains only a modest share of wage variance. \texttt{df=5} is the \emph{entire} complexity choice: it fixes how many basis columns are built, and the knots then go at quantiles of \texttt{age}.
\end{solutionbox}
\end{frame}
...
X = Wage[['age', 'year']].values  # col 0 = age, col 1 = year
y = Wage['wage'].values           # response
gam = LinearGAM(s(0) + s(1)).fit(X, y)  # a penalised spline per column
gam.summary()  # EDF per term: how wiggly each one got
\end{lstlisting}
\emph{What the output shows:} with pyGAM's default penalty the \texttt{age} smooth uses $\approx15$ EDF and \texttt{year} $\approx6$ --- both flexible; tuning $\lambda$ by \texttt{gam.gridsearch(X, y)} shrinks the smooths toward simpler shapes. Note the contrast with (a): there \emph{you} fixed df $=5$; here $\lambda$ decides, and the EDF merely \emph{reports} the flexibility it granted.
\end{solutionbox}
\end{frame}
...
\end{solutionbox}
\begin{mistake}
Interpreting single spline-basis coefficients (or their $t$-tests) as ``the effect of age''. Judge the term by its fitted curve and a \emph{joint} test of all its basis columns.
\end{mistake}
\end{frame}
...
\end{longexercise}
\begin{labnote}[{{Run this live}}]
The worked solution sits in \texttt{chapter\_07\_lab.ipynb} under \emph{Lecture exercises --- worked Python solutions}: the cell headed \emph{Extended Exercise 7.3 --- Three fits for wage vs.\ age [Python]} --- the nested ANOVA, the three fits and the overlay plot, ready to run.
\end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution --- Extended Exercise 7.3 (1/5)}
\begin{solutionbox}
...
W = pd.read_csv('../ALL CSV FILES - 2nd Edition/Wage.csv')  # load Wage
terms = lambda d: '+'.join(f'np.power(age,{k})' for k in range(1, d+1))  # age..age^d
models = [smf.ols('wage ~ '+terms(d), data=W).fit() for d in range(1, 6)]  # nested: +1 term each
print(anova_lm(*models)[['df_diff', 'F', 'Pr(>F)']].round(4))  # row d: is degree d+1 worth it?
\end{lstlisting}
...
\begin{frame}[fragile]{Solution --- Extended Exercise 7.3 (2/5)}
\begin{solutionbox}
\textbf{Step function, natural spline, and the overlay.}
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
import matplotlib.pyplot as plt
poly = smf.ols('wage ~ '+terms(3), data=W).fit()  # the degree chosen above
W['bin'] = pd.cut(W['age'], [17,25,35,45,55,65,81])  # 6 bins -> 5 dummies
step = smf.ols('wage ~ C(bin)', data=W).fit()  # a constant inside each bin
ns   = smf.ols('wage ~ cr(age, df=5)', data=W).fit()   # 5 cols, linear tails

g = pd.DataFrame({'age': np.arange(18, 81)})  # one row per age to plot
...
for m, lab in [(poly,'poly (d=3)'), (step,'step'), (ns,'natural spline')]:
    plt.plot(g['age'], m.predict(g), label=lab)  # three curves, one grid
plt.legend(); plt.xlabel('age'); plt.ylabel('wage'); plt.show()
\end{lstlisting}
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 7.3 (3/5)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 7.3 (4/5)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 7.3 (5/5)}
\begin{solutionbox}
  \begin{itemize}
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{description}
    ...
    \item[Cubic spline] truncated-power basis $1,x,x^2,x^3,(x-\xi_k)_+^3$; \ $K$ knots $\Rightarrow$ df $=K+4$.
    ...
    \item[Effective df] $\mathrm{df}_\lambda=\operatorname{trace}(S_\lambda)$, where $\hat g=S_\lambda y$; \ larger $\lambda$ $\Rightarrow$ \emph{smaller} $\mathrm{df}_\lambda$: from $n$ (at $\lambda=0$, interpolation) down to $2$ (as $\lambda\to\infty$, a straight line).
    \item[GAM] $y_i=\beta_0+f_1(x_{i1})+f_2(x_{i2})+\dots+f_p(x_{ip})+\varepsilon_i$.
    \item[Backfitting] re-estimate each $f_j$ in turn on the partial residual $y-\beta_0-\sum_{k\neq j}f_k(X_k)$, cycling through the terms until convergence.
  \end{description}
\end{frame}
...
\begin{frame}{Connections to the rest of the course}
  \begin{itemize}
    ...
    \item Module A9's deep nets generalise GAMs by learning the basis.
  \end{itemize}
\end{frame}
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{Wage vs.\ age: four models side by side} --- a second illustration of the cubic-vs-natural contrast already made in the main deck.
      \item \textbf{How the spline basis is built} --- one truncated-power term $(x-\xi)_+^3$ drawn on its own, and the constraint counting that turns loose cubic pieces into one smooth curve. The main deck already states the basis and the degrees-of-freedom rule; this is the same argument taken slowly.
...
\begin{frame}{The building block: one truncated-power term per knot}
  \begin{columns}[T]
    ...
    \begin{tikzpicture}[scale=0.86,>=Stealth,font=\small]
      ...
      \node[cP,font=\scriptsize] at (3.0,2.55) {cubic rise};
    \end{tikzpicture}
    \end{center}
    \end{column}
...
\begin{frame}{Solution --- Extended Exercise 7.1 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 7.1 (2/4)}
\begin{solutionbox}
\textbf{(b) Evaluate $\hat g(50)$.} Here the quadratic and cubic \emph{base} coefficients happen to be zero, so only the linear base term and the \emph{active} truncated terms contribute.

\medskip
\textbf{The general rule:} before any arithmetic, compare $x$ with each knot --- a truncated term is switched \emph{on} only where $x>\xi_k$:
\begin{center}\footnotesize
\begin{tabular}{lccc}
\toprule
Term & $(x-25)^3_+$ & $(x-40)^3_+$ & $(x-60)^3_+$ \\
\midrule
At $x=50$ & on ($50>25$) & on ($50>40$) & \textbf{off} ($50\le 60$) \\
\bottomrule
\end{tabular}
\end{center}
So $\hat g(50)$ is a sum of the base terms plus \emph{two} truncated contributions --- the third knot has not been reached yet. This bookkeeping step is where the marks are lost; the arithmetic on the next slide is routine.
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 7.1 (3/4)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
Evaluating the third term as $(50-60)^3=(-10)^3=-1000$ instead of $0$. The ``$+$'' truncation zeroes the term for $x\le 60$; forgetting it would give the wrong value $71.05-0.30=70.75$.
\end{mistake}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 7.1 (4/4)}
\begin{solutionbox}
