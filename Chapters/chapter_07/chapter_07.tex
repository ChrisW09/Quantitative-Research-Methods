...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.5cm}p{5.5cm}p{4.7cm}@{}}
    \toprule
  ...
  \begin{industry}[The common thread]
    In every row the fitted \emph{curve} is the deliverable: it gets plotted, argued about and signed off. That rules out a black box --- but a straight line would be plainly wrong.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: pricing a motor insurance policy}
  \footnotesize
  \begin{industry}[The model behind the tariff]
    Insurers price in two pieces --- \textbf{frequency} (claims per policy-year, Poisson) and \textbf{severity} (cost per claim, gamma) --- and increasingly fit each as an additive model with smooth terms:
    \[ \log\mathbb{E}[\text{claims}]=\beta_0+f_1(\text{driver age})+f_2(\text{vehicle age})+f_3(\text{mileage})+\text{region}+\text{bonus--malus}. \]
  \end{industry}
  \begin{readme}[What the fitted age curve looks like]
    It is famously \emph{non-monotone}: risk is high for the youngest drivers, falls through middle age, then rises again for the oldest. No linear term in age can represent that; a smooth can, and prints as one readable curve --- the curve an underwriter shows a broker or a customer who asks why a premium moved.
  \end{readme}
  \begin{takeaway}[Who signs it off --- and one caveat]
    The curves become tariff tables reviewed by pricing and the actuarial function, so they must be smooth and defensible: that, not accuracy, is why a GAM beats a black box here. \emph{Caveat:} the tariff must also price rarely observed cells, where a high-degree polynomial would swing wildly --- and additivity means an age~$\times$~vehicle-power interaction has to be added deliberately.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Polynomial regression: the simplest extension}
  ...
  \end{readme}
  \begin{industry}[Retail and travel: price-response curves]
...
\begin{frame}{Step functions: piecewise constant}
  ...
  \end{readme}
  \begin{industry}[Banking: scorecard bands]
...
\begin{frame}{Choosing knots and degrees of freedom}
  ...
  \end{readme}
  \begin{industry}[Fixed income: how many knots in a yield curve?]
...
\begin{frame}{Smoothing splines: let the data choose}
  ...
  \begin{block}{Penalised RSS}
    ...
    \]
  \end{block}
  \begin{industry}[Utilities: the temperature-response curve]
...
\begin{frame}{Why GAMs are so useful}
  ...
  \end{alertblock}
  \begin{industry}[Insurance and utilities: why they reach for GAMs]
    Insurers, utilities and banks need a model that bends \emph{and} can be defended one predictor at a time: each $f_j$ is a picture an actuary, a regulator or a model-risk reviewer can hold against domain knowledge.
  \end{industry}
\end{frame}
