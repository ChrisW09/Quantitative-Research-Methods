...
X = Wage[["year", "age"]].values  # two predictors
y = Wage["wage"].values           # response
gam = LinearGAM(s(0) + s(1)).fit(X, y)  # smooth terms in year and age
...
\end{lstlisting}
\emph{What the output shows:} $6$ coefficients (intercept $+$ $5$ basis columns), $R^2\approx0.087$, and an overall $F$-test with $p\approx6\times10^{-58}$ --- the spline terms are jointly highly significant, even though \texttt{age} alone explains only a modest share of wage variance.
\end{solutionbox}
\end{frame}
