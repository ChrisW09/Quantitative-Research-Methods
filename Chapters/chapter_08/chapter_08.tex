...
\subtitle{Chapter 8: Tree-Based Methods}
...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{Ch.\ 8} & \textbf{Tree-based methods}\,{\color{accent!75!black}--- trees, forests, boosting} \\
    \bottomrule
  \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}{Notation in this chapter}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    $E$ & Error rate $1-\max_k \hat p_{mk}$: node impurity \\
...
\begin{frame}{Learning objectives for this chapter}
  ...
  \begin{readme}[{{How to use this deck --- self-study and revision}}]
    ...
    \texttt{chapter\_08\_lab.ipynb} alongside the lab section. Revise from the
...
\begin{frame}{Regression trees: idea}
  ...
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Switch to \texttt{chapter\_08\_lab.ipynb} \S2 and run the \texttt{plot\_tree} cell: a depth-4 regression tree on the Carseats data, with \texttt{ShelveLoc} and \texttt{Price} taking the top splits and a test MSE of $3.612$ printed by the next cell.
  \end{labnote}
  \par{\tiny\itshape\color{gray}History: this is CART --- Breiman,
  Friedman, Olshen \& Stone (1984), the book that made trees a serious
  statistical tool.}
\end{frame}
...
\begin{frame}{Recursive binary splitting: one step}\label{frm:greedy}
  \begin{block}{The greedy step}
    At each node:
    \begin{enumerate}
      \item For every predictor $X_j$ and every cutpoint $s$, compute
            the RSS (residual sum of squares) that would result from splitting on $X_j < s$.
...
\begin{frame}{Classification trees: splitting rules}\label{frm:gini}
  \footnotesize
  Same recursive splitting; predict the majority class in each leaf. RSS no
  longer applies, so we score a node by its \emph{impurity}:
  \begin{block}{Error rate, Gini and cross-entropy in node $m$}
    \[
      E = 1 - \max_k \hat p_{mk}, \quad
      G = \sum_k \hat p_{mk}(1 - \hat p_{mk}), \quad
      D = -\sum_k \hat p_{mk}\log \hat p_{mk}.
    \]
  \end{block}
  \begin{readme}[{{Reading the impurities, and scoring a split}}]
    $\hat p_{mk}$ is the fraction of node-$m$ points in class $k$; $E$ is the
    share the majority vote gets wrong. All three are \emph{small} when one
    class dominates ($\hat p_{mk}\!\approx\!1$) and \emph{large} at a 50/50
    mix. A \emph{split} is scored by the \textbf{size-weighted} average of its
    two children, $\frac{n_L}{n}Q_L + \frac{n_R}{n}Q_R$ --- a big impure child
    must count for more than a small one.
  \end{readme}
  \begin{takeaway}[Why not the error rate?]
    $E$ is blind to purity gains that do not flip the majority class, so it
    ties too often to grow a tree with. Split on Gini or cross-entropy; keep
    $E$ for pruning and for reporting accuracy. \hfill{\tiny\itshape\color{gray}ISLP \S8.1.2, (8.5)--(8.7)}
  \end{takeaway}
\end{frame}


\begin{frame}{The three impurity measures, drawn}
  \begin{center}
    \includegraphics[width=0.72\textwidth,height=0.52\textheight,keepaspectratio]{ch08_impurity_measures.png}
  \end{center}
  \begin{takeaway}
    Gini and cross-entropy are strictly concave, so any split that moves a
    node toward purity lowers them. Classification error is piecewise
    \emph{linear} --- flat to purity gains that do not flip the majority ---
    which is exactly what Exercise 8.2 exploits.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Strengths and weaknesses of single trees}
  ...
  \end{columns}
  \begin{takeaway}
    A small pruned tree survives precisely because it is weak but
    \emph{printable} --- a rule set you can read off one page. Grow it
    deep and you lose that without gaining much accuracy.
  \end{takeaway}
\end{frame}
...
\end{solutionbox}
\begin{mistake}
Interpolating between the two leaf means for a point near the cut (e.g.\ predicting $\approx5$ at $x=3.2$). Trees do not interpolate; the prediction jumps only when $x$ crosses $3.5$.
\end{mistake}
\end{frame}
...
\end{enumerate}
\end{exercise}
\begin{labnote}[{{Check it in code --- \S6 of the notebook}}]
\S6 of \texttt{chapter\_08\_lab.ipynb} recomputes this exercise: both splits print error $0.250$, while Gini prints $0.375$ vs.\ $0.333$ and picks the split with the pure node.
\end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 8.2 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 8.2 (2/4)}
\begin{solutionbox}
\medskip
...
\begin{frame}{Solution --- Exercise 8.2 (3/4)}
\begin{solutionbox}
\textbf{(b) Gini.}
...
\begin{frame}{Solution --- Exercise 8.2 (4/4)}
\begin{solutionbox}
\medskip
...
Run the \S3 cell of \texttt{chapter\_08\_lab.ipynb}: ten-fold CV picks $\alpha\approx0.013$, yet the pruned tree's test MSE ($4.62$) is \emph{worse} than the depth-4 tree's $3.612$ --- CV pruned a tree that \texttt{max\_depth=4} had already regularised.
\end{labnote}
\end{frame}
...
\end{solutionbox}
\begin{mistake}
Selecting $\alpha$ on the \emph{training} criterion --- the RSS term always favours the biggest tree, so training data alone would pick $\alpha=0$. Only held-out error can choose the penalty.
\end{mistake}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 8.1 (1/6)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 8.1 (2/6)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
Evaluating the second split on \emph{all six} points. Recursive splitting works within one node at a time --- only the right child's four points $\{12,13,20,21\}$ enter this computation.
\end{mistake}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 8.1 (5/6)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 8.1 (6/6)}
\begin{solutionbox}
\medskip
...
\begin{frame}{How much does averaging actually buy?}\label{frm:avggain}
  \footnotesize
  Each tree has variance $\sigma^2$; because they are grown on overlapping
  bootstrap samples, any two are correlated, $\mathrm{Cor}(\hat f_b, \hat
  f_{b'}) = \rho$. Expanding the variance of a sum (Ch.~0: $B$ variance terms
  and $B(B-1)$ covariance terms) gives
  \begin{block}{The averaging gain}
    \[
      \operatorname{Var}\Bigl(\tfrac1B \sum_{b=1}^{B}\hat f_b\Bigr)
        = \frac{1}{B^2}\bigl[B\sigma^2 + B(B-1)\rho\sigma^2\bigr]
        = \underbrace{\rho\sigma^2}_{\text{floor}}
          + \underbrace{\frac{1-\rho}{B}\,\sigma^2}_{\to\,0}.
    \]
  \end{block}
  \begin{takeaway}[{{Two levers, and only one is free}}]
    More trees ($B\uparrow$) kills the second term --- that is why $B$ is
    never a tuning parameter and more trees never hurt. But the first term
    does not move: no matter how many trees you average, the variance stops
    at $\rho\sigma^2$. Independent trees ($\rho = 0$) would leave
    $\sigma^2/B \to 0$; identical trees ($\rho = 1$) gain nothing at all.
    \textbf{To go below the floor you must lower $\rho$} --- which is
    precisely what a random forest does next.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Why averaging helps: one deep tree vs.\ a bagged hundred}
  \begin{center}\includegraphics[width=0.88\textwidth,height=0.52\textheight,keepaspectratio]{ch08_x_bagging_variance.png}\end{center}
...
\begin{frame}{Random forests: the de-correlation trick}\label{frm:rf}
  ...
  \begin{takeaway}
    ...
    pairwise correlation $\rho$ --- and so lowers the variance floor
    $\rho\sigma^2$ that bagging alone cannot get past. With $m = p$ random
    forests reduce to bagging.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Random forest: error vs.\ number of trees}
  ...
  \par{\tiny\itshape\color{gray}History: bagging (1996) and random forests
  (2001) are both Leo Breiman's --- the same Breiman as in CART.}
...
\begin{frame}{Variable importance in random forests}
  ...
  \begin{alertblock}{Caveat}
    ...
    interpret pairs together. Impurity importance is also biased
    towards high-cardinality variables (customer ID, postcode), so
    prefer permutation importance when the ranking will be quoted.
  \end{alertblock}
\end{frame}
...
In \texttt{chapter\_08\_lab.ipynb} \S4, fit the $500$-tree forest with \texttt{oob\_score=True}: OOB $R^2=0.629$ appears without touching the test set (test MSE $2.312$), and the next cell draws the importance plot.
\end{labnote}
\end{frame}

\begin{frame}{Solution --- Exercise 8.4 (1/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 8.4 (2/3)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Exercise 8.4 (3/3)}
\begin{solutionbox}
\medskip
...
\end{solutionbox}
\begin{mistake}
Scoring OOB points with \emph{all} $B$ trees. Trees that saw the point in-bag have memorised it, so including them makes the error estimate optimistically low.
\end{mistake}
\end{frame}
...
\end{solutionbox}
\begin{mistake}
Expecting small $m$ to reduce \emph{bias}. Restricting candidates slightly \emph{raises} each tree's bias; the payoff is purely in decorrelation, i.e.\ variance reduction of the average.
\end{mistake}
\end{frame}
...
\begin{frame}{Boosting: idea}\label{frm:boost}
  ...
  \end{block}
  \begin{takeaway}
    Nothing here fits the response directly after the first step: every
    tree after it fits the \emph{residual} left by the ones before, and
    $\nu$ decides how much of each correction is kept.
  \end{takeaway}
  \par{\tiny\itshape\color{gray}History: AdaBoost (Freund \& Schapire, 1997)
  made boosting practical; Friedman (2001) recast it as gradient boosting,
  XGBoost's ancestor.}
\end{frame}
...
\begin{frame}{Boosting tuning parameters}
  \begin{readme}[The trio]
    \begin{itemize}
      \item $B$ --- number of trees. Too large $\Rightarrow$
            overfitting.
      \item $\nu$ --- learning rate (typically $0.01$ or $0.001$).
  ...
  \begin{takeaway}[Trade-off]
    ...
    by CV --- but on time-ordered data, hold out \emph{later} periods
    rather than random folds (Chapter~5), or the future leaks into
    training and flatters every one of the three.
  \end{takeaway}
\end{frame}

\begin{frame}{Boosting vs.\ random forest: head-to-head}\label{frm:headtohead}
  \begin{center}
...
\begin{frame}{Modern boosting libraries}
  ...
  \end{readme}
  \begin{takeaway}
    Native handling of missing values and categorical variables removes
    most of the feature engineering, and histogram splits make millions
    of rows routine --- which is why these three, not \texttt{sklearn}'s
    textbook implementation, are what actually ships.
  \end{takeaway}
\end{frame}
...
\end{enumerate}
\end{exercise}
\begin{labnote}[{{Live demo --- \S5 and \S7 of the notebook}}]
Run \S5 of \texttt{chapter\_08\_lab.ipynb} ($B{=}2000$, $\lambda{=}0.01$, $d{=}3$ $\to$ test MSE $2.195$ on Carseats), then the \S7 table: forest $2.312$ vs.\ boosting $2.195$ --- both cut the single tree's $3.612$ by more than a third.
\end{labnote}
\end{frame}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_08\_lab.ipynb}]
    The code in this section is meant to be \textbf{demonstrated live}. Open the companion Jupyter notebook and run it cell by cell, section by section:
    \begin{itemize}
      \item \S2, \emph{Single regression tree}, and \S3, \emph{Pruning via cost-complexity} --- on the Carseats data loaded in \S1, \emph{The Carseats data};
      \item \S4, \emph{Random forest}, and \S5, \emph{Gradient boosting} --- with the \emph{Optional: XGBoost} subsection that closes \S5;
      \item \S6, \emph{Why Gini and not classification error?} --- Exercise 8.2 re-done in code --- and \S7, \emph{The four models side by side}.
    \end{itemize}
    Data loads via the \texttt{ISLP} package or the bundled CSV files; \S8, \emph{Exercises}, closes the notebook.
  \end{labnote}
\end{frame}
...
from sklearn.tree import DecisionTreeRegressor, plot_tree  # fit and draw
from sklearn.model_selection import train_test_split  # honest test error
from ISLP import load_data  # the book's data loader

Carseats = load_data("Carseats")  # 400 stores, Sales in 1000s
X = pd.get_dummies(Carseats.drop(columns="Sales"), drop_first=True)  # factors -> 0/1
y = Carseats["Sales"]  # response: unit sales per store

Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=0.3, random_state=0)  # seeded 70/30
tree = DecisionTreeRegressor(max_depth=4).fit(Xtr, ytr)  # depth cap = the regulariser
plot_tree(tree, feature_names=X.columns, filled=True, fontsize=7)  # read rules top-down
  \end{lstlisting}
  \begin{readme}
    \texttt{max\_depth=4} is the only regulariser here: splitting stops after
    four levels, so the tree stays readable instead of growing one leaf per
    store. Each leaf predicts the mean \texttt{Sales} of its training cases
    --- test MSE $3.612$. The alternative, \texttt{ccp\_alpha}$\,=\alpha$,
    charges $\alpha$ per leaf and prunes a deep tree back: CV picks
    $\alpha\approx0.013$ and scores a \emph{worse} $4.62$.
  \end{readme}
\end{frame}

\begin{frame}[fragile]{Random forest}
  \begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
from sklearn.ensemble import RandomForestRegressor  # B bootstrap trees
import numpy as np

rf = RandomForestRegressor(n_estimators=500, max_features="sqrt",  # sqrt(p) per split
                            random_state=0, oob_score=True).fit(Xtr, ytr)  # keep OOB score
print("OOB R^2:", rf.oob_score_)  # 0.629: validation without a test set
print("Test MSE:", ((yte - rf.predict(Xte))**2).mean())  # 2.312 vs 3.612 one tree

imp = pd.Series(rf.feature_importances_, index=X.columns)  # total RSS drop per variable
imp.sort_values().plot.barh()  # which predictors the forest leans on
  \end{lstlisting}
  \begin{readme}
    \texttt{max\_features="sqrt"} is what makes this a \emph{forest} rather
    than bagging: each split may use only $\sqrt p$ of the $p$ predictors, so
    no single strong variable heads all $500$ trees --- less correlated trees
    average better. \texttt{oob\_score=True} scores each store with the
    ${\approx}37\,\%$ of trees that never drew it, so $R^2=0.629$ costs no
    test data. Test MSE $2.312$ beats the single tree's $3.612$.
  \end{readme}
\end{frame}

\begin{frame}[fragile]{Gradient boosting and XGBoost}
  \begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
from sklearn.ensemble import GradientBoostingRegressor  # trees in sequence
gb = GradientBoostingRegressor(n_estimators=2000, learning_rate=0.01,  # B=2000, step 0.01
                               max_depth=3, random_state=0).fit(Xtr, ytr)  # d=3, weak
print("Test MSE:", ((yte - gb.predict(Xte))**2).mean())  # 2.195, best of the four

# Optional: XGBoost, the production-grade implementation
import xgboost as xgb  # same idea, much faster
bst = xgb.XGBRegressor(n_estimators=2000, learning_rate=0.05,  # bigger step, same B
                       max_depth=4, subsample=0.8).fit(Xtr, ytr)  # 80% of rows per tree
  \end{lstlisting}
  \begin{readme}
    Boosting is sequential: each of the $B=2000$ trees fits what the ensemble
    still gets wrong, and only \texttt{learning\_rate} $\lambda=0.01$ of it is
    added. Small $\lambda$ learns cautiously and so needs large $B$: the
    product $\lambda B$ is the effective fitting effort, so lowering one means
    raising the other. \texttt{max\_depth=3} keeps each tree a weak learner.
    Test MSE $2.195$, just ahead of the forest's $2.312$.
  \end{readme}
\end{frame}
...
The worked solution sits in \texttt{chapter\_08\_lab.ipynb} under \emph{Lecture exercises --- worked Python solutions}: the cell headed \emph{Exercise 8.7 --- Random forest \& boosting on Boston [Python]}.
\end{labnote}
\end{frame}
...
\begin{solutionbox}
\begin{lstlisting}
import pandas as pd  # CSV in, DataFrame out
from sklearn.ensemble import (RandomForestRegressor,  # the two ensembles
                              GradientBoostingRegressor)  # to compare
from sklearn.model_selection import train_test_split  # one fixed split
from sklearn.metrics import mean_squared_error  # the scoring rule

Boston = pd.read_csv('../ALL CSV FILES - 2nd Edition/Boston.csv')  # 506 tracts
Boston = Boston.drop(columns=[c for c in ['Unnamed: 0']  # stray index column
                              if c in Boston.columns])  # only if present
X = Boston.drop(columns='medv'); y = Boston['medv']  # target: median value
Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=0.3,  # 70/30 split
                                      random_state=0)  # same split each run
\end{lstlisting}
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
rf = RandomForestRegressor(n_estimators=500, max_features='sqrt',  # sqrt(p) per split
                           random_state=0).fit(Xtr, ytr)  # train only; test held back
print('RF  test MSE:', mean_squared_error(yte, rf.predict(Xte)))  # ~19.6 here

gb = GradientBoostingRegressor(n_estimators=1000, learning_rate=0.01,  # small steps
                               max_depth=3, random_state=0).fit(Xtr, ytr)  # weak learners
print('GBM test MSE:', mean_squared_error(yte, gb.predict(Xte)))  # ~13.5: beats the RF

imp = pd.Series(rf.feature_importances_,  # RSS drop credited per variable
                index=X.columns).sort_values(ascending=False)  # biggest first
print(imp.head())  # lstat and rm lead
\end{lstlisting}
...
\end{solutionbox}
\begin{mistake}
Comparing models fitted or evaluated on \emph{different} splits --- always fix one train/test split (or use CV) before declaring a winner, and remember that single-split MSEs carry noticeable seed-to-seed noise.
\end{mistake}
\end{frame}
...
\end{longexercise}
\begin{labnote}[{{Run this live}}]
The worked solution sits in \texttt{chapter\_08\_lab.ipynb} under \emph{Lecture exercises --- worked Python solutions}: the cell headed \emph{Extended Exercise 8.3 --- Bagging vs.\ forest vs.\ boosting [Python]} --- all four fits on one split (seed 1), with $m$ and $(B,\lambda,d)$ tuned by CV \emph{inside} the training set.
\end{labnote}
\end{frame}

\begin{frame}[fragile]{Solution --- Extended Exercise 8.3 (1/6)}
\begin{solutionbox}
\textbf{Load the data; fit a single tree and bagging} (\texttt{max\_features=p} $=$ bagging).
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
import pandas as pd  # CSV loading
from sklearn.model_selection import train_test_split  # one split, reused
from sklearn.tree import DecisionTreeRegressor  # the baseline
from sklearn.ensemble import RandomForestRegressor, GradientBoostingRegressor
from sklearn.metrics import mean_squared_error as mse  # scoring rule
B = pd.read_csv('../ALL CSV FILES - 2nd Edition/Boston.csv')  # 506 tracts
B = B.drop(columns=[c for c in ['Unnamed: 0', ''] if c in B.columns])  # tidy
X, y = B.drop(columns='medv'), B['medv']; p = X.shape[1]   # p=12 predictors
Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=.3, random_state=1)  # fixed
tree = DecisionTreeRegressor(random_state=1).fit(Xtr, ytr)  # unpruned baseline
bag  = RandomForestRegressor(n_estimators=500, max_features=p,  # m=p -> bagging
                             random_state=1).fit(Xtr, ytr)  # all p vars per split
print('tree', round(mse(yte, tree.predict(Xte)), 2),  # 26.7 against 8.6
      'bag', round(mse(yte, bag.predict(Xte)), 2))  # averaging kills variance
\end{lstlisting}
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 8.3 (2/6)}
\begin{solutionbox}
\emph{What it prints (seed 1):} tree $\approx26.7$, bagging $\approx8.6$ --- averaging $500$ bootstrap trees alone cuts the single tree's test error to a third.
\end{solutionbox}
\end{frame}

\begin{frame}[fragile]{Solution --- Extended Exercise 8.3 (3/6)}
\begin{solutionbox}
\textbf{Tune the forest's $m$ and boosting's $(B,\lambda,d)$; read importances.}
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
for m in ['sqrt', 4, 6]:                          # vars tried per split
    rf = RandomForestRegressor(n_estimators=500, max_features=m,  # the m dial
                               random_state=1).fit(Xtr, ytr)  # only m varies
    print('RF  m=', m, round(mse(yte, rf.predict(Xte)), 2))  # 9.81, 9.43, 8.73

for lr, Bn, d in [(0.1, 1000, 2), (0.01, 2000, 3)]:   # big step/few vs small/many
    gb = GradientBoostingRegressor(learning_rate=lr, n_estimators=Bn,  # lambda*B
                                   max_depth=d, random_state=1).fit(Xtr, ytr)  # depth d
    print('GBM', lr, Bn, d, round(mse(yte, gb.predict(Xte)), 2))  # 10.24 vs 7.34

imp = pd.Series(rf.feature_importances_,  # from the last (m=6) forest
                index=X.columns).sort_values(ascending=False)  # biggest first
print(imp.round(3).head())  # lstat and rm dominate
\end{lstlisting}
\emph{What it prints (seed 1):} RF $m{=}\sqrt p$: $9.81$, $m{=}4$: $9.43$, $m{=}6$: $8.73$; GBM $(0.1,1000,2)$: $10.24$, $(0.01,2000,3)$: $7.34$ --- slow learning with more trees wins.
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 8.3 (4/6)}
\begin{solutionbox}
\textbf{Typical test MSE} (with \texttt{random\_state}${=}1$; exact figures vary):
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 8.3 (5/6)}
\begin{solutionbox}
\[
\begin{array}{l|c}
\text{single tree} & \approx 27\\
\text{bagging } (m=p) & \approx 8.6\\
\text{random forest } (m=\sqrt p \ \to\ 6) & \approx 9.8\ \to\ 8.7\\
\text{boosting } (\lambda{=}0.01,\,B{=}2000,\,d{=}3) & \approx 7.3
\end{array}
\]
\end{solutionbox}
\end{frame}

\begin{frame}{Solution --- Extended Exercise 8.3 (6/6)}
\begin{solutionbox}
...
\end{solutionbox}
\begin{mistake}
Reading importances as causal effects. They measure how much the ensemble \emph{uses} a variable, and correlated predictors share the credit --- \texttt{lstat}'s $0.40$ partly stands in for the whole socio-economic cluster.
\end{mistake}
\end{frame}
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{description}
    ...
    \item[Gini] $G=\sum_{k}\hat p_{mk}(1-\hat p_{mk})$; \quad \textbf{entropy} $D=-\sum_k \hat p_{mk}\log \hat p_{mk}$.
    ...
    \item[Averaging gain] $\operatorname{Var}\!\big(\tfrac1B\sum_b \hat f_b\big)=\rho\sigma^2+\frac{1-\rho}{B}\,\sigma^2 \xrightarrow{B\to\infty} \rho\sigma^2$ \ (each $\operatorname{Var}=\sigma^2$, pairwise corr.\ $\rho$).
...
\begin{frame}{Decision rules of thumb}\label{frm:rules}
  \begin{itemize}
  ...
  \begin{alertblock}{Don't}
    \begin{enumerate}
      ...
      \item Forget that trees fit step-functions --- they cannot
            extrapolate.
      \item Skip validation: OOB error is free for a forest, but boosting's
            $B$, $\lambda$ and $d$ still need CV or early stopping.
    \end{enumerate}
  \end{alertblock}
\end{frame}
...
\begin{frame}{What's next}
  This was the last taught chapter --- from here the semester turns to
  revision and the final exam.
  \begin{itemize}
    \item \textbf{Module A9 --- Deep Learning} (ISLP Ch.\ 10): MLPs,
          convolutional networks, backpropagation, SGD, regularisation.
          Self-study, with its own deck and lab; the final mock exam
          draws on it.
    \item \textbf{Module A7 --- Multiple Testing} (ISLP Ch.\ 13): also
          assigned as self-study; Short Exam E and the final papers carry
          a problem from it.
    \item The remaining modules (A1--A6, A8) are optional depth ---
          SVMs, survival analysis, unsupervised learning, and more.
  \end{itemize}
\end{frame}
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{ISLP Figures 8.1 and 8.2 --- the Hitters tree and its partition} --- the book's rendering; the main deck teaches from the redrawn flowchart version.
      \item \textbf{The partition picture, drawn a second time} --- a schematic duplicate of the Hitters partition already shown in the main deck.
      \item \textbf{Impurity measures and pruning (Extended Exercise 8.2)} --- Gini, entropy and cost-complexity pruning worked through in full. Exercises 8.2 and 8.3 cover both in the main deck.
      \item \textbf{BART} --- Bayesian additive regression trees --- a fourth ensemble, listed for completeness. Random forests and boosting are the two you will be asked about.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 8.2 (1/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 8.2 (2/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 8.2 (3/4)}
\begin{solutionbox}
...
\begin{frame}{Solution --- Extended Exercise 8.2 (4/4)}
\begin{solutionbox}
...
\medskip
\emph{Where the breakpoints come from.} Two neighbouring subtrees swap
places where their $R_\alpha$ values tie: pruning one leaf pays when the
per-leaf price exceeds the training error it costs,
$\alpha > \Delta R/\Delta|T|$. Here the trade-offs are
$(4-0)/1 = 4$ leaves$\,4\to3$, $(10-4)/1 = 6$ for $3\to2$ and
$(40-10)/1 = 30$ for $2\to1$ --- the breakpoints $\alpha = 4, 6, 30$ of
the table. This ``price per leaf'' is exactly the weakest-link ordering
that software uses to build the pruning sequence.

\smallskip
% Kept as italic text: lifting this into a mistake box overflows the slide.
\textit{Common mistake:} choosing $\alpha$ by training error --- $\alpha=0$ (the full tree) always wins there, by construction. $\alpha$ is chosen by cross-validating over the pruning sequence.
\end{solutionbox}
\end{frame}

\begin{frame}{BART: Bayesian Additive Regression Trees}\label{frm:bart}
  \footnotesize
  A sum of $K$ small trees, $\hat f(x)=\sum_{k=1}^{K}\hat f_k(x)$, updated
  over $B$ sweeps --- but unlike boosting, a sweep does not fit a fresh
  tree:
  \begin{block}{One update of tree $k$ in sweep $b$ (Algorithm 8.3)}
    \begin{enumerate}
      \item Form the \textbf{partial residual} --- what the \emph{other}
            $K-1$ trees currently leave unexplained:
            $r_i = y_i - \sum_{k'<k}\hat f^{\,b}_{k'}(x_i)
                       - \sum_{k'>k}\hat f^{\,b-1}_{k'}(x_i)$.
      \item Randomly \textbf{perturb} the previous version of tree $k$ ---
            grow or prune a branch, or change its leaf values --- favouring
            perturbations that fit $r_i$ better.
    \end{enumerate}
  \end{block}
  \begin{takeaway}[Why perturb instead of refit --- and what you get]
    Refitting greedily would chase the residual hard and overfit; small
    random perturbations keep every tree weak, so BART regularises the way
    slow learning does in boosting. The final prediction \emph{averages}
    the sweeps after a burn-in of $L$:
    $\hat f(x)=\frac{1}{B-L}\sum_{b=L+1}^{B}\hat f^{\,b}(x)$ --- bagging's
    trick, applied across iterations, and the spread of those sweeps gives
    uncertainty bands for free. Defaults ($K=200$, $B=1000$, $L=100$) work
    well out of the box. Implementations: \texttt{ISLP.bart}, \texttt{PyMC}.
  \end{takeaway}
  \islpsource{Section 8.2.4, Algorithm 8.3}
\end{frame}
