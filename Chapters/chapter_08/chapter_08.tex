...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.5cm}p{5.6cm}p{4.6cm}@{}}
    \toprule
...
\begin{frame}{Industry case in depth: retail demand forecasting}
  \footnotesize
  \begin{industry}[The model]
    \textbf{Response:} units sold per article $\times$ store $\times$ week (or day). \textbf{Predictors:} lagged and rolling sales, own and relative price, promotion and display flags, holidays and seasonality, article hierarchy and store attributes. Gradient boosting handles the mix natively --- LightGBM-based solutions dominated the M5 forecasting competition.
  \end{industry}
  \begin{readme}[What comes out --- and who uses it]
    Not one number but a forecast \emph{distribution} per article--store--week: the replenishment system orders against a service-level quantile, while the same forecasts feed the sales and operations planning round that finance and supply chain sign off. A store or category planner then releases or overrides each proposed order, with attributions (e.g.\ SHAP) explaining the unusual ones.
  \end{readme}
  \begin{takeaway}[The honest caveat]
    Trees predict step functions, so they \emph{cannot extrapolate}: a price point or a trend outside the training range gets clipped to the nearest observed level. Practitioners keep an explicit trend or price term, model in logs, and retrain on a cadence with drift monitoring.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Strengths and weaknesses of single trees}
  \begin{columns}[T]
    ...
    \end{column}
  \end{columns}
  \begin{industry}[Governance: the tree that fits on one page]
...
\begin{frame}{Variable importance in random forests}
  ...
  \end{alertblock}
  \begin{industry}[Model risk: an importance ranking is not an explanation]
...
\begin{frame}{Boosting: idea}
  ...
  \begin{block}{Gradient-boosting algorithm}
    ...
    \end{enumerate}
  \end{block}
  \begin{industry}[Banking: champion and challenger]
...
\begin{frame}{Boosting tuning parameters}
  ...
  \begin{takeaway}[Trade-off]
    ...
    by CV.
  \end{takeaway}
  \begin{industry}[Forecasting and fraud: validate by time --- never at random]
...
\begin{frame}{Modern boosting libraries}
  ...
  \end{readme}
  \begin{industry}[Production: why these libraries are the default]
