...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}r l >{\raggedright\arraybackslash}p{0.60\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{10} & \textbf{Ch.\ 8} & \textbf{Tree-based methods}\,{\color{accent!75!black}--- trees, forests, boosting} \\
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_08\_lab.ipynb}]
    ...
    \begin{itemize}
      \item \S2, \emph{Single regression tree}, and \S3, \emph{Pruning via cost-complexity} --- on the Carseats data loaded in \S1;
      \item \S4, \emph{Random forest}, and \S5, \emph{Gradient boosting};
      \item the \emph{Optional: XGBoost} subsection that closes \S5.
    \end{itemize}
...
\end{longexercise}
\begin{labnote}[Run this live: \emph{Extended Exercise 8.3 --- Bagging vs.\ random forest vs.\ boosting}]
Under \emph{Lecture exercises --- worked Python solutions} in \texttt{chapter\_08\_lab.ipynb}: all four fits on one split, with the importance ranking, ready to run.
\end{labnote}
\end{frame}
