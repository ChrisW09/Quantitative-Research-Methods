...
Run \S5 of \texttt{chapter\_08\_lab.ipynb} ($B{=}2000$, $\lambda{=}0.01$, $d{=}3$ $\to$ test MSE $2.195$ on Carseats), then the \S7 table: forest $2.312$ vs.\ boosting $2.195$ --- both roughly halve the single tree's $3.612$.
\end{labnote}
\end{frame}
  ...
  \begin{alertblock}{Don't}
    \begin{enumerate}
      ...
      \item Forget that trees fit step-functions --- they cannot
            extrapolate.
      \item Skip CV: even RF can overfit if individual trees are grown to unlimited depth.
    \end{enumerate}
  \end{alertblock}
\end{frame}
...
\begin{frame}{Self-check questions}
  ...
  \begin{enumerate}
    ...
    \item Rank baseline methods (logistic, RF, gradient boosting) for
          tabular data with $p = 100$, $n = 1000$. (p.~79)
  \end{enumerate}
\end{frame}
