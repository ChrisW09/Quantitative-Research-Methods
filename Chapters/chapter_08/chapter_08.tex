...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{3.0cm}p{5.2cm}p{4.5cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{Which decision it drives} \\
    \midrule
    Retail, supply chain & Demand per article, store and week from lagged sales, price, promotion and calendar features (LightGBM won the M5 competition) & The reorder quantity and safety stock released for next week \\
    Payments, e-commerce & Card-fraud and account-takeover scoring at transaction speed & Let through, challenge, or block --- decided in milliseconds \\
    Insurance & Claim triage and fraud referral from claim, policy and text-derived features & Straight-through settlement, or routed to a human adjuster \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{takeaway}[The common thread: wide mixed-type tabular data]
    All three feed hundreds of columns of mixed types, with missing values and interactions, into a model that must score fast and be retrained often. That is exactly the setting where boosted trees beat both linear models and neural networks --- and why they are the default choice there today.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Self-check questions}
  ...
  \begin{enumerate}
    ...
    \item Why use cross-entropy or Gini, not the misclassification
          rate, to grow classification trees? (p.~21)
    ...
    \item Rank baseline methods (logistic, RF, gradient boosting) for
          tabular data with $p = 100$, $n = 1000$. (p.~77)
  \end{enumerate}
\end{frame}
