...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}r l >{\raggedright\arraybackslash}p{0.60\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{11} & \textbf{Ch.\ 9} & \textbf{Support vector machines} \\
...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.5cm}p{5.1cm}p{4.5cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{Which decision it drives} \\
    \midrule
    Bioinformatics & Tumour classification from gene expression, $p$ in the
      thousands, $n$ in the dozens & Which tissue class a sample is reported as \\
    Text and search & Spam filters, ticket routing, sentiment on
      bag-of-words features & Which queue or folder a message lands in \\
    Manufacturing & One-class SVMs on sensor traces for novelty detection &
      Whether a unit is pulled off the line \\
    Finance, credit & Small-sample scorecards where a hard margin is
      auditable & Approve, decline or refer to a human \\
    Medical imaging & Lesion / no-lesion calls on engineered image
      features & Whether a scan is escalated to a radiologist \\
    Chemistry, materials & Property prediction from molecular descriptors with
      custom kernels & Which candidates get synthesised \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{industry}[The common thread]
    SVMs are the tool of choice when $p$ is large relative to $n$, when the
    signal is a \emph{boundary} rather than a probability, and when a custom
    similarity (a kernel) is more natural than a list of engineered features.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: a small-sample diagnostic assay}
  \scriptsize
  \begin{industry}[{{Why the widest street, and not maximum likelihood}}]
    A diagnostics firm has $n=180$ patients and $p=1\,200$ assay features. A
    logistic regression separates the training data perfectly and its
    maximum-likelihood coefficients diverge. An SVM instead asks for the
    \emph{widest} separating street --- well posed even here, with one finite,
    reproducible answer.
  \end{industry}
  \begin{readme}[{{The rule the team enforces, and who signs off}}]
    Predictors are standardised \emph{inside} the cross-validation pipeline;
    $C$ and $\gamma$ come from nested CV, never from the test fold. The
    reported number is the ROC AUC from \texttt{decision\_function}, because
    the operating threshold is set later by the clinical team. The lab director
    signs the validation report and defends the threshold to the notified body.
  \end{readme}
  \begin{takeaway}[Two honest caveats]
    The model returns no probability, so it cannot feed an expected-cost
    calculation without extra calibration; and with $p \gg n$ most of the
    sample ends up a support vector, which weakens the ``only a few points
    matter'' story.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Why maximum likelihood can fail where the margin does not}
  ...
  \begin{takeaway}
    ...
    unique answer.
  \end{takeaway}
  \begin{industry}[Bioinformatics: the $p \gg n$ regime]
    Gene-expression studies with $p$ in the thousands and $n$ in the dozens are
    \emph{always} separable --- where SVMs were first adopted in earnest, and
    where unpenalised logistic regression will not converge.
  \end{industry}
\end{frame}
...
\begin{frame}{The two conventions side by side}
  ...
  \begin{takeaway}
    ...
    scikit-learn convention: \textbf{big $C$, narrow street}.
  \end{takeaway}
  \begin{industry}[{Model risk: put the convention in the documentation}]
    Validation teams routinely reject SVM documentation that reports ``$C=100$,
    a highly regularised model''. Naming the library and version next to the
    hyperparameter is the cheapest possible fix, and reviewers look for it.
  \end{industry}
\end{frame}
...
\begin{frame}{$C$ and $\gamma$ pull on the same rope}
  ...
  \begin{takeaway}
    ...
    after the other.
  \end{takeaway}
  \begin{industry}[{Manufacturing: a tuned $\gamma$ is a maintenance liability}]
    A fault detector tuned to $\gamma$ on last quarter's sensor scale degrades
    silently once a sensor is replaced and the feature scale shifts. Freeze the
    scaler with the model, and re-tune $(C,\gamma)$ on a schedule.
  \end{industry}
\end{frame}
...
\begin{frame}{Solution --- Exercise 9.4}
\begin{solutionbox}
  \scriptsize
...
\begin{labnote}[Run this live]
  The worked solution sits in \texttt{chapter\_09\_lab.ipynb} under
...
print('linear', lin.best_params_, round(lin.best_score_, 3))
print('radial', rbf.best_params_, round(rbf.best_score_, 3))
print('poly d=2  CV', round(pol.mean(), 3))
\end{lstlisting}
\end{solutionbox}
\end{frame}
...
\end{exercise}
\begin{labnote}[Run this live]
  Worked in \texttt{chapter\_09\_lab.ipynb} under \emph{Lecture exercises ---
...
print('scaled  ', round(sc.score(Xte, yte), 3))
print('median d^2: raw %.0f  scaled %.1f'                         # (3) why
      % (np.median(pdist(X, 'sqeuclidean')),
...
\begin{frame}{When to reach for an SVM}
  \begin{readme}[Decision guidance]
    ...
    \end{itemize}
  \end{readme}
  \begin{industry}[{Credit and insurance: why SVMs lost the mainstream}]
    Scorecards must give a monotone, explainable probability of default and
    survive a regulator asking ``why was this applicant declined?''. Penalised
    logistic regression and boosted trees with SHAP answer that; a
    radial-kernel SVM does not. SVMs stayed where explanation matters less and
    $p/n$ is brutal --- assays, spectra, text, novelty detection.
  \end{industry}
\end{frame}
...
pipe = lambda **kw: make_pipeline(StandardScaler(), SVC(**kw))
...
for nm, gs in [('radial', rbf), ('linear', lin)]:
...
scores = {'radial SVM': rbf.best_estimator_.decision_function(Xte),  # NOT proba
          ...
          'logistic':   lg.predict_proba(Xte)[:, 1],
          'forest':     rf.predict_proba(Xte)[:, 1]}
for nm, sc in scores.items():                            # roc_auc_score
    print('%-11s AUC=%.3f' % (nm, roc_auc_score(yte, sc)))
\end{lstlisting}
\end{solutionbox}
\end{frame}
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[{Companion notebook: \texttt{chapter\_09\_lab.ipynb}}]
...
y = np.repeat([-1, 1], 10)                           # labels coded +-1
X[y == 1] += 2.5                                     # shift one class apart

hard = SVC(kernel='linear', C=1e6).fit(X, y)         # huge C ~ hard margin
print('n support vectors:', hard.n_support_.sum())   # 3
