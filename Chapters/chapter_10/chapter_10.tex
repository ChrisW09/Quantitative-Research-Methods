...
\begin{frame}{The course at a glance}
  ...
  \begin{tabular}{@{\hspace{2pt}}l >{\raggedright\arraybackslash}p{0.66\textwidth}@{\hspace{2pt}}}
    ...
    \rowcolor{accent!18} \textbf{Ch.\ 10} & \textbf{Deep learning}\,{\color{accent!75!black}--- MLPs, CNNs, training} \\
...
\begin{frame}{What is deep learning?}
  ...
  \end{takeaway}
  \begin{industry}[Banking and insurance: the tabular reality check]
    On the wide customer, policy and transaction tables that dominate a bank's or
    insurer's analytics stack, gradient boosting (Chapter~8) usually matches or beats a
    neural network at a fraction of the cost --- and is far easier to document for model
    validation. Deep learning earns its keep on the \emph{unstructured} assets those firms
    also hold: scanned documents, call recordings, images.
  \end{industry}
\end{frame}
...
\begin{frame}{Where this chapter is used in industry}
  \scriptsize
  \begin{center}
  \begin{tabular}{@{}p{2.5cm}p{5.3cm}p{4.6cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{Which decision it drives} \\
    \midrule
    Manufacturing & CNN on line-scan camera images: surface defects, missing components,
      assembly faults & Scrap, rework or stop the line \\
    Healthcare & Radiology screening and triage; many AI imaging devices are cleared by
      regulators & Which scans the radiologist reads first \\
    Insurance, banking & OCR plus sequence models on claims forms, invoices, contracts,
      KYC documents & Straight-through processing; route exceptions to a human \\
    Retail, streaming & Embedding-based recommendation and two-tower retrieval &
      What appears on the home page and in the ranking \\
    Telco, services & Speech recognition and call-centre analytics &
      Route, summarise and quality-check conversations \\
    Retail, energy & One network trained across thousands of related demand series &
      Purchasing, staffing and dispatch volumes \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{industry}[The common thread]
    Every row has an \emph{unstructured} input --- pixels, characters, waveforms --- where
    the network learns the features a human would otherwise hand-code.
  \end{industry}
\end{frame}

\begin{frame}{Industry case in depth: automated visual inspection}
  \footnotesize
  \begin{industry}[Semiconductors and automotive: end-of-line inspection]
    \textbf{Response.} A binary label per image, \emph{defect} / \emph{no defect}, usually
    with a location (bounding box or heat map) so the operator can see what was flagged.\\
    \textbf{Inputs.} Line-scan or area-scan camera images of each unit under fixed
    lighting; often several views per unit, sometimes with the process metadata (machine,
    shift, recipe) attached for later diagnosis.
  \end{industry}
  \begin{readme}[What the fitted model produces and who acts on it]
    A defect probability per image. A threshold turns it into an action: pass, divert to
    \textbf{rework}, \textbf{scrap}, or --- if the flagged rate spikes --- \textbf{stop the
    line} and call process engineering. Quality engineering owns the threshold; the
    production manager signs off on any rule that can halt output. The flag itself lands on
    the \textbf{shift supervisor}, who must act on it and answer for the stopped line.
  \end{readme}
  \begin{takeaway}[Honest caveats]
    Defects are \emph{rare}, so labelled positives are scarce and badly imbalanced ---
    augmentation, transfer learning and a cost-weighted threshold do the heavy lifting. A
    false stop is expensive, a missed defect can be worse, and the model must be
    \emph{revalidated} whenever the product, the fixture or the lighting changes.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Single-layer network as a graph}
  ...
  \begin{labnote}[{{Live demo --- \S2 of the notebook}}]
    Switch to \texttt{chapter\_10\_lab.ipynb} \S2 and run the model cell: the \texttt{MLP}
...
\begin{frame}{Where plain MLPs still pay off in business}
  \begin{industry}[Retail and energy: one network for many series]
    Retailers and grid operators forecast demand for thousands of related items or nodes. A
    single ``global'' network trained across \emph{all} series shares structure ---
    holidays, promotions, weather --- that a per-series model cannot borrow, and its output
    sets purchasing, staffing and dispatch volumes. The error is paid in stock-outs at one
    end and write-offs at the other.
  \end{industry}
  \begin{industry}[High-cardinality categories: learned embeddings]
    Business tables are full of identifiers with thousands of levels --- SKU, store,
    postcode, merchant. A network can map each level to a short learned vector
    (an \emph{embedding}) instead of a wide one-hot block, which is often the one place a
    neural model clearly beats a tree ensemble on tabular data.
  \end{industry}
\end{frame}
...
\begin{labnote}[{{Live demo --- \S4 of the notebook}}]
  After the hand count, run \texttt{chapter\_10\_lab.ipynb} \S4: it prints
...
\begin{frame}{Why convolutions?}
  ...
  \begin{readme}[Two key layer types]
    ...
    \end{itemize}
  \end{readme}
  \begin{industry}[Healthcare: radiology triage]
    Many AI-enabled imaging devices hold regulatory clearance, mostly in radiology. They
    \emph{prioritise or flag} studies rather than decide --- a radiologist still signs the
    report.
  \end{industry}
\end{frame}
...
\begin{frame}{Sequence models in commercial use}
  \begin{industry}[Insurance and banking: document and claims processing]
    OCR plus sequence models read claims forms, invoices, contracts and KYC documents and
    extract the fields --- claim amount, IBAN, policy number, counterparty. This is one of
    the highest-volume commercial uses of deep learning: it decides which files clear
    \emph{straight through} and which are routed to a human handler. A silently
    mis-extracted amount becomes a wrong payment, so confidence thresholds and sampling
    audits are part of the design, not an afterthought.
  \end{industry}
  \begin{industry}[Telco and services: speech and language]
    Speech recognition turns call recordings into text for routing, summarising and
    compliance checks; large language models over retrieved document embeddings answer
    customer and internal support queries. The business case is handling time, not
...
\begin{frame}{Loss and optimisation}
  ...
  \begin{readme}[Reading the update]
    ...
    \end{itemize}
  \end{readme}
  \begin{industry}[Operations: what training costs an organisation]
    Behind this one line sit GPU capacity, large labelled data sets and an MLOps stack:
    pipelines, versioning, monitoring, retraining. Projects stall on that plumbing far more
    often than on the optimiser.
  \end{industry}
\end{frame}
...
\begin{frame}{Regularisation in deep learning}
  ...
  \begin{readme}
    ...
    \end{itemize}
  \end{readme}
  \begin{industry}[Regulated deployment: generalising is not the only bar]
    Where a network is used to assess creditworthiness, to screen job applicants or for a
    medical purpose, the EU AI Act places it in the \emph{high-risk} category, with
    documentation, logging, monitoring and human-oversight obligations. That raises the bar
    for an opaque model: a validation team will ask not only ``does it generalise?'' but
    ``can you show why it said this, and who reviewed it?''
  \end{industry}
\end{frame}
...
\begin{labnote}[{{Live demo --- \S3 of the notebook}}]
  Every iteration of the training loop in \texttt{chapter\_10\_lab.ipynb} \S3 is this
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_10\_lab.ipynb}]
    ...
    \begin{itemize}
      ...
      \item \S4, \emph{Counting the parameters} --- Exercises 10.3 and 10.4 re-done in code, confirmed by PyTorch;
...
\begin{labnote}[{{Run this live}}]
  The worked solution sits in \texttt{chapter\_10\_lab.ipynb} under
...
\begin{frame}{Key formulas at a glance}
  ...
  \begin{description}
    ...
    \item[Conv params] $\#\text{params}=(F^2 d_{\text{in}}+1)\times(\text{\#filters})$; \ pooling has none.
  \end{description}
\end{frame}
...
\begin{frame}{Decision rules of thumb}
  ...
  \begin{labnote}[{{Live demo --- \S5 of the notebook}}]
    \S5 of \texttt{chapter\_10\_lab.ipynb} runs the first rule on \texttt{Default}:
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{Gradient descent on a loss surface: the real path} --- the seeded 2-D descent figure; the learning-rate trade-off it illustrates is stated in the main deck and proved in Exercise 10.5.
