...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.5cm}p{5.3cm}p{4.6cm}@{}}
    ...
    \midrule
    Manufacturing & Convolutional network on line-scan camera images: surface defects,
      missing components, assembly faults & Scrap it, send it to rework, or stop the line \\
    Insurance, banking & OCR plus sequence models reading claims forms, invoices and KYC
      documents & Which files clear straight through and which go to a human handler \\
    Retail, energy & One network trained across thousands of related demand series &
      The purchasing, staffing and dispatch volumes booked for next week \\
    \bottomrule
  \end{tabular}
  \end{center}
  \begin{takeaway}[Each row ends in a threshold somebody owns]
    In all three the network learns features a human would otherwise hand-code --- from
    pixels, characters or thousands of related series --- but the number it emits only
    matters once a threshold turns it into an action, and someone has to answer for
    that threshold.
  \end{takeaway}
\end{frame}
  ...
  \begin{readme}[Reading the recurrence]
    \begin{itemize}
      ...
      \item The \emph{same} $W_x,W_h,b$ are reused at every step --- weight sharing
            across time (convolutions share weights across space; see the appendix).
    \end{itemize}
  \end{readme}
  \begin{takeaway}
