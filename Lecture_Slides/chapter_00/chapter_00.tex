...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.3cm}p{4.7cm}p{5.4cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{What it delivers / decision it drives} \\
    \midrule
    Manufacturing & Statistical process control: a chart on a critical
      dimension, limits at $\pm 3\sigma$ &
      Flags a drifting process before it makes scrap: stop the line, or keep
      running \\
    Insurance & Claim severity is strongly right-skewed, so the mean sits
      above the typical claim &
      Median and upper quantiles inform reserving and where reinsurance
      attaches \\
    E-commerce & Online A/B test of a checkout or ranking change &
      Ship the variant or roll it back; the test size fixes the smallest
      effect worth chasing \\
    Market research & Brand and satisfaction trackers on a sample of a few
      hundred to a few thousand &
      The ``margin of error'' says which movements in the tracker are real \\
    Supplier quality & Acceptance sampling of an incoming lot against an
      agreed plan &
      Accept or reject the lot from a sample, at an agreed producer and
      consumer risk \\
    \bottomrule
  \end{tabular}
  \end{center}
...
\begin{frame}{Variable types decide everything that follows}
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    \bottomrule
  \end{tabular}
  \begin{industry}[ERP and CRM exports lie about types]
