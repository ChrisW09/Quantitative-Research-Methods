...
\begin{frame}{Quantiles, the five-number summary, and the boxplot}
  \begin{center}
    \includegraphics[width=0.84\textwidth,height=0.50\textheight,keepaspectratio]{ch00_boxplot_anatomy.png}
  \end{center}
  \scriptsize
  ...
  \begin{labnote}[Companion notebook]
    Rebuild this plot yourself with \texttt{df["wage"].describe()}.
  \end{labnote}
\end{frame}
...
\begin{frame}{Four shapes you will actually meet}
  ...
  \textbf{A spike at one value} --- 22\% of \texttt{Credit} customers carry a
...
\begin{frame}{When a transformation helps}
  ...
  \end{center}
  \footnotesize
  Taking logs pulls in the long right tail: skewness falls from $+1.0$ to
  $+0.3$, and mean and median almost coincide.
...
\begin{frame}{Simpson's paradox, drawn}
  \begin{center}
    \includegraphics[width=0.86\textwidth,height=0.48\textheight,keepaspectratio]{ch00_simpson.png}
  \end{center}
  \footnotesize
  Simulated data for three age groups: older people here both exercise more
  \emph{and} have higher cholesterol. Ignoring the group, exercise looks
  harmful ($+0.66$); within every group it helps.
  \begin{takeaway}[The lesson]
    The overall slope and the within-group slope answer different questions.
    Adding the group variable to the model --- what multiple regression does
    --- recovers the within-group answer. Which one you \emph{want} depends on
...
\begin{frame}{Power: the error nobody reports}
  ...
  \end{center}
  \footnotesize
  Moving the critical value to the right shrinks the orange area ($\alpha$)
...
\begin{frame}{One point can move the whole line}
  ...
  $y$ at a typical $x$ lifts the line a little. \textbf{Right:} the same
  vertical error placed far out in $x$ swings the slope --- that point has
  high \emph{leverage}, because the line must pivot to reach it.
...
\end{lstlisting}
  \begin{readme}[Column by column]
    ...
    the estimate is $17.7$ standard errors from zero.
...
\end{lstlisting}
  ...
  \begin{takeaway}[The shape of every analysis in this course]
    ...
    \emph{with} uncertainty $\to$ check the residuals. Chapters 2--13 change
    step 4 and enrich step 7; the skeleton never changes.
  \end{takeaway}
  \begin{labnote}[Companion notebook]
