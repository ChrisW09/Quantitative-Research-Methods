...
\begin{frame}{Notation introduced in this session}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    $\mathcal{O}(\cdot)$ & how the cost grows with the size of the problem \\
    \bottomrule
  \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}{Sums and products}
  ...
  \begin{numexample}[With numbers]
    ...
    In Python: \texttt{a.sum()} and \texttt{a.prod()} --- or
    \texttt{np.log(a).sum()} for the log of the product.
  \end{numexample}
...
\begin{frame}{Why logs are everywhere in statistics}
  ...
  \begin{numexample}[Numerical underflow --- the practical reason]
    ...
    around $0.5$, the product is $\approx 10^{-301}$: the computer rounds it
    to zero and every model becomes equally good. Its log is
    $\approx -693$, a perfectly ordinary number.
  \end{numexample}
\end{frame}
...
\begin{frame}{Three ways to say the same thing}
  ...
  \end{tabular}
  \end{center}
  \begin{takeaway}[Note the symmetry]
    Probabilities $0.25$ and $0.75$ give log-odds $-1.10$ and $+1.10$: the
    logit scale is symmetric about zero, while probabilities are squeezed
    against $0$ and $1$. That symmetry is why models work on the logit scale.
  \end{takeaway}
\end{frame}
...
\begin{frame}{The idea in one sentence}
  ...
  \begin{readme}[Read the direction of the conditioning]
    ...
    $\theta$. That is the single most common misunderstanding here.
  \end{readme}
\end{frame}
...
\begin{frame}{Least squares is maximum likelihood in disguise}
  \begin{center}
    \includegraphics[width=0.88\textwidth,height=0.42\textheight,keepaspectratio]{ch00b_likelihood_normal.png}
  \end{center}
  \footnotesize
  With normal errors, $\log P(y_i \mid \mu) = -\frac{(y_i-\mu)^2}{2\sigma^2} +
  \text{const}$, so
  \[
...
\begin{frame}{Solution --- Exercise 0b.4}
  \begin{solutionbox}
    ...
    \textbf{(3) Correct, and harmless.} Each extra observation multiplies in
    another probability $\le 1$, so $L$ shrinks (and $\ell$ becomes more
    negative). Only \emph{comparisons at the same data set} are meaningful ---
...
% ============================================================
\section{Counting and computational cost}
% ============================================================
...
\begin{frame}{The cost of ``just try everything''}
  \begin{center}
    \includegraphics[width=0.82\textwidth,height=0.50\textheight,keepaspectratio]{ch00b_subset_growth.png}
  \end{center}
  \footnotesize
  Forward stepwise selection fits about $p^2/2$ models instead of $2^p$: for
  $p = 40$, roughly $800$ instead of $10^{12}$. That is the difference between
  a second and longer than the age of the universe --- with no guarantee of
  finding the same answer, which is the trade-off Chapter 6 discusses.
\end{frame}
...
\begin{frame}[fragile]{Functions and loops --- used in all fourteen labs}
\begin{lstlisting}
...
\end{lstlisting}
  \begin{center}
    \includegraphics[width=0.80\textwidth,height=0.28\textheight,keepaspectratio]{ch00b_split.png}
  \end{center}
  \footnotesize
  Every notebook in this course seeds its randomness, so your numbers match
  the slides exactly. Change the seed and the third decimal moves --- that is
  not a bug, it is sampling variability, and Chapter 5 is about measuring it.
\end{frame}
...
\end{lstlisting}
  \begin{takeaway}[Learn it once and you know every model in the course]
    Trees, forests, boosting, SVMs, neural nets and clustering use the same
    three methods: swapping \texttt{LinearRegression()} for
...
\begin{frame}{Solution --- Extended Exercise 0b.2}
  \begin{solutionbox}
    ...
    ``best'' degrees appear. Never pick a seed until the answer looks good.
    Use $k$-fold cross-validation (Chapter 5), which averages over splits.
