...
\begin{frame}{Industry case in depth: price elasticity of demand}
  \footnotesize
  A retailer or an airline needs one number: how much volume is lost when the
  price goes up?
  \begin{industry}[The model]
    ...
    regression \emph{is} the elasticity. A coefficient on $\log(\text{price})$
    is a multiplicative effect on units, so a $1\%$ price rise moves volume by
    $\beta_1\%$ --- read straight off the output, nothing to transform back.
  \end{industry}
  \begin{numexample}[Illustrative]
    An estimated $\beta_1 \approx -1.8$ says a $10\%$ price cut lifts volume by
    roughly $18\%$, so revenue \emph{rises}. Comparing $|\beta_1|$ with $1$ is
    the whole calculation: above $1$ demand is elastic and a price rise loses
    revenue; below $1$ it gains.
  \end{numexample}
  \begin{takeaway}[Who signs off --- and the honest caveat]
    Pricing and revenue management or category management own the list price,
    promotion depth, markdown plan and fare-class prices; finance checks the
    margin. Caveat: past prices were cut \emph{because} demand was weak, so the
    estimate is confounded unless it comes from experiments or genuinely
    exogenous price moves --- and never extrapolate it to a price never tried.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Solution --- Exercise 0b.1}
  \begin{solutionbox}
    ...
    \medskip
    \textit{Common mistake:} writing $\min$ when $\arg\min$ is meant.
...
\begin{frame}{Solution --- Exercise 0b.2}
  \begin{solutionbox}
    ...
    \medskip
    \textit{Common mistake:} inventing a rule for sums. $\log(a+b)$
...
\begin{frame}{Solution --- Exercise 0b.3}
  \begin{solutionbox}
    ...
    \medskip
    \textit{Common mistake:} reading odds as probabilities. Odds of $0.818$
...
\begin{frame}{Solution --- Extended Exercise 0b.1 (2/2)}
  \begin{solutionbox}
    ...
    \medskip
    \textit{Common mistake:} treating $\hat\pi = k/n$ as exact. With $k=0$
...
\begin{frame}{Solution --- Exercise 0b.5}
  \begin{solutionbox}
    ...
    \medskip
    \textit{Common mistake:} assuming the $22$ stepwise fits search the same
