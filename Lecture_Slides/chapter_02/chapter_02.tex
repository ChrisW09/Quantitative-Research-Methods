...
\begin{frame}{Industry case in depth: an item-to-item recommender}
  ...
  \begin{readme}[The fitted output --- and who signs it off]
    ...
    and nobody asks it to be. Merchandising and personalisation own the module.
  \end{readme}
...
\begin{frame}{Bias, variance, and test MSE across flexibility}
  ...
  MSE = bias$^2$ + variance + $\sigma^2$. Horizontal dashed line: the
  irreducible $\sigma^2$. Vertical dotted line: the flexibility that
  minimises test MSE.
...
\smallskip
\textit{Common mistake:} averaging the two tail probabilities without the priors --- $(0.115+0.212)/2 = 0.164$, not $0.154$. Each class's error must be weighted by how often that class occurs.
\end{solutionbox}
\end{frame}
