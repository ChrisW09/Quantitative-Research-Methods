...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.85cm}p{5.15cm}p{4.6cm}@{}}
    \toprule
    \textbf{Sector} & \textbf{Concrete application} & \textbf{Decision it drives} \\
    \midrule
    Consumer goods, retail & Marketing mix modelling: weekly sales on spend per channel, price, promotion, seasonality & The annual media budget split across channels \\
    Real estate, mortgage lending & Hedonic pricing / automated valuation: price on floor area, rooms, age, location & Listing prices, collateral values, index quality adjustment \\
    Asset management & Factor models: excess return on market, size and value factors & Beta and hedge ratios, cost of equity, performance attribution \\
    Energy, utilities & Load forecasting: demand on temperature, day type, calendar effects & Day-ahead procurement and capacity bids \\
    HR, legal & Pay-equity audit: log salary on grade, tenure, location \emph{plus} a group indicator & The adjusted pay gap reported to the board \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{2pt}
  \begin{industry}[The common thread]
    In all five, the \emph{coefficient} is the deliverable --- not the
...
\begin{frame}{Industry case in depth: marketing mix modelling (MMM)}
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{block}{The model an agency actually fits}
        \begin{itemize}
          ...
          \item $n$: a few hundred brand--weeks.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{block}{What the output is used for}
        Each channel coefficient is read as a contribution and an
        elasticity, then aggregated into a decomposition of the year's
        sales. That decomposition is the starting point for next year's
        budget split --- signed off by the CMO together with finance,
        and re-fitted each quarter.
      \end{block}
    \end{column}
  \end{columns}
  ...
  \end{industry}
  \begin{alertblock}{The honest caveat}
    \footnotesize MMM estimates \emph{association}, so turning a coefficient
