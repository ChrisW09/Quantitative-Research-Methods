...
\subtitle{Chapter 3: Linear Regression \textemdash{} Improved Edition}
...
\begin{frame}{Deriving the slope: step 1 of 2}
  ...
  \begin{readme}
    ...
    sample variance of $X$. Strong covariation \emph{and} variation in
    $X$ both push the slope up.
  \end{readme}
\end{frame}
...
\begin{frame}{Confidence interval: construction}
  Approximately,
  \[
    \boxed{\;\hat\beta_j \;\pm\; 2\cdot \mathrm{SE}(\hat\beta_j)\;}
  \]
  is a $95\,\%$ confidence interval for $\beta_j$. (Strictly, use
  $t_{n-2,\,0.975}$ instead of $2$.)
...
\begin{frame}{Diagnostic 1: nonlinearity}
  ...
  \begin{block}{What to look for}
    ...
    randomly around zero. A clear pattern (curve, U-shape, fan) signals
...
\begin{frame}{Solution --- Exercise 3.10}
  \begin{solutionbox}
    \textbf{(1) Non-linearity.} A systematic curve means the linear
...
\begin{frame}{Detecting outliers and leverage in practice}
  ...
  \begin{block}{Studentised residuals}
    ...
    potential outliers.
  \end{block}
  \begin{block}{Leverage statistic $h_i$}
    The $i$-th diagonal of the hat matrix, $h_i = (\mathbf{H})_{ii}$.
    Always $0 \le h_i \le 1$ with average $(p+1)/n$. Flag points with
    $h_i > 2(p+1)/n$ as high-leverage.
  \end{block}
  \begin{takeaway}
  ...
  \begin{exercise}
    ...
    \begin{enumerate}
      ...
      \item Flag high-leverage points using the rule
            ...
            $|\text{studentized residual}| > 3$.
      \item In one sentence each, explain what leverage and a
            studentized residual measure.
    \end{enumerate}
  \end{exercise}
\end{frame}
...
\begin{frame}{Solution --- Exercise 3.11 (2/2)}
  \begin{solutionbox}
    ...
    \begin{itemize}
      ...
      \item A \emph{studentized residual} is the residual divided by its
...
\begin{frame}{Solution --- Extended Exercise 3.L5 (1/2)}
  \begin{solutionbox}
    \textbf{Non-linearity.} A $\cup$-shaped residuals-vs-fitted plot is
...
\begin{frame}{Vocabulary checklist}
  ...
  \begin{tabular}{ll}
    ...
    Leverage $h_i$ & Diagonal of $\mathbf{H}$; flag if $> 2(p+1)/n$ \\
