...
\begin{frame}{Learning objectives for this chapter}
  ...
  \begin{itemize}
    ...
    \item \textbf{Read} confusion matrices, sensitivity, specificity,
          ROC (receiver operating characteristic) curves, and AUC; \textbf{tune} the threshold to your loss
          function.
...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.5cm}p{5.0cm}p{5.4cm}@{}}
    ...
    Healthcare, pharma &
      Diagnostic and screening tests; biomarker and spectral panels (a classic home of LDA) &
...
\begin{frame}{Python lab --- run it live in the notebook}
  \begin{labnote}[Companion notebook: \texttt{chapter\_04\_lab.ipynb}]
    ...
    \begin{itemize}
      ...
      \item \S2 \emph{LDA, QDA, naive Bayes, KNN --- head-to-head}, and its \emph{ROC curves} subsection;
      \item \S3 \emph{Poisson regression on Bikeshare}.
    \end{itemize}
  ...
  \begin{alertblock}{Don't}
    \begin{enumerate}
      ...
      \item Use LDA on data with obviously different class
            covariances.
      \item Forget that QDA has $\sim K p^2/2$ parameters --- it overfits
            with small $n$.
...
\begin{frame}{Self-check questions}
  \begin{enumerate}
    ...
    \item Derive the LDA discriminant from Bayes' rule, assuming
          ...
          (pp.~47--49; step-by-step in the appendix, pp.~117--119)
    \item When does naive Bayes outperform logistic regression even
          though its independence assumption is wrong? (pp.~62 and~67)
...
\begin{frame}{Appendix --- what is in here, and why}
  ...
  formal derivation, a longer worked exercise, or a side topic.
  \begin{block}{Contents of the appendix}
    \begin{itemize}
      \item \textbf{How logistic regression is actually fitted} --- log-likelihood, deviance and IRLS as three views of one optimisation. \texttt{statsmodels} does it for you.
      \item \textbf{Multinomial logistic regression (the softmax)} --- the $K>2$ extension; the binary case carries every idea you need.
      \item \textbf{Deriving LDA from Bayes' theorem (Extended Exercise 4.2)} --- where the discriminant comes from. The main deck states it; Exercise 4.5 applies it.
      \item \textbf{Naive Bayes by hand (Extended Exercise 4.3)} --- a longer second pass over the calculation of Exercise 4.7.
      \item \textbf{GLMs and Poisson regression} --- the umbrella over linear, logistic and Poisson regression, with the Bikeshare example and its lab code. Not examinable.
    \end{itemize}
  \end{block}
\end{frame}
