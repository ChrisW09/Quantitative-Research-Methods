...
\begin{frame}{Where this chapter is used in industry}
  ...
  \begin{tabular}{@{}p{2.5cm}p{5.1cm}p{4.6cm}@{}}
    ...
    Banking &
      ...
      Out-of-sample \emph{and} out-of-time error --- the evidence for sign-off \\
    Retail, energy &
      Walk-forward backtest of a demand or load forecast &
      Honest error per horizon; replenishment and hedging settings \\
    Insurance &
      Bootstrap of a loss-reserve or capital figure &
      A \emph{range} rather than a point --- the booked reserve \\
    Pharma, medtech &
      Patient-grouped CV of a diagnostic or triage model &
      Error on \emph{unseen patients}; evidence for a submission \\
    Tech, platforms &
      Bootstrap interval for revenue per user in an A/B test &
      Ship / no-ship on a heavy-tailed KPI \\
    Manufacturing &
      $k$-fold CV to tune a quality- or yield-prediction model &
      The complexity choice, made before the line depends on it \\
    \bottomrule
  \end{tabular}
  \end{center}
...
\begin{frame}{LOOCV's hidden cost}
  ...
  \end{takeaway}
  \begin{industry}[Where $n$ is genuinely small]
...
\begin{frame}{Three pitfalls that quietly invalidate CV}
  \begin{alertblock}{The classic CV mistakes}
    ...
    \end{enumerate}
  \end{alertblock}
  \begin{industry}[These are expensive mistakes --- not textbook ones]
...
\begin{frame}{Why we need the bootstrap}
  ...
  \begin{industry}[Where the formula is missing]
...
\begin{frame}{Decision rules of thumb}
  ...
  \end{itemize}
  \begin{industry}[What you report to a steering committee]
