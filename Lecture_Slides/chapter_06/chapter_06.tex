...
print("ridge alpha:", round(ridge.alpha_, 3),
  ...
  \begin{solutionbox}[Expected result and interpretation]
    ...
    \medskip
    \textit{Common mistake:} comparing the printed \texttt{alpha} with a
    textbook $\lambda$ --- \texttt{scikit-learn} scales the lasso penalty by
    $1/(2n)$, so the numbers differ across formulations even when the
    fitted models agree.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 6.3 (3/3)}
  \begin{solutionbox}[Expected result and interpretation]
    ...
    \smallskip
    \textit{Common mistake:} reading the dropped variables as ``irrelevant
    to salary''. \texttt{Years} and \texttt{CRBI} are highly correlated
    with the kept career totals --- the lasso drops \emph{redundant}
    predictors, and a different split can swap which of a correlated group
    survives.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Solution --- Extended Exercise 6.2 (2/2)}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} thinking the lasso shrinks proportionally.
    It subtracts the same \emph{absolute} amount from every coefficient;
    ridge is the proportional one --- compare the $3.0$ and $0.4$ rows.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Solution --- Exercise 6.6}
  \begin{solutionbox}
    ...
    \smallskip
    \textit{Common mistake:} concluding supervised must be better ---
    using $Y$ also raises variance, so PCR often matches PLS. And neither
    selects variables: every component $Z_m$ mixes \emph{all} $p$
    predictors.
  \end{solutionbox}
\end{frame}
