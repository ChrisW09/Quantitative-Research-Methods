...
\subtitle{Chapter 7: Moving Beyond Linearity \textemdash{} Improved Edition}
...
\begin{frame}{Notation in this chapter}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    $\mathrm{df}_\lambda$ & Effective degrees of freedom, $\operatorname{trace}(S_\lambda)$ \\
...
\begin{frame}{Wage data: degree-4 polynomial}
  ...
  \end{center}
  \begin{takeaway}Left: the degree-4 fit; right: its pointwise band. The curve captures the rise-then-plateau, but bends and widens at the sparse upper edge.\end{takeaway}
...
\begin{frame}{Step functions: piecewise constant}
  ...
  constant.
  \begin{block}{Model}
    Bin $X$ into $K$ ranges and fit a constant in each:
    \[
...
\begin{frame}{Polynomial vs.\ spline on Wage}
  \begin{center}
    \includegraphics[width=0.85\textwidth,height=0.50\textheight,keepaspectratio]{7_5.pdf}
  \end{center}
  \begin{takeaway}Same data, two fits: the polynomial oscillates at the edges while the natural spline stays controlled --- boundary behaviour is the deciding difference.\end{takeaway}
  \islpsource{Figure 7.5}
\end{frame}
  ...
  \begin{takeaway}[Three observations]
    \begin{itemize}
      \item The minimiser is a natural cubic spline with knots at every
            $x_i$.
      \item $\lambda = 0$: interpolation. $\lambda = \infty$: linear fit.
