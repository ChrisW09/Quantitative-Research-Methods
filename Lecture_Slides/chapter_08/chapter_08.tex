...
\begin{frame}{Industry case in depth: retail demand forecasting}
  ...
  \begin{readme}[What comes out --- and who uses it]
    Not one number but a forecast \emph{distribution} per article--store--week: the replenishment system orders against a service-level quantile, while the same forecasts feed the sales and operations planning round that finance and supply chain sign off. Attributions (e.g.\ SHAP) explain to a planner why an unusual order was proposed.
  \end{readme}
...
\begin{frame}{Boosting tuning parameters}
  ...
  \begin{industry}[Forecasting and fraud: validate by time --- never at random]
    In production, $B$ is fixed by early stopping on a held-out period. Random $K$-fold CV on transaction or sales data leaks the future into training and flatters the model; teams validate on later weeks than they train on.
  \end{industry}
\end{frame}
...
\begin{frame}{Modern boosting libraries}
  ...
  \end{readme}
  \begin{industry}[Why these libraries are the production default]
...
\smallskip
\textit{Common mistake:} reading importances as causal effects. They measure how much the ensemble \emph{uses} a variable, and correlated predictors share the credit --- \texttt{lstat}'s $0.40$ partly stands in for the whole socio-economic cluster.
\end{solutionbox}
\end{frame}
