...
\subtitle{Chapter 10: Deep Learning \textemdash{} Improved Edition}
...
\begin{frame}{Contents}\footnotesize\tableofcontents[hideallsubsections]\end{frame}
...
\begin{frame}{Notation in this chapter}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    softmax & output activation $e^{z_m}/\sum_\ell e^{z_\ell}$ for multiclass labels \\
...
\begin{frame}{Feed-forward network as a labeled graph}
  \begin{center}
  \begin{tikzpicture}[
...
\begin{frame}{The convolution operation}
  ...
  \end{center}
  \begin{takeaway}
    Each output cell is a dot product of the shared-weight filter with a local input patch. Weight sharing plus locality make CNNs data-efficient for images.
  \end{takeaway}
\end{frame}
...
\begin{frame}{Modern CNN tricks}
  \begin{itemize}
    ...
    \item \textbf{Dropout}: randomly switch off units so the net cannot rely on any
          single one (detailed in the next section).
...
\begin{frame}{Gradient descent on a loss surface: the real path}
  \begin{center}\includegraphics[width=0.88\textwidth,height=0.56\textheight,keepaspectratio]{ch10_x_gradient_descent.png}\end{center}
...
\begin{frame}{Regularisation in deep learning}
  ...
  \begin{readme}
    \begin{itemize}
      ...
      \item Dropout, weight decay (L2), early stopping.
...
\begin{frame}{Dropout: randomly switch off units during training}
  ...
  \end{center}
  \begin{takeaway}
    On each mini-batch, every hidden unit is kept with probability $1-p$ and otherwise
...
\begin{frame}{What's next}
  Chapter 11: \emph{Survival Analysis}.
  \begin{itemize}
    \item Censored time-to-event data.
