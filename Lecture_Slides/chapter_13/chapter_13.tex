...
\begin{frame}{Industry case in depth: an online experimentation platform}
  ...
  \begin{readme}[The rule the platform enforces and who signs off]
    ...
    owner proposes, a launch review with product and data science approves.
  \end{readme}
...
\begin{solutionbox}
  ...
  \medskip
  \textit{Common mistake:} defaulting to FWER ``to be safe''. Safety is
  not free --- the stricter criterion buys its guarantee with missed true
  effects, so the choice is a design decision, not a formality.
\end{solutionbox}
\end{frame}
...
\begin{solutionbox}
  ...
  \medskip
  \textit{Common mistake:} treating $q$ like $\alpha$. They bound
  different things: $\alpha$ caps the chance of \emph{any} false
  rejection; $q$ caps only the expected \emph{fraction} of the rejected
  list that is false.
\end{solutionbox}
\end{frame}
...
\begin{solutionbox}[Comparison and error guarantees]
  ...
  \medskip
  \textit{Common mistake:} reading ``FDR $\le 0.10$'' as ``each discovery is $90\%$
  likely true''. The FDR is an average over the whole rejected list (and over
  repetitions), not a per-discovery probability.
\end{solutionbox}
\end{frame}
...
\begin{solutionbox}[The power trade-off]
  ...
  \smallskip
  \textit{Common mistake:} quoting $E[V]=m\alpha$ as a fact about your study --- it is
  the all-null worst case ($m_0=m$). With real effects present the count is smaller,
  though still ruinous at scale.
\end{solutionbox}
\end{frame}
