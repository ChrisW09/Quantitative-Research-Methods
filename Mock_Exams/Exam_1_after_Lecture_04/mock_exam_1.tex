...
{\small\itshape Lecture slides: Chapters 1--2 --- Introduction and Statistical Learning (Lectures 1--2).}\par\vspace{1mm}
...
{\small\itshape Lecture slides: Chapter 2 --- Statistical Learning (Lecture 2).}\par\vspace{1mm}
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{3} Describe how squared bias, variance, training MSE and expected test MSE
    ...
    \emph{(Recall the expected test MSEs from part~(b): $3.50$ for A, $2.60$ for B and $4.61$
    for C.)}
\end{enumerate}
...
{\small\itshape Lecture slides: Chapter 2 --- Statistical Learning: $K$-nearest neighbours (Lecture 2).}\par\vspace{1mm}
...
{\small\itshape Lecture slides: Chapter 3 --- Linear Regression (Lectures 3--4).}\par\vspace{1mm}
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  \item \pp{6} Compute the least-squares estimates $\hat{\beta}_1$ and $\hat{\beta}_0$, and
    ...
    problem (with units).
  \item \pp{4} Using the fitted line from part~(a), $\hat{y}=2.000+3.000\,x$, compute the
    fitted values $\hat{y}_i$ and the residuals $e_i$ for all five weeks, and verify that
    $\sum_{i=1}^{5} e_i = 0$.
  \item \pp{3} Using the residuals from part~(b), $(e_1,\dots,e_5)=(1,-1,0,-1,1)$, compute the
    residual sum of squares (RSS) and the residual standard error (RSE), and interpret the RSE.
  \item \pp{6} Using $\hat{\beta}_1=3.000$ from part~(a), $\operatorname{RSE}=1.155$ from
    part~(c) and $\sum_i (x_i-\bar{x})^2 = 10$, compute
    $\operatorname{SE}(\hat{\beta}_1)=\operatorname{RSE}/\sqrt{\sum_i (x_i-\bar{x})^2}$
  ...
  \item \pp{3} Compute the total sum of squares (TSS) and, using $\operatorname{RSS}=4.000$ from
    part~(c), the coefficient of determination $R^2$, and interpret $R^2$.
  \item \pp{2} Using the fitted line from part~(a), $\hat{y}=2.000+3.000\,x$, predict the weekly
    sales for a week with $x=6$ radio spots. Name one reason why this particular prediction
