...
{\small\itshape Lecture slides: Chapter 4 --- Classification (Lectures 5--6).}\par\vspace{1mm}
...
0 to 400 EUR) with a logistic regression
\[
\hat{p}(x)=\Pr(Y=1\mid X=x)
  =\frac{e^{\hat{\beta}_0+\hat{\beta}_1 x}}{1+e^{\hat{\beta}_0+\hat{\beta}_1 x}} .
\]
Fitting it by maximum likelihood in Python with \texttt{statsmodels}
(\texttt{sm.Logit(y, X).fit()}) gives the coefficients:
...
\noindent where \texttt{const} is the intercept $\hat{\beta}_0$ and \texttt{balance} is the
coefficient $\hat{\beta}_1$ on the card balance.

\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  \item \pp{4} Read the estimated coefficients $\hat{\beta}_0$ and $\hat{\beta}_1$ from the
    \texttt{statsmodels} output above, and use them to compute the predicted default
    probability $\hat{p}(x)$ for a customer with balance $x=100$ EUR and for a customer with
    balance $x=250$ EUR.
  \item \pp{4} For the same two customers ($x=100$ EUR and $x=250$ EUR), compute the
    \emph{odds} of default $\hat{p}(x)/(1-\hat{p}(x))$ and the \emph{log-odds} (logit).
    Interpret the odds of the customer with $x=250$ in one sentence.
...
\begin{solutionbox}
\textbf{(a) Predicted probabilities.} Reading the coefficients from the \texttt{statsmodels}
output: $\hat{\beta}_0=-4$ (row \texttt{const}) and $\hat{\beta}_1=0.02$ (row \texttt{balance}).
The linear predictor (log-odds) is
...
$\hat{p}(\text{default}\mid x)>0.5$. The labels are coded $1=$ ``default'' and
$0=$ ``no default''. Evaluated in Python with \texttt{scikit-learn}, the call
\texttt{confusion\_matrix(y\_true, y\_pred)} returns:
...
\noindent Recall that \texttt{scikit-learn} places the \emph{true} classes in the rows and
the \emph{predicted} classes in the columns, with the class labels in increasing order (so
row and column~1 refer to label $0=$ ``no default'', and row and column~2 to label
$1=$ ``default'').

\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  \item \pp{7} Read TP, FP, FN and TN from the \texttt{scikit-learn} \texttt{confusion\_matrix}
    array above (taking ``default'' as the positive class), and compute (i) the overall
    accuracy and the overall error rate, (ii) the sensitivity (recall, true positive rate),
    (iii) the specificity, (iv) the precision, and (v) the false positive rate. Interpret
    sensitivity and precision in one sentence each in the context of this application.
...
\ifdefined\withsolutions
\begin{solutionbox}
\textbf{(a) Metrics.} In the \texttt{scikit-learn} array the rows are the true classes and
the columns the predicted classes, ordered $0$ (``no default'') then $1$ (``default''); with
``default'' as the positive class this gives $\text{TN}=855$ (top-left), $\text{FP}=45$
(top-right), $\text{FN}=40$ (bottom-left) and $\text{TP}=60$ (bottom-right).
\begin{align*}
...
{\small\itshape Lecture slides: Chapter 5 --- Resampling Methods (Lecture 7).}\par\vspace{1mm}
...
{\small\itshape Lecture slides: Chapter 6 --- Linear Model Selection and Regularisation (Lecture 8).}\par\vspace{1mm}
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{2} Using the criterion values from (a) and (b) ---
    $C_p=5.080,\,4.600,\,4.520,\,4.560$ and $\operatorname{BIC}=5.184,\,4.808,\,4.833,\,4.977$
    for $d=1,2,3,4$ --- state which model size each criterion selects, and explain the reason
    for the disagreement by comparing the two penalty terms.
