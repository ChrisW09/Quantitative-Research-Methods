...
\begin{frame}{Problem 1 --- solution (a) and (b)}
  \begin{solutionbox}[Solution 1(a) --- predicted probabilities]
    Read $\hat{\beta}_0=-4$ (\texttt{const}) and $\hat{\beta}_1=0.02$ (\texttt{balance})
    from the output; then $\hat{\eta}(x)=-4+0.02\,x$ gives $\hat{\eta}(100)=-2$ and
    $\hat{\eta}(250)=1$, hence
    $\hat{p}(100)=e^{-2}/(1+e^{-2})=\mathbf{0.119}$ and
    $\hat{p}(250)=e^{1}/(1+e^{1})=\mathbf{0.731}$.
  \end{solutionbox}
  \begin{solutionbox}[Solution 1(b) --- odds and log-odds]
    ...
    \end{tabular}
    \end{center}
    Check: $0.731/0.269=2.718$. \checkmark\ At $x=250$ the odds are $\approx 2.72{:}1$ ---
    default is about $2.7\times$ as probable as non-default.
  \end{solutionbox}
\end{frame}
...
\begin{frame}{Problem 3 --- solution (a)}
  \begin{solutionbox}[Solution 3(a) --- the five metrics]
    Rows $=$ true, columns $=$ predicted, labels $0$ then $1$; with ``default'' ($=1$) as the
    positive class: $\text{TP}=60$ (bottom-right), $\text{FP}=45$ (top-right),
    $\text{FN}=40$ (bottom-left), $\text{TN}=855$ (top-left).
    \begin{center}
    \begin{tabular}{llll}
      \toprule
