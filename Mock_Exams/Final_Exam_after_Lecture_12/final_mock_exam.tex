...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{2} Using the split chosen in part~(a), namely $s=4.5$ (so $R_1=\{i:x_i<4.5\}$ with
    mean $\hat{y}_{R_1}=3$ and $R_2=\{i:x_i\ge 4.5\}$ with mean $\hat{y}_{R_2}=9$), predict the
    weekly sales for a new product with advertising budget $x=5$.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{4} How do \emph{random forests} modify bagging at each split, and what is the
    ...
    Explain --- with reference to the ensemble-variance formula
    $\operatorname{Var}=\rho\,\sigma^{2}+\frac{1-\rho}{B}\,\sigma^{2}$ from part~(a) ---
    why this modification improves on bagging.
  \item \pp{4} A data scientist tunes a \emph{boosting} model for regression trees with the
    following \texttt{scikit-learn} call:
    \begin{center}
    \texttt{GradientBoostingRegressor(n\_estimators=5000,}\\
    \texttt{\ \ \ \ learning\_rate=0.01, max\_depth=1)}
    \end{center}
    These three constructor arguments are exactly the three tuning parameters of boosting for
    regression trees. State in one sentence each what \texttt{n\_estimators},
    \texttt{learning\_rate} and \texttt{max\_depth} control. What is the ``slow learning'' idea
    behind boosting, and how is it reflected in the values chosen here?
...
\medskip
\textbf{(c) Boosting --- the three tuning parameters (constructor arguments).}
\begin{itemize}[nosep,leftmargin=5mm]
  \item \texttt{n\_estimators}~$=B$, the \emph{number of trees}: how many small trees are added
    sequentially; unlike bagging, boosting \emph{can} overfit if $B$ is too large, so it is
    chosen by cross-validation (here $B=5000$).
  \item \texttt{learning\_rate}~$=\lambda$, the \emph{shrinkage} (e.g.\ $0.01$ or $0.001$):
    scales the contribution of each new tree and thus the speed of learning; smaller $\lambda$
    usually needs larger $B$ (here $\lambda=0.01$).
  \item \texttt{max\_depth}~$=d$, the \emph{number of splits per tree} (interaction depth): the
    complexity of each tree and the interaction order of the model; often even $d=1$ (stumps,
    additive model) works well (here \texttt{max\_depth}$=1$).
\end{itemize}
...
which tends to generalise better. The chosen values encode exactly this: a \emph{small}
\texttt{learning\_rate=0.01} with \emph{many} trees (\texttt{n\_estimators=5000}) and shallow
\texttt{max\_depth=1} stumps.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{6} Apply the \emph{Benjamini--Hochberg} (BH) procedure with FDR level $q=0.05$ to
    the same six ordered $p$-values from part~(b), namely
    $p_{(1)},\dots,p_{(6)}=0.001,\ 0.004,\ 0.011,\ 0.030,\ 0.045,\ 0.230$:
    give the thresholds $jq/m$, determine which hypotheses are
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{4} A bank fits a logistic regression for loan default in \texttt{statsmodels},
    $\log\bigl(\frac{p(x)}{1-p(x)}\bigr)=\beta_0+\beta_1\,\texttt{missed\_payments}$, where
    \texttt{missed\_payments} is the number of missed payments $x$. The call
    \texttt{sm.Logit(default, X).fit()} produces:
    \begin{center}
    \begin{tabular}{lrrrr}
    \toprule
     & coef & std err & $z$ & $P>|z|$\\
    \midrule
    \texttt{const}            & $-3.5000$ & 0.420 & $-8.333$ & 0.000\\
    \texttt{missed\_payments} &   0.7000  & 0.150 &   4.667  & 0.000\\
    \bottomrule
    \end{tabular}
    \end{center}
    Using the estimated coefficient of \texttt{missed\_payments}, interpret the effect of
    \emph{one additional missed payment} on the \emph{odds} of default
    \emph{(use $e^{0.7}=2.014$)}. Why can the effect on the \emph{probability} of default not be
...
\medskip
\textbf{(b) Odds ratio.}
The \texttt{coef} column gives $\hat{\beta}_1=0.7$ for \texttt{missed\_payments} (highly
significant, $z=4.667$). One additional missed payment increases the log-odds by
$\hat{\beta}_1=0.7$, i.e.\ it \emph{multiplies the odds} of default by $e^{0.7}=2.014$ --- the
odds roughly double, regardless of the starting value of $x$.
