...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{2} Using the split at $s=3.5$ from part~(a) (so $R_2=\{x\ge 3.5\}$), predict the
    monthly revenue for a new branch that runs $x=5$ promotional events.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{4} How do \emph{random forests} modify bagging at each split, and what is the
    ...
    $p=15$)? Explain, with reference to the variance formula
    $\operatorname{Var}=\rho\sigma^2+\frac{1-\rho}{B}\sigma^2$ from part~(a), why this
    modification improves on bagging.
  \item \pp{4} The following \texttt{scikit-learn} code fits a boosted regression-tree model:
    \begin{center}\small\ttfamily
    \begin{tabular}{@{}l@{}}
    from sklearn.ensemble import GradientBoostingRegressor\\
    gbr = GradientBoostingRegressor(\\
    \hspace*{2em}n\_estimators=500, learning\_rate=0.01, max\_depth=2)\\
    gbr.fit(X\_train, y\_train)\\
    \end{tabular}
    \end{center}
    The three keyword arguments \texttt{n\_estimators}, \texttt{learning\_rate} and
    \texttt{max\_depth} are exactly the three tuning parameters of boosting. State in one
    sentence each what each of them controls, and explain the ``slow learning'' idea behind
    boosting --- in particular why one chooses a small \texttt{learning\_rate} such as
    \texttt{0.01}.
...
\medskip
\textbf{(c) Boosting.}
The three keyword arguments \emph{are} the three tuning parameters:
\begin{itemize}[nosep,leftmargin=5mm]
  \item \texttt{n\_estimators}${}=B$, the \emph{number of trees} added sequentially; unlike
    bagging, boosting \emph{can} overfit if $B$ is too large, so $B$ is chosen by
    cross-validation.
  \item \texttt{learning\_rate}${}=\lambda$, the \emph{shrinkage} (here $0.01$): it scales the
    contribution of each new tree and thus the speed of learning; smaller $\lambda$ usually
    needs larger $B$.
  \item \texttt{max\_depth}${}=d$, the \emph{number of splits / interaction depth} per tree
    (here $2$): the complexity of each tree and the interaction order of the model; often even
    $d=1$ (stumps, additive model) works well.
\end{itemize}
...
which tends to generalise better. Choosing a small \texttt{learning\_rate} such as
\texttt{0.01} is precisely how this slow learning is implemented (at the cost of needing more
trees, i.e.\ a larger \texttt{n\_estimators}).
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{6} Apply the \emph{Benjamini--Hochberg} (BH) procedure with FDR level $q=0.10$ to
    the same six ordered $p$-values from part~(b)
    ($p_{(1)},\dots,p_{(6)}=0.002,\,0.007,\,0.009,\,0.020,\,0.040,\,0.320$): give the
    thresholds $jq/m$, determine which hypotheses are rejected, and state precisely what
    ``FDR control at $q=0.10$'' guarantees for the resulting set of rejections.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  \item \pp{4} True or false --- justify briefly: ``By choosing a sufficiently flexible model
    ...
    way down to zero.''
  \item \pp{4} A logistic regression predicts whether a candidate passes a certification exam
    (\texttt{passed}$=1$) from the number of \texttt{hours} of study,
    $\log\bigl(\frac{p}{1-p}\bigr)=\beta_0+\beta_1\,\texttt{hours}$. The model was fitted in
    Python (\texttt{statsmodels} \texttt{Logit}), with the following output:
    \begin{center}
    \begin{tabular}{lrrrr}
    \toprule
     & coef & std err & $z$ & $P>|z|$\\
    ...
    \end{tabular}
    \end{center}
    Using the estimated coefficient on \texttt{hours}, interpret the effect of \emph{one
    additional hour of study} on the \emph{odds} of passing \emph{(use $e^{0.8}=2.226$)}. Why
    can the effect on the \emph{probability} of passing not be stated as a single number?
...
\medskip
\textbf{(b) Odds ratio.}
The fitted coefficient on \texttt{hours} is $\hat{\beta}_1=0.8000$ (from the \texttt{Logit}
output). One additional hour of study increases the log-odds of passing by $0.8$, i.e.\ it
\emph{multiplies the odds} of passing by $e^{0.8}=2.226$ --- the odds go up by about $123\%$
(more than double), regardless of the current number of hours.
The effect on the \emph{probability} depends on where the candidate sits on the S-shaped
logistic curve: the same change in log-odds moves $p$ a lot near $p\approx 0.5$ but very little
when $p$ is already close to $0$ or $1$ --- hence no single number describes it.
