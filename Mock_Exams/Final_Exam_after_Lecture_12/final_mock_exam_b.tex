...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  \item \pp{4} A \emph{cubic spline} is a piecewise cubic polynomial that is continuous and has
    continuous first and second derivatives at every knot. Derive how many degrees of freedom
    (free parameters, counting the intercept) such a spline uses for a general number $K$ of
    interior knots, by counting the parameters of the polynomial pieces and subtracting the
    constraints imposed at the knots. Then evaluate the count for $K=5$.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{2} Using the preferred split, predict the number of parcels for a new depot with
    $x=13$ vans.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{4} How do \emph{random forests} modify bagging at each split, and what is the
    ...
    $p=36$)? Using the formula in (a), explain why this modification improves on bagging.
  \item \pp{4} Name the three tuning parameters of \emph{boosting} for regression trees and say
    in one sentence each what they control. What is the ``slow learning'' idea behind boosting?
...
\begin{itemize}[nosep,leftmargin=5mm]
  \item \emph{Number of trees $B$:} how many small trees are added in sequence; unlike bagging,
    boosting \emph{can} overfit if $B$ is too large, so $B$ is tuned by cross-validation.
  \item \emph{Shrinkage $\lambda$} (learning rate, e.g.\ $0.01$ or $0.001$): scales the
    contribution of each new tree and hence the speed of learning; a smaller $\lambda$ usually
    requires a larger $B$.
  \item \emph{Number of splits $d$ per tree} (interaction depth): the complexity of each tree
    and the interaction order captured; often even $d=1$ (stumps, an additive model) suffices.
\end{itemize}
\emph{Slow learning:} each new tree is fit to the current \emph{residuals}, and only a small
fraction $\lambda$ of it is added to the model, so the fit improves gradually and precisely
where it is still weak, rather than fitting the data hard in a single step --- which tends to
generalise better.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{6} Apply the \emph{Benjamini--Hochberg} (BH) procedure at FDR level $q=0.05$ to the
    same six $p$-values: give the thresholds $jq/m$, determine which hypotheses are rejected, and
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  \item \pp{4} A medical screening classifier is evaluated on $200$ patients, giving the
    confusion matrix
    \begin{center}
    \begin{tabular}{lcc}
    \toprule
    ...
    \end{tabular}
    \end{center}
    Compute the overall error rate, the sensitivity, the specificity and the precision (positive
    predictive value). Which single number would trace out the ROC curve if it were varied, and
    in which direction would sensitivity and specificity move?
...
\ifdefined\withsolutions
\begin{solutionbox}
\textbf{(a) Confusion-matrix metrics.}
Here $TP=40$, $FN=10$, $FP=30$, $TN=120$, total $200$; $50$ patients are truly diseased and
$150$ truly healthy.
\[
