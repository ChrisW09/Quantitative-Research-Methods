...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  \item \pp{4} A \emph{cubic spline} is a piecewise cubic polynomial that is continuous and has
    continuous first and second derivatives at each knot. How many degrees of freedom
    ...
    constraints at the knots, and evaluate for $K=3$.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{2} Using the chosen split at $s=3.5$ from (a), predict the number of parcels for a
    new depot that will operate $x=5$ vans.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{4} How do \emph{random forests} modify bagging at each split, and what is the
    ...
    $p=30$)? Using the bagged-variance formula
    $\operatorname{Var}=\rho\sigma^{2}+\tfrac{1-\rho}{B}\sigma^{2}$ from (a), explain why this
    modification improves on bagging.
  \item \pp{4} A colleague fits a boosted regression-tree model in scikit-learn with the call
    below; its three arguments are exactly the three tuning parameters of \emph{boosting}:
    \begin{center}\small
    \begin{tabular}{@{}l@{}}
    \texttt{from sklearn.ensemble import GradientBoostingRegressor}\\
    \texttt{boost = GradientBoostingRegressor(}\\
    \texttt{~~~~n\_estimators=5000, learning\_rate=0.01, max\_depth=1)}\\
    \texttt{boost.fit(X\_train, y\_train)}\\
    \end{tabular}
    \end{center}
    For each of \texttt{n\_estimators}, \texttt{learning\_rate} and \texttt{max\_depth}, state in
    one sentence what it controls, and explain the ``slow learning'' idea behind boosting: why is
    a small \texttt{learning\_rate} paired with a large \texttt{n\_estimators}?
...
\medskip
\textbf{(c) Boosting.}
The three arguments of \texttt{GradientBoostingRegressor} are exactly the three tuning
parameters of boosting:
\begin{itemize}[nosep,leftmargin=5mm]
  \item \texttt{n\_estimators} (\emph{number of trees $B$}): how many small trees are added
    sequentially; unlike bagging, boosting \emph{can} overfit if $B$ is too large, so it is
    chosen by cross-validation --- here $B=5000$.
  \item \texttt{learning\_rate} (\emph{shrinkage $\lambda$}, e.g.\ $0.01$ or $0.001$): scales
    the contribution of each new tree and thus the speed of learning; a smaller $\lambda$
    usually needs a larger $B$ --- here $\lambda=0.01$.
  \item \texttt{max\_depth} (\emph{splits per tree / interaction depth $d$}): the complexity of
    each tree and the interaction order of the model; often even $d=1$ (stumps $\Rightarrow$
    additive model, as here) works well.
\end{itemize}
\emph{Slow learning:} each tree is fitted to the current \emph{residuals}, and only a small
fraction $\lambda$ of it is added to the model; the small \texttt{learning\_rate=0.01} combined
with the large \texttt{n\_estimators=5000} makes the model improve slowly and exactly in the
regions where it still performs poorly, instead of fitting the data hard in one step --- which
tends to generalise better.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{6} Apply the \emph{Benjamini--Hochberg} (BH) procedure with FDR level $q=0.05$ to
    the same six ordered $p$-values ($0.001$, $0.008$, $0.011$, $0.030$, $0.041$, $0.260$;
    $m=6$): give the thresholds $jq/m$, determine which hypotheses are rejected, and state
    precisely what ``FDR control at $q=0.05$'' guarantees for the resulting set of rejections.
...
\begin{enumerate}[label=(\alph*),leftmargin=7mm,itemsep=2.5mm]
  ...
  \item \pp{4} At a fixed test point $x_0$ the expected test error of an estimate $\hat f$
    ...
    A simulation stores these three components for two candidate estimators in a \texttt{pandas}
    \texttt{DataFrame} and prints it:
    \begin{center}\small
    \begin{tabular}{@{}l@{}}
    \texttt{comp = pd.DataFrame(\{"var": [3.5, 0.5], "bias": [-1.0, -2.5],}\\
    \texttt{~~~~~~~~~~~~~~~~~~~~"noise\_var": [2.0, 2.0]\},}\\
    \texttt{~~~~~~~~~~~~~~~~~~~~index=["flexible\_spline", "linear\_model"])}\\
    \texttt{print(comp)}\\
    \end{tabular}
    \end{center}
    \begin{center}
    \begin{tabular}{lccc}
    \toprule
     & \texttt{var} & \texttt{bias} & \texttt{noise\_var}\\
    \midrule
    \texttt{flexible\_spline} & 3.5 & $-1.0$ & 2.0\\
    \texttt{linear\_model} & 0.5 & $-2.5$ & 2.0\\
    \bottomrule
    \end{tabular}
    \end{center}
    Reading the three components off this output, compute the expected test MSE of each estimator,
    say which you would deploy at $x_0$, and state the lowest test MSE \emph{any} method could
    achieve here.
...
Reading the two rows of the \texttt{DataFrame} and applying
$\text{MSE}=\operatorname{Var}+\operatorname{Bias}^2+\operatorname{Var}(\varepsilon)$:
\[
...
\]
Deploy the \textbf{flexible\_spline} (row 1): its higher estimation variance ($3.5$ vs.\ $0.5$)
is more than offset by its much smaller squared bias ($1.0$ vs.\ $6.25$), giving the lower
expected test error ($6.5<8.75$). The lowest test MSE any method can achieve at $x_0$ is the
\emph{irreducible error} $\operatorname{Var}(\varepsilon)=\texttt{noise\_var}=\mathbf{2.0}$; no
estimator can go below it.
