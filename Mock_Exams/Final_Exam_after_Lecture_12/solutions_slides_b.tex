...
\begin{frame}{Problem 4 --- solution (c) and (d)}
  \begin{solutionbox}[Solution P4 --- (c) and (d)]
    \textbf{(c)} Boosting tuning parameters (from the call):
    \begin{itemize}
      \item \texttt{n\_estimators}$=B=500$: number of trees added sequentially; too many
        $\Rightarrow$ overfitting; choose by CV.
      \item \texttt{learning\_rate}$=\lambda=0.01$ (shrinkage): scales each new tree's
        contribution; smaller $\lambda$ $\Rightarrow$ larger $B$ needed.
      \item \texttt{max\_depth}$=d=1$ (interaction depth): splits per tree / interaction order;
...
\begin{frame}{Problem 7 --- solution (a) and (b)}
  \begin{solutionbox}[Solution P7 --- (a) confusion matrix and (b) KNN]
    \textbf{(a)} Read the array (order $[0,1]$, rows true / cols predicted); diagonal $=$ correct,
    so $TN{=}120$, $FP{=}30$, $FN{=}10$, $TP{=}40$ ($50$ diseased, $150$ healthy, $n=200$):
    \[
