...
\begin{frame}{Problem 4 --- solution (c) and (d)}
  \begin{solutionbox}[Solution P4 --- (c) and (d)]
    \textbf{(c)} The three arguments of \texttt{GradientBoostingRegressor} are the tuning parameters:
    \begin{itemize}
      \item \texttt{n\_estimators} \textbf{$=B$, number of trees:} added sequentially; too many
        $\Rightarrow$ slow overfitting; choose by CV (here $B=5000$).
      \item \texttt{learning\_rate} \textbf{$=\lambda$, shrinkage} (e.g.\ $0.01$): scales each
        new tree's contribution; smaller $\lambda$ $\Rightarrow$ slower learning $\Rightarrow$
        larger $B$ needed.
      \item \texttt{max\_depth} \textbf{$=d$, splits per tree} (interaction depth): complexity and
        interaction order of each tree; often stumps ($d=1$, as here) suffice (additive model).
    \end{itemize}
    \emph{Slow learning:} each tree is fitted to the current \emph{residuals} and only the
    fraction $\lambda$ is added --- small \texttt{learning\_rate=0.01} with large
    \texttt{n\_estimators=5000} makes the model improve gradually exactly where it still errs,
...
\begin{frame}{Problem 7 --- solution (a) to (c)}
  \begin{solutionbox}[Solution P7 --- (a) to (c)]
    ...
    \textbf{(b)} Reading each row and using
    $\text{MSE}=\operatorname{Var}+\operatorname{Bias}^2+\operatorname{Var}(\varepsilon)$:
    \[
      \text{MSE}_{\texttt{flexible\_spline}}=3.5+(-1.0)^2+2.0=\mathbf{6.5}\qquad
      \text{MSE}_{\texttt{linear\_model}}=0.5+(-2.5)^2+2.0=\mathbf{8.75}
    \]
    Deploy \textbf{flexible\_spline} ($6.5<8.75$): its higher variance is more than offset by its
    much smaller squared bias. Lowest achievable MSE $=$ irreducible error
    $\operatorname{Var}(\varepsilon)=\texttt{noise\_var}=\mathbf{2.0}$.
    \medskip
