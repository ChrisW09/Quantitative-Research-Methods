...
\begin{frame}{Problem 4 --- solution (c) and (d)}
  \begin{solutionbox}[Solution P4 --- (c) and (d)]
    \textbf{(c)} Boosting for regression trees --- the three constructor arguments:
    \begin{itemize}
      \item \texttt{n\_estimators}~$=B$ --- \textbf{number of trees:} added sequentially; too
        many $\Rightarrow$ slow overfitting; choose by CV (here $5000$).
      \item \texttt{learning\_rate}~$=\lambda$ --- \textbf{shrinkage} (e.g.\ $0.01$ or $0.001$):
        scales each new tree's contribution; smaller $\lambda$ $\Rightarrow$ slower learning
        $\Rightarrow$ larger $B$ needed (here $0.01$).
      \item \texttt{max\_depth}~$=d$ --- \textbf{splits per tree} (interaction depth):
        complexity and interaction order of each tree; often stumps ($d=1$) suffice (here $1$).
    \end{itemize}
    ...
    instead of fitting the data hard at once; the small \texttt{learning\_rate} with many trees
    and shallow stumps encodes exactly this.
    \medskip
...
\begin{frame}{Problem 7 --- solution (a) to (c)}
  \begin{solutionbox}[Solution P7 --- (a) to (c)]
    ...
    \textbf{(b)} From the \texttt{sm.Logit} output, \texttt{coef}~$=0.7$ for
    \texttt{missed\_payments}: one more missed payment adds $0.7$ to the log-odds, i.e.\
    \emph{multiplies} the odds of default by $e^{0.7}=\mathbf{2.014}$ (roughly doubles them) ---
    the same factor at every $x$. The \emph{probability} change depends on the position on the
    S-curve: large near $p\approx 0.5$, tiny near $0$ or $1$ $\Rightarrow$ no single number.
    \medskip
