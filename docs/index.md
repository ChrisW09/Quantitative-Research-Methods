...
**twelve polished Beamer decks, fifteen Jupyter labs, three mock exams** and
the course datasets.
...
Run a lab in Colab with zero setup, or install the pinned local environment.
:::
...
The 12-lecture plan, chapter map and suggested stopping points.
:::
...
: Twelve decks, fifteen labs and three exams that share one notation, one
...
: Fifteen notebooks run locally *and* on Google Colab; data loads from the
...
: LaTeX sources for every deck and exam, a pinned Python environment, and
  datasets that resolve automatically.
